...
- **TradeCpi**: production matcher path; the LP owner configures a matcher program/context once on
  ...
  `portfolio_id`, position epoch, and matcher sequence, then advances the sequence. Fills then run
  without the LP owner signing each transaction. Direct LP-signed bilateral trading is
  `TradeNoCpi`.
...
The signed instruction carries `portfolio_id`, `position_epoch`, and `expected_sequence`. It
succeeds only for the current portfolio incarnation, current position episode, and current
sequence, then increments the sequence exactly once. This prevents a retained enable from crossing
either a later explicit revoke or an automatic revoke caused by a position mutation that the
external matcher did not observe. Prior-layout portfolios with a portfolio ID but no sequence tail
read sequence zero and grow atomically on their first mutation.
...
- **SetMatcherConfig** (tag 68)
  - LP-owner-signed opt-in/out for unsigned LP matcher fills. This writes the matcher config tail
    ...
    base-fee cap. The payload is `portfolio_id: u64, expected_sequence: u64, position_epoch: u64,
    enabled: u8, trade_fee_cap_bps: u16, expiry_slot: u64`; stale incarnation, position episode,
    or sequence values reject before mutation.
