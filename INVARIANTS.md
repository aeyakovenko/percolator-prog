...
is the retained envelope: it binds the transaction-message version, deployed program ID, static
account keys, exact instruction bytes, and recent blockhash. For v0 address lookups, it binds the
table addresses, indices, and writable/readonly classifications; authenticated runtime lookup
resolution supplies the account keys checked by the wrapper. In that architecture, cluster admission
