//! Tag 2 — InitLP. LP-account materialization. Same 6-account layout
//! and pure-deposit semantics as InitUser; additionally registers the
//! LP's `matcher_program` + `matcher_context` (the CPI identity
//! binding consulted by TradeCpi).
//!
//! Wire format:
//!   `[2u8] [matcher_program: [u8; 32]] [matcher_context: [u8; 32]]`
//!   `      [fee_payment: u64]`                                   (73 bytes)
//!
...
pub fn handler(
    ...
    engine
        ...
        .map_err(map_risk_error)?;
    // LP-specific fields: kind + matcher binding.
