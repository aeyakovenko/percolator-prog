...
pub mod processor {
    ...
    fn portfolio_touches_requested_active_asset_view(
        ...
        let mut seen_asset_indices = [usize::MAX; percolator::V16_MAX_PORTFOLIO_ASSETS_N];
        let mut seen_len = 0usize;
        let mut any_request_has_stored_positions = false;
        for request in requests {
            if request.asset_index >= group.markets.len() {
                return Err(PercolatorError::EngineInvalidConfig.into());
            }
            let asset = group.markets[request.asset_index].engine.asset;
            if asset.stored_pos_count_long.get() != 0 || asset.stored_pos_count_short.get() != 0 {
                any_request_has_stored_positions = true;
            }
        }
        if !any_request_has_stored_positions {
            return Ok(false);
        }
        ...
        while slot < percolator::V16_MAX_PORTFOLIO_ASSETS_N {
            ...
            for request in requests {
                ...
                let mut seen = 0usize;
                while seen < seen_len {
                    if seen_asset_indices[seen] == asset_index {
                        return Err(PercolatorError::EngineHiddenLeg.into());
                    }
                    seen += 1;
                }
                seen_asset_indices[seen_len] = asset_index;
                seen_len += 1;
