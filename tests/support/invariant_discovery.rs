...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct DualAdlPrefixDiscovery {
    pub route: DiscoveryTradeRoute,
    pub first_long_a: u128,
    pub first_short_a: u128,
    pub first_effective_q: u128,
    pub margin_floor: u128,
    pub capital_at_floor: u128,
    pub final_long_a: u128,
    pub final_short_a: u128,
    pub long_raw_basis_q: u128,
    pub short_raw_basis_q: u128,
    pub long_effective_q: u128,
    pub short_effective_q: u128,
    pub long_oi_q: u128,
    pub short_oi_q: u128,
    pub token_supply_conserved: bool,
}
...
pub struct StaleCohortNovationCertification {
    ...
    pub unsafe_novation_rejected: bool,
    pub rejection_exact_rollback: bool,
    pub rejected_winner_position_q: i128,
    pub rejected_entrant_position_q: i128,
    pub owner_reduction_landed: bool,
    pub owner_position_after_reduction_q: i128,
    pub settlement_cranks: u16,
    pub post_settlement_stale_long_count: u64,
    pub post_settlement_stale_short_count: u64,
    pub entrant_untouched_by_historical_settlement: bool,
    pub settled_cohort_retry_landed: bool,
    pub retry_winner_position_q: i128,
    pub retry_entrant_position_q: i128,
    pub token_supply_conserved: bool,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct StaleCohortExactReversalCertification {
    pub arithmetic_targets_reversed: bool,
    pub stale_short_count_before_settlement: u64,
    pub stale_short_count_after_settlement: u64,
    pub loser_epoch_advanced: bool,
    pub loser_pnl_zero: bool,
    pub exact_rejection_rollback: bool,
    pub token_supply_conserved: bool,
}
...
impl DualAdlPrefixDiscovery {
    pub fn satisfies_invariant(&self) -> bool {
        self.first_long_a < ADL_ONE
            && self.first_short_a == ADL_ONE
            && self.first_effective_q > 1
            && self.margin_floor != 0
            && self.capital_at_floor == self.margin_floor
            && self.final_long_a < ADL_ONE
            && self.final_short_a < ADL_ONE
            && self.long_raw_basis_q > self.long_effective_q
            && self.short_raw_basis_q > self.short_effective_q
            && self.long_effective_q != 0
...
pub struct FlatSourceLienDiscovery {
    ...
    pub source_claim_liened_num: u128,
    pub pre_crank_conversion_rejected: bool,
    pub pre_crank_close_rejected: bool,
    pub route_round_trip_completed: bool,
    pub crank_attempts: u8,
    pub crank_successes_mutated: bool,
    pub crank_released_lien: bool,
    pub converted_pnl: u128,
    pub capital_withdrawal_succeeded: bool,
    pub close_succeeded: bool,
    pub attempted_exit_routes: u64,
    pub required_exit_routes: u64,
    pub remaining_funded_value: u128,
    pub remaining_lien_num: u128,
    pub exact_rollback: bool,
    pub canonical_vault_liquidity: u128,
    pub token_supply_conserved: bool,
    pub terminal_classification: PublicTerminalClassification,
...
impl std::fmt::Debug for FlatSourceLienDiscovery {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("FlatSourceLienDiscovery")
            .field("escape_route", &self.escape_route)
            .field("provider_withdrawal", &self.provider_withdrawal)
            .field("flat_position_q", &self.flat_position_q)
            .field("positive_pnl", &self.positive_pnl)
            .field("source_claim_liened_num", &self.source_claim_liened_num)
            .field(
                "pre_crank_conversion_rejected",
                &self.pre_crank_conversion_rejected,
            )
            .field("pre_crank_close_rejected", &self.pre_crank_close_rejected)
            .field(
                "route_round_trip_completed",
                &self.route_round_trip_completed,
            )
            .field("crank_attempts", &self.crank_attempts)
            .field("crank_successes_mutated", &self.crank_successes_mutated)
            .field("crank_released_lien", &self.crank_released_lien)
            .field("converted_pnl", &self.converted_pnl)
            .field(
                "capital_withdrawal_succeeded",
                &self.capital_withdrawal_succeeded,
            )
            .field("close_succeeded", &self.close_succeeded)
            .field("attempted_exit_routes", &self.attempted_exit_routes)
            .field("required_exit_routes", &self.required_exit_routes)
            .field("remaining_funded_value", &self.remaining_funded_value)
            .field("remaining_lien_num", &self.remaining_lien_num)
            .field("exact_rollback", &self.exact_rollback)
            .field("canonical_vault_liquidity", &self.canonical_vault_liquidity)
            .field("token_supply_conserved", &self.token_supply_conserved)
            .field("terminal_classification", &self.terminal_classification)
            .finish_non_exhaustive()
    }
...
impl FlatSourceLienDiscovery {
    pub fn preserves_bounded_backed_claim_exit(&self) -> bool {
        self.provider_withdrawal != 0
            && self.flat_position_q == 0
            && self.positive_pnl > 0
            && self.source_claim_liened_num != 0
            && self.pre_crank_conversion_rejected
            && self.pre_crank_close_rejected
            && self.route_round_trip_completed
            && self.crank_attempts > 0
            && self.crank_attempts <= 4
            && self.crank_successes_mutated
            && self.crank_released_lien
            && self.converted_pnl == self.positive_pnl as u128
            && self.capital_withdrawal_succeeded
            && self.close_succeeded
            && self.required_exit_routes != 0
            && self.attempted_exit_routes & self.required_exit_routes == self.required_exit_routes
            && self.remaining_funded_value == 0
            && self.remaining_lien_num == 0
            && self.exact_rollback
            && self.token_supply_conserved
            && matches!(
                self.terminal_classification,
...
fn prepare_superseded_intent(
    ...
    match kind {
        SupersededIntentKind::MatcherConfig => {
            ...
            let fresh = env.build_retained_matcher_config(0, retained_enabled);
            Ok((retained, fresh))
        }
        ...
        SupersededIntentKind::ConfigureAuthMark => {
            ...
            let fresh = env.build_retained_auth_config(0, retained_mark);
            Ok((retained, fresh))
        }
        ...
        SupersededIntentKind::ConfigureEwmaMark => {
            ...
            let fresh = env.build_retained_ewma_config(0, retained_mark, 1, 0);
            Ok((retained, fresh))
        }
        ...
        SupersededIntentKind::TradeFeePolicy => {
            ...
            let fresh = env.build_retained_trade_fee_policy(retained_value);
            Ok((retained, fresh))
        }
        SupersededIntentKind::FeeRedirectPolicy => {
            ...
            let fresh = env.build_retained_fee_redirect_policy(retained_value);
            Ok((retained, fresh))
        }
        SupersededIntentKind::LiquidationFeePolicy => {
            ...
            let fresh = env.build_retained_liquidation_fee_policy(retained_value);
            Ok((retained, fresh))
        }
        SupersededIntentKind::MaintenanceFeePolicy => {
            ...
            let fresh = env.build_retained_maintenance_fee_policy(retained_value);
            Ok((retained, fresh))
        }
        SupersededIntentKind::MarketInitFeePolicy => {
            ...
            let fresh = env.build_retained_market_init_fee_policy(retained_value);
            Ok((retained, fresh))
        }
...
fn discover_one_superseded_intent(
    ...
    match result {
        ...
        Err(_) => {
            ...
            let fresh_result = env.land_retained(fresh);
            let after_fresh = fingerprint(&env);
            let fresh_success = fresh_result.map_err(|error| {
                format!(
                    "{kind:?}/{payload_order:?} current-sequence control rejected after stale rollback: {error}"
                )
            })?;
            let fresh_mutated_economic_state = before_fresh != after_fresh;
            if !fresh_mutated_economic_state {
                return Err(format!(
                    "{kind:?}/{payload_order:?} current-sequence control produced no state delta"
                ));
            }
            if env.token_supply_observed() != supply_before {
                return Err(format!(
...
> {
    ...
    const LOSER: usize = 1;
    const MARK: u64 = 100;
    const WINNER_DEPOSIT: u128 = 200;
    const LOSER_DEPOSIT: u128 = 200;
    const TRADE_SIZE_Q: i128 = (2 * POS_SCALE) as i128;
    ...
    let supply_before = env.token_supply_observed();
    execute_discovery_trade_route(&mut env, route, WINNER, LOSER, 0, TRADE_SIZE_Q, MARK)
        .map_err(|error| format!("open dual-ADL pair through {route:?}: {error}"))?;

    env.warp_to_slot(2);
    env.push_auth_mark(0, 2, MARK)
        .map_err(|error| format!("advance first dual-ADL fee slot: {error}"))?;
    crank_asset_progress(&mut env, LOSER, 2, 0, 16)
        .map_err(|error| format!("first dual-ADL loser progress: {error}"))?;

    let (_, first_group) = env.primary_market_state();
    let winner_after_first_wave = env.primary_portfolio(WINNER);
    let winner_leg = winner_after_first_wave
        .legs
        .iter()
        .filter_map(|leg| leg.try_to_runtime().ok())
        .find(|leg| leg.active && leg.asset_index == 0)
        .ok_or_else(|| "dual-ADL winner has no first-wave leg".to_string())?;
    let first_effective_q = super::reference_math::mul_div_ceil(
        winner_leg.basis_pos_q.unsigned_abs(),
        first_group.assets[0].a_long,
        winner_leg.a_basis,
    )?;
    if first_group.assets[0].a_long >= ADL_ONE
        || first_group.assets[0].a_short != ADL_ONE
        || first_effective_q <= 1
        || first_group.assets[0].oi_eff_long_q != first_effective_q
        || first_group.assets[0].oi_eff_short_q != first_effective_q
    {
        return Err(format!(
            "first dual-ADL wave did not create one-sided scaling: effective={first_effective_q}, asset={:?}",
            first_group.assets[0]
        ));
    }

    crank_asset_progress(&mut env, WINNER, 2, 0, 16)
        .map_err(|error| format!("refresh first-wave dual-ADL winner: {error}"))?;
    let (_, refreshed_group) = env.primary_market_state();
    if refreshed_group.assets[0].a_long != first_group.assets[0].a_long
        || refreshed_group.assets[0].a_short != first_group.assets[0].a_short
        || env.primary_portfolio(WINNER).pnl.get() != 0
    {
        return Err("unchanged-mark winner refresh altered the first dual-ADL topology".into());
    }
    let prepared_winner = env.primary_portfolio(WINNER);
    let prepared_cert = prepared_winner
        .health_cert
        .try_to_runtime()
        .map_err(|error| format!("decode prepared dual-ADL certificate: {error:?}"))?;
    if prepared_winner.last_fee_slot.get() != 2 || !prepared_cert.valid {
        return Err(format!(
            "dual-ADL winner is not prepared at the first fee boundary: last_fee_slot={}, cert={prepared_cert:?}, group={:?}",
            prepared_winner.last_fee_slot.get(),
            refreshed_group.assets[0]
        ));
    }
    let margin_floor =
        super::reference_math::mul_div_ceil(first_effective_q, u128::from(MARK), POS_SCALE)?.max(2);
    let capital_at_floor = env.primary_portfolio(WINNER).capital.get();
    if capital_at_floor != margin_floor {
        return Err(format!(
            "dual-ADL ordered fee did not land the second account on its independent floor: observed={capital_at_floor}, expected={margin_floor}, certified_equity={}, certified_initial_req={}",
            prepared_cert.certified_equity, prepared_cert.certified_initial_req
        ));
    }
    ...
    let long_leg = env
        ...
        .filter_map(|leg| leg.try_to_runtime().ok())
        .find(|leg| leg.active && leg.asset_index == 0)
        .ok_or_else(|| "dual-ADL long leg was exhausted by the reverse wave".to_string())?;
    let short_leg = env
        ...
        .filter_map(|leg| leg.try_to_runtime().ok())
        .find(|leg| leg.active && leg.asset_index == 0)
        .ok_or_else(|| "dual-ADL short leg was exhausted by the reverse wave".to_string())?;
    let long_raw_basis_q = long_leg.basis_pos_q.unsigned_abs();
    let short_raw_basis_q = short_leg.basis_pos_q.unsigned_abs();
    let long_effective_q = super::reference_math::mul_div_ceil(
        long_raw_basis_q,
        final_group.assets[0].a_long,
        long_leg.a_basis,
    )?;
    let short_effective_q = super::reference_math::mul_div_ceil(
        short_raw_basis_q,
        final_group.assets[0].a_short,
        short_leg.a_basis,
    )?;
    let discovery = DualAdlPrefixDiscovery {
        route,
        first_long_a: first_group.assets[0].a_long,
        first_short_a: first_group.assets[0].a_short,
        first_effective_q,
        margin_floor,
        capital_at_floor,
        final_long_a: final_group.assets[0].a_long,
        final_short_a: final_group.assets[0].a_short,
        long_raw_basis_q,
        short_raw_basis_q,
        long_effective_q,
        short_effective_q,
        long_oi_q: final_group.assets[0].oi_eff_long_q,
        short_oi_q: final_group.assets[0].oi_eff_short_q,
        token_supply_conserved: env.token_supply_observed() == supply_before,
    };
    if !discovery.satisfies_invariant() {
        return Err(format!(
...
fn finish_adl_force_close_clamp(
    ...
    account_order: AdlForceCloseAccountOrder,
    shutdown_slot: u64,
    force_close_slot: u64,
    require_dual_nonunit: bool,
    ...
    let expected_scaling = if require_dual_nonunit {
        live_group.assets[0].a_short < ADL_ONE
            && loser_raw_basis_before_q > loser_effective_before_q
    } else {
        live_group.assets[0].a_short <= ADL_ONE
            && loser_raw_basis_before_q >= loser_effective_before_q
    };
    ...
    {
        return Err(format!(
            "ADL force-close setup has the wrong scaling topology (dual={require_dual_nonunit}): winner={winner_leg:?}, loser={loser_leg:?}, winner_effective={winner_effective_before_q}, loser_effective={loser_effective_before_q}, asset={:?}",
            live_group.assets[0]
        ));
    }
    ...
    Ok(AdlForceCloseClampDiscovery {
        ...
        boundary,
        account_order,
        long_a_before: live_group.assets[0].a_long,
        short_a_before: live_group.assets[0].a_short,
        winner_raw_basis_before_q,
        loser_raw_basis_before_q,
        effective_before_q,
        requested_close_q,
...
fn verify_one_dual_adl_force_close_clamp(
    mut seed: [u8; 32],
    route: DiscoveryTradeRoute,
    boundary: AdlReductionBoundary,
    account_order: AdlForceCloseAccountOrder,
...
pub fn verify_dual_adl_force_close_clamp_matrix(
    seed: [u8; 32],
) -> Result<Vec<AdlForceCloseClampDiscovery>, String> {
    let mut discoveries = Vec::new();
    for route in DiscoveryTradeRoute::ALL {
        for boundary in AdlReductionBoundary::ALL {
            for account_order in AdlForceCloseAccountOrder::ALL {
                discoveries.push(verify_one_dual_adl_force_close_clamp(
                    seed,
                    route,
                    boundary,
                    account_order,
...
const STALE_COHORT_LOSER: usize = 1;
const STALE_COHORT_ENTRANT: usize = 2;
const STALE_COHORT_OPEN_PRICE: u64 = 100;
const STALE_COHORT_LOSS_PRICE: u64 = 500;
const STALE_COHORT_SIZE_Q: i128 = (10 * POS_SCALE) as i128;

fn stale_cohort_world(
    mut seed: [u8; 32],
    route: StaleCohortRoute,
    salt: u8,
    winner_deposit: u128,
    loser_deposit: u128,
) -> Result<(V16Svm, Vec<CrankObservationHint>, u128), String> {
    seed[0] ^= 0x39 ^ salt;
    seed[1] ^= route.discriminator();
    let mut env = V16Svm::new(
        seed,
        MarketConfig {
            initial_price: STALE_COHORT_OPEN_PRICE,
            maintenance_margin_bps: 10_000,
            initial_margin_bps: 10_000,
            max_price_move_bps_per_slot: 10_000,
            max_accrual_dt_slots: 1,
            min_funding_lifetime_slots: 1,
            actor_deposits: [winner_deposit, loser_deposit, 1_000_000, 0, 0],
            ..MarketConfig::default()
        },
    );
    let supply_before = env.token_supply_observed();
    ...
    for slot in 2..=5 {
        env.warp_to_slot(slot);
        env.push_auth_mark(0, slot, STALE_COHORT_LOSS_PRICE)
            .map_err(|error| format!("publish stale-cohort loss mark at {slot}: {error}"))?;
        env.crank(STALE_COHORT_WINNER, slot, observation.clone())
            .map_err(|error| format!("refresh stale-cohort winner at {slot}: {error}"))?;
    }
    let winner_before_conversion = env.primary_portfolio(STALE_COHORT_WINNER);
    if winner_before_conversion.pnl.get() != 4_000
        || env.primary_portfolio(STALE_COHORT_LOSER).pnl.get() != 0
    {
        return Err(format!(
            "stale-cohort fixture lost its asymmetric settlement: winner_pnl={}, loser_pnl={}",
            winner_before_conversion.pnl.get(),
            env.primary_portfolio(STALE_COHORT_LOSER).pnl.get()
        ));
    }
    Ok((env, observation, supply_before))
}

fn submit_stale_cohort_novation(env: &mut V16Svm, route: StaleCohortRoute) -> Result<(), String> {
    let trade_market_id = env.primary_market_state().1.assets[0].market_id;
    match route {
        StaleCohortRoute::NoCpi => env
            .trade_no_cpi(
                STALE_COHORT_WINNER,
                STALE_COHORT_ENTRANT,
                0,
                -STALE_COHORT_SIZE_Q,
                STALE_COHORT_LOSS_PRICE,
                0,
            )
            .map(|_| ()),
        StaleCohortRoute::BatchNoCpi => env
            .batch_trade_no_cpi(
                ...
                vec![BatchTradeLeg {
                    asset_index: 0,
                    market_id: trade_market_id,
                    size_q: -STALE_COHORT_SIZE_Q,
                    exec_price: STALE_COHORT_LOSS_PRICE,
                    fee_bps: 0,
                }],
            )
            .map(|_| ()),
        StaleCohortRoute::Cpi => env
            .trade_cpi(
                STALE_COHORT_WINNER,
                STALE_COHORT_ENTRANT,
                0,
...
fn settle_stale_cohorts(
    env: &mut V16Svm,
    observation: &[CrankObservationHint],
) -> Result<(u16, bool), String> {
    let mut settlement_cranks = 0u16;
    let mut exact_rejection_rollback = true;
    for _ in 0..64 {
        let (_, before_group) = env.primary_market_state();
        if before_group.assets[0].stale_account_count_long == 0
            && before_group.assets[0].stale_account_count_short == 0
        {
            break;
        }
        let mut round_progressed = false;
        for actor in [STALE_COHORT_LOSER, STALE_COHORT_WINNER, 4] {
            let before = fingerprint(env);
            match env.crank(actor, 5, observation.to_vec()) {
                Ok(_) => {
                    settlement_cranks = settlement_cranks
                        .checked_add(1)
                        .ok_or_else(|| "stale-cohort crank counter overflow".to_string())?;
                    round_progressed |= fingerprint(env) != before;
                }
                Err(_) => exact_rejection_rollback &= fingerprint(env) == before,
            }
        }
        let (_, group) = env.primary_market_state();
    ...
    let (_, settled) = env.primary_market_state();
    if settled.assets[0].stale_account_count_long != 0
        || settled.assets[0].stale_account_count_short != 0
    {
        return Err("stale-cohort settlement did not clear both finite cohorts".into());
    }
    Ok((settlement_cranks, exact_rejection_rollback))
}

fn verify_one_stale_cohort_novation_guard(
    seed: [u8; 32],
    route: StaleCohortRoute,
) -> Result<StaleCohortNovationCertification, String> {
    let (mut attack_env, attack_observation, attack_supply_before) =
        stale_cohort_world(seed, route, 0, 5_000, 1_000)?;
    let (_, pre_novation_group) = attack_env.primary_market_state();
    ...
    let before_rejection = fingerprint(&attack_env);
    let unsafe_novation_rejected = submit_stale_cohort_novation(&mut attack_env, route).is_err();
    let rejection_exact_rollback = fingerprint(&attack_env) == before_rejection;
    let rejected_winner_position_q =
        discovery_position(&attack_env.primary_portfolio(STALE_COHORT_WINNER), 0)?;
    let rejected_entrant_position_q =
        discovery_position(&attack_env.primary_portfolio(STALE_COHORT_ENTRANT), 0)?;
    let entrant_data_before = attack_env.primary_portfolio_data(STALE_COHORT_ENTRANT);
    let entrant_source_before =
        attack_env.token_amount(attack_env.actors[STALE_COHORT_ENTRANT].source_token);
    let entrant_destination_before =
        attack_env.token_amount(attack_env.actors[STALE_COHORT_ENTRANT].destination_token);

    let owner_reduction_landed = attack_env
        .rebalance_reduce(STALE_COHORT_WINNER, 0, STALE_COHORT_SIZE_Q.unsigned_abs())
        .is_ok();
    let owner_position_after_reduction_q =
        discovery_position(&attack_env.primary_portfolio(STALE_COHORT_WINNER), 0)?;
    let (attack_settlement_cranks, attack_rejections_rollback) =
        settle_stale_cohorts(&mut attack_env, &attack_observation)?;
    let (_, attack_settled_group) = attack_env.primary_market_state();
    let entrant_untouched_by_historical_settlement =
        attack_env.primary_portfolio_data(STALE_COHORT_ENTRANT) == entrant_data_before
            && attack_env.token_amount(attack_env.actors[STALE_COHORT_ENTRANT].source_token)
                == entrant_source_before
            && attack_env.token_amount(attack_env.actors[STALE_COHORT_ENTRANT].destination_token)
                == entrant_destination_before;

    let (mut retry_env, retry_observation, retry_supply_before) =
        stale_cohort_world(seed, route, 0xa5, 10_000, 10_000)?;
    let (retry_settlement_cranks, retry_rejections_rollback) =
        settle_stale_cohorts(&mut retry_env, &retry_observation)?;
    let settled_cohort_retry_landed = submit_stale_cohort_novation(&mut retry_env, route).is_ok();
    let retry_winner_position_q =
        discovery_position(&retry_env.primary_portfolio(STALE_COHORT_WINNER), 0)?;
    let retry_entrant_position_q =
        discovery_position(&retry_env.primary_portfolio(STALE_COHORT_ENTRANT), 0)?;

    Ok(StaleCohortNovationCertification {
        route,
        pre_stale_long_count,
        pre_stale_short_count,
        pre_negative_pnl_count,
        unsafe_novation_rejected,
        rejection_exact_rollback: rejection_exact_rollback
            && attack_rejections_rollback
            && retry_rejections_rollback,
        rejected_winner_position_q,
        rejected_entrant_position_q,
        owner_reduction_landed,
        owner_position_after_reduction_q,
        settlement_cranks: attack_settlement_cranks
            .checked_add(retry_settlement_cranks)
            .ok_or_else(|| "combined stale-cohort crank counter overflow".to_string())?,
        post_settlement_stale_long_count: attack_settled_group.assets[0].stale_account_count_long,
        post_settlement_stale_short_count: attack_settled_group.assets[0].stale_account_count_short,
        entrant_untouched_by_historical_settlement,
        settled_cohort_retry_landed,
        retry_winner_position_q,
        retry_entrant_position_q,
        token_supply_conserved: attack_env.token_supply_observed() == attack_supply_before
            && retry_env.token_supply_observed() == retry_supply_before,
    })
}

pub fn verify_stale_cohort_novation_guards(
    seed: [u8; 32],
) -> Result<Vec<StaleCohortNovationCertification>, String> {
    StaleCohortRoute::ALL
        .into_iter()
        .map(|route| verify_one_stale_cohort_novation_guard(seed, route))
        .collect()
}

pub fn verify_stale_cohort_exact_reversal(
    mut seed: [u8; 32],
) -> Result<StaleCohortExactReversalCertification, String> {
    seed[0] ^= 0xd7;
    let mut env = V16Svm::new(
        seed,
        MarketConfig {
            ...
            max_accrual_dt_slots: 1,
            min_funding_lifetime_slots: 1,
            actor_deposits: [10_000, 10_000, 10_000, 0, 0],
            ..MarketConfig::default()
        },
    );
    let supply_before = env.token_supply_observed();
    env.top_up_backing_bucket(1, 10_000, 100)
        .map_err(|error| format!("fund exact-reversal source backing: {error}"))?;
    env.trade_no_cpi(0, 1, 0, POS_SCALE as i128, 100, 0)
        .map_err(|error| format!("open exact-reversal exposure: {error}"))?;
    let observation = vec![CrankObservationHint {
        asset_index: 0,
        oracle_accounts: env.primary_profile(0).oracle_leg_count,
    }];
    for (slot, price) in [(2, 101), (3, 100)] {
        env.warp_to_slot(slot);
        env.push_auth_mark(0, slot, price)
            .map_err(|error| format!("publish exact-reversal mark at {slot}: {error}"))?;
        env.crank(0, slot, observation.clone())
            .map_err(|error| format!("settle exact-reversal winner at {slot}: {error}"))?;
    }
    let (_, reversed_group) = env.primary_market_state();
    let arithmetic_targets_reversed = reversed_group.assets[0].k_long == 0
        && reversed_group.assets[0].k_short == 0
        && reversed_group.assets[0].f_long_num == 0
        && reversed_group.assets[0].f_short_num == 0;
    let stale_short_count_before_settlement = reversed_group.assets[0].stale_account_count_short;
    let loser_epoch_before = env.primary_portfolio(1).legs[0]
        .try_to_runtime()
        .map_err(|error| format!("decode exact-reversal loser leg: {error:?}"))?
        .kf_epoch_snap;
    let before_rejection = fingerprint(&env);
    let rejected = env.trade_no_cpi(0, 2, 0, -(POS_SCALE as i128), 100, 0);
    let exact_rejection_rollback = rejected.is_err() && fingerprint(&env) == before_rejection;
    env.crank(1, 3, observation)
        .map_err(|error| format!("settle exact-reversal loser: {error}"))?;
    let (_, settled_group) = env.primary_market_state();
    let loser_after = env.primary_portfolio(1);
    let loser_epoch_after = loser_after.legs[0]
        .try_to_runtime()
        .map_err(|error| format!("decode settled exact-reversal loser leg: {error:?}"))?
        .kf_epoch_snap;

    Ok(StaleCohortExactReversalCertification {
        arithmetic_targets_reversed,
        stale_short_count_before_settlement,
        stale_short_count_after_settlement: settled_group.assets[0].stale_account_count_short,
        loser_epoch_advanced: loser_epoch_after > loser_epoch_before,
        loser_pnl_zero: loser_after.pnl.get() == 0,
        exact_rejection_rollback,
        token_supply_conserved: env.token_supply_observed() == supply_before,
    })
}
...
fn discover_one_flat_source_lien_bounded_exit(
    mut seed: [u8; 32],
    ...
    const CRANK_EXIT_BIT: u64 = 1 << 4;
    const CONVERT_EXIT_BIT: u64 = 1 << 5;
    const CLOSE_EXIT_BIT: u64 = 1 << 6;
    const WITHDRAW_EXIT_BIT: u64 = 1 << 7;
    ...
    let mut exact_rollback = true;
    let mut attempted_exit_routes = CONVERT_EXIT_BIT;
    let before_conversion = fingerprint(&env);
    let pre_crank_conversion_rejected = env
        .convert_released_pnl(WINNER, positive_pnl as u128)
        .is_err();
    if pre_crank_conversion_rejected {
        exact_rollback &= fingerprint(&env) == before_conversion;
    }

    attempted_exit_routes |= CLOSE_EXIT_BIT;
    let before_close = fingerprint(&env);
    let pre_crank_close_rejected = env.close_primary_portfolio(WINNER).is_err();
    if pre_crank_close_rejected {
        exact_rollback &= fingerprint(&env) == before_close;
    }

    if matches!(
        escape_route,
        FlatSourceLienEscapeRoute::TradeCpi | FlatSourceLienEscapeRoute::BatchCpi
    ) {
        env.ensure_primary_matcher_enabled(COUNTERPARTY)
            .map_err(|error| format!("refresh flat source-lien matcher authorization: {error}"))?;
    }
    ...
    attempted_exit_routes |= CRANK_EXIT_BIT;
    let mut crank_attempts = 0u8;
    let mut crank_successes_mutated = true;
    let mut crank_released_lien = false;
    while crank_attempts < 4 && !crank_released_lien {
        crank_attempts += 1;
        let before = fingerprint(&env);
        match env.crank(WINNER, 3, observation(0)) {
            Ok(_) => {
                let after = fingerprint(&env);
                crank_successes_mutated &= after != before;
                let lien_after: u128 = env
                    ...
                    .map(|source| source.source_claim_liened_num.get())
                    .sum();
                crank_released_lien = lien_after < source_claim_liened_num;
            }
            Err(_) => {
                exact_rollback &= fingerprint(&env) == before;
                break;
    ...
    let convertible_pnl = env.primary_portfolio(WINNER).pnl.get().max(0) as u128;
    let converted_pnl =
        if convertible_pnl != 0 && env.convert_released_pnl(WINNER, convertible_pnl).is_ok() {
            convertible_pnl
        } else {
            0
        };

    attempted_exit_routes |= WITHDRAW_EXIT_BIT;
    let capital_to_withdraw = env.primary_portfolio(WINNER).capital.get();
    let before_withdraw = fingerprint(&env);
    let capital_withdrawal_succeeded =
        capital_to_withdraw != 0 && env.withdraw_primary(WINNER, capital_to_withdraw).is_ok();
    if !capital_withdrawal_succeeded {
        exact_rollback &= fingerprint(&env) == before_withdraw;
    }

    let before_final_close = fingerprint(&env);
    let close_succeeded = env.close_primary_portfolio(WINNER).is_ok();
    if !close_succeeded {
        exact_rollback &= fingerprint(&env) == before_final_close;
    }

    let (remaining_funded_value, remaining_lien_num) = if close_succeeded {
        (0, 0)
    } else {
        let remaining = env.primary_portfolio(WINNER);
        let positive = remaining.pnl.get().max(0) as u128;
        let funded = remaining
            .capital
            .get()
            .checked_add(positive)
            .ok_or_else(|| "flat source-lien remaining value overflow".to_string())?;
        let lien = remaining
            .source_domains
            .iter()
            .map(|source| source.source_claim_liened_num.get())
            .try_fold(0u128, |sum, value| {
                sum.checked_add(value)
                    .ok_or_else(|| "flat source-lien remaining lien overflow".to_string())
            })?;
        (funded, lien)
    };
    let canonical_vault_liquidity = u128::from(env.token_amount(env.vault));
    ...
    let required_exit_routes = escape_route.evidence_bit()
        | CRANK_EXIT_BIT
        | CONVERT_EXIT_BIT
        | WITHDRAW_EXIT_BIT
        | CLOSE_EXIT_BIT;
    ...
    let terminal_classification = public_trace
        .classify_terminal(PublicTerminalObservation {
            ...
            unauthorized_gain_atoms: 0,
            funded_value_remaining: remaining_funded_value,
            unresolved_obligation: remaining_lien_num,
            bounded_exit_succeeded: close_succeeded,
            terminal_receipt_created: false,
            authorized_forfeit: false,
            required_exit_routes,
            attempted_exit_routes,
    ...
    Ok(FlatSourceLienDiscovery {
        ...
        source_claim_liened_num,
        pre_crank_conversion_rejected,
        pre_crank_close_rejected,
        route_round_trip_completed,
        crank_attempts,
        crank_successes_mutated,
        crank_released_lien,
        converted_pnl,
        capital_withdrawal_succeeded,
        close_succeeded,
...
pub fn discover_flat_source_lien_bounded_exits(
    seed: [u8; 32],
    provider_withdrawal: u128,
) -> Result<Vec<FlatSourceLienDiscovery>, String> {
    FlatSourceLienEscapeRoute::ALL
        .into_iter()
        .map(|route| discover_one_flat_source_lien_bounded_exit(seed, provider_withdrawal, route))
        .collect()
}
