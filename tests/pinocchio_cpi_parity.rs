...
fn assert_ix_eq(label: &str, got: &Instruction, want: &Instruction) {
    ...
    for (i, (g, w)) in got.accounts.iter().zip(want.accounts.iter()).enumerate() {
        assert_eq!(
            g.pubkey, w.pubkey,
            "{label}: account[{i}].pubkey mismatch"
        );
        assert_eq!(
...
fn test_transfer_typical_amount() {
    ...
    let our = percolator_prog::spl_token::transfer(
        &token_prog, &source, &dest, &authority, &[], amount,
    )
    .expect("our transfer failed");
    let spl = spl_token::instruction::transfer(
        &token_prog, &source, &dest, &authority, &[], amount,
    )
    .expect("spl transfer failed");
...
fn test_transfer_u64_max() {
    ...
    let our = percolator_prog::spl_token::transfer(
        &token_prog, &source, &dest, &authority, &[], u64::MAX,
    )
    .expect("our transfer failed");
    let spl = spl_token::instruction::transfer(
        &token_prog, &source, &dest, &authority, &[], u64::MAX,
...
fn test_mint_to_amount_zero() {
    ...
    let our =
        percolator_prog::spl_token::mint_to(&token_prog, &mint, &dest, &authority, &[], 0)
            .expect("our mint_to failed");
    let spl =
        spl_token::instruction::mint_to(&token_prog, &mint, &dest, &authority, &[], 0)
...
fn test_mint_to_typical() {
    ...
    let our = percolator_prog::spl_token::mint_to(
        &token_prog, &mint, &dest, &authority, &[], amount,
    )
    .expect("our mint_to failed");
    let spl = spl_token::instruction::mint_to(
        &token_prog, &mint, &dest, &authority, &[], amount,
    )
    .expect("spl mint_to failed");
...
fn test_mint_to_u64_max() {
    ...
    let our = percolator_prog::spl_token::mint_to(
        &token_prog, &mint, &dest, &authority, &[], u64::MAX,
    )
    .expect("our mint_to failed");
    let spl = spl_token::instruction::mint_to(
        &token_prog, &mint, &dest, &authority, &[], u64::MAX,
...
fn test_burn_amount_zero() {
    ...
    let our =
        percolator_prog::spl_token::burn(&token_prog, &account, &mint, &authority, &[], 0)
            .expect("our burn failed");
    let spl =
        spl_token::instruction::burn(&token_prog, &account, &mint, &authority, &[], 0)
...
fn test_burn_typical() {
    ...
    let our = percolator_prog::spl_token::burn(
        &token_prog, &account, &mint, &authority, &[], amount,
    )
    .expect("our burn failed");
    let spl = spl_token::instruction::burn(
        &token_prog, &account, &mint, &authority, &[], amount,
    )
    .expect("spl burn failed");
...
fn test_burn_u64_max() {
    ...
    let our = percolator_prog::spl_token::burn(
        &token_prog, &account, &mint, &authority, &[], u64::MAX,
    )
    .expect("our burn failed");
    let spl = spl_token::instruction::burn(
        &token_prog, &account, &mint, &authority, &[], u64::MAX,
...
fn test_initialize_mint_no_freeze() {
    ...
    let our = percolator_prog::spl_token::initialize_mint(
        &token_prog, &mint, &authority, None, 6,
    )
    .expect("our initialize_mint failed");
    let spl = spl_token::instruction::initialize_mint(
        &token_prog, &mint, &authority, None, 6,
    )
...
fn test_initialize_mint_with_freeze() {
    ...
    let our = percolator_prog::spl_token::initialize_mint(
        &token_prog, &mint, &authority, Some(&freeze), 9,
    )
    .expect("our initialize_mint failed");
    let spl = spl_token::instruction::initialize_mint(
        &token_prog, &mint, &authority, Some(&freeze), 9,
...
fn test_initialize_mint_decimals_boundary() {
    ...
    for dec in [0u8, 1, 8, 9, 18, 255] {
        let our = percolator_prog::spl_token::initialize_mint(
            &token_prog, &mint, &authority, None, dec,
        )
        .expect("our initialize_mint failed");
        let spl = spl_token::instruction::initialize_mint(
            &token_prog, &mint, &authority, None, dec,
...
fn test_initialize_mint_data_length_with_freeze() {
    ...
    let our = percolator_prog::spl_token::initialize_mint(
        &token_prog, &mint, &authority, Some(&freeze), 6,
    )
    .expect("our initialize_mint failed");
    let spl = spl_token::instruction::initialize_mint(
        &token_prog, &mint, &authority, Some(&freeze), 6,
...
fn test_instruction_tags() {
    ...
    let transfer_ix = percolator_prog::spl_token::transfer(&prog, &a, &b, &c, &[], 1)
        .expect("transfer");
    assert_eq!(
    ...
    let mint_to_ix = percolator_prog::spl_token::mint_to(&prog, &a, &b, &c, &[], 1)
        .expect("mint_to");
    assert_eq!(
        mint_to_ix.data[0], 7,
    ...
    let burn_ix = percolator_prog::spl_token::burn(&prog, &a, &b, &c, &[], 1)
        .expect("burn");
    assert_eq!(
        burn_ix.data[0], 8,
