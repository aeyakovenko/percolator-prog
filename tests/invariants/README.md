...
`beeea60a37d7a3a7f3d6207502f35c5485b329739bf71d7b3c70af16f06685fa`. Resolved-mode
...
roster passes 160/160. Arbitrary relational wide scale division remains behind the arithmetic wall
...
| Focused INV-089 activation/reactivation equivalence | 15/15 public CU tests | both permissionless and privileged reuse reset an old generation's `u64::MAX` replay watermark; byte-complete persisted-slot differentials cover public trade/oracle/backing, exact insurance spend/owner forfeit/provider settlement, and a converted source claim whose stale certificate rejects until public refresh. Every retired history matches fresh activation after normalizing only generation IDs; fresh same-generation trading plus full withdrawal remains live and engine/SPL custody stays exact. |
...
`beeea60a37d7a3a7f3d6207502f35c5485b329739bf71d7b3c70af16f06685fa`.
...
8. Extend INV-089's byte-complete fresh/reuse differential beyond matched trade, mark, backing,
   spent-insurance, prior-claim, and current-certificate history to maximum supported account shape
   and unsupported-shape rejection. Replay-watermark poisoning and the three byte-complete history
   classes are closed; do not duplicate those cells with field-by-field assertions.
...
The current ledger is 3 **CLOSED**, 54 **OPEN-T**, 12 **OPEN-D**, 6 **PARTIAL**, 13 **FRONTIER**,
...
| AUDIT-089 | OPEN-T | Fresh/reuse authority and price checks are broad. The shared model proves both a public Active-to-DrainOnly-to-empty-Retired lifecycle and an empty-Recovery restart that advances generation, preserves authority/fee/insurance scope, frames unrelated assets/accounts/custody, and admits a fresh-generation trade under independent leg/OI checks. A byte-complete dynamic-slot differential independently found that wrapper replay watermarks survived engine/profile reset: an old generation could set `oracle_observation = u64::MAX` and permanently poison the replacement authority. Both privileged and permissionless reuse now reset all eleven per-generation lanes. The first persisted-state route composes matched trade, mark movement, backing top-up/withdrawal, retirement, and reuse. A second public-only route consumes an exact insurance budget through bankruptcy, explicitly forfeits only the winner's junior claim, returns all senior capital, preserves and withdraws the provider-attributed backing, retires the empty Recovery asset, and proves fresh-slot equality and liveness. A third route converts a real 50,000-atom source-backed claim only after exact stale-certificate rollback and public epoch refresh, returns exact user capital, refills and withdraws the provider receivable, retires audit-only source history, and again proves byte equality, fresh trading, and custody. Maximum-shape and unsupported-shape permutations remain open. |
