...
substitution and concurrent partial-receipt framing across every route pair. INV-028/071/073/082
...
| `cargo test --test v16_program_stateful_fuzz` | 187/187 | full engine-`592d538c`/SBF-`909c24cc` rerun in 175.03 seconds, including both active-close barrier orientations across every trade route, the independent raw-basis/effective-OI ledger, sub-minimum-ADL and cross-domain fractional-source progress, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, concurrent partial receipts, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
