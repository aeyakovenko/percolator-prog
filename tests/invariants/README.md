...
871/871 LiteSVM/CU tests, 100/100 public fuzz regressions, and 206/206 stateful/model tests. Static
...
Every matrix starts from a valid mutation control. That narrows but does not close its remaining
instruction-schema matrix.
...
| `cargo test --test v16_cu` | 871/871 | full engine-`b10b3454`/SBF-`fb16e50e` exact-source rerun passed in 202.87 seconds on the 2026-08-26 checkpoint; zero failures, including every prior maximum-shape and invariant route plus exhaustive account-pair and required-privilege matrices for authority handoff, initialization, matcher configuration, auxiliary-ledger sync, fee/resolve policy, managed-mark, authority-resolution, drain-only, and stale-resolution routes |
...
| `cu/` | 871 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, 60 partial-fill route/ratio/maximum-domain worlds, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
| AUDIT-017 | OPEN-T | All four trade routes exhaust ten direct or 21 CPI/matcher core-account pairs; deposit, withdraw, resolved close, and a genuinely partial value-moving resolved claim exhaust 15, 21, 21, and 21 custody/payout pairs; nine reserve-custody shapes exhaust 126 more pairs plus 40 privilege downgrades; flat close, unilateral reduction, both maintenance-sync shapes, no-tail crank, and liquidation-reward crank add 19 pairs plus 17 downgrades; both market and asset-oracle authority handoffs add all six account-pair aliases, four signer downgrades, and two writable downgrades; portfolio initialization plus disabled/enabled matcher configuration add 21 pairs and seven downgrades; both ledger synchronizers add six pairs and six downgrades; all seven two-account fee/resolve policy routes add seven pairs and fourteen downgrades; and all managed-mark, authority-resolve, drain-only, and stale-resolve routes add six pairs and thirteen downgrades. Provider earnings are generated through public marks, maintenance, lien growth, and fee charging before the tag-52 matrix. Every shape starts from a successful mutating public control, hostile cases reject with exact rollback, both authority routes require two distinct signers and writable market state, self-cranker aliasing conserves aggregate capital/insurance/custody against a distinct cranker, no-tail crank is signer-independent, and the reward-tail matrix proves a readonly signer can receive a real reward while every pair alias rolls back. Remaining instruction schemas still need generated matrices and explicit controls for intentionally safe aliases. |
...
On engine pin `b10b3454dd03dcf4c04a020dc1a90381ff179200`, the full 871-test `v16_cu` inventory is
