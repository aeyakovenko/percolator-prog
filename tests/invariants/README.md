...
876/876 LiteSVM/CU tests, 100/100 public fuzz regressions, and 206/206 stateful/model tests. Static
...
prevents this progress from being overstated: 43 instruction variants have exhaustive current-shape
matrices, two have an explicitly named dynamic-tail/action gap, and five remain open. Fresh
...
exhaustive evidence without a live test, or dropping a gap fails the CU gate.
...
| Focused INV-017 account-pair and privilege matrices | 78 custody/payout pairs plus 18 downgrades; 204 reserve-custody pairs plus 59 downgrades; 19 core/crank pairs plus 17 downgrades; 6 authority-handoff pairs plus 6 privilege downgrades; 24 initialization/matcher-config pairs plus 9 privilege downgrades; 6 ledger-sync pairs plus 6 privilege downgrades; 7 policy-route pairs plus 14 privilege downgrades; 7 managed-mark/lifecycle aliases plus 16 privilege downgrades | every covered schema starts from a successful public mutation control; every hostile pair or privilege downgrade rejects with exact rollback; ResetPending and Recovery controls are publicly reached; the source-complete roster enforces 40 exhaustive, 2 partial, and 8 open variants |
...
| `cargo test --test v16_cu` | 876/876 | full engine-`b10b3454`/SBF-`fb16e50e` exact-source rerun passed in 199.11 seconds on the 2026-08-26 checkpoint; zero failures, including the reserve, Hybrid-oracle, public Recovery-forfeit, public close-cure, maximum-shape, and prior invariant routes |
...
| `cu/` | 876 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership and account-role disposition rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, 60 partial-fill route/ratio/maximum-domain worlds, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
| INV-017 | SVM/CU + Partial M | `cu/inv_017_signer_writable_role_and_account_alias_safety.rs`, `stateful/inv_017_signer_writable_role_and_account_alias_safety.rs`, and `inv_017_account_role_coverage.tsv` exhaust all ten direct and all 21 CPI semantic account-pair aliases for single/batch trade; all 15 deposit, 21 withdraw, 21 resolved-close, and 21 value-moving resolved-claim pairs; 204 reserve-custody pairs plus 59 privilege downgrades across market/domain insurance top-ups, backing top-ups, all optional-ledger withdrawal forms, terminal insurance, and independently generated backing earnings; 37 core/crank pairs plus 25 downgrades across flat close, unilateral reduction, public Recovery forfeit, a publicly generated cancellable close with SPL cure, maintenance sync, no-tail crank, and value-moving liquidation-reward crank; 19 Hybrid-oracle pair aliases plus six downgrades across all legal one-, two-, and three-provider tails; all six account-pair aliases plus six privilege downgrades across market and asset-oracle authority handoffs; all three fresh-market aliases plus both required privilege downgrades; 21 account-pair aliases plus seven privilege downgrades across portfolio initialization and disabled/enabled matcher configuration; six ledger-sync pairs plus six downgrades; all seven two-account fee/resolve policy pairs plus fourteen downgrades; and seven managed-mark/lifecycle aliases plus sixteen privilege downgrades across all four managed-mark routes, authority resolve, drain-only transition, permissionless stale resolve, public ResetPending finalization, and public Recovery restart. Every shape starts from a successful mutating fixture and hostile cases require exact rollback. The source-locked roster prevents drift and currently classifies 43 variants exhaustive, two partial, and five open with exact remaining shapes. |
...
| AUDIT-017 | OPEN-T | A source-complete roster binds all 50 production variants to an account-role disposition and live evidence. Forty-three variants are exhaustive; two remain partial (`PermissionlessCrank` authenticated-oracle tails and `UpdateAssetLifecycle` permissionless activation/actions); five remain open (`CloseSlab`, `ConvertReleasedPnl`, `UpdateBaseUnitMints`, `SwapSecondaryForPrimary`, and `ForceCloseAbandonedAsset`). Covered shapes begin with successful mutating public controls and hostile aliases/privilege downgrades require exact rollback; Hybrid-oracle controls cover every legal provider-tail length, Recovery forfeit is publicly reached, and cure uses a publicly generated cancellable close plus real SPL deposit. The executable queue is `inv_017_account_role_coverage.tsv`; closure requires exhausting those seven remaining schemas and documenting intentional safe aliases. |
...
On engine pin `b10b3454dd03dcf4c04a020dc1a90381ff179200`, the full 876-test `v16_cu` inventory is
