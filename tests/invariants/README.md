...
| `v16_program_recovery_seeded_frontier_preserves_bounded_owner_exit` | `run_bounded_recovery_reference_frontier`: step/graph checks, then `run_direct_user_exit_campaign` and increased SPL destinations. | Fresh/exact backing-expiry seeds; 366 worlds / 702 word transitions. |
...
ResetPending's 16. Counts exclude seed construction and exit steps. Recovery and receipt conflict
have no late-expiry seed; oracle failure has no post-maturity seed. The base graph does not replay
