...
| `cargo test --test v16_program_stateful_fuzz` | 184/184 | full engine-`78c73bc8`/SBF-`6a56ba5c` rerun in 172.31 seconds, including the independent raw-basis/effective-OI ledger, the persisted sub-minimum-ADL discovery seed, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`78c73bc8`/SBF-`6a56ba5c` rerun in 26.67 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 794/794 | full engine-`78c73bc8`/SBF-`6a56ba5c` exact-source rerun in 194.52 seconds, including all maximum-source and maximum-shape exits, post-ADL effective conversion, liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and every cross-invariant progress/rollback audit |
...
| Engine runtime/property suites | 184/184 base and 231/231 with `--features fuzz` | production-equivalent rerun on engine `53adf4d8`; `78c73bc8` changes only the `cfg(kani)` proof harness. All unit, reference-model, arithmetic-discharge, source-credit, insolvency, and v16 specification tests pass, including sub-minimum-A DrainOnly refresh/exit and full nonzero-current-A rounding partitions |
