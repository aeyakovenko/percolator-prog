...
| `cargo check --tests` | pass | exact engine-`b10b3454` build completed in 6.11 seconds on the 2026-08-21 PR135 test head |
| `cargo test --lib` | 7/7 | exact engine-`b10b3454` rerun on the 2026-08-21 PR135 production head |
| `cargo test --test v16_program_stateful_fuzz` | 201/201 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun completed in 255.43 seconds on the underfunded authority/claim checkpoint; two value-moving partial-receipt worlds join the 48 authority/policy and two full-funded authority/resolve worlds plus every prior stateful check |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 26.96 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
...
| `stateful/` | 201 | Generated public routes with exact rollback on rejection and shared state, custody, OI, source-credit, and liveness oracles on success. Coverage includes randomized retained retries and trade-route switching, lifecycle and terminal compositions, all 2,380 public action words through depth three, 14 underfunded terminal worlds including both authority/resolve orders, and all 5! basic claimant orders. Per-invariant rows below are the source of truth for uncovered dimensions. |
