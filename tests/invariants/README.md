...
`3b76b7948700ecc69fef69c84e7f5222c8a8a686`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`97eb59e91bcb2b1a8580041e2f85c2ad4cb82f3a4b054ad6072f2f42e3ca040d`. Resolved-mode
...
| Focused INV-052 canonical crank/ADL/insurance/claim partition matrix | 11/11 CU plus 10/10 stateful, 5/5 engine Kani, and 1/1 wrapper Kani | generated live, resolved, shutdown/Recovery, owner-reduction, live asset-insurance, terminal market-wide insurance, and atomic backed-claim conversion partitions plus exact post-ADL zero-sum settlement rerun on the 2026-08-21 PR135 production head |
...
| Focused INV-076 same-asset close-drift matrix | 4/4 public worlds | each trade route creates a real flat bankruptcy close while an independent pair keeps the same asset exposed; two authenticated same-asset accruals cross both price directions and funding enabled/disabled, enabled worlds require nonzero F-index movement, the untouched close ledger remains exact, one empty-hint crank strictly books the residual in Live, and every owner then closes and withdraws |
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `3b76b794` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `3b76b794` |
| `cargo test --test v16_program_stateful_fuzz` | 179/179 | full engine-`3b76b794`/SBF-`97eb59e9` rerun in 169.08 seconds, including generated aggregate/split/reversed terminal-insurance withdrawal, Recovery K/F refresh, same-asset close drift with real funding, success-must-mutate/fixed-point rollback checks, both active-close/shutdown landing orders, and every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`3b76b794`/SBF-`97eb59e9` rerun in 27.51 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 794/794 | full engine-`3b76b794`/SBF-`97eb59e9` exact-source rerun in 190.67 seconds, including the discarded-result nonvacuity audit, the 80-call cross-invariant progress audit, Hybrid/Pyth/AuthMark/EWMA fixed-point rejection, maximum-source exits, maximum-shape Recovery obligation and K/F cohort cleanup, liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and B-before-claim reset-carry retirement |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 137/137 | full wrapper-only exact-pin rerun on engine `3b76b794`; zero harness failures, including all declared nonvacuity covers |
| Engine runtime/property suites | 181/181 base and 224/224 with `--features fuzz` | rerun on engine `3b76b794`; all unit, reference-model, arithmetic-discharge, source-credit, insolvency, and v16 specification tests pass, including latent Recovery B and K/F settlement |
...
`97eb59e91bcb2b1a8580041e2f85c2ad4cb82f3a4b054ad6072f2f42e3ca040d`.
...
3. Apply INV-052's split/merge oracle to liquidation, multi-domain/expiry lien consumption,
   resolved payout claims, cooldowns, rates, and policy limits. Live asset-insurance withdrawal,
   terminal market-wide insurance withdrawal, and atomic live backed-claim conversion now have
   generated partition coverage. Keep each remaining operation's conservative-rounding envelope
   explicit instead of assuming byte identity.
4. Extend INV-076 from its reachable same-asset flat-close matrix to table-driven public fault
   injection around each close phase and complete account/market/custody snapshots. Compose the
   already-owned open-risk liquidation-to-Recovery boundary with atomic OI/basis release rather
   than synthesizing an impossible Live close that retains uncovered exposure.
...
| AUDIT-052 | OPEN-T | The current-anchor compounding and endpoint-funding sampling violations are fixed by one bounded canonical accrual path. Eleven CU and ten stateful public tests now include generated target replacement plus live, resolved, shutdown/Recovery, owner-reduction, live and terminal insurance, and backed-claim conversion partitions, exact normalized/SPL outcomes, both resolved claimant orders, full 14-leg/32-step CU coverage, exact post-ADL zero-sum settlement, five engine proofs, and a wrapper carry-domain proof. Owner-reduction split/reversed rounding is independently specified and conservatively bounded. The live insurance model crosses both asset domains in aggregate, split, and reversed order, checks exact per-call value flow, rejects one-atom overspend atomically, and converges every economic byte. The terminal model resolves and pays all claimants, dematerializes all portfolios, and proves aggregate/split/reversed market-wide insurance drains converge before one exact-rollback exhausted retry. The all-four-route claim model proves conversion is deliberately atomic: strict partition caps roll back rather than partially consume claim/lien atoms, then one full conversion and one reuse rejection converge exactly. A direction-reversal witness proves gross paid/received funding telemetry is cadence-dependent and unsafe as a paid-only reward basis; the wrapper has no such consumer. Add exact stale/fresh and mixed-oracle histories plus arbitrary partition/permutation fuzz for liquidation, multi-domain/expiry lien consumption, resolved claims, cooldowns, rates, and policy limits. |
...
On engine pin `3b76b7948700ecc69fef69c84e7f5222c8a8a686`, the full `v16_cu` inventory is
