...
868/868 LiteSVM/CU tests, 100/100 public fuzz regressions, and 206/206 stateful/model tests. Static
...
routes now have the same complete pair/privilege treatment. Every matrix starts from a valid
mutation control. That narrows but does not close its remaining instruction-schema matrix.
...
| `cargo test --test v16_cu` | 868/868 | full engine-`b10b3454`/SBF-`fb16e50e` exact-source rerun passed in 204.40 seconds on the 2026-08-26 checkpoint; zero failures, including every prior maximum-shape and invariant route plus exhaustive account-pair and required-privilege matrices for authority handoff, initialization, matcher configuration, auxiliary-ledger sync, and all two-account fee/resolve policy routes |
...
| `cu/` | 868 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, 60 partial-fill route/ratio/maximum-domain worlds, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
On engine pin `b10b3454dd03dcf4c04a020dc1a90381ff179200`, the full 868-test `v16_cu` inventory is
