...
for INV-005; INV-012 remains open only for explicit capability expiry.
...
`support_consumed`; the prior standalone subtrahend would double-count the cure. INV-037 remains
open for a different, narrower contradiction: engine `spec.md` still names
`pending_obligation_credits` and corresponding proof obligations, but the deployed
`CloseProgressLedgerV16`, its begin/advance writers, and the exact residual validator expose no
canonical field or reconstruction for that category. Map it disjointly from the actual pending-
obligation ledgers or remove the abstract term and stale proof claims, then compose every writer.
INV-075 owns the aligned exclusive same-domain contention semantics.
...
requests now bind the persisted matcher-config sequence, with public ABA rejection and fresh-
request liveness plus symbolic wire preservation. Explicit capability expiry remains a genuine
schema requirement; it cannot be closed by another test against the current wire.
...
The current INV-084 audit derives the complete effective proof roster directly from 29 mounted
modules: 168 direct harnesses plus 38 macro-generated trade-decoder harnesses, exactly matching the
206 reported by `cargo kani list`. Every direct harness is classified into 99 symbolic-total, 30
...
| Focused INV-012 capability scope and matcher synchronization | 6/6 public route worlds, 1/1 source-complete route roster, 1/1 full-key authorization Kani, and 3/3 matcher-sequence decoder Kani | both CPI routes consume one exact typed capability predicate and bind the persisted matcher-config sequence before CPI; all three 32-byte tuple fields and every packed control word are symbolic, while INV-016 and INV-002/003/004 compose PDA domain plus asset/portfolio/episode generations. Public partial liquidation, force-close/reuse, no-CPI mutation, configured CPI preservation, old-grant ABA rollback, and fresh post-regrant liveness remain green on the exact current SBF |
...
| INV-010 | CLOSED - Independent + P + SVM/CU + M + R | The public products exhaust all `3!` matcher/control/trade and value/control orders, both deposit/reduction orders, all 48 authority/policy worlds, both funded and underfunded authority/resolve orders, and all 144 policy/boundary/handoff/resolve terminal cells. `kani/inv_010_out_of_order_safety.rs` proves exact current-sequence admission and strict one-step consumption over full `u64`; `cu/inv_010_out_of_order_safety.rs` source-composes that induction with every retained family, delayed-control lane, identity/authority binding, successful attributed delta, and exact rollback. Every finite landing permutation is a repetition of this checked serial step. Closure applies to the bounds carried by the current schema; absent aggregate intent budgets remain OPEN-D under INV-008/009/011/059. A retained route, binding lane, rollback disposition, or signed field change reopens this row. |
...
| INV-012 | P + Static roster + SVM/CU + Cross-invariant composition | `cu/inv_012_capability_and_delegate_scope.rs`, `stateful/inv_012_capability_and_delegate_scope.rs`, `kani/inv_004_position_episode_binding.rs`, and `kani/inv_022_instruction_decoding_and_schema_upgrade_safety.rs` prove the exact enabled/program/context/delegate predicate over full symbolic keys and bind it plus the persisted matcher-config sequence to both production CPI handlers. Public partial liquidation, force-close/reuse, and no-CPI mutations invalidate; configured CPI fills preserve only the participating LP; old-grant single and batch CPI requests reject after identical disable/re-enable with exact rollback; and fresh current-sequence requests install real exposure. INV-016 exhausts the delegate PDA domain, while INV-002/003/004 bind asset generation, portfolio incarnation, and position episode. The capability authorizes only CPI matching and every requested leg carries its own generation. Explicit capability expiry remains absent. |
...
The current ledger is 81 **CLOSED**, 0 **OPEN-T**, 6 **OPEN-D**, 0 **PARTIAL**, 0 **FRONTIER**,
...
| AUDIT-012 | OPEN-D | The tractable current-surface scope and incarnation work is complete. One production predicate owns enabled/program/context/delegate authorization; Kani proves its exact equivalence over all full 32-byte keys and every control word; a source roster binds both CPI handlers to both portfolio/episode tuples, per-leg asset generation, PDA derivation, the typed check, and the persisted matcher-config sequence. Public single/batch ABA traces reject old requests after identical disable/re-enable with exact rollback and admit fresh current-sequence requests; Kani preserves the full-width sequence in both wire layouts and rejects prior sequence-less schemas. INV-016 exhausts every PDA seed substitution, while INV-002/003/004 compose the public generation boundaries. CPI matching is the capability's only operation and asset scope is per-request, so separate operation/asset allowlists are structurally inapplicable. Explicit capability expiry is the sole remaining schema gap; add and bind it before this row can close. |
...
| AUDIT-037 | OPEN-D | One mutation-killed oracle owns the deployed exact equation across public continuation, cancellation, Recovery support/face retirement, insurance-covered finalization, and both same-domain close landing orders under normative exclusive serialization. The cancellation matrix exhausts all four trade routes and both winning sides before and after cure; INV-075 proves the rejected contender cannot mutate or double-book the accepted ledger. The charter is corrected to match v16.9.1: counterparty-credit lien cure is already included once in `support_consumed`, and a separate subtrahend is forbidden double counting. The remaining concrete spec/implementation gap is `pending_obligation_credits`: engine `spec.md` names it and related proof items, but deployed `CloseProgressLedgerV16`, its begin/advance writers, and its residual validator expose no field or canonical reconstruction. Either define and prove a disjoint source-ledger map or remove the abstract term and stale proof claims; then source-compose every residual writer and finalizer through that corrected map. |
