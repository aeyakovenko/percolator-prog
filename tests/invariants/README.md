...
875/875 LiteSVM/CU tests, 100/100 public fuzz regressions, and 206/206 stateful/model tests. Static
...
prevents this progress from being overstated: 40 instruction variants have exhaustive current-shape
matrices, two have an explicitly named dynamic-tail/action gap, and eight remain open. Fresh
...
| `cargo test --test v16_cu` | 875/875 | full engine-`b10b3454`/SBF-`fb16e50e` exact-source rerun passed in 199.72 seconds on the 2026-08-26 checkpoint; zero failures, including the 204-pair/59-downgrade reserve matrix and every prior maximum-shape and invariant route |
...
| `cu/` | 875 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership and account-role disposition rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, 60 partial-fill route/ratio/maximum-domain worlds, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
| AUDIT-017 | OPEN-T | A source-complete roster binds all 50 production variants to an account-role disposition and live evidence. Forty variants are exhaustive; two remain partial (`PermissionlessCrank` authenticated-oracle tails and `UpdateAssetLifecycle` permissionless activation/actions); eight remain open (`CloseSlab`, `ConvertReleasedPnl`, `ConfigureHybridOracle`, `CureAndCancelClose`, `ForfeitRecoveryLeg`, `UpdateBaseUnitMints`, `SwapSecondaryForPrimary`, and `ForceCloseAbandonedAsset`). Covered shapes begin with successful mutating public controls and hostile aliases/privilege downgrades require exact rollback; reset finalization and Recovery restart use publicly reached lifecycle states. The executable queue is `inv_017_account_role_coverage.tsv`; closure requires exhausting those ten remaining schemas and documenting intentional safe aliases. |
...
On engine pin `b10b3454dd03dcf4c04a020dc1a90381ff179200`, the full 875-test `v16_cu` inventory is
