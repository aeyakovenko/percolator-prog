...
complete exact-artifact gates cover 892 LiteSVM/CU tests, 102 public fuzz regressions, and 203
...
| `cargo test --test v16_program_stateful_fuzz` | 204/204 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 266.18 seconds; includes the 16-world two-account shared-source reservation composition, four-route late conversion rollback, half-backed and pending-close principal-attribution oracles, and the independent resolved-payout seniority oracle; every public-trace token-moving step composes checked quote conservation with owner/vault attribution, and all authority, policy, lifecycle, liveness, and model campaigns pass |
...
| `cargo test --test v16_cu` | 892/892 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 202.53 seconds; zero behavioral failures, including the seven-case certificate-stale own-lien oracle, exact user-before-insurance terminal partition, single SPL-account parser gateway, source-complete authority-epoch route/decoder matrices, canonical permissionless lifecycle epoch, proof/deployed-dispatch source lock, and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
