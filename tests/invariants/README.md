...
public fuzz regressions, 205/205 stateful/model tests, 886/886 LiteSVM/CU tests, 174/174 wrapper
Kani harnesses, and 129/129 engine runtime/property tests. INV-034 remains
`OPEN-T` for a source-complete generator over every public instruction and valid cross-domain
account substitution; this counterexample does not promote the whole invariant to closed.
...
| Focused INV-034 unattributed cross-domain loss | one finding-blind public stateful campaign, one deterministic exact-SBF regression, 2/2 focused engine Kani proofs, and exact engine runtime/property suite | parent `b10b3454` spends 100,100 atoms of unrelated insurance and yields 95,500 atoms of coalition profit after public multi-asset loss/detach/liquidation; engine `9b737fd` retains the existing unattributed-loss lock, permits one strict risk-reducing crank, spends zero unrelated insurance, rejects settled nonprogress with exact rollback, preserves a bounded owner exit, and reconciles token supply. The fixed artifact is `320e8162`; 101/101 regressions, 205/205 stateful tests, 886/886 CU tests, and 129/129 engine tests pass. |
...
| `cargo test --test v16_cu` | 886/886 | full engine-`9b737fd`/SBF-`320e8162` exact-source rerun passed in 204.11 seconds on 2026-08-28; zero failures, including all 50 account-role matrices, reserve, Hybrid-oracle, public Recovery, terminal slab, lifecycle, maximum-shape, and prior invariant routes |
...
| INV-034 | Independent + Direct + P(engine) + F + SVM/CU | `public_sbf/inv_034_domain_and_instance_isolation.rs`, `stateful/inv_034_domain_and_instance_isolation.rs`, and `cu/inv_034_domain_and_instance_isolation.rs`. The finding-blind public campaign independently exposed a multi-asset realized-loss detach followed by foreign-domain insurance consumption on parent `b10b3454`; engine `9b737fd` reuses the existing unattributed-loss lock and central detach/liquidation paths. Fixed-pin evidence requires a strict risk-reducing automatic crank, zero foreign insurance spend, zero coalition profit, exact rollback once no work remains, a bounded owner exit, and exact SPL supply reconciliation. Two engine Kani harnesses prove the uncovered-loss postcondition and sticky-lock predicate lifecycle. The source-complete all-instruction cross-market/domain substitution matrix remains tractable work. |
