...
| `cargo test --test v16_program_stateful_fuzz` | 207/207 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 255.81 seconds; includes the 16-world two-account shared-source reservation composition, the new 16-world Resolved/Recovery encumbrance lifecycle, four-route late conversion rollback, half-backed, pending-close, eight-world pending-domain principal attribution, independent resolved-payout seniority, and all three INV-037 partition owners. The public INV-026 matrix independently checks account/source/bucket ownership after every transition and distinguishes consumed claim support from a surviving live-leg risk lien before bounded release. The public INV-037 matrices retire 1,000 junior face atoms against exactly 251 realizable support atoms on every trade route and preserve the equation across all eight route/side cancellation worlds; the mutation oracle rejects every one-atom value-term error and ignores face-only changes. Every public-trace token-moving step composes checked quote conservation with owner/vault attribution, and all authority, policy, lifecycle, liveness, and model campaigns pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 27.41 seconds; includes all public regressions, 48 trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 892/892 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 197.12 seconds; zero behavioral failures, including the seven-case certificate-stale own-lien oracle, exact user-before-insurance terminal partition, shared INV-037 close-partition oracle, single SPL-account parser gateway, source-complete authority-epoch route/decoder matrices, canonical permissionless lifecycle epoch, proof/deployed-dispatch source lock, and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
