...
`0f1bc6dc352329ea08d7751963c392f090162fb634097578de8c7b343e0ef20c`. Resolved-mode
...
rejection; the complete wrapper proof gate is 143/143. No provider parsing rule changed and no new
defect was found. The exact rebuilt SBF slightly reduced measured parser-route CU: the
freshness configuration/crank maxima are `9,061`/`47,617`, the epoch configuration/crank maxima are
`9,132`/`47,461`, and composite liquidation ingestion is `59,356` CU. This establishes the
...
| --- | ---: | --- |
| Focused INV-020 authenticated timestamp, parser, composite epoch, and lifecycle matrix | 1/1 direct public, 2/2 stateful, 6/6 Kani, 5 deterministic CU/host matrices, plus the complete 804-test CU gate | full-width timestamp/freshness/owner/key predicates plus all-provider short-data rejection; 7,183 provider words prove the thin `AccountInfo` entrypoint delegates exactly to the pure production parser; 126 provider/transform configurations reject 114 selected-leg skews and 126 coherent rewinds; 39 provider words cross freshness ages 59/60/61; three provider-role worlds reach a genuine coherent liquidation then exact effective exit; one composite shutdown/force-close/restart world clears old provenance and resumes trading; mixed-time controls preserve value and terminal payout |
...
`0f1bc6dc352329ea08d7751963c392f090162fb634097578de8c7b343e0ef20c`.
...
2. Extend the extracted pure Pyth, Switchboard, and Chainlink parser boundary from exact
   `AccountInfo` delegation, fixed-layout mutation corpus, and dispatch contracts to symbolic valid
   byte-layout parsing or an independent bounded reference parser. The public matrix includes
   ordinary trade exit, a real liquidation, shutdown, force close, restart, and terminal resolution,
   but it remains an orthogonal provider-role sample rather than the full provider-by-lifecycle
   Cartesian product. Do not treat delegation equivalence as a proof of every provider layout.
