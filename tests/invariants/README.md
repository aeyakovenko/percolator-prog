...
assets. ResetPending stale-leg reachability, Resolved terminal fallback, and Retired exposure
unreachability are now explicit public compositions. Its next tractable partition is wider interior
quantity/ADL generation. The next broader
...
| AUDIT-050 | OPEN-T | The engine-134 fix is integrated atop the canonical-accrual engine and the public all-four-route matrix is inverted: both OI preflight branches reject fresh-basis reissue with exact rollback, including preexisting auxiliary OI that reaches the common ADL gate. Each rejection retains a bounded owner reduction, and focused engine proofs establish route-complete admission. Zero/one/max/max+1 quantities, account-local effective/raw boundaries, both single-barrier orientations, two opposite-side barriers active simultaneously across two assets, Active/DrainOnly/Recovery admission, ResetPending stale raw-leg cleanup, Resolved terminal fallback, and Retired nonempty-state unreachability are public-route covered with exact rollback, bounded exits, obligation attribution/release, payout, and withdrawal. Add wider interior quantity/ADL generation. |
