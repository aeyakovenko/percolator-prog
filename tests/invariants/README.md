...
census owns 183 harnesses across 21 mounted modules, and the public-trace guard owns 33 consumers.
...
all 34 current public-trace consumers. Every successful token-moving wrapper step must have unique
...
SPL, and program-lamport rollback. A recursive source-complete guard requires all 34 current
...
extraction, terminal-generation replay, terminal dust, cross-domain backing double-consume, and
retained source-fee consent are classifier-bound public LoF evidence, while the other 22 are mechanically limited to economic-delta, local-safety, or
...
| `cargo test --test v16_program_stateful_fuzz` | 203/203 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 261.71 seconds; every public-trace token-moving step now composes checked quote conservation with owner/vault attribution, and the authority-incarnation, retained-policy, lifecycle, and model campaigns all pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 26.96 seconds; includes all public regressions, 34 trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 892/892 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 200.90 seconds; zero behavioral failures, including the single SPL-account parser gateway, source-complete authority-epoch route/decoder matrices, canonical permissionless lifecycle epoch, proof/deployed-dispatch source lock, and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
...
| AUDIT-024 | PARTIAL | Aggregate conservation is supplemented by a 32-world public route-pair matrix that proves exact realized-PnL ownership through settlement, route-switched close, conversion, and SPL withdrawal for both sides. All 34 public-trace consumers now share an external token-flow contract: every changed tracked SPL account is writable and authority-attributed, quote deltas balance exactly through one configured vault authority, and wrong-owner or one-atom corruptions fail. The remaining gap is internal engine-domain attribution for every value-bearing transition plus formal whole-route composition; exact rejected-route snapshots already exist. |
