...
Updated 2026-08-29. The current engine pin is
`6c8d94bcacb8110c8e86ce1b1fc46594bb672a7c` (engine PR
[#191](https://github.com/aeyakovenko/percolator/pull/191), stacked on canonical-allocation PR
[#190](https://github.com/aeyakovenko/percolator/pull/190)).
...
short; bounded public accrual makes the funding checkpoint replayable, and an authority-signed
shutdown plus owner forfeit creates a simultaneous nonzero close ledger without touching program
bytes out of band. `PermissionlessCrank` selects the higher-priority close, strictly decreases its
residual on every accepted call, frames the independent B leg and SPL custody, terminates within the
explicit bound, and then exposes a successful B-settlement continuation. The same public state is
also advanced past the close deadline: authenticated `Clock` expiry outranks the B obligation,
declares Recovery without touching the portfolio or custody, finalizes Recovery permissionlessly,
and then disposes the deferred B leg through bounded Resolved continuations. These are net-new
concrete summary-fidelity and overlap witnesses; they found no new production violation and add no
deployed code or state.
...
This adds no deployed code. A source-complete search of pinned engine `6c8d94bc` also finds no
...
N/A until that optional mechanism enters production; complete-state reachability remains the honest
INV-029 frontier.
...
refill lifecycle and the eight-world live-lien impairment matrix call the same oracle explicitly.
This centralizes the available-backing formula instead of adding a parallel test model or production
field. Twenty malformed relation cases plus two exact source-boundary truncations cross both source
sides and require a real instruction error with exact market, backing-ledger, token, and lamport
rollback. A pin-bound source lock permits only four non-BPF typed-codec copies and forbids direct
wrapper rate/epoch writers; INV-088's complete wrapper-to-engine callsite roster and the pinned
engine contracts own the internal mutations without duplicating them here. Universal public-state
...
caller-input and boundary rosters now own all 230 public fields across 52 input types, the proof
census owns 183 harnesses across 21 mounted modules, and the public-trace guard owns 49 consumers.
...
INV-018 now has one source-locked classic-SPL account parser boundary. The canonical user, vault,
and withdrawal validators return only the already validated balance and destination-encumbrance
facts needed downstream, so handlers no longer borrow and unpack the same 165-byte account again
for balance, empty-vault, or permissionless-payout checks. This removes duplicate parser paths
without adding policy or changing the deployed account format.

INV-024 now composes that custody boundary with a general external owner-attribution contract over
all 49 current public-trace consumers. Every successful token-moving wrapper step must have unique
...
snapshot; no deployed code or SBF byte changed. Internal engine-domain attribution and a formal
whole-route value-flow composition remain open.
...
current-state reconciliation. Full same-pubkey market recreation remains blocked by INV-001:
...
base-unit routes, reject
same-market `A -> B -> A`; remaining destructive-consent gaps are same-pubkey whole-market
recreation. The source-rostered lifecycle authority route is now epoch-bound through the same
canonical lane. Whole-market recreation remains owned by INV-001/005.
...
inventory of all 13 explicit `kani::assume` sites across all 21 mounted wrapper Kani modules. A
...
sources: 151 direct harnesses plus 36 macro-generated trade-decoder harnesses, exactly matching the
187 reported by `cargo kani list`. Every direct harness is classified into 86 symbolic-total, 26
...
`6c8d94bc`; approximate-bucket rebucketing is N/A for this profile, while complete-state reachability
remains.
...
| Focused INV-083 field-complete boundary inventory | 10/10 CU plus 187/187 wrapper Kani composition | all 234 fields across 52 public input types are source-locked into 20 semantic boundary profiles with per-field and profile-level executable evidence; the class roster retains all eleven required classes, and 25 public invalid `InitMarket` partitions reject with pristine-account rollback before a valid retry creates a usable portfolio |
| Focused INV-084 proof-harness nonvacuity inventory | 3/3 CU plus 13/13 assumption-focused Kani and 187/187 wrapper Kani composition | a source parser derives all 151 direct and 36 generated harnesses and locks 86 symbolic-total, 26 branch-witnessed, 10 explicitly constrained, 29 concrete-exact, and 36 generated-symbolic dispositions; all 13 explicit assumptions across 21 modules retain exact ownership and public evidence; public routes reach admitted domains, reject invalid partitions with exact rollback, and terminate with exact custody |
...
| Focused INV-018 token boundary and quote-delta matrix | 45/45 public CU tests plus the source-complete 15-handler roster | exact SBF-`8ac842fe` rerun; the single-parser gateway guard, real Token-2022 fee/hook mint rejection, six primary-decimal worlds, all 15 finding-blind token-moving handlers, independently generated public backing earnings, partial-receipt claim, cure, swap, and terminal surplus sweep all pass |
...
| Focused INV-029 exact-only deployed profile | 211/211 stateful public tests plus 1/1 source lock | after every generated successful public transition, each domain's exact claim equals its positive bound and the complete portfolio attribution; the wrapper exposes no non-exact bound or rebucketing ingress, so approximate-bucket range/rebucketing proofs remain N/A unless that optional profile is introduced |
| Focused INV-030 source-credit transition causality and malformed state | 211/211 stateful public tests plus 3/3 focused CU | every generated public action and successful permissionless crank compares both markets before and after: unchanged formula inputs preserve the rate, every formula-input mutation advances `credit_epoch`, and a live claim's rate rises only with more independently available backing or a smaller claim bound. Twenty malformed relation cases and two exact source-boundary truncations cross both source sides with instruction-error and complete economic rollback; a pin-bound source lock leaves on-chain rate/epoch mutation exclusively in the engine |
...
| Focused INV-071/072/078/082 concrete close/B selector overlap | 2/2 public two-asset worlds plus 15/15 focused INV-071, 31/31 focused INV-073, 7/7 focused INV-078, and 6/6 focused INV-056 CU tests | one portfolio simultaneously carries a real active-close residual and a real B-stale leg created exclusively by public trades, marks, cranks, shutdown, and owner forfeit. Before expiry, every close call strictly decreases the higher-priority residual while byte-framing the B leg and SPL custody, then deferred B settlement succeeds. After expiry, authenticated Clock selects Recovery despite a stale caller slot, frames the account and custody, reaches Resolved permissionlessly, and bounded resolved progress disposes the deferred B leg. |
...
| `cargo check --tests` | pass | exact engine-`6c8d94bc` build completed on commit `7497a8cc` |
| `cargo test --lib` | 7/7 | exact engine-`6c8d94bc` rerun on commit `7497a8cc` |
| `PERCOLATOR_FUZZ_CASES=2 cargo test --test v16_program_stateful_fuzz` | 211/211 | full exact engine-`6c8d94bc`/SBF-`a76dd082` rerun completed in 225.53 seconds on `7497a8cc`; every generated public action and successful crank composes the shared invariant oracles for both markets. The suite includes the 56-world one-domain, 48-world two-domain, and new 48-world mixed-expiry INV-052 source-lien matrices, the asymmetric-fee INV-041 order regression, INV-076 rollback frames, the finding-blind 21-family retained asset-generation matrix, source-domain single-use, retained source-fee consent, and backing-provider landing-order campaigns. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
| Registry/manifest checks in the INV-079 module | 12/12 | includes the source-complete 27-oracle evidence roster, sixteen exact terminal-LoF dispositions, complete-route terminal classification, and the recursive 50-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 106/106 | full exact engine-`6c8d94bc`/SBF-`a76dd082` rerun completed in 27.69 seconds after the expired-close/B overlap increment; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 897/897 | full exact engine-`6c8d94bc`/SBF-`a76dd082` rerun completed in 203.39 seconds after the public expired-close/B overlap increment; zero behavioral failures across reserve, fee-consent, oracle, Recovery, terminal, lifecycle, source-lien normalization, selector composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 187/187 | full wrapper-only exact-source rerun on commit `7497a8cc` with engine `6c8d94bc`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied |
| Engine runtime/property suites | 189/189 | exact `cargo test --all-targets` rerun on engine `6c8d94bc`: 50 library, 13 backing, 2 resolved-insolvency, and 124 v16 specification tests pass |
...
exists. The locally rebuilt 1,239,736-byte production SBF used by the 2026-08-29 exact-pin gates has
SHA-256 `a76dd082c832749d01fca98e48bd9d09cf308a1ef8481ddb4ca5900e981e11dd`.
The 3,688-byte increase from the preceding artifact is the engine's bounded concrete source-lien
normalization and dispatch; wrapper production code is unchanged, and the complete 897-test CU
suite remains green.
...
1. Generalize the concrete actionability-fidelity oracle that exposed the mixed-expiry DoS. For
   ...
   same expired public state. Continue with publicly reachable close/source-lien and
   stale/lower-priority overlaps; replace any apparently impossible pair with a production-builder
   unreachability proof rather than injected state.
...
replay (including PR296/325/326) and non-epoch authority-scope retention remain explicit
INV-001/INV-005 gaps. The 34 epoch-bearing authority cases now
...
reaches the exact target before terminal payouts reconcile. Other residue owners listed under
AUDIT-038 remain open and are not implied by these four certifications.
...
work, the entrant remains untouched, and the same route becomes live only after settlement. Reset,
account-close, partial-liquidation, and other unenumerated obligation-removal transitions remain
the explicit AUDIT-039 frontier.

Seven additional fixed-pin public families are now reconciled with the manifest: PR220/366 require
...
source-local claim/backing single use. The eight remaining quarantines are intentionally still red:
PR293/294/295/296/307/317/325/326 whole-market incarnation replay.
...
| INV-018 | Static roster + SVM/CU + M | `cu/inv_018_quote_mint_vault_token_program_and_authority_integrity.rs` rejects a real Token-2022 transfer-fee/transfer-hook mint at both mint-admission routes and rejects its executable program on a live deposit with exact rollback. Six primary-decimal worlds preserve raw-atom source/vault/capital/`c_tot` equality. A generic public matrix compares actual SPL and internal quote deltas across all 15 production token-moving handlers, including independently generated backing earnings, public cure, partial-receipt claim, swap, and terminal surplus sweep; `cu/inv_016_canonical_pda_and_seed_binding.rs` owns the source-complete exact-program/canonical-vault roster. A source-complete gateway check confines every classic-SPL account unpack to the canonical validators, which return the exact balance/encumbrance facts consumed by handlers. Formal equivalence to SPL Token's trusted unpack and CPI execution semantics remains. |
...
| INV-026 | F + SVM/CU + Partial | `stateful/inv_026_reservation_and_encumbrance_conservation.rs`, `stateful/inv_030_credit_rate_determinism_and_fail_closed_behavior.rs`, and `cu/inv_026_reservation_and_encumbrance_conservation_is_separate_from_token_value.rs` run a shared independent account/bucket/reservation census after every generated public action. One 16-world matrix crosses all four trade families, both source sides, and Resolved/Recovery through nonzero counterparty-lien creation: Resolved consumes the lien exactly once into provider receivable; Recovery consumes the exact realizable claim tranche, keeps the surviving-leg risk lien live, and releases it only after that leg closes and bounded permissionless cranks run. Separate route matrices cover live expiry/impairment, exact stale-route rollback, bounded owner reduction, out-of-order resolved close, and exact backing consumption/provider-receivable accounting. The census treats expired counterparty backing as account-local backing matched by market valid-plus-impaired state, rather than assuming it remains valid. Direct insurance-backed lien creation has no wrapper route and is owned by INV-033's engine proof; internal fault/retry points, pending obligations, and close reserves remain. |
...
| INV-029 | Independent + F + SVM/CU + Partial R | `stateful/inv_029_positive_claim_bounds_never_understate.rs` and `cu/inv_029_positive_claim_bounds_never_understate.rs` cover a whole-route source-claim lifecycle census, a 16-cell min/max and odd/even partial-burn partition, generated interior prices, both claimant orders, and exact unreceipted-bound-to-partial-receipt ledger replacement under the shared terminal stock/custody oracle. One eight-world matrix crosses all trade routes and both position orientations through pure favorable funding with unchanged effective price, exact account/domain/global claim attribution, exact conversion burn, principal reconciliation, and unrelated-user frames. A second underfunded eight-world matrix proves unmaterialized favorable price claims remain behind an independently reconstructed stale/stored-position snapshot barrier, then materialize exactly before counterparty principal and the remaining junior face are partitioned. The deployed exact-only profile requires exact and bound totals to agree after every generated public transition; a source lock excludes non-exact bound/rebucketing ingress, making approximate-bucket rebucketing N/A until that optional mechanism is introduced. Complete-state reachability remains. |
| INV-030 | Independent + SVM/CU | `stateful/inv_030_credit_rate_determinism_and_fail_closed_behavior.rs`, `cu/inv_030_credit_rate_determinism_and_fail_closed_behavior.rs`, `cu/inv_028_source_domain_realizability_cap.rs`, and `cu/inv_063_backing_expiry_normalization.rs` cover the deterministic credit-rate lifecycle plus secondary expiry/progress ownership. The shared runner wraps every generated public action and successful permissionless crank with a before/after oracle for both markets: unchanged formula inputs preserve the rate, any input mutation advances `credit_epoch`, and a live claim's rate cannot rise without greater independently available backing or a smaller claim bound. An eight-world public matrix crosses all four trade families with both source sides, requires a real lien to move exactly from valid to impaired, checks the independent rate and stock/encumbrance oracles throughout, proves stale bilateral routes roll back exactly, and retains a bounded owner reduction. The mixed-expiry 48-world matrix adds simultaneous Impaired and Fresh source domains, requires zero credit for the impaired claim, exact Fresh-sibling conversion, and one epoch-advancing normalization per selected domain. Twenty malformed persisted relations and two omitted embedded-source boundaries reject exactly on both sides. The wrapper source lock permits only four host-codec copies and no direct rate/epoch writer; INV-088 owns every wrapper-to-engine callsite and the exact pinned engine owns internal mutation proofs. Universal public-state reachability remains frontier work. |
| INV-031 | Independent + Direct + F + SVM/CU | `public_sbf/inv_031_no_double_use_of_claim_backing_or_insurance_atoms.rs`, `stateful/inv_031_no_double_use_of_claim_backing_or_insurance_atoms.rs`, and `cu/inv_031_no_double_use_of_claim_backing_or_insurance_atoms.rs` cover shared user credit, live domain insurance, terminal insurance, and primary/secondary collateral rails. A 32-world public matrix crosses every ordered pair of single/batch CPI/no-CPI trade routes and both source sides: a real source claim grows one live lien through multiple strict steps, the account/source/bucket owners remain exactly equal after every mutation, both route attempts reject atomically at the same admission frontier, and bounded crank release restores the exact fresh backing atoms. A separate 16-world composition crosses all four routes, both source sides, and both account orders while two portfolios reserve one source bucket against risk in different assets; the independently summed account liens equal the sole source/bucket aggregate at every step, both accounts reach exact-rollback frontiers, and finite release restores the original pool. The four-route half-backed conversion matrix additionally forces the deployed post-conversion cap rejection one atom below the exact payout, requires complete SVM rollback, and then consumes the same claim/backing tranche exactly once at the valid cap. Exhaustive internal failure points and the engine-only insurance-backed lien lifecycle remain. |
| INV-032 | Independent + F + SVM/CU | `stateful/inv_026_reservation_and_encumbrance_conservation.rs`, `stateful/inv_030_credit_rate_determinism_and_fail_closed_behavior.rs`, `stateful/inv_031_no_double_use_of_claim_backing_or_insurance_atoms.rs`, `cu/inv_032_exact_counterparty_lien_lifecycle.rs`, and the shared-expiry lifecycle in `cu/inv_028_source_domain_realizability_cap.rs` use one independent account/source/bucket census across all four trade routes and both source sides. The public matrices cover exact create/grow, alternate-route rejection, valid-to-impaired relabeling, terminal release/consume, provider-label retirement, sibling preservation, and zero residual aggregate impairment without insurance-provenance substitution. The mixed-expiry matrix now composes those labels across two simultaneous domains: one crank retires exactly the selected Impaired provider label into the account's impaired lane while preserving the Fresh sibling, whose later release/conversion clears exactly once. The half-backed conversion matrix additionally reaches the post-conversion cap rejection on every originating trade route, proves complete SVM rollback of the claim/backing lifecycle, and then consumes the same tranche exactly once. Recovery and the other fallible internal steps remain. |
| INV-033 | CLOSED - SVM/CU + P(engine) + public unreachability | `cu/inv_033_insurance_backed_lien_single_classification.rs` creates a genuine counterparty-backed source lien and proves it never populates insurance categories, then funds real domain insurance and proves the same public risk increase rejects exactly without reserving or consuming it. A source-complete absence guard pins zero wrapper callsites to the engine reservation/create/release/consume/impair methods and binds that claim to engine `6c8d94bcacb8110c8e86ce1b1fc46594bb672a7c`. On that exact pin, the engine's create/release/terminal-release/impair/consume function contracts were rerun and own the otherwise unreachable lifecycle without duplicating those proofs in the wrapper. A public reservation route or engine-pin change reopens this row. |
| INV-034 | Independent + Direct + P(engine) + F + SVM/CU + complete public-role matrix | `public_sbf/inv_034_domain_and_instance_isolation.rs`, `stateful/inv_034_domain_and_instance_isolation.rs`, `cu/inv_034_domain_and_instance_isolation.rs`, `stateful/inv_068_receipt_uniqueness_and_monotonic_topups.rs`, and `inv_034_instance_role_coverage.tsv`. The finding-blind public campaign independently exposed a multi-asset realized-loss detach followed by foreign-domain insurance consumption on parent `b10b3454`; engine `9b737fd` reuses the existing unattributed-loss lock and central detach/liquidation paths. Fixed-pin evidence requires a strict risk-reducing automatic crank, zero foreign insurance spend, zero coalition profit, exact rollback once no work remains, a bounded owner exit, and exact SPL supply reconciliation. Two engine Kani harnesses prove the uncovered-loss postcondition and sticky-lock predicate lifecycle. A source-locked roster covers all 49 public variants and rejects source/evidence drift: 20 variants have one existing instance anchor, all 29 mixed-role variants exhaust every current type-correct instance-bound role with exact rollback and mutating controls, and zero rows are partial/open. This closes the finite role-substitution frontier without claiming exhaustive arbitrary-length semantic sequences. |
...
| INV-036 | Independent + Direct + P + SVM/CU + M | `public_sbf/inv_036_fee_destination_and_policy_version_integrity.rs`, `stateful/inv_036_fee_destination_and_policy_version_integrity.rs`, `kani/inv_036_fee_destination_and_policy_version_integrity.rs`, and `cu/inv_036_fee_destination_and_policy_version_integrity.rs`. The retained source-fee matrix crosses single/batch CPI/no-CPI and both debited-account roles after policy changes: stale zero-cap requests reject exactly, out-of-domain caps reject exactly, bounded single-route retries reconcile trader debit to provider earning and SPL withdrawal, and batch policy admission remains fail-closed. Single no-CPI uses one bilateral signed cap; single CPI uses the signed account-A cap plus matcher-authenticated account-B cap through one shared collector. Issue 408 adds exact canonical-insurance attribution before matcher/liquidation value transfer, and the withdrawal reference independently partitions owner payout from maintenance credit. The finding-blind signed-direction counterexample is fixed by one account-order-to-economic-side mapping shared by single and batch bookkeeping. An eight-world terminal matrix crosses both signed directions with single/batch CPI/no-CPI, while a second eight-world multi-asset matrix crosses both mixed-leg orders with sequential/batch CPI/no-CPI under asymmetric fee collection. Together they prove independently reconstructed side budgets, winner payout, user exit, terminal custody, and route equivalence. Kani exhausts all full-width fee pairs and every signed direction for the pure mapping; INV-022 separately proves strict signed-cap wire preservation. |
...
| INV-039 | Independent + Direct + SVM/CU | `public_sbf/inv_039_pending_loss_obligation_durability.rs`, `stateful/inv_039_pending_loss_obligation_durability.rs`, and `cu/inv_041_deterministic_allocation_and_caller_order_independence.rs`; the four-party Recovery matrix exhausts all landing orders while independently checking retained weight and pending counts. Prospective accrual across all four trade routes, pending-mark terminal resolve, zero-effective-price-move terminal funding, shutdown-funding ordering, and accrual before CPI close, batch CPI close, unilateral reduction, or Recovery forfeit all execute paired public worlds through complete resolution, recompute victim and coalition payouts from both traces' destination-token deltas, and require terminal portfolios plus classifier-bound exact loss or bounded exit. Recovery forfeit's two-atom aggregate residue is derived from two positive terminal claimants, with no destination gain and at most one floor atom lost per claimant; it fails certification under a one-atom aggregate bound and cannot classify as LoF. A shared bounded round-robin drain revisits claimants after other users progress and rejects nonterminal fixed points. INV-027 owns the cross-cutting stale-cohort novation guard and finite-settlement certification. |
...
Audit last reconciled: 2026-08-26. The answer to "is every invariant exhaustively proven or tested as much as
...
The current ledger is 19 **CLOSED**, 36 **OPEN-T**, 15 **OPEN-D**, 5 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-018 | PARTIAL | SPL custody substitutions are extensive. A real Token-2022 transfer-fee/transfer-hook mint rejects at both admission routes, its executable program rejects on a live value route with exact rollback, and six primary-decimal worlds prove exact raw-atom accounting. Finding-blind public oracles compare actual SPL movement with internal quote deltas on all 15 source-complete token-moving handlers, including independently generated backing-provider earnings. The wrapper parser side is now canonical by construction: one source-locked classic-SPL account unpack gateway feeds the user/vault/withdrawal validators, and handlers reuse the returned balance and destination-encumbrance facts. Closure still needs a formal equivalence assumption or theorem for SPL Token's trusted `Account::unpack` and downstream CPI execution semantics. |
...
| AUDIT-024 | PARTIAL | Aggregate conservation is supplemented by a 32-world public route-pair matrix that proves exact realized-PnL ownership through settlement, route-switched close, conversion, and SPL withdrawal for both sides. All 49 public-trace consumers now share an external token-flow contract: every changed tracked SPL account is writable and authority-attributed, quote deltas balance exactly through one configured vault authority, and wrong-owner or one-atom corruptions fail. The shared paired-terminal contract additionally reconstructs victim and counterparty payouts from exact destination-account deltas for prospective accrual, accrual/removal ordering, terminal mark commit, zero-move terminal funding, and shutdown funding ordering. The remaining gap is internal engine-domain attribution for every value-bearing transition plus formal whole-route composition; exact rejected-route snapshots already exist. |
...
| AUDIT-026 | PARTIAL | A common independent census checks account-local face/backing classification and every market bucket/reservation equality after each generated public step. One 16-world public matrix requires nonzero counterparty-lien creation and exact terminal release/consumption for every trade family, source side, and Resolved/Recovery path; Recovery separately proves exact claim consumption, surviving-leg lien retention, and bounded release after the live leg closes. A second eight-world matrix crosses routes/sides through live expiry and impairment, exact stale-route rollback, and bounded owner reduction. Internal fault/retry and double-use transitions, pending obligations, and close reserves remain. Insurance-backed lien lifecycle is wrapper-unreachable and owned by INV-033's engine contracts plus source-complete absence guard. |
...
| AUDIT-028 | OPEN-T | Source reversal, expiry, rounding, sparse capacity, omitted backing, and the reciprocal `A-backs-B-backs-A` control now have public-route owners. The cyclic matrix proves recertification nets the adverse leg before credit use and backing without a claim remains unusable across all trade families and both close orders. A finding-blind two-order matrix exposed estimator/consumer disagreement when each source supplied less than one quote atom; engine `592d538c` centralizes per-domain atom rounding, and the public regression plus runtime, randomized, and Kani checks now agree. Exact/late expiry additionally retains a nonzero flat claim after principal exit and proves exact terminal payout, lien cleanup, and account close through configured permissionless resolution rather than mistaking principal-only withdrawal for complete liveness. The public maximum-shape route now constructs all 28 source claims and live liens, proves each one-domain release preserves the remaining attribution, and converts the exact unchanged 28,000-atom claim only after the last lien clears. Engine `422893fa` closes PR267 by consuming backing and burning claims in one source-local loop; generated and fixed-seed public evidence requires exact single use and atomic retry rejection, while symbolic engine proofs cover both lien-bearing flat claims and active source exposure at the production guard. Insurance impairment and a generalized bounded multi-domain transition proof against an independent per-domain consume/burn model remain. |
| AUDIT-029 | OPEN-T | The exact public claim census exhausts 16 fixed lifecycle cells over min/max positions, odd/even partial-burn edges, and both claimant orders, while generated seeds cover interior price moves. The shared terminal oracle observes genuine partial receipt creation and proves exact-face receipts replace precisely their recorded prior unreceipted contribution without increasing total claim mass; the receipt then moves SPL value and remains terminally live. Eight pure-funding worlds cover every trade route and both position orientations with zero mark movement, exact favorable-claim attribution and burn, aggregate principal conservation, and unrelated-user frames. Eight underfunded stale-price worlds independently bind terminal snapshot admission to complete stale/stored-position settlement, exact claim materialization, and exact principal/junior partition. Every generated public transition requires exact claims to equal their bounds, and a pin-bound source lock excludes non-exact wrapper ingress. Approximate-bucket rebucketing is N/A for this deployed profile; the complete production state graph remains. |
| AUDIT-030 | OPEN-T | The independent rate oracle covers claim/add/expiry/reduce/refill. A public eight-world route/source-side matrix covers real live-lien impairment, exact valid-to-impaired relabeling, zero post-impairment credit, stale-route rollback, and owner-reduction liveness under both independent censuses. The exact/late flat-claim matrix proves impairment cannot create live conversion, yet configured terminal reconciliation pays the retained claim exactly and clears every impaired aggregate. A generic before/after oracle wraps every generated public action and successful crank for both markets, requires formula-input mutations to advance `credit_epoch`, forbids rate changes with unchanged inputs, and permits a live-claim rate increase only when independently available backing rises (including valid encumbrance release) or the claim bound falls. Twenty malformed relation cases plus two exact omission boundaries fail closed across both source sides. A pin-bound source lock proves the wrapper has only four host-codec record copies and no independent rate/epoch writer; INV-088 separately owns every wrapper-to-engine callsite, leaving internal mutation arithmetic to the exact engine contracts rather than duplicating it. Remaining closure is exhaustive public-state reachability beyond the generated action/state corpus. |
| AUDIT-031 | OPEN-T | Shared credit and insurance rails are tested. A 32-world public route-pair matrix supplies the atom-ownership oracle for repeated live-lien creation, same-frontier alternate-route retry, exact rollback, and bounded release across all single/batch CPI/no-CPI combinations and both source sides. The added 16-world multi-account composition gives two portfolios nonzero simultaneous claims on one source bucket while taking risk in different assets; all routes, source sides, and account orders preserve the exact account-sum/source/bucket equality, reject both frontier retries atomically, and restore the original pool in finite cranks. The four-route haircut matrix now covers a real late failure: a one-atom-low cap rejects after the conversion call with complete rollback, while the exact-cap retry consumes the same claim and backing once. These matrices found no current violation. Exhaustive internal fault injection and the engine-only insurance-backed lien path remain. |
| AUDIT-032 | OPEN-T | A common independent census now recomputes account, source, and backing-bucket ownership after each generated transition. Public matrices cover create/grow/rejected retry/release across 32 route-pair worlds, impairment across all eight route/side worlds, and terminal consumption with exact provider-label retirement. The four-route half-backed conversion matrix now forces a real post-conversion cap failure, requires exact rollback, and proves the valid retry consumes the claim/backing ownership once. Recovery and the other internal failure points remain; direct insurance-backed lifecycle is still a wrapper-API gap. |
| AUDIT-033 | CLOSED | The wrapper exposes counterparty-backed liens but intentionally exposes no insurance-credit reservation route. The public control creates a real counterparty lien and proves all insurance categories stay zero; a separately funded insurance-only attempt rejects with exact rollback and cannot silently reserve or consume insurance. A source-complete guard proves the wrapper calls none of the five engine reservation/lien mutators and binds the claim to exact engine pin `6c8d94bcacb8110c8e86ce1b1fc46594bb672a7c`. That pin's create, live-release, terminal-release, impairment, and consume contracts were rerun and establish lockstep source/reservation classification while leaving counterparty backing untouched. This is closed by public unreachability plus engine proof, not by adding a redundant wrapper API; any new callsite or pin reopens it. |
| AUDIT-034 | OPEN-T | A finding-blind public multi-asset campaign independently found that detaching the first loss-bearing leg discarded source-domain attribution and let a later liquidation spend 100,100 atoms of unrelated insurance for 95,500 atoms of coalition profit. Engine `9b737fd` fixes the central detach/liquidation semantics without new persisted state: the existing unattributed-loss lock survives while uncovered loss and other risk coexist, and liquidation under that lock can reduce risk but cannot move insurance, B, explicit loss, or fees. Exact-SBF evidence now proves strict crank progress, zero foreign spend/profit, exact settled rollback, bounded owner exit, and SPL conservation; Kani proves the lock predicate and uncovered-loss postcondition. Closure still requires a generated roster covering every public instruction with every type-correct cross-market/domain account substitution and normalized success/rollback oracles. |
...
| AUDIT-039 | OPEN-D | Many accrual-before-weight-removal routes are covered, and INV-027 now certifies stale-cohort novation rejection plus finite discharge. The public four-party Recovery matrix exhausts all owner landing orders and checks retained loss weight and pending counts after every removal. Prospective trade/catch-up, pending-mark resolve, zero-move terminal funding, shutdown-funding ordering, CPI close, batch CPI close, unilateral reduction, and Recovery forfeit now share paired terminal traces with exact destination-token payout reconstruction, complete public resolution, terminal users, and classifier-bound loss/bounded-exit evidence. Recovery forfeit's no-gain terminal residue is derived from two positive claimants with at most one floor atom per claimant and fails under a tighter aggregate bound. Reset, account close, partial liquidation, and remaining obligation-removal transitions still need the common obligation-before-removal state machine. |
...
83 are `Certified`, 8 remain `Quarantined`, 8 are `Nonqualifying`, and none are `Missing`. Its
...
On engine pin `6c8d94bcacb8110c8e86ce1b1fc46594bb672a7c`, the full 897-test `v16_cu` inventory is
