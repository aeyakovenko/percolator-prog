...
| AUDIT-072 | OPEN-T | A public three-asset matrix exhausts all 40 hint words through length three, including every bounded subset, ordering, and duplicate placement, plus selected out-of-range, malformed/absent oracle, and unclaimed account tails. Every case rejects atomically or lowers rank before an honest completion to rank zero. The backing-expiry matrix proves that an authenticated late slot with no observation hint still discovers bounded source-lien progress; this caught and fixed a wrapper/engine composition path that classified against the stored slot. A nine-world hybrid-oracle matrix reaches ResetPending/Recovery publicly and crosses absent, zero-account, stale-profile, malformed, overdeclared, missing, unclaimed, duplicate, and out-of-range tails through exact rollback or identical detach/restart/exit outcomes. The active-close frontier proves complete and empty hints remain order-robust across both sides and all close-expiry boundaries; it independently found and fixed irrelevant already-settled terminal hints and stale Live hints landing after Recovery declaration. The oracle-failure frontier adds all 366 missing/wrong-owner/stale/fresh/empty-hint orderings before/equal hard-stale maturity and caught the lagging checkpoint plus discarded-normalization locks. The ResetPending frontier separately crosses complete/empty stale account cranks before and after every other modeled public action; rejected stale landings roll back exactly and canonical side finalization remains discoverable. Extend equivalence over every other account-actionable crank class, remaining Active/DrainOnly multi-feed parser permutations, account aliases, and maximum tail shape. |
