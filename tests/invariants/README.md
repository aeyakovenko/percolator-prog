...
explicit bound, and then exposes a successful B-settlement continuation. This is net-new concrete
summary-fidelity and overlap evidence; it found no new production violation and adds no deployed
code or state.
...
| Focused INV-071/072/082 concrete close/B selector overlap | 1/1 public two-asset world plus 14/14 focused INV-071 and 6/6 focused INV-056 CU tests | one portfolio simultaneously carries a real active-close residual and a real B-stale leg created exclusively by public trades, marks, cranks, shutdown, and owner forfeit. Every close call strictly decreases the higher-priority residual while byte-framing the B leg and SPL custody; after finite close completion, the deferred B continuation succeeds below the CU ceiling. |
...
| `cargo test --test v16_program_fuzz_regressions` | 106/106 | full exact engine-`6c8d94bc`/SBF-`a76dd082` rerun completed in 27.63 seconds on `7497a8cc`; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 896/896 | full exact engine-`6c8d94bc`/SBF-`a76dd082` rerun completed in 207.54 seconds after the public close/B overlap increment; zero behavioral failures across reserve, fee-consent, oracle, Recovery, terminal, lifecycle, source-lien normalization, selector composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling. |
...
normalization and dispatch; wrapper production code is unchanged, and the complete 896-test CU
suite remains green.
...
1. Generalize the concrete actionability-fidelity oracle that exposed the mixed-expiry DoS. For
   ...
   termination and deferred-B progress. Continue with publicly reachable close/source-lien,
   expired-close/recovery, and stale/lower-priority overlaps; replace any apparently impossible
   pair with a production-builder unreachability proof rather than injected state.
...
On engine pin `6c8d94bcacb8110c8e86ce1b1fc46594bb672a7c`, the full 896-test `v16_cu` inventory is
