...
INV-089 charter. It is a baseline, not completion of the expanded goal. Requirements 6 and 7 now
have the current-surface proof-equivalence closures described below; requirements 8-10 remain open
...
Two assumption-free Kani theorems execute the pinned engine's actual auto-crank selector: one uses
...
`NoAction` after at most seven class-completion steps. The existing source-complete liveness gate
...
modules: 174 direct harnesses plus 38 macro-generated trade-decoder harnesses, exactly matching the
212 reported by `cargo kani list`. Every direct harness is classified into 103 symbolic-total, 32
...
| INV-073 | CLOSED - Independent + P(engine/wrapper composition) + F + SVM/CU + R + C | Public routes cover DrainOnly retirement, Recovery owner/nonowner exits, stale-market resolution, claimant schedules, bankruptcy obligations, ADL cleanup, source/backing/oracle failure, and funded terminal disposition across all bounded frontiers. The shared runner has no quarantine and propagates progress and owner-exit failures independently. INV-071 source-locks all nine progress classes, ten plan shapes, twenty-nine engine postconditions, and twenty-two wrapper witnesses; the INV-082 selector/rank induction closes arbitrary sequence length and class overlap, while INV-077 proves each required step executable at supported maxima. Closure is conditional on authenticated time/oracle/recovery availability, fair submission, SVM rollback, validated engine state, and the named arithmetic axiom. A new funded-state class, terminal rail, selector/dispatch path, pin, or supported shape reopens it. |
...
| INV-082 | CLOSED - P(engine/wrapper composition) + F + SVM/CU + R + C | `kani/inv_082_state_indexed_liveness_theorem.rs` executes the pinned engine's actual selector over arbitrary full-width magnitudes and proves each non-fixed-point choice strictly lowers the finite seven-class lexicographic rank under its named engine continuation postcondition. A second theorem exhausts all `2^8` actionable-summary overlaps and reaches `NoAction` in at most seven class-completion steps. `cu/inv_071_crank_progress.rs` source-binds that induction to all ten selector shapes, nine concrete progress classes, twenty-nine engine contracts/proofs, and twenty-two public witnesses; bounded public graphs independently reconstruct rank and maximum-shape routes establish CU. Closure assumes classifier/postcondition fidelity on validated engine state, authenticated inputs, fair submission, SVM rollback, and the named arithmetic boundary. Any new rank class, selector/dispatch shape, engine pin, public ingress, or supported shape reopens it. |
...
| AUDIT-082 | CLOSED | The public graph independently reconstructs and decreases close, B, K/F, obligation, reset, source-lien, health, Recovery, and resolved ranks across every seeded ordering and documented overlap. Two assumption-free Kani proofs remove the finite-depth limitation: the first executes the actual pinned selector over arbitrary full-width magnitudes and proves every non-fixed-point result strictly lowers the seven-class lexicographic rank under the exact named continuation postcondition; the second exhausts all `2^8` actionable summaries and proves class-priority convergence to `NoAction`. The source-complete INV-071 gate connects those theorems to all plan shapes, engine contracts, wrapper dispatch witnesses, and supported maximum shapes. Closure is conditional on classifier/postcondition fidelity for validated state, authenticated inputs, fair submission, SVM rollback, and the named arithmetic boundary. A new actionable flag, rank class, plan/dispatch path, pin, or supported shape reopens it. |
