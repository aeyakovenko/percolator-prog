...
without consuming that second episode, and then charges exactly once again. INV-009's one-shot
episode-consumption composition and INV-011's exact-fill aggregate batch caps close the execution
half without adding a redundant persistent intent ledger. A durable or persistent partial-intent
API would reopen the invariant.
...
| AUDIT-059 | CONDITIONAL | `PermissionlessCrank` is the sole public liquidation ingress and exposes no close quantity; the engine selects the minimum health-restoring close. Sub-minimum partials reject, a full residual close may pay the configured minimum once, accepted partial fees are proportional/capped, and sixteen fixed-point retries are exact no-ops. A two-episode public matrix proves a new fee requires a new authenticated mark plus certified deficit, while a malformed intervening call rolls back exactly; both charges match an independent oracle. On the execution side, single-CPI partials consume both one-shot episodes and stale retries frame exactly; a residual is newly signed, while exact-fill batch CPI enforces aggregate fee/slippage caps. Persistent partial authorization, caller-sized liquidation, or durable-nonce economic consent reopens this row. |
