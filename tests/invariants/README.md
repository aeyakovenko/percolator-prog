...
census owns 191 harnesses across 22 mounted modules, and the public-trace guard owns 49 consumers.
...
tests. The mounted wrapper proof census is 191 harnesses; exact final rerun results are recorded in
...
theorem. The exact current artifact passes 903/903 CU tests; the full Kani result is recorded below.
...
inventory of all 13 explicit `kani::assume` sites across all 22 mounted wrapper Kani modules. A
...
| Focused INV-083 field-complete boundary inventory | 10/10 CU plus 191/191 wrapper Kani composition | all 234 fields across 52 public input types are source-locked into 20 semantic boundary profiles with per-field and profile-level executable evidence; the class roster retains all eleven required classes, and 25 public invalid `InitMarket` partitions reject with pristine-account rollback before a valid retry creates a usable portfolio |
| Focused INV-084 proof-harness nonvacuity inventory | 3/3 CU plus 13/13 assumption-focused Kani and 191/191 wrapper Kani composition | a source parser derives all 155 direct and 36 generated harnesses and locks 89 symbolic-total, 27 branch-witnessed, 10 explicitly constrained, 29 concrete-exact, and 36 generated-symbolic dispositions; all 13 explicit assumptions across 22 modules retain exact ownership and public evidence; public routes reach admitted domains, reject invalid partitions with exact rollback, and terminate with exact custody |
...
| `cargo test --test v16_program_fuzz_regressions` | 101/101 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 27.17 seconds; includes the generic 11-operation whole-market no-reuse matrix, the source-complete account-incarnation and detached-signature censuses, all public regressions, trace consumers, executable registries, and the INV-079 evidence roster |
| `cargo test --test v16_cu` | 903/903 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 201.67 seconds; zero behavioral failures across market tombstone/rent/fresh-address behavior, matcher identity/incarnation and stateless-PDA composition, reserve, fee-consent, oracle, Recovery, terminal, lifecycle, source-lien normalization, selector composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling; the run observed 1,116,641 CU for max-source liquidation and 1,314,291 CU for max-shape resolved close. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 191/191 | full wrapper-only exact-source rerun with engine `d604ca0` completed in 2342.440 seconds; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider/token-parser, deployed-arithmetic, and arbitrary-prestate tombstone harnesses, with required constructive covers satisfied |
...
| INV-084 | P + Source audit + SVM/CU | `cu/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` derives all 191 mounted harnesses, locks their five nonvacuity categories, follows local assertion helpers, requires constructive covers for every branch-limited claim, and binds every concrete-fixture module to public evidence. It separately discovers all 13 explicit `kani::assume` sites across all 22 modules and requires an exact row naming the owning proof, constructive witness, classification, and public evidence. `kani/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` exhausts every finite admitted/excluded partition with boundary mutation killers. Public LiteSVM reaches the valid identity, sequence, episode, fee, enable, and signed-size domains and proves invalid partitions roll back exactly before terminal exit. Any proof-roster or category drift reopens this current-surface closure. |
...
The current ledger is 25 **CLOSED**, 36 **OPEN-T**, 11 **OPEN-D**, 3 **PARTIAL**, 12 **FRONTIER**,
