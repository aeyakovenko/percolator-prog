...
| `cargo test --test v16_program_stateful_fuzz` | 205/205 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun completed in 264.08 seconds; the four public single/batch CPI/no-CPI routes now share an exact normalized nonzero-fee state oracle across fixed boundaries and generated size, side, fee, and seed words, alongside every prior stateful check |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.72 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 28 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
...
| `stateful/` | 205 | Generated public routes with exact rollback on rejection and shared state, custody, OI, source-credit, and liveness oracles on success. Coverage includes randomized retained retries and trade-route switching, exact normalized nonzero-fee equivalence across all four trade route classes, lifecycle and terminal compositions, all 2,380 public action words through depth three, 14 underfunded terminal worlds including both authority/resolve orders, one public two-stage exact receipt-top-up lifecycle, and all 5! basic claimant orders. Per-invariant rows below are the source of truth for uncovered dimensions. |
