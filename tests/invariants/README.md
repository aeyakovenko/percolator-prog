...
position epochs, custody, mint supply, and bounded CU. Public wire
...
| Focused INV-008 retained-operation retry matrix and contracts | 12/12 public, 11/11 stateful, 4/4 local Kani, 4/4 source/layout CU, plus 3/3 real failed-CPI retry probes; all 11 retained families reject stale retries with exact rollback, every family rejects an identical same-transaction duplicate with bundle-wide rollback before exactly one standalone execution, all 50 public variants have a machine-checked replay disposition, both direct/domain insurance cross-entrypoint orders remain atomic, and all 16 ordered single/batch CPI/no-CPI trade pairs are exact-once | the source-locked economic-family partition proves trade and insurance top-up are the only current multi-entrypoint retained families; arbitrary successful partial ratios, expiry, and aggregate signed budgets remain open |
| Focused INV-009 partial-fill and retry accounting | 5/5 public CU (including 12 repeated partitions and a 16-world cross-route residual matrix) plus 1/1 local Kani | configured single CPI partial fills cumulatively book only returned quantity/fees/OI, each consumed request rolls back exactly, every stale route rejects after a real half fill, and every fresh route lands the exact residual; uniform or asymmetric partial CPI batches reject atomically and retain honest full-retry liveness |
...
| `cargo check --tests` | pass | exact engine-`b10b3454` build completed in 6.16 seconds on the 2026-08-21 PR135 production head |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.15 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
| `cargo test --test v16_cu` | 847/847 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun passed in 196.15 seconds on the 2026-08-21 checkpoint; zero failures, including the 16-world partial-fill stale-route/fresh-residual matrix, exact/late impaired-lien terminal disposition, all-28 simultaneous source-lien chunked release and terminal exit, every prior maximum-shape route, the source-complete INV-085 arithmetic census, bigint boundary/provider references, exact host-to-SBF comparisons, and INV-080 error composition |
...
| `cu/` | 847 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, the 16-world partial-fill cross-route residual matrix, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
| INV-008 | Independent + P + Direct + F + SVM/CU | `public_sbf/inv_008_intent_uniqueness_and_bounded_replay.rs`, `stateful/inv_008_intent_uniqueness_and_bounded_replay.rs`, `cu/inv_008_intent_uniqueness_and_bounded_replay.rs`, `cu/inv_080_error_propagation_and_exact_rollback.rs`, `kani/inv_008_intent_uniqueness_and_bounded_replay.rs`, and `inv_008_replay_disposition.tsv` cover all eleven retained-operation families and classify all 50 public variants. They prove stale-retry rejection, exact rollback, fresh liveness, sequence/episode/watermark invalidation, legacy-schema rejection, all-family same-transaction atomicity, both insurance landing orders, and all 16 ordered trade-route pairs. The source-locked economic-family partition proves trade and insurance top-up are the only current multi-entrypoint retained families. A real half fill additionally invalidates every stale route before every fresh residual route lands exactly. Arbitrary partial ratios, expiry, and aggregate-budget semantics remain open. |
| INV-009 | P + SVM/CU | `cu/inv_009_partial_fill_and_retry_accounting.rs` and `kani/inv_009_partial_fill_and_retry_accounting.rs` prove deployed single-CPI partial-fill accounting and residual liveness, a 12-world integral repeated-partition matrix, and a 16-world stale-route/fresh-residual matrix with cumulative quantity/OI/two-sided-fee equality, exact rollback, custody, epoch, and CU checks. Atomic full-fill-only batch consent and exact matcher-result binding remain covered. Arbitrary matcher-selected ratios and aggregate slippage/expiry/per-intent-minimum-fee semantics remain; the current request schema has no fields for the latter obligations. |
...
| AUDIT-008 | OPEN-T | All eleven retained families reject stale retries and atomically reject same-transaction duplicates before one standalone execution remains live. Both insurance orders and all sixteen ordered trade-route pairs are exact-once. A source-locked partition proves those are the only current multi-entrypoint retained families. A genuine single-CPI half fill invalidates every stale prebuilt trade encoding, after which every fresh route lands the exact residual with cumulative quantity, fee, OI, epoch, custody, mint, and CU checks. Real failed SPL CPIs preserve retry liveness for all three top-up routes, and all 50 public variants retain an explicit replay disposition. Remaining closure is arbitrary successful partial ratios, expiry, and aggregate-budget semantics where applicable. |
| AUDIT-009 | OPEN-T | Public single-CPI coverage accepts explicitly flagged partials. Twelve repeated-halving worlds prove cumulative quantity, OI, two-sided fees, episode consumption, stale rollback, and final residual liveness. A separate sixteen-world matrix rejects every stale single/batch CPI/no-CPI encoding after a real half fill and executes the exact fresh residual through every route. Batch CPI remains full-fill-only and atomically rejects uniform or asymmetric matcher-selected partials; Kani proves that predicate symbolically. Arbitrary matcher-selected single-fill ratios remain tractable. Aggregate slippage, expiry, and one-minimum-fee-per-intent closure require adding the absent request/ledger fields. |
