...
tests, 110 public regressions, 214 stateful tests, 916 LiteSVM/CU tests, and 193 wrapper Kani
...
remaining finding-blind public counterexample runner. A source-complete roster now covers all 32
...
deltas. Fee redirection now has downstream terminal domain-loss quantification; the next
retained-control target is a complete funded route-mask liveness observation for `ResolvePolicy`.
The sealed holdout remains unopened while this generic evidence path is extended.
...
| `cargo test --test v16_program_stateful_fuzz` | 214/214 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 264.86 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets, including generated route selection for all four terminal retained-trade retry worlds, both exact external-debit retry families, the revoked-matcher paired terminal world with a fresh-grant nonvacuity witness, all five oracle-supersession terminal worlds, the exact fee-redirection terminal world, and fee-bearing maintenance/liquidation share worlds. The oracle worlds prove exact stale rollback and fresh terminal victim loss offset by counterparty gain, protected-recipient loss, or protocol-defined mint burn. The suite also owns the complete pending-obligation and close-ledger encumbrance census, direct per-domain `usable_positive_credit_num <= available_backing_num`, exact/bound claim transitions, credit-rate edge causality, and the independent resolved-payout quotient/remainder oracle. A persisted INV-086 seed additionally locks zero-amount withdrawal semantics: an admitted economic no-op may charge only the exact maintenance amount, move no SPL payout, and cannot satisfy value-withdrawal coverage. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
| Registry/manifest checks in the INV-079 module | 14/14 | includes the source-complete 32-oracle evidence roster, 22 exact terminal-LoF dispositions, all 11 retained-retry dispositions, all 14 retained-control supersession dispositions with seven terminal-LoF rows, two attribution-only fee-share rows, and zero terminal-value candidates, complete-route terminal classification, and the recursive 56-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 110/110 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 29.47 seconds; includes the generic 11-operation whole-market no-reuse matrix, deterministic terminal retained-trade, exact external-debit retry, revoked-matcher, all five oracle-supersession terminal worlds, the fee-redirection terminal world, and both fee-share attribution worlds, the source-complete 11-kind retry and 14-kind supersession disposition censuses, the source-complete account-incarnation and detached-signature censuses, all public regressions, 56 trace consumers, executable registries, and the 32-oracle INV-079 evidence roster |
