...
frames the active ledger, finalizes it before snapshot capture, and preserves independent source
claims. Two 32-world public matrices now own the
...
| Focused INV-086 terminal reference composition | 12/12 public terminal worlds, one active-close-to-partial-receipt bridge, plus all 183 depth-two base words | every seed is rebuilt through public top-up, trade, mark, crank, resolve, and close instructions; all claim-priority worlds move real SPL value, exact/late expiry normalize on a recorded edge, route/claimant orders converge under independent position/OI/source-credit/encumbrance/stock/custody oracles, and the separate three-asset bridge preserves an active value-bearing close through resolution before finalizing it into a genuine partial receipt |
...
| `cargo test --test v16_program_stateful_fuzz` | 196/196 | exact engine-`592d538c`/SBF-`909c24cc` rerun in 175.94 seconds, including the public three-asset active-close-to-partial-receipt bridge, both active-close barrier orientations, the 32-world cross-asset active-close admission matrix, the inverse 40-world direct/prior-leg terminal-liability matrix, and paired 32-world active-close/ResetPending and active-close/Recovery-reset rank compositions across every trade route, the independent raw-basis/effective-OI ledger, sub-minimum-ADL and cross-domain fractional-source progress, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, concurrent partial receipts, disjoint- and shared-portfolio lifecycle-local reset/shutdown ordering, retained-reduction/shutdown landing orders, simultaneous two-asset lifecycle ordering, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
