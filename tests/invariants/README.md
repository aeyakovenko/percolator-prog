...
gate rather than a larger duplicate scenario generator. Nineteen independently executable layers
...
wrapper-to-engine calls and wrapper-owned persisted fields, position/OI and scope induction, every
external-token/internal-quote route, the independent transition model, and the complete error
boundary. Four mounted Kani theorems supply the engine-value/wrapper-custody, exact-stock,
certificate-disposition, and engine-error composition steps. The gate pins the engine revision and
...
| Focused INV-081 whole-route success-state composition | 1/1 source-complete composition gate over 19 executable layers, 49/49 public routes, and 4/4 mounted Kani owners | decoder/domain/identity/account/input/admission inventories join all engine calls, wrapper fields, value/stock/certificate/position/scope postconditions, independent model dimensions, and exact error propagation on the immutable engine pin; any omitted public or CU route fails closed. |
...
| INV-081 | CLOSED - P(engine/wrapper composition) + F + Direct + SVM/CU | `cu/inv_081_success_state_validity_over_complete_public_routes.rs` joins twenty source-complete executable layers over all 49 public instructions: decoding/domain, identity/authority, account roles/PDA/caller fields, admission, every engine call and wrapper persisted field, position/OI, scope, token/value, per-episode entitlement, independent model, and error propagation. It additionally source-locks five mounted Kani composition theorems for value flow, owner-episode entitlement, exact stock, conservative certificate commit, and engine-error mapping, and pins the exact engine revision. The generated public model and seeded frontiers retain exact per-step success/rollback, value, OI, episode, lifecycle, terminal, and funded-exit oracles. This is a current-surface proof-equivalence decomposition under the named engine and SVM assumptions, not one monolithic arbitrary-byte query. A route, field, callsite, effect class, proof owner, witness, or pin change reopens it. |
...
| AUDIT-081 | CLOSED | The shared public model directly applies independent success/rollback, value, OI, episode, lifecycle, certificate, terminal, and funded-exit oracles to 25 decoder variants and all generated action/frontier schedules. The remaining 24 variants are not inferred from examples: a new fail-closed gate composes nineteen independently executable source owners over the exact 49-route registry. Those layers own canonical decoding and transaction domain, all identity/authority/account/PDA/caller-field checks, state admission, every wrapper-to-engine call and wrapper persisted field, position/OI and scope induction, all external-token/internal-quote routes, the independent transition model, and every error boundary. Four mounted Kani theorems supply arbitrary value-flow, exact-stock, certificate-commit, and engine-error composition. The gate rejects a route omission and pins engine `495a5590`. This is current-surface proof-equivalence under each owner's named assumptions, not one arbitrary-byte monolithic query; a route, state/effect class, callsite, field, theorem, witness, or pin change reopens it. |
