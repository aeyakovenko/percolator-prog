...
`573c4e900c015a5653b52ab494d10562cbbcb3aa`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`a165aada1e420fa8e08405a80b649be88b79fcbe6af9a29727a6c6733a6a5bfa`. Resolved-mode
...
now covered and fixed. The next broader INV-074 target is the remaining side/domain/receipt
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `573c4e90` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `573c4e90` |
| `cargo test --test v16_program_stateful_fuzz` | 173/173 | full exact-pin rerun in 162.66 seconds |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full exact-pin rerun in 25.86 seconds; the sealed benchmark census accepts the independently expanded post-restart insurance/trading route |
| `cargo test --test v16_cu` | 791/791 | full exact-pin rerun in 191.93 seconds, including maximum-source exits, maximum-shape liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and fresh-generation insurance/restart progress |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 137/137 | full wrapper-only exact-pin rerun on engine `573c4e90`; zero harness failures, including all 6/6 INV-074 withdrawal-scope covers |
| Engine runtime/property suites | 216/216 | rerun with `cargo test --features fuzz` on engine `573c4e90`; whole-body restart success proves 0/4,842 failures with 3/3 covers, restart rejection proves 0/6,519 failures with 1/1 cover, and one-call retirement proves 0/5,629 failures with 1/1 cover over symbolic spent-domain/source, social, K/F, and senior-value inputs |
...
On engine pin `573c4e900c015a5653b52ab494d10562cbbcb3aa`, the full `v16_cu` inventory is
