...
theorem. The exact current artifact passes 909/909 CU tests; the full Kani result is recorded below.
...
| `cargo test --test v16_program_fuzz_regressions` | 101/101 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 36.26 seconds; includes the generic 11-operation whole-market no-reuse matrix, the source-complete account-incarnation and detached-signature censuses, all public regressions, trace consumers, executable registries, and the INV-079 evidence roster |
| `cargo test --test v16_cu` | 909/909 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 197.38 seconds; zero behavioral failures across market tombstone/rent/fresh-address behavior, matcher identity/incarnation and stateless-PDA composition, reserve, fee-policy/destination ownership, claim/backing/insurance single-use, exact lien lifecycle, source-realizability composition, oracle, Recovery, terminal, source-lien normalization, selector composition, equivalent-route source composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling; the run observed 1,116,641 CU for max-source liquidation and 1,314,291 CU for max-shape resolved close. The non-exit 28th-lien admission reached 1,398,554 CU and remains an explicit thin-headroom risk. |
...
The current ledger is 35 **CLOSED**, 29 **OPEN-T**, 11 **OPEN-D**, 0 **PARTIAL**, 12 **FRONTIER**,
