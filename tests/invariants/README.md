...
`78c73bc8f6a7703374d7ca68f9c84ec8293209bc`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`6a56ba5c13a967584f84375a8ef79a52ead12516ec9c6fa53b3bd7d238dbb74e`. Resolved-mode
...
assets. Its next tractable partition is explicit reachability/unreachability arguments for
ResetPending, Resolved, and Retired exposure, followed by wider interior quantity generation. The next broader
...
| Focused INV-050 scalar, lifecycle, and active-close barrier matrix | 4/4 scalar route worlds, 8/8 DrainOnly/Recovery route worlds, 8/8 stateful single-barrier route/orientation worlds, and 4/4 simultaneous two-asset route worlds covering 8/8 barrier-asset cells | zero rejects exactly; one-atom reduction/flip/close and exact maximum open/close land below route CU caps; max+1 rejects exactly; both exit-only modes reject flips while exact closes remain live; long and short barriers reject reissue, frame every concurrent close, retain exact account-local loss obligations, release through the actual owner accounts' automatic cranks, and preserve post-cleanup withdrawal |
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `78c73bc8` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `78c73bc8` |
| `cargo test --test v16_program_stateful_fuzz` | 185/185 | full engine-`78c73bc8`/SBF-`6a56ba5c` rerun in 172.68 seconds, including both active-close barrier orientations across every trade route, the independent raw-basis/effective-OI ledger, the persisted sub-minimum-ADL discovery seed, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`78c73bc8`/SBF-`6a56ba5c` rerun in 26.93 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 795/795 | full engine-`78c73bc8`/SBF-`6a56ba5c` exact-source rerun in 189.18 seconds, including all maximum-source and maximum-shape exits, exact maximum cross-zero quantity boundaries, all-route DrainOnly/Recovery admission, post-ADL effective conversion, liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and every cross-invariant progress/rollback audit |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 137/137 | full wrapper-only exact-pin rerun on engine `78c73bc8`; zero harness failures, including all declared nonvacuity covers |
| Engine runtime/property suites | 184/184 base and 231/231 with `--features fuzz` | exact rerun on engine `78c73bc8`. All unit, reference-model, arithmetic-discharge, source-credit, insolvency, and v16 specification tests pass, including sub-minimum-A DrainOnly refresh/exit and full nonzero-current-A rounding partitions |
...
| INV-050 | Independent + SVM/CU + F + M + Regression | `cu/inv_050_cross_zero_decomposition.rs` covers lifecycle exact-close admission, initial-margin flips, and all four public trade routes after a real partial liquidation. The post-ADL matrix holds the signed Flip request fixed and varies only unrelated auxiliary OI: both preflight branches reject raw-basis reissue, and the aggregate-admitted branch separately rejects effective exposure plus one at the account-local ADL gate with exact market, portfolio, matcher, SPL, and lamport rollback. The independently ceiled exact effective exposure remains trade-closeable on every route and removes the retained raw leg without minting value. A separate all-route scalar matrix covers zero, one-atom same-side reduction, one-atom cross-zero, exact close, exact `MAX_TRADE_SIZE_Q` open/close, and max+1 atomic rollback under measured CU. `stateful/inv_050_cross_zero_decomposition.rs` publicly creates both long- and short-domain bankruptcy-close barriers for every route: cross-zero reissue rejects exactly, the same-side pair exit clears effective OI without rewriting the close, the matching-side raw leg becomes a zero-basis obligation, and the retained cure plus owner-specific automatic crank releases it in bounded work. The CU matrix additionally composes both orientations simultaneously on two Active assets through every route, requires exact close-ledger framing and bounded multi-asset exits, releases all independently owned obligations, and proves post-cleanup withdrawal. Interior full-domain generation and formal unreachability for lifecycle modes that cannot retain exposure remain. |
...
| AUDIT-050 | OPEN-T | The engine-134 fix is integrated atop the canonical-accrual engine and the public all-four-route matrix is inverted: both OI preflight branches reject fresh-basis reissue with exact rollback, including preexisting auxiliary OI that reaches the common ADL gate. Each rejection retains a bounded owner reduction, and focused engine proofs establish route-complete admission. Zero/one/max/max+1 quantities, account-local effective/raw boundaries, both single-barrier orientations, two opposite-side barriers active simultaneously across two assets, and Active/DrainOnly/Recovery route admission are now public-route covered with exact rollback, bounded exits, obligation attribution/release, and withdrawal. Add interior full-domain generation plus explicit ResetPending/Resolved/Retired exposure reachability or unreachability arguments. |
