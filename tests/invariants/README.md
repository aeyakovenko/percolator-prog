...
853/853 LiteSVM/CU tests, 100/100 public fuzz regressions, and 206/206 stateful/model tests. Static
...
No open invariant is promoted solely because code was consolidated or one deterministic lifecycle
was made public.
...
| Focused INV-019 matcher return-data provenance | 19/19 public CU plus 1/1 local Kani | a second program's nested return before the configured matcher is superseded and remains live; the same nested return after the matcher rejects with exact rollback because the producer is not the configured matcher. A separate fully public lifecycle closes and recreates the same wrapper portfolio and external-program matcher context, writes a stale old-incarnation response through the matcher, proves the next wrapper CPI rejects with exact market/portfolio/context rollback, and then proves a fresh response remains live. |
...
| INV-019 | P + SVM/CU | `kani/inv_019_cpi_invocation_and_return_data_binding.rs` and `cu/inv_019_cpi_invocation_and_return_data_binding.rs` prove full-width matcher field/flag binding, request freshness, single-context and batch return replay rejection, tail isolation, and current-producer provenance. A distinct nested program's return is harmless before the configured matcher overwrites it but rejects with exact rollback when emitted afterward. A deployed external matcher now publicly closes and recreates the same context while the wrapper publicly closes and reinitializes the same LP address; a publicly written stale response rejects exactly before a fresh response succeeds. Oracle routes consume authenticated account data rather than CPI return data. Generated stateful reconfiguration/context-incarnation sequences across single and batch CPI remain. |
...
The current ledger is 11 **CLOSED**, 43 **OPEN-T**, 16 **OPEN-D**, 6 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-019 | OPEN-T | Matcher return fields, stale data, req_id, tails, local validation, and nested-CPI producer ordering are covered. A second program's return before the configured matcher is superseded and live; replacement after the matcher rejects with exact writable-state rollback. The former injected ABA setup is replaced by an external program's public context init/close/recreate/configure path composed with public LP close/reinit: the old-incarnation response rejects exactly and a fresh retry succeeds. Oracle paths read authenticated accounts rather than CPI return data. Add a generated stateful campaign that composes repeated context incarnations and matcher reconfiguration with both single and batch CPI before closure. |
