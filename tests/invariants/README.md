...
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun completed in 255.27 seconds; the independent exact-receipt/unreceipted-portfolio-bound oracle, 224 bidirectional supersession worlds, and two value-moving underfunded authority/claim worlds join every prior stateful check |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.26 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
...
| `stateful/` | 202 | Generated public routes with exact rollback on rejection and shared state, custody, OI, source-credit, and liveness oracles on success. Coverage includes randomized retained retries and trade-route switching, lifecycle and terminal compositions, all 2,380 public action words through depth three, 14 underfunded terminal worlds including both authority/resolve orders, and all 5! basic claimant orders. Per-invariant rows below are the source of truth for uncovered dimensions. |
