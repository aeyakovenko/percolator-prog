...
change exactly once, and SPL supply remains conserved. Public wire
...
| `cargo check --tests` | pass | exact engine-`b10b3454` build completed in 6.19 seconds on the 2026-08-21 PR135 production head |
| `cargo test --lib` | 7/7 | exact engine-`b10b3454` rerun on the 2026-08-21 PR135 production head |
| `cargo test --test v16_program_stateful_fuzz` | 198/198 | exact engine-`b10b3454`/SBF-`342f7bb5` rerun in 263.98 seconds, including randomized same-transaction retained-intent duplication, randomized switching across all single/batch CPI/no-CPI trade-route pairs, all 2,380 public base words through depth three and their 6,942 independently checked edges, the 32-world live source-lien route-pair ownership matrix, four independent flat-source-lien route worlds with complete terminal observations, source-complete validation of every current public trace consumer, and every prior rank/reference-model, lifecycle, source-lien, terminal, and route-composition check |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.32 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
| `cargo test --test v16_cu` | 846/846 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun passed in 198.28 seconds on the 2026-08-21 checkpoint; zero failures, including exact/late impaired-lien terminal disposition, all-28 simultaneous source-lien chunked release and terminal exit, every prior maximum-shape route, the source-complete INV-085 arithmetic census, bigint boundary/provider references, exact host-to-SBF comparisons, and INV-080 error composition |
...
| --- | ---: | --- |
| `public_sbf/` | 95 | Deterministic public SBF/LiteSVM counterexamples, regressions, decoder corpora, trace-schema checks, and manifest checks, including normalized public terminal classification, source-complete trace-consumer validation, paired-world conversion-retry rejection with unchanged victim payout, fixed-pin rebalance-retry rejection, issue-402 delayed-close red/green plus failed-deposit rollback, asset-authority/lifecycle generation replay plus exact activation-frontier controls, all eleven retained-operation families with stale-retry rejection, same-transaction duplicate atomicity, fresh-intent liveness, and both direct/domain insurance cross-entrypoint orders |
| `stateful/` | 197 | Proptest-generated public routes, including randomized same-transaction retained-intent duplication, the composite-oracle exact-epoch rejection/ignore/control matrix, Hybrid terminal payout, three-asset active-close-to-partial-receipt bridge, the 16-world pending-target/funding/withdrawal composition, the 32-world clock-first/trade-first mark-discovery matrix, the 40-world aggregate/equal-two/asymmetric-three source-lien expiry matrix, the 32-world live source-lien route-pair ownership matrix, concurrent partial receipts, cross-domain fractional-source liveness in both asset orders plus a minimized three-mark crank trace, 32 lifecycle-local reset/shutdown versus unrelated-exit schedules across disjoint and shared portfolios, 16 retained-reduction/shutdown landing-order worlds with an exact Recovery fallback, 128 simultaneous two-asset lifecycle worlds over every route/side/order combination, and 32 active-close cross-asset admission worlds. The suite also covers generated authority and permissionless-stale resolution, all three resolved payout rails, retained-operation retry and ABA matrices, complete stock/encumbrance censuses, all four trade routes, claim/liquidation/insurance partitions, Recovery and ResetPending lifecycles, authenticated expiry, bounded crank ranks, all 2,380 public action words through depth three, 12 underfunded terminal reference worlds, and all 5! claimant orders. Every modeled rejection uses exact program/SPL/lamport rollback; every modeled success runs the shared state, custody, OI, source-credit, and liveness oracles. The detailed per-invariant rows below remain the source of truth for dimensions not yet covered. |
...
| INV-008 | Independent + P + Direct + F + SVM/CU | `public_sbf/inv_008_intent_uniqueness_and_bounded_replay.rs`, `stateful/inv_008_intent_uniqueness_and_bounded_replay.rs`, `cu/inv_008_intent_uniqueness_and_bounded_replay.rs`, `cu/inv_080_error_propagation_and_exact_rollback.rs`, `kani/inv_008_intent_uniqueness_and_bounded_replay.rs`, and `inv_008_replay_disposition.tsv` cover all eleven retained-operation families and classify all 50 public variants. They prove stale-retry rejection, exact rollback, fresh-intent liveness, sequence/episode/watermark invalidation, intentless legacy-schema rejection, cross-route insurance ordering, same-intent liveness after a real failed SPL CPI, and bundle-wide rollback when each retained family is duplicated in one transaction before exactly one standalone request lands. Both shared-watermark insurance routes are additionally bundled in either order. Direct and domain insurance top-ups share one per-asset watermark; backing top-up has a separate lane. The source-bound gate requires every retry and supersession generator kind to map to a public route. Cross-entrypoint bundles beyond insurance, remaining partial-failure partitions, expiry, and aggregate-budget semantics remain open. |
...
| AUDIT-008 | OPEN-T | The current public retained-operation roster covers eleven families and reports no duplicate-execution violation. Deposit/withdraw, conversion/rebalance, all four trade routes, retired-slot activation, both insurance top-up routes, and backing top-up each reject a stale retained request with exact rollback and admit a fresh request. Every family now duplicates one request in a single transaction: the second guard aborts the complete bundle, exact economic state and SPL supply roll back, one standalone request remains live and bounded, and the remaining duplicate rejects exactly. The two insurance routes additionally cross entrypoints in both landing orders. All sixteen ordered single/batch CPI/no-CPI trade pairs share one retained economic pre-state: each bundle rejects and rolls back exactly, one first route lands below the transaction ceiling, the alternate route rejects, and basis/OI/SPL supply prove exact-once economics. Real failed SPL CPIs preserve an identical retry for all three top-up routes. A source-complete registry classifies all 50 public variants and proves exact correspondence with every retry and supersession generator kind, so a new public route cannot bypass replay ownership silently. Remaining closure is non-trade cross-entrypoint bundles, the other partial-failure permutations, successful partial-fill partitions, expiry, and aggregate-budget semantics where applicable. |
