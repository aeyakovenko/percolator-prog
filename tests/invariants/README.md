...
not only this fixture. No production defect was found. One-field receipt identity substitutions,
cross-portfolio replay, asset-slot reuse, and recovery/episode compositions remain.
...
| Focused INV-068 split receipt top-ups | 1/1 public two-asset/two-source lifecycle plus shared route oracle | two authenticated source releases produce two positive exact top-ups on one immutable partial receipt; each paid-counter delta equals its SPL payout, three immediate retries are exact no-ops, all five portfolios terminate, and engine/SPL vault custody reconciles |
...
| `cargo test --test v16_program_stateful_fuzz` | 203/203 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun completed in 255.06 seconds; the independent exact-receipt/unreceipted-portfolio-bound oracle, public two-stage exact receipt top-ups, 224 bidirectional supersession worlds, and two value-moving underfunded authority/claim worlds join every prior stateful check |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.30 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
