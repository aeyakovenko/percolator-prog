...
The prior 87 `CLOSED` / 2 `N/A` audit ledger recorded closure of the original INV-001 through
INV-089 charter. The 2026-09-03 cross-lane matcher-grant counterexample disproved that blanket
claim for the pinned public surface: explicit enable/disable supersession was covered, but a
retained enable could cross an automatic revoke caused by a bilateral position mutation. The
public parent trace then created unsigned LP exposure and transferred 10,000 quote atoms from the
LP to the attacker. INV-004, INV-008, INV-010, INV-012, INV-047, INV-062, and INV-081 are reopened
until the position-epoch binding below is merged and certified. More generally, a route, role,
state dimension, supported shape, holdout finding, or assumption change reopens the owning row;
individual ADV/SUG regressions remain corroborating evidence rather than substitutes for generic
history and authority properties.
...
matcher, and unsynchronized position mutations clear it together with the grant. The signed tag-68
payload also binds the current position epoch. This closes the distinct cross-lane replay in which
an old enable retained the unchanged matcher sequence, landed after automatic revocation, and had
the handler copy the new epoch into stale consent. The exact-parent public trace performs a signed
open/close round trip, replays the old enable, creates unsigned LP exposure, settles an honest
adverse mark, and withdraws an exact 10,000-atom attacker gain and independent LP loss. The fixed
route rejects before mutation and a freshly signed current-epoch grant succeeds. Public single and
batch routes also succeed one slot before expiry, reject at and after expiry with exact market,
portfolio, matcher-context, token, and lamport frames, and recover only after a fresh owner grant.
Full-width Kani proves the enabled/fee/expiry predicate and preserves every tag-68 wire field; the
prior 28-byte schema rejects rather than being reinterpreted. The previous account length reads
expiry as disabled and grows zero-initialized, and the measured config/CPI routes remain below
their CU limits. The exact rebuilt default-feature SBF is 1,259,208 bytes with SHA-256
`7b8f3035f880876ab7388dfcdbef73bef5dc7ca6c43dd2013f10574ce62b854a`.
...
| Focused INV-012 capability scope, incarnation, episode, and expiry | 17/17 focused CU routes, 2/2 stateful public traces, 1/1 source-complete route roster, 2/2 capability Kani, and 4/4 matcher wire Kani | both CPI routes consume the exact program/context/delegate/config-sequence tuple plus persisted authenticated expiry before CPI. Public partial liquidation, force-close/reuse, and no-CPI mutation clear the grant; configured CPI preserves only its participating LP. Tag 68 now binds the position epoch as well as the independent matcher sequence, so a retained enable cannot cross automatic position-mutation revocation. The exact-parent trace realizes a 10,000-atom victim loss and attacker gain; the fixed route rejects with exact rollback and admits a fresh current-epoch control. Old-grant ABA and at/after-expiry requests also roll back exactly, one-slot-before and fresh post-regrant controls install real exposure, and prior-layout accounts read the appended expiry lane as disabled before zero-initialized growth. |
...
The current ledger is 87 **CLOSED**, 0 **OPEN-T**, 0 **OPEN-D**, 0 **PARTIAL**, 0 **FRONTIER**,
...
1. The charter requests `P` for 76 invariants, `F` for 85, `I` for 66, `M` for 32, `R` for 22,
   ...
   obligations: all 32 `M` rows, all 22 `R` rows, and both `C` rows are current-surface `CLOSED` by
   executable coverage or proof-equivalence composition. The final four `M` rows close through
