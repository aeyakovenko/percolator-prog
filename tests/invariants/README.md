...
Separate provenance fields absent from the close ledger remain an honest INV-037 specification
gap.
...
complete exact-artifact gates cover 892 LiteSVM/CU tests, 102 public fuzz regressions, and 206
...
| `cargo test --test v16_program_stateful_fuzz` | 206/206 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 260.08 seconds; includes the 16-world two-account shared-source reservation composition, four-route late conversion rollback, half-backed, pending-close, eight-world pending-domain principal attribution, independent resolved-payout seniority, and both INV-037 partition oracles. The public INV-037 matrix retires 1,000 junior face atoms against exactly 251 realizable support atoms on every trade route; the mutation oracle rejects every one-atom value-term error and ignores face-only changes. Every public-trace token-moving step composes checked quote conservation with owner/vault attribution, and all authority, policy, lifecycle, liveness, and model campaigns pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 37.69 seconds; includes all public regressions, 48 trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 892/892 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 208.00 seconds; zero behavioral failures, including the seven-case certificate-stale own-lien oracle, exact user-before-insurance terminal partition, shared INV-037 close-partition oracle, single SPL-account parser gateway, source-complete authority-epoch route/decoder matrices, canonical permissionless lifecycle epoch, proof/deployed-dispatch source lock, and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
...
| `stateful/` | 204 | Generated public routes with exact rollback on rejection and shared state, custody, OI, source-credit, and liveness oracles on success. Coverage includes randomized retained retries and trade-route switching, exact normalized nonzero-fee equivalence across all four trade route classes, lifecycle and terminal compositions, all 2,380 public action words through depth three, underfunded terminal authority/resolve and retained-policy/boundary/order products, the 27-case authority-incarnation matrix, one public two-stage exact receipt-top-up lifecycle, all 5! basic claimant orders, and the mutation-killed plus four-route public INV-037 residual-partition oracles. Per-invariant rows below are the source of truth for uncovered dimensions. |
