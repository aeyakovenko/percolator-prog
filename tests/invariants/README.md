...
`ba7a84b774870c3db9dd14d9747e8326b31f3a94`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`6650b2dd8ff92610a60efb875e004bbe35c6e7e6d0f0ba4ec1cffc847203b457`. Resolved-mode
...
| Focused INV-045 mark staging, fee isolation, liquidation, exit, and terminal-retirement matrix | 7/7 public, 19/19 stateful, 19/19 CU, and 4/4 wrapper Kani | the 80-cell boundary matrix, 64-case generated interior campaign, 64-world route-order composition, 16-world/four-step repeated-movement campaign, and 32-world clock-first schedule matrix cover all modes/routes, varied anchors, up/down targets, caps, nonterminal elapsed slots, ordered partial-reduction route pairs, and 64 catch-up boundaries; 32 stale no-CPI-to-CPI transitions and 16 missing-observation terminal refreshes reject exactly before public refresh/retry, clock-first and trade-first schedules converge economically, same-slot movement cannot compound, valid movement is fee-supported, invalid prices roll back exactly, position exits remain live, paid movement cannot be reclaimed by the controlling coalition, and the 14-asset paid-movement/full-exit composition remains below the SVM compute ceiling |
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `ba7a84b7` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `ba7a84b7` |
| `cargo test --test v16_program_stateful_fuzz` | 183/183 | full engine-`ba7a84b7`/SBF-`6650b2dd` rerun in 175.38 seconds, including the new 32-world clock-first mark-discovery schedule matrix, 24-world source-lien partition/expiry/exit matrix, generated aggregate/split/reversed terminal-insurance withdrawal, resolved-claim and liquidation partition matrices, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`ba7a84b7`/SBF-`6650b2dd` rerun in 26.85 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 794/794 | full engine-`ba7a84b7`/SBF-`6650b2dd` exact-source rerun in 191.52 seconds, including all maximum-source and maximum-shape exits, liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and every cross-invariant progress/rollback audit; the mark-clock fix adds no CU failure and same-slot exits cannot compound mark movement |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 137/137 | full wrapper-only exact-pin rerun on engine `ba7a84b7`; zero harness failures, including all declared nonvacuity covers |
| Engine runtime/property suites | 181/181 base and 226/226 with `--features fuzz` | rerun on engine `ba7a84b7`; all unit, reference-model, arithmetic-discharge, source-credit, insolvency, and v16 specification tests pass, including deterministic and generated conservative-margin partition checks |
...
`6650b2dd8ff92610a60efb875e004bbe35c6e7e6d0f0ba4ec1cffc847203b457`.
...
| AUDIT-045 | OPEN-T | The ten known target-staging, pending-target, fee-support, reserve-reclaim, and liquidation-reward adapters plus one finding-blind clock-first violation now assert safe fixed-pin outcomes. Seven deterministic public tests, nineteen stateful tests, nineteen CU tests, and four local Kani contracts cover all four trade routes, EWMA/hybrid modes, exact pending-state rollback, permissionless catch-up, risk-reducing exits, coalition value, terminal burn, and bounded liquidation. The 80-cell model crosses four mark regimes, four routes, same/max configured dt, valid `1`/`MAX_ORACLE_PRICE` targets, invalid zero/above-domain inputs, and repeated partial reductions with exact fee/value/supply/rollback/exit/CU oracles. A 64-case saturation run reuses the same oracle over generated interior anchors, up/down spreads, caps, and nonterminal dt; its persisted after-hours seed also guards the fresh/fallback regime boundary. A separate 64-world matrix exhausts every ordered pair of partial-reduction routes in both trade-driven mark modes and directions; all reversed orders converge economically, and all 32 stale no-CPI-to-CPI transitions reject exactly before public refresh and successful retry. The new 32-world landing-order matrix proves clock-only cranks cannot pin trade discovery by consuming the engine clock first: both schedules produce the same bounded, fee-backed mark and complete position exits, and a same-slot second reduction cannot compound movement. The 16-world repeated campaign adds 64 sequential paid movements and 64 bounded catch-ups, then proves exact omitted-observation rollback, authenticated recertification, and complete owner exit. The 14-asset paid-EWMA composition covers maximum shape through DrainOnly full clear, released-PnL conversion, owner withdrawal, and terminal custody under the CU ceiling. Other lifecycle/route maximum-shape compositions remain. Whole-domain wrapper arithmetic remains behind CBMC's deployed 128-bit division circuit; closure needs a named arithmetic axiom/equivalence result rather than another narrowed duplicate. |
...
On engine pin `ba7a84b774870c3db9dd14d9747e8326b31f3a94`, the full `v16_cu` inventory is
