...
INV-089 charter. It is a baseline, not completion of the expanded goal. Requirements 6 through 9
now have the current-surface proof-equivalence closures described below; requirement 10 remains open
until the adversarial-role matrix is connected to production authority callsites and discharged
with executable whole-route evidence. Individual ADV/SUG regressions demonstrate required
counterexamples but cannot substitute for the generic authority properties above.
...
gate rather than a larger duplicate scenario generator. Twenty-one independently executable layers
...
obligation census, and scope induction, every
external-token/internal-quote route, the independent transition model, and the complete error
boundary. Five mounted Kani theorems supply the engine-value/wrapper-custody, owner-episode
...
| Focused INV-081 whole-route success-state composition | 1/1 source-complete composition gate over 21 executable layers, 49/49 public routes, and 5/5 mounted Kani owners | decoder/domain/identity/account/input/admission inventories join all engine calls, wrapper fields, value/stock/certificate/position/typed-obligation/scope postconditions, independent model dimensions, and exact error propagation on the immutable engine pin; any omitted public or CU route fails closed. |
...
| INV-081 | CLOSED - P(engine/wrapper composition) + F + Direct + SVM/CU | `cu/inv_081_success_state_validity_over_complete_public_routes.rs` joins twenty-one source-complete executable layers over all 49 public instructions: decoding/domain, identity/authority, account roles/PDA/caller fields, admission, every engine call and wrapper persisted field, position/OI, typed matched-book obligations, scope, token/value, per-episode entitlement, independent model, and error propagation. It additionally source-locks five mounted Kani composition theorems for value flow, owner-episode entitlement, exact stock, conservative certificate commit, and engine-error mapping, and pins the exact engine revision. The generated public model and seeded frontiers retain exact per-step success/rollback, value, OI, episode, lifecycle, terminal, and funded-exit oracles. This is a current-surface proof-equivalence decomposition under the named engine and SVM assumptions, not one monolithic arbitrary-byte query. A route, field, callsite, effect class, proof owner, witness, or pin change reopens it. |
