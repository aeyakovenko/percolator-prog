...
INV-089 charter. It is a baseline, not completion of the expanded goal. Requirements 6 through 8
now have the current-surface proof-equivalence closures described below; requirements 9-10 remain open
until they are connected to source-complete public-transition rosters and discharged with
executable whole-route evidence. Individual ADV/SUG regressions demonstrate required
counterexamples but cannot substitute for the generic history properties above.
...
current-sequence admission and strict one-step consumption; a new fourteen-layer gate joins every
retained family, delayed-control lane, identity/authority binding, successful value attribution,
and exact error rollback to the existing `2!`, `3!`, and 144-cell noncommuting public products.
...
modules: 175 direct harnesses plus 38 macro-generated trade-decoder harnesses, exactly matching the
213 reported by `cargo kani list`. Every direct harness is classified into 103 symbolic-total, 33
...
| Focused INV-007/010 arbitrary-depth ABA and landing-order composition | 1/1 no-reuse account census plus 1/1 fourteen-layer ordering-induction gate and existing full-width Kani step | first-reuse impossibility or monotonic incarnation identity closes repeated ABA depth; exact current-sequence admission plus strict consumption/rollback closes every finite request permutation while `2!`, `3!`, 48-policy, and 144 terminal worlds witness noncommuting boundaries. |
...
| INV-084 | P + Source audit + SVM/CU | `cu/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` derives all 206 mounted harnesses, locks their five nonvacuity categories, follows local assertion helpers, requires constructive covers for every branch-limited claim, and binds every concrete-fixture module to public evidence. It separately discovers all 13 explicit `kani::assume` sites across all 29 modules and requires an exact row naming the owning proof, constructive witness, classification, and public evidence. `kani/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` exhausts every finite admitted/excluded partition with boundary mutation killers. Public LiteSVM reaches the valid identity, sequence, episode, fee, enable, and signed-size domains and proves invalid partitions roll back exactly before terminal exit. Any proof-roster or category drift reopens this current-surface closure. |
...
| AUDIT-010 | CLOSED | Matcher/control/trade, deposit/withdraw/control, and deposit/reduction orders are exhausted at their documented boundaries. The authority products span all eight policy lanes, low/mid/max values, funded and underfunded resolution, and all 144 policy/boundary/handoff/resolve schedules with genuine partial receipts and terminal convergence. The full-width Kani step proves a request commits iff its exact sequence is current and then advances once. A fourteen-layer source gate composes that step with every retained family, delayed-control lane, incarnation/authority binding, attributed successful delta, and exact instruction-error rollback. Every finite landing order is an induction over this serial step, so factorially larger schedules add no new semantic branch. INV-008/009/011/059 own one-shot execution and current aggregate trade bounds. A retained route, binding lane, rollback disposition, signed field, detached authorization, or durable-nonce economic-intent change reopens this row. |
