...
INV-089 charter. Requirements 6 through 10 now also have the current-surface proof-equivalence
closures described below. The expanded goal is therefore closed for the pinned public surface under
the named engine, arithmetic, authenticated-input, SVM rollback, and submission assumptions. This
is not an unconditional claim that future code or every unbounded state is LoF/DoS-free; a route,
role, state dimension, supported shape, holdout finding, or assumption change reopens the owning
row. Individual ADV/SUG regressions remain corroborating evidence rather than substitutes for the
generic history and authority properties.
