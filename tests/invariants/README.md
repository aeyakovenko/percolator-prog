...
1. The charter requests `P` for 76 invariants, `F` for 85, `I` for 66, `M` for 32, `R` for 22,
   ...
   obligations: 19 `M` rows, 20 `R` rows, and both `C` rows are current-surface `CLOSED` by
   executable coverage or proof-equivalence composition. Thirteen `M` rows and two `R` rows retain
   partial named evidence and are the remaining special-method work.
