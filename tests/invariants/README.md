...
asset-local reset/shutdown versus an unrelated all-route bilateral exit. INV-028/071/073/082
...
| Focused INV-065/074 lifecycle-local exit ordering | 16/16 public worlds over 4 trade routes, 2 reset sides, and 2 landing orders | asset-0 `ResetPending` shutdown frames the unrelated asset/profile, the asset-1 bilateral exit frames the reset episode, permissionless cleanup/finalization/restart remains bounded, every owner withdraws, both schedules converge exactly, and SPL/engine stock reconcile |
...
| `cargo test --test v16_program_stateful_fuzz` | 188/188 | exact engine-`592d538c`/SBF-`909c24cc` rerun in 173.84 seconds, including both active-close barrier orientations across every trade route, the independent raw-basis/effective-OI ledger, sub-minimum-ADL and cross-domain fractional-source progress, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, concurrent partial receipts, lifecycle-local reset/shutdown ordering, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
...
| AUDIT-074 | OPEN-T | The unrelated-accrual close-drift path is fixed and function-contract/public-route covered. Eight same-asset worlds preserve complete unrelated reductions across every route/orientation; two asset-local closes advance independently; shutdown/close ordering converges through canonical B discovery. Historical bankruptcy no longer blocks unrelated exact backing, provider principal, or remaining insurance after active blockers clear. Twelve underfunded worlds preserve unrelated flat principal across expiry, claimant order, and payout-route priority. INV-075 already exhausts both landing orders for same-domain close contenders and proves rejected-contender terminal liveness. The split-claim composition now covers all sixteen route pairs with two simultaneous partial receipts: destination substitution rejects exactly and a nonzero canonical claim frames the other receipt. Another sixteen public worlds prove `ResetPending`/Recovery shutdown on asset 0 and a bilateral exit on asset 1 commute economically across every trade route and reset side while framing each other's scoped state. Complete the remaining portfolio/domain and lifecycle locality cross-product before promotion. |
