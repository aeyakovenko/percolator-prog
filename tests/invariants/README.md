...
production LoF/DoS defect. INV-084 remains `FRONTIER`: branch-implied and concrete-fixture-implied
proof preconditions still need a source-complete harness inventory and nonvacuity disposition.
...
| Focused INV-084 explicit proof-assumption inventory | 2/2 CU plus 11/11 focused Kani and 161/161 wrapper Kani composition | a source parser owns all 13 explicit `kani::assume` sites across all 19 mounted modules and binds each exact source row to its owning proof, constructive two-sided witness, classification, and public evidence; public routes reach the admitted ID/sequence/episode/fee/enable/trade domains, reject invalid partitions with exact rollback, and terminate with exact custody |
...
| INV-084 | P + Source audit + SVM/CU + Partial R + Proof-harness gap | `cu/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` discovers all 13 explicit `kani::assume` sites across all 19 mounted modules and requires an exact inventory row naming the owning proof, constructive Kani witness, classification, and public evidence. `kani/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` exhausts every finite admitted/excluded partition with boundary mutation killers. Public LiteSVM evidence reaches the valid identity, sequence, episode, fee, enable, and signed-size domains and proves invalid partitions roll back exactly before terminal exit. Implicit branch- and concrete-fixture-imposed proof preconditions remain open. |
...
The current ledger is 7 **CLOSED**, 50 **OPEN-T**, 12 **OPEN-D**, 6 **PARTIAL**, 13 **FRONTIER**,
...
| AUDIT-084 | FRONTIER | A host source audit discovers all 13 explicit `kani::assume` sites across all 19 mounted Kani modules and binds every exact file/line/predicate row to its owning proof, constructive Kani witness, classification, and public-route evidence. Full-width symbolic partitions prove admitted and excluded models and pin off-by-one, widening, dropped-mark-clause, cap, epoch, identity, sequence, and zero-size mutation killers. Public LiteSVM composition establishes the admitted domains and exact rollback controls. The prior eight-site/ten-module statement was a stale coverage contradiction. Implicit branch/fixture proof preconditions are not yet source-inventoried, so the row remains open. |
