...
| Focused INV-038 wrapper truncation ownership and partition products | 9/9 stateful plus 914/914 CU; 36/36 production functions, 62/62 truncating operations, 2/2 odd public value partitions, and 16/16 EWMA public worlds | one source-derived semantic census includes plain division, modulo, and checked division rather than only wide-arithmetic markers. Structural index arithmetic is separated from oracle/policy rounding, exact value partitions, aggregate ceilings, the atom-aligned backing counter floor, and the engine-owned funding mirror. Executable evidence owns every nonstructural row. Public activation proves `41 = 20 + 21`; public batch redirect proves two one-atom per-side fees become `2 = 0 + 2`, with exact insurance and SPL reconciliation. The four single/batch CPI/no-CPI trade transports compare aggregate, paid multi-slot split, and zero-movement dust-prefix execution. Every mark-moving segment pays at least the independently reconstructed externality-notional ceiling, same-slot lock rejection rolls back exactly before bounded cohort catch-up, and a zero-move dust prefix consumes neither fee nor discovery capacity. |
...
| `cargo test --test v16_program_stateful_fuzz` | 205/205 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 263.64 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets, including the complete pending-obligation and close-ledger encumbrance census, direct per-domain `usable_positive_credit_num <= available_backing_num`, exact/bound claim transitions, credit-rate edge causality, and the independent resolved-payout quotient/remainder oracle. The suite includes the finding-blind 11-operation whole-market no-reuse property, 56-world one-domain, 48-world two-domain, and 48-world mixed-expiry INV-052 source-lien matrices, exact per-portfolio source attribution and close exclusion, the asymmetric-fee INV-041 order regression, INV-076 rollback frames, the finding-blind 21-family retained asset-generation matrix, source-domain single-use, retained source-fee consent, backing-provider landing-order campaigns, and all-route EWMA aggregate/partition/dust fee conservation. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
...
| `cargo test --test v16_program_fuzz_regressions` | 101/101 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 27.74 seconds; includes the generic 11-operation whole-market no-reuse matrix, the source-complete account-incarnation and detached-signature censuses, all public regressions, trace consumers, executable registries, and the INV-079 evidence roster |
| `cargo test --test v16_cu` | 914/914 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 201.21 seconds; zero behavioral failures across market tombstone/rent/fresh-address behavior, matcher identity/incarnation and stateless-PDA composition, reserve, fee-policy/destination ownership, claim/backing/insurance single-use, exact lien lifecycle, source-realizability, exact-claim, credit-rate transition composition, exact B-booking/account-settlement/carry-normalization partitions, the source-complete truncating-arithmetic ownership census, odd activation/redirect partitions, oracle, Recovery, terminal, source-lien normalization, selector composition, equivalent-route source composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling; the run observed 1,116,641 CU for max-source liquidation and 1,314,291 CU for max-shape resolved close. The non-exit 28th-lien admission reached 1,398,554 CU and remains an explicit thin-headroom risk. |
...
prefix consumes no fee or movement capacity. The remaining ratio-change frontier is explicitly
non-EWMA: source-complete aggregate-versus-partition products for social-loss booking/settlement and
backing/claim-rate changes. These tests do not imply an unbounded cross-product.
...
| INV-038 | OPEN-T - Independent + Direct + F + SVM/CU + M | `public_sbf/inv_038_rounding_and_ratio_conservation.rs`, `stateful/inv_038_rounding_and_ratio_conservation.rs`, and `cu/inv_038_rounding_and_ratio_conservation.rs` cover exact fractional mark carry, fee and funding rounding, composite rational composition, two resolved-receipt rate increases, social-loss B booking, per-account B settlement, zero-OI carry normalization, odd wrapper-owned partitions, and trade-driven EWMA aggregate/split behavior. The public bankruptcy test independently reconstructs every persisted quotient/remainder partition and exact side-local dust/explicit-loss split while reconciling SPL custody; the receipt test uses a separate shift/add oracle and immutable face so sequential floors cannot erase retained entitlement. A source-derived census owns all 36 production functions and all 62 plain division, modulo, or checked-division operations, classifies structural indexing separately, and requires executable evidence for every semantic rounding owner. Public odd-value routes prove exact complement assignment. Sixteen EWMA worlds cross all four trade transports with aggregate, paid multi-slot split, and zero-move dust-prefix execution; every moving segment pays its independent externality-notional ceiling and retains bounded public progress. Close the remaining non-EWMA aggregate-versus-partition products for social-loss booking/settlement and backing/claim-rate changes. Engine arithmetic equivalence remains owned by INV-085 and is not duplicated here. |
...
The current ledger is 35 **CLOSED**, 27 **OPEN-T**, 11 **OPEN-D**, 0 **PARTIAL**, 14 **FRONTIER**,
...
| AUDIT-038 | OPEN-T | Fractional price movement carries exact sub-basis-point residue, reaches the target in finite public cranks, and preserves exact terminal payout; denominator boundaries and reserved bytes fail closed. Independent public oracles reconstruct both resolved-receipt top-up floors from immutable face and the exact market B-booking, account B-settlement, and zero-OI carry-normalization quotient/remainder equations, including side-local dust and explicit loss. The source-complete semantic census now owns all 36 truncation-bearing production functions and 62 operations, and odd public activation/batch routes prove exact complement assignment. The all-route EWMA aggregate/partition/dust product is complete with per-segment fee and progress oracles. Cover the remaining non-EWMA social-loss and backing/claim ratio-change products before closure. |
