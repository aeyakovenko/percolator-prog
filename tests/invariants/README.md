...
| `cargo test --test v16_program_fuzz_regressions` | 106/106 | full exact engine-`6c8d94bc`/SBF-`a76dd082` rerun completed in 27.67 seconds after the retained-source/adverse-leg composition increment; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 899/899 | full exact engine-`6c8d94bc`/SBF-`a76dd082` rerun completed in 205.67 seconds after the retained-source/adverse-leg composition increment; zero behavioral failures across reserve, fee-consent, oracle, Recovery, terminal, lifecycle, source-lien normalization, selector composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling. |
...
normalization and dispatch; wrapper production code is unchanged, and the complete 899-test CU
suite remains green.
...
1. Generalize the concrete actionability-fidelity oracle that exposed the mixed-expiry DoS. For
   ...
   canonical loss attribution; close that pair with a production-builder unreachability proof rather
   than an injected state. A fourth public composition proves stale refresh and liquidation cannot
   hide a retained source label and preserve the owner's remaining-capital exit. Continue with
   reachable three-class and lifecycle/lower-priority overlaps.
...
On engine pin `6c8d94bcacb8110c8e86ce1b1fc46594bb672a7c`, the full 899-test `v16_cu` inventory is
