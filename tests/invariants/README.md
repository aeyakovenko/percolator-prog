...
census owns 183 harnesses across 21 mounted modules, and the public-trace guard owns 37 consumers.
...
all 38 current public-trace consumers. Every successful token-moving wrapper step must have unique
...
INV-039/079 now promote four ordering oracles from internal economic deltas to one shared paired-
terminal evidence contract: prospective accrual across all four trade routes, pending-mark commit
before permissionless resolve, zero-effective-price-move funding before terminal resolve, and
funding commit before asset shutdown. Each control and reordered
...
before the earlier claimant's later top-up. The fixed pin produces bounded exits with identical
victim payouts in all four families, so this strengthens detection without changing production.
...
an explicit evidence class. Ten oracles now carry classifier-bound public LoF evidence. In
...
The pending-mark resolve, zero-move terminal funding, and shutdown-funding ordering oracles now use
the same paired terminal contract as prospective accrual instead of duplicating their payout
classifier. The remaining 18
...
SPL, and program-lamport rollback. A recursive source-complete guard requires all 38 current
...
| `cargo test --test v16_program_stateful_fuzz` | 203/203 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 304.26 seconds; every public-trace token-moving step composes checked quote conservation with owner/vault attribution, all four paired terminal-ordering families bind exact SPL identities, and all authority, policy, lifecycle, liveness, and model campaigns pass |
| Registry/manifest checks in the INV-079 module | 12/12 | includes the source-complete 28-oracle evidence roster, ten exact terminal-LoF dispositions, complete-route terminal classification, and the recursive 38-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 27.02 seconds; includes all public regressions, 38 trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 892/892 | full exact engine-`9b737fd`/SBF-`8ac842fe` rerun completed in 246.23 seconds; zero behavioral failures, including the single SPL-account parser gateway, source-complete authority-epoch route/decoder matrices, canonical permissionless lifecycle epoch, proof/deployed-dispatch source lock, and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
...
| INV-039 | Independent + Direct + SVM/CU | `public_sbf/inv_039_pending_loss_obligation_durability.rs`, `stateful/inv_039_pending_loss_obligation_durability.rs`, and `cu/inv_041_deterministic_allocation_and_caller_order_independence.rs`; the four-party Recovery matrix exhausts all landing orders while independently checking retained weight and pending counts. Prospective accrual across all four trade routes, pending-mark terminal resolve, zero-effective-price-move terminal funding, and shutdown-funding ordering all execute paired public worlds through complete resolution, recompute victim and coalition payouts from both traces' destination-token deltas, and require terminal portfolios plus classifier-bound exact loss or bounded exit. A shared bounded round-robin drain revisits claimants after other users progress and rejects nonterminal fixed points. INV-027 owns the cross-cutting stale-cohort novation guard and finite-settlement certification. |
...
| AUDIT-039 | OPEN-D | Many accrual-before-weight-removal routes are covered, and INV-027 now certifies stale-cohort novation rejection plus finite discharge. The public four-party Recovery matrix exhausts all owner landing orders and checks retained loss weight and pending counts after every removal. Prospective trade/catch-up, pending-mark resolve, zero-move terminal funding, and shutdown-funding ordering now share paired terminal traces with exact destination-token payout reconstruction, complete public resolution, terminal users, and classifier-bound loss/bounded-exit evidence. Reset, account close, partial liquidation, and the nonterminal rebalance/forfeit accrual worlds still need the common obligation-before-removal state machine. |
...
| AUDIT-079 | OPEN-T | The LiteSVM trace schema records actual transaction signers, compiled account metas, exact authority-attributed tracked token/lamport deltas, rejected writable-account rollback with the fee-payer network charge separated from program effects, and between-transaction economic mutation. The shared validator requires an allowlisted public construction sequence containing a real wrapper call and rejects malformed success, rollback, signer, account, payload, program, CU, token-owner, vault-participation, and quote-balance evidence. A recursive scan source-locks all 38 current trace consumers to validate or classify immediately. The normalized terminal classifier distinguishes exact loss/unauthorized gain, persistent funded lock, bounded exit, atomic rejection, and progress; real public exit/rejection witnesses plus negative anti-overclaim controls pass. Required, attempted, and progressing route masks prevent incomplete searches or successful no-op continuations from being labeled persistent DoS. Ten of 28 finding-blind violation oracles now carry classifier-bound exact LoF evidence; the enforced roster limits the remaining 18 to their narrower semantic claims. Attach independently measured terminal observations and complete exit-route attempts to every remaining qualifying finding-blind counterexample before promotion. |
