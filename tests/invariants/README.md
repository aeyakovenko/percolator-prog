...
| `cu/` | 737 | Full `v16_cu` public-route, metamorphic, rollback, liveness, arithmetic-differential, and max-shape CU inventory, including issue-404 transient-rent boundaries, issue-405 selected-Switchboard-result provenance, issue-406 matcher-inventory synchronization, issue-408 maintenance seniority, and five nonvacuous issues-407/409 crank-cadence counterexamples with exact independent-user SPL attribution; the cadence violations remain open, with no standalone top-level tests |
