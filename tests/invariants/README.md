...
frontier; active-close, explicit B-settlement, lien-impairment, receipt, and oracle-failure seeds
remain. No production source or persisted state changed.

The current SBF is 1,255,960 bytes with SHA-256
`d70956aa6559f9e3cdd5dde9553866227cea53b8df560e8d2da30b7f4252d837`. The exact pin passes
...
The Recovery extension adds a 233rd stateful/model test; that focused test passes 1/1 in 27.74
seconds. The unchanged prior 232 tests were not redundantly rerun for this proof-only addition.
...
8. The base graph now extends every exact authenticated tracked depth-three wrapper state by every
   action. The first lifecycle seed is complete: fresh- and exact-expiry Recovery states exhaust
   all 366 empty/one/two-action words and 702 transitions, and every result retains a constructible
   value-moving owner exit. Next add public-prefix seeds for active close, explicit B work, lien
   impairment, receipt conflict, and oracle failure, then extend those frontiers with the same
   exact-state reduction. Keep the graph explicitly non-universal and require every abstract node
   to retain a public reachability witness.
