...
| INV-028 | Independent + SVM/CU | `stateful/inv_028_source_domain_realizability_cap.rs`, `cu/inv_028_source_domain_realizability_cap.rs` cover source reversal, expiry, rounding, the 28-domain admission-order boundary, and eight reciprocal cross-asset worlds proving full recertification cannot turn mutually offsetting claims or unattached backing into usable credit. The shared-expiry matrix reaches PR300's staged foreign-impaired account-lien state, but records that PR302 currently masks its claimed counter-underflow stage. Retained public counterexamples also expose cross-domain conversion attribution and funded-exit failures on the pinned engine. |
...
| INV-030 | Independent + SVM/CU | `stateful/inv_030_credit_rate_determinism_and_fail_closed_behavior.rs`, `cu/inv_030_credit_rate_determinism_and_fail_closed_behavior.rs`, `cu/inv_028_source_domain_realizability_cap.rs`, `cu/inv_063_backing_expiry_normalization.rs` cover the deterministic credit-rate lifecycle plus secondary expiry/progress ownership. The shared-expiry matrix independently reaches PR302's impaired-domain prospective-loss rollback fixed point. |
...
| INV-048 | Independent + F + SVM/CU | `cu/inv_048_matched_trade_and_open_interest_coherence.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs`, and `stateful/inv_081_success_state_validity_over_complete_public_routes.rs`. Fresh-state scans cover all four trade routes; the stateful model separately derives exact pooled-OI deltas across matched/retained trades, crank liquidation, owner rebalance, reset cleanup, and recovery forfeit, then compares that ghost ledger to deployed `oi_eff` after every public step. |
...
| INV-051 | Independent + F + SVM/CU | `cu/inv_051_canonical_adl_effective_quantity.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs`, and `stateful/inv_081_success_state_validity_over_complete_public_routes.rs`. Directed ADL exit matrices are joined by an exact transition-derived pooled-OI ledger that deliberately separates retained raw basis from effective OI. |
...
| INV-086 | Direct + F + Partial M + Partial R | `public_sbf/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs` (fifteen generated public operation classes plus all 133 words through depth two over eleven value, trade, crank, policy, authority, backing, and lifecycle actions; every edge runs exact custody/account frames and independent position/effective-OI/source-credit/stock oracles against one production SBF hash. The OI oracle derives transition deltas rather than equating ADL-retained raw basis with pooled OI. The retained corpus covers that ADL/rebalance distinction, a final prior-epoch ResetPending leg clearing into finalization, and an unrelated empty portfolio correctly rejecting while another account owns the remaining leg. Identity, lien, payout, authority-epoch, terminal-resolution, deeper sequences, and complete lifecycle models remain) |
...
| AUDIT-048 | OPEN-T | All four fresh trade routes scan raw OI, and the stateful model now keeps an exact independent effective-OI transition ledger across matched/retained trades, crank liquidation, owner rebalance, prior-reset cleanup, and recovery forfeit. A retained public ADL/rebalance trace prevents regression to the invalid assumption that raw basis equals pooled OI. Add directed resolved-close/recovery schedules, pending-obligation transitions, and larger multi-account ADL topologies. |
...
| AUDIT-086 | OPEN-T | The reference runner checks fifteen generated public classes and exhausts all 133 words through depth two over eleven value, trade, crank, matcher, backing, authority, resolve-policy, and lifecycle actions. The graph binds one production SBF hash, distinguishes 50+ normalized states and 100+ edges, and runs exact custody/account frames plus independent position/effective-OI/source-credit/stock oracles after every edge. Its retained corpus now includes ADL/rebalance raw-versus-effective separation, ResetPending clear/finalize, and unrelated-account rejection rank regressions. It still lacks independent identity, all-balance, lien, payout, authority-epoch, terminal-resolution, deeper sequences, and complete lifecycle state components; add those projections and extend the bounded alphabet/depth without treating this finite graph as universal equivalence. |
...
On engine pin `e57296cd73f3f6f7233f2efab223be6b300c6084`, the full `v16_cu` inventory is
