...
`320e816254bba5761fbe06b2b5e2bdabf3d2f26de5019939c4f67877191f87f5` and passes 101/101
public fuzz regressions, 205/205 stateful/model tests, 890/890 LiteSVM/CU tests, 174/174 wrapper
Kani harnesses, and 129/129 engine runtime/property tests. INV-034 now has a source-complete role
roster over all 50 public variants: 20 have no mixed-instance role, 14 have exhaustive current-role
substitution evidence, 16 are partial, and none is unclassified or `OPEN`. New public two-market
tests cover permissionless activation-fee vaults, base-unit old-reserve rotation, and matcher
portfolio/delegate capability tuples with exact rollback and successful same-instance controls.
The 16 named partial role families remain `OPEN-T`; the roster and three closed families do not
promote the whole invariant to closed. This tranche adds no deployed state or production branch.
...
| Focused INV-034 unattributed cross-domain loss and public role roster | one finding-blind public stateful campaign, one deterministic exact-SBF regression, 2/2 focused engine Kani proofs, 31/31 focused CU tests, a source-complete 50-variant role roster, and exact engine/runtime suites | parent `b10b3454` spends 100,100 atoms of unrelated insurance and yields 95,500 atoms of coalition profit after public multi-asset loss/detach/liquidation; engine `9b737fd` retains the existing unattributed-loss lock, permits one strict risk-reducing crank, spends zero unrelated insurance, rejects settled nonprogress with exact rollback, preserves a bounded owner exit, and reconciles token supply. The roster currently classifies 20 single-anchor, 14 exhaustive, 16 partial, and zero open/unclassified variants. Public two-market controls now close activation-fee vault, base-unit old-reserve, and matcher portfolio/delegate substitutions. The fixed artifact is `320e8162`; 101/101 regressions, 205/205 stateful tests, 890/890 CU tests, and 129/129 engine tests pass. |
...
| `cargo test --test v16_program_fuzz_regressions` | 101/101 | full engine-`9b737fd`/SBF-`320e8162` exact-source rerun; includes all public regressions, trace consumers, and executable registries with PR290 promoted only after the finding-blind fixed-pin oracle passed |
| `cargo test --test v16_cu` | 890/890 | full engine-`9b737fd`/SBF-`320e8162` exact-source rerun passed in 440.94 seconds on 2026-08-28; zero failures, including the source-complete INV-034 role roster, all 50 account-role matrices, reserve, Hybrid-oracle, public Recovery, terminal slab, lifecycle, maximum-shape, and prior invariant routes |
...
| INV-034 | Independent + Direct + P(engine) + F + SVM/CU + Partial matrix | `public_sbf/inv_034_domain_and_instance_isolation.rs`, `stateful/inv_034_domain_and_instance_isolation.rs`, `cu/inv_034_domain_and_instance_isolation.rs`, and `inv_034_instance_role_coverage.tsv`. The finding-blind public campaign independently exposed a multi-asset realized-loss detach followed by foreign-domain insurance consumption on parent `b10b3454`; engine `9b737fd` reuses the existing unattributed-loss lock and central detach/liquidation paths. Fixed-pin evidence requires a strict risk-reducing automatic crank, zero foreign insurance spend, zero coalition profit, exact rollback once no work remains, a bounded owner exit, and exact SPL supply reconciliation. Two engine Kani harnesses prove the uncovered-loss postcondition and sticky-lock predicate lifecycle. A source-locked roster covers all 50 public variants and rejects source/evidence drift; 20 variants have one existing instance anchor, 14 have exhaustive current-role public substitutions, 16 retain an explicit finite partial-role gap, and zero are unclassified. The matrix remains `OPEN-T` until those 16 rows are discharged or reduced through proven canonical-route equivalence. |
