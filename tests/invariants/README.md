...
The current INV-005 tranche extends the canonical per-asset authority epoch to terminal slab close
and all three value-bearing reserve top-up routes. Combined with the prior authority handoff,
authenticated resolution, sequence-bearing policy, and managed-oracle routes, the source-derived
matrix now owns 20 epoch-bearing instruction variants as 24 semantic authority cases. Every case
...
campaign covers 192 public route instances, and the deterministic policy/handoff/resolve product
...
adds the explicit epoch wire field and canonical handler check to four routes, so it is a necessary
schema extension rather than a raw source-line reduction. A source-derived decoder roster,
one-byte truncation/trailing-byte proofs, and exact field-preservation proofs prevent route drift.
The caller-input and boundary rosters now own all 225 public fields, the proof census owns 178
harnesses across 21 mounted modules, and the public-trace guard owns 33 consumers.

The exact rebuilt SBF is 1,245,384 bytes with SHA-256
`444be368b3d0fe48ea1eb09c2a84ddc0da89037fd6e8710777ff0816f1e2dcc9`. The complete exact-artifact
gates pass 891/891 LiteSVM/CU tests, 102/102 public fuzz regressions, and 202/202 stateful/model
tests. The mounted wrapper proof census is 178 harnesses; its exact-source result is recorded in
...
routes that reach configured-authority logic: 20 are epoch-bound above; `ClosePortfolio` is
independently portfolio/sequence/episode-bound; and both auxiliary-ledger synchronizers are
deterministic current-state reconciliation. Seven explicit epoch gaps remain:
`WithdrawBackingBucket`, `WithdrawBackingBucketEarnings`, `WithdrawInsuranceAsset`, the legacy
multi-domain `WithdrawInsurance`, `UpdateAssetLifecycle`, `UpdateBaseUnitMints`, and
`SwapSecondaryForPrimary`. Full same-pubkey market recreation is separately blocked by INV-001:
...
authority routes, including terminal `CloseSlab`, reject same-market `A -> B -> A`; remaining
destructive-consent gaps are same-pubkey whole-market recreation and the seven source-rostered
non-epoch authority routes named in the current checkpoint. Those remain owned by INV-001/005.
...
all 225 fields across 53 public input types map to exactly one of 20 semantic profiles, one
...
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`9b737fd`/SBF-`444be368` rerun completed in 258.54 seconds; the 24-case authority-incarnation matrix, every retained-policy landing order, and every prior stateful/model check pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full engine-`9b737fd`/SBF-`444be368` exact-source rerun; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 891/891 | full engine-`9b737fd`/SBF-`444be368` exact-source rerun passed in 208.99 seconds on 2026-08-28; zero failures, including the source-complete authority-epoch route/decoder matrices and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 178/178 | full wrapper-only exact-source rerun with engine `9b737fd`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied |
...
fee consent remain explicit INV-001/INV-005/INV-014 gaps. The 24 epoch-bearing authority cases now
...
| AUDIT-005 | OPEN-D | A source-derived call graph classifies all 30 configured-authority routes. The 20 epoch-bearing source variants, represented by 24 semantic authority cases, reject same-market `A -> B -> A` with exact rollback and admit fresh current-epoch controls. `ClosePortfolio` is independently incarnation/sequence/episode-bound, and both auxiliary-ledger synchronizers are deterministic current-state reconciliation. Seven routes remain explicit epoch gaps: both backing withdrawals, both insurance withdrawals, lifecycle, base-unit mint replacement, and secondary-to-primary swap. Three Kani harnesses prove exact epoch admission, atomic checked handoff, and the migration-aware floor. Persistent market incarnation remains separately open because close/recreate resets account-local epochs. |
