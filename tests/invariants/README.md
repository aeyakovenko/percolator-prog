...
| `cargo check --tests` | pass | exact engine-`422893fa` build completed on the 2026-08-29 PR135 test head |
| `cargo test --lib` | 7/7 | exact engine-`422893fa` rerun on the 2026-08-29 PR135 production head |
| `PERCOLATOR_FUZZ_CASES=2 cargo test --test v16_program_stateful_fuzz` | 208/208 | full exact engine-`422893fa`/SBF-`4314e6b0` rerun completed in 224.97 seconds on `73f9ff7b`; every generated public action and successful crank composes the shared invariant oracles for both markets. The suite includes the 56-world INV-052 source-lien partition matrix, INV-076 post-refresh/post-accrual rollback partitions and complete success frames, the finding-blind 21-family retained asset-generation matrix with nonvacuous backing earnings in both generations, source-domain single-use, retained source-fee consent, and backing-provider landing-order campaigns. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
...
| `cargo test --test v16_program_fuzz_regressions` | 106/106 | full exact engine-`422893fa`/SBF-`4314e6b0` rerun completed in 27.75 seconds on `73f9ff7b`; includes the nonvacuous same-world backing-earnings generation replay, deterministic source-domain single-use, retained source-fee cap, and both backing-provider consent landing-order certifications, plus all other public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 895/895 | full exact engine-`422893fa`/SBF-`4314e6b0` rerun completed in 203.91 seconds on `73f9ff7b`; zero behavioral failures across reserve, fee-consent, oracle, Recovery, terminal, lifecycle, and maximum-shape routes. The observed required exits remain below the 1.4M CU ceiling. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 187/187 | full wrapper-only exact-source rerun on commit `68254277` with engine `422893fa`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied. A source-diff guard confirms that production, manifests, the Kani target, and all mounted Kani modules are byte-identical through `73f9ff7b`. The current proofs establish the exact five-lane funded-provider policy predicate, reject the old intentless top-up schema, and preserve both signed provider terms through the exact 55-byte production decoder. |
...
exists. The locally rebuilt 1,235,368-byte production SBF used by the 2026-08-28 exact-pin gates has
SHA-256 `4314e6b0c57dfbf5206f601c7526c2bc728d9cef759b2d9c4f0c497375171647`.
...
4. Apply INV-052's split/merge oracle to multi-asset or larger-account liquidation and to
   multi-domain lien consumption, cooldowns, rates, and policy limits. The source-lien expiry route
   now compares aggregate, equal two-account, asymmetric three-account, and asymmetric four-account
   shapes across all four trade families and both exit orders at exact/late expiry. Four targets
   plus one shared counterparty exhaust the current five-actor fixture; testing five or more target
   partitions requires a larger public topology rather than another permutation of this fixture.
