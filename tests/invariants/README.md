...
terminal-generation classification, and source-locks all 26 finding-blind `is_violation` oracles to
an explicit evidence class. Four oracles now carry classifier-bound public LoF evidence. In
...
remaining 22 oracles retain narrower replay, local-safety, or economic-delta claims until equivalent
...
remaining finding-blind public counterexample runner. A source-complete roster now covers all 26
`is_violation` oracles: terminal-generation replay, terminal dust, cross-domain backing
double-consume, and retained source-fee consent are classifier-bound public LoF evidence, while the other 22 are mechanically limited to economic-delta, local-safety, or
...
| Registry/manifest checks in the INV-079 module | 12/12 | includes the source-complete 26-oracle evidence roster, complete-route terminal classification, and the recursive 34-consumer source guard |
...
| INV-079 | Direct + Static rosters + Partial R | `public_sbf/inv_079_public_reachability_evidence.rs`, `public_sbf/inv_007_no_aba_reuse.rs` enforce the finding manifest and production/method rosters, mutation-test the public trace recorder, and replay all 11 whole-market ABA request classes with actual transaction signers, compiled account metas, exact token/lamport deltas, rejected-call rollback with the network fee classified separately, and zero out-of-band economic mutation. The shared validator rejects malformed provenance and a recursive source guard requires all 34 current trace consumers to validate or classify immediately. A normalized terminal classifier has real bounded-exit and atomic-rejection witnesses plus anti-overclaim controls for LoF and persistent DoS. Persistent-lock evidence requires complete required/attempted masks with zero progressing bits, so successful no-op routes cannot masquerade as liveness. The flat-source-lien and cross-domain rounding campaigns carry route-level progress evidence; terminal-generation replay, terminal dust, cross-domain backing double-consume, and retained source-fee consent now bind exact public victim loss to the classifier. A source-complete 26-oracle roster prevents the remaining 22 semantic or local discoveries from being promoted to terminal LoF/DoS evidence. Independent terminal observations are not yet attached to every qualifying counterexample. |
...
| AUDIT-079 | OPEN-T | The LiteSVM trace schema records actual transaction signers, compiled account metas, exact tracked token/lamport deltas, rejected writable-account rollback with the fee-payer network charge separated from program effects, and between-transaction economic mutation. The shared validator requires an allowlisted public construction sequence containing a real wrapper call and rejects malformed success, rollback, signer, account, payload, program, and CU evidence. A recursive scan source-locks all 34 current trace consumers to validate or classify immediately. The normalized terminal classifier distinguishes exact loss/unauthorized gain, persistent funded lock, bounded exit, atomic rejection, and progress; real public exit/rejection witnesses plus negative anti-overclaim controls pass. Required, attempted, and progressing route masks prevent incomplete searches or successful no-op continuations from being labeled persistent DoS. Four of 26 finding-blind violation oracles now carry classifier-bound exact LoF evidence; the enforced roster limits the remaining 22 to their narrower semantic claims. Attach independently measured terminal observations and complete exit-route attempts to every remaining qualifying finding-blind counterexample before promotion. |
