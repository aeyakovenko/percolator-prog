...
| Focused INV-010 retained-operation landing orders | 4 fixed matrices, 74 public worlds | all `3!` deposit/withdraw/disable orders at three value boundaries enforce one shared-sequence winner and exact stale rollback; both deposit/reduction orders converge outside the conservative health-certificate cache; all eight market/asset-0 policy lanes cross both authority-handoff orders at low/mid/max values; both authority/resolve orders then pay and close all five users in opposite claimant orders, reject stale authority exactly, preserve fresh resolve liveness, prevent retry double-pay, and close the slab under the rotated authority |
...
| `cargo check --tests` | pass | exact engine-`b10b3454` build completed in 6.03 seconds on the 2026-08-21 PR135 production head |
| `cargo test --lib` | 7/7 | exact engine-`b10b3454` rerun on the 2026-08-21 PR135 production head |
| `cargo test --test v16_program_stateful_fuzz` | 200/200 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun passed in 250.75 seconds; the two new terminal worlds join 48 authority/policy worlds, and every prior randomized retry, trade-route, depth-three graph, source-lien, rank/reference-model, lifecycle, terminal, and route-composition check remains green |
...
| `stateful/` | 198 | Generated public routes with exact rollback on rejection and shared state, custody, OI, source-credit, and liveness oracles on success. Coverage includes randomized retained retries and trade-route switching, lifecycle and terminal compositions, all 2,380 public action words through depth three, 12 underfunded terminal worlds, and all 5! claimant orders. Per-invariant rows below are the source of truth for uncovered dimensions. |
...
| INV-010 | Independent + P + SVM/CU + Partial R | `stateful/inv_010_out_of_order_safety.rs`, `kani/inv_010_out_of_order_safety.rs`, `cu/inv_010_out_of_order_safety.rs` exhaust all `3!` landing orders of conflicting same-sequence matcher controls plus retained CPI trade; all `3!` deposit/withdraw/disable orders at three value boundaries; both deposit/reduction orders; both authority-handoff orders against all eight market/asset-0 policy lanes at low/mid/max values; and both authority/resolve orders through exact five-user terminal payout and slab closure. They enforce lane-specific postconditions, exact stale rollback, fresh incoming-authority liveness, claim fixed points, conservative certificate normalization, and complete public exits. Underfunded partial-receipt/authority cross-products remain; independent underfunded claim-priority worlds are covered by INV-086. |
...
| AUDIT-010 | OPEN-T | Matcher/control/trade, deposit/withdraw/control, and deposit/reduction orders are exhausted at their documented boundaries. A 48-world matrix crosses market-authority handoff with all eight market/asset-0 policy lanes at low/mid/max values: policy-first preserves authorized state, handoff-first rejects the former authority exactly, every incoming-authority policy remains live, and users exit. Two additional worlds cross authority handoff and resolution in both orders, enforce exact stale rollback and fresh-authority resolve, pay all five funded users in opposite claimant orders, prove claim-retry fixed points, close every portfolio, and close the slab with the rotated authority's signer and SPL destination. Add underfunded partial-receipt/authority permutations, then widen the bounded cross-product; INV-086 independently covers twelve value-moving underfunded claim-priority worlds. |
