...
191 engine host/runtime tests, 111 public regressions, 219 public stateful/model tests, and all 916
...
INV-086 now owns 96 public unilateral-close worlds plus four public dual-ADL prefix controls. The
...
independently exposed and now certifies best-effort clamping of stale/raw work. The next tractable
work is permissionless liquidation and Recovery forfeit composition; each must use the pre-state
oracle rather than trust deployed post-state counters.
...
| AUDIT-048 | OPEN-T | All four fresh trade routes scan raw OI, and the stateful model keeps an exact independent effective-OI transition ledger across matched/retained trades, crank liquidation, owner rebalance, prior-reset cleanup, and recovery forfeit. Two 32-world public matrices create nonzero ADL through every trade transport and cross `effective - 1`, exact effective, `effective + 1`, and raw basis. Owner reduction crosses both terminal claimant orders; one-sided-ADL Recovery force-close crosses both account orders. Each requires the independently derived clamp to match both side OI deltas and recomputed remaining legs before exact terminal value returns. A separate 24-world public bankruptcy matrix pins the zero-OI pending-obligation boundary: the final matched reduction clears effective OI while retaining exactly one zero-basis stored leg and one obligation, and terminal payout clears both without resurrecting OI. Directed nonzero-ADL liquidation/Recovery-forfeit, dual-nonunit-A force-close, and larger multi-account ADL topologies remain. |
...
| AUDIT-051 | FRONTIER | Zero-effective-OI directed matrices and the stateful transition ledger cover resize, matched trade, rebalance, liquidation, reset clear, and recovery forfeit without collapsing raw basis into effective OI. Two 32-world matrices add nonzero ADL, all four opening routes, and the complete one-atom/exact/overshoot/raw-basis boundary to owner reduction and one-sided-ADL Recovery force-close, with independently recomputed remaining effective quantity and terminal value. The bankruptcy matrix carries zero effective OI through a nonzero pending-obligation epoch and terminal close. Transfer, nonzero-ADL liquidation/Recovery-forfeit, dual-nonunit-A force-close and retirement compositions, and a pure whole-transition equivalence proof remain. |
