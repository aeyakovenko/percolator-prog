...
`ce590d9bd3e8f55eaf2e0321f36ef11fbc003d26` (engine PR
[#193](https://github.com/aeyakovenko/percolator/pull/193), stacked on roadmap/source-loss PR
[#192](https://github.com/aeyakovenko/percolator/pull/192) and source-lien certification PR
[#191](https://github.com/aeyakovenko/percolator/pull/191)). The parent work includes two production
source-claim accounting fixes, one bounded canonical whole-account K/F settlement plan, live
target/phase/domain validation at the mutation boundary, matching specification equations, and
three full-width Kani proofs.
...
close-ledger deltas to agree exactly. No production source or persisted state changed.

The resulting SBF is 1,248,160 bytes with SHA-256
`0f7702706247db46d11572b5d1e7be2b40d6a4a21d6f4a9cf3449da811db109e`. The exact pin passes
191 engine host/runtime tests, 111 public regressions, 226 public stateful/model tests, and all 916
LiteSVM/CU tests, including the 14-leg/28-source-domain required exits and dense terminal-shape
matrices.
...
| `cargo check --tests` | pass | exact engine-`a6e3c79` build completed at the current checkpoint |
| `cargo test --lib` | 7/7 | exact engine-`a6e3c79` rerun at the current checkpoint |
| `cargo test --test v16_program_stateful_fuzz` | 217/217 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 262.93 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets, including generated route selection for all four terminal retained-trade retry worlds, both exact external-debit retry families, the revoked-matcher paired terminal world with a fresh-grant nonvacuity witness, all five oracle-supersession terminal worlds, the exact fee-redirection terminal world, both resolve-policy threshold directions across 48 complete funded lifecycles, and fee-bearing maintenance/liquidation share worlds. The oracle worlds prove exact stale rollback and fresh terminal victim loss offset by counterparty gain, protected-recipient loss, or protocol-defined mint burn; the resolve worlds distinguish finite bounded delay from persistent DoS with complete route masks. The suite also owns the complete pending-obligation and close-ledger encumbrance census, direct per-domain `usable_positive_credit_num <= available_backing_num`, exact/bound claim transitions, credit-rate edge causality, and the independent resolved-payout quotient/remainder oracle. INV-086 additionally locks zero-amount withdrawal semantics and 64 all-route ADL unilateral-close compositions: owner reduction and one-sided-ADL Recovery force-close match the pre-state clamp, two-sided OI, remaining effective quantity, and exact terminal payouts across one-atom/exact/overshoot/raw-basis requests. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
| Registry/manifest checks in the INV-079 module | 14/14 | includes the source-complete 32-oracle evidence roster, 22 exact terminal-LoF dispositions, all 11 retained-retry dispositions, all 14 retained-control supersession dispositions with seven terminal-LoF rows, two attribution-only fee-share rows, one bounded-liveness resolve-policy row, zero terminal-value or liveness candidates, complete-route terminal classification, and the recursive 57-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 111/111 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 29.17 seconds; includes the generic 11-operation whole-market no-reuse matrix, deterministic terminal retained-trade, exact external-debit retry, revoked-matcher, all five oracle-supersession terminal worlds, the fee-redirection terminal world, both resolve-policy funded liveness directions, and both fee-share attribution worlds, the source-complete 11-kind retry and 14-kind supersession disposition censuses, the source-complete account-incarnation and detached-signature censuses, all public regressions, 57 trace consumers, executable registries, and the 32-oracle INV-079 evidence roster |
| `cargo test --test v16_cu` | 916/916 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 1,308.92 seconds; the INV-044 public matrix agrees across both permissionless account-crank orders and both portfolio leg-slot orders on exact claims, certified equity, source stock, terminal SPL value, and zero residue. The 14-feed refresh uses 931,087 CU and the 14-leg/28-source-domain required exit uses 933,246 CU; all required exits remain below the 1.4M-CU ceiling. The non-exit 28th-lien admission remains the explicit thin-headroom risk at 1,398,554 CU. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 193/193 | full wrapper-only exact-source rerun with engine `a6e3c79`; zero failures across decoder, identity-binding, assumption-nonvacuity, provider/token-parser, deployed-arithmetic, arbitrary engine-flow/SPL-custody, engine-stock/SPL-custody, and arbitrary-prestate tombstone harnesses, with every required constructive cover satisfied. The largest trailing-byte decoder query completed in 476.83 seconds with 0/1,676 failures rather than being skipped. |
| Engine runtime/property suites | 239/239 | exact engine-`a6e3c79` reruns: 50 library, 13 backing, 15 reference, 2 resolved, 19 rounding, 7 fuzzing, and 133 v16 specification tests pass |
...
| AUDIT-070 | OPEN-T | A complete public two-asset lifecycle now resolves, pays and dematerializes all five funded portfolios, proves zero accounting, and reaches `CloseSlab` across all 5! claimant orders while a foreign market remains byte-identical. A separate lifecycle funds prior insurance through the legacy public route, reconciles every user claim and portfolio close with independent stock/encumbrance censuses, drains terminal insurance exactly, and reaches `CloseSlab`. The underfunded four-route graph now consumes 123 exact-domain insurance atoms before resolution and carries that historical spend through partial receipt, later payout, all five terminal portfolios, and exact residual custody. Compose that spent-insurance world with slab retirement; then extend it with rounding, recovery, impairment, and surplus sweep. |
...
On engine pin `a6e3c79f2d6c3afdfb82260951d8a5be85f8fa5d`, the full 916-test `v16_cu` inventory is
