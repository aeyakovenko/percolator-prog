...
modules: 173 direct harnesses plus 38 macro-generated trade-decoder harnesses, exactly matching the
211 reported by `cargo kani list`. Every direct harness is classified into 103 symbolic-total, 31
...
| INV-081 | CLOSED - P(engine/wrapper composition) + F + Direct + SVM/CU | `cu/inv_081_success_state_validity_over_complete_public_routes.rs` joins nineteen source-complete executable layers over all 49 public instructions: decoding/domain, identity/authority, account roles/PDA/caller fields, admission, every engine call and wrapper persisted field, position/OI, scope, token/value, independent model, and error propagation. It additionally source-locks four mounted Kani composition theorems for value flow, exact stock, conservative certificate commit, and engine-error mapping, and pins the exact engine revision. The generated public model and seeded frontiers retain exact per-step success/rollback, value, OI, episode, lifecycle, terminal, and funded-exit oracles. This is a current-surface proof-equivalence decomposition under the named engine and SVM assumptions, not one monolithic arbitrary-byte query. A route, field, callsite, effect class, proof owner, witness, or pin change reopens it. |
