...
witness that transfers terminal value from the LP to the attacker. Seven oracle and fee-policy
families remain explicit terminal-value candidates; the other six remain constrained to signed
economic bounds, attribution, liveness, or provider-consent claims until stronger evidence exists.
...
inheriting that label; the supersession table keeps seven terminal-value candidates explicit. The
...
grant mutation witness; seven mark/configuration and liquidation/maintenance families remain named
terminal-value candidates for the next paired-world tranche.
