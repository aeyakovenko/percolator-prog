...
| `cargo test --test v16_cu` | 915/915 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 202.70 seconds; zero behavioral failures across market tombstone/rent/fresh-address behavior, matcher identity/incarnation and stateless-PDA composition, reserve, fee-policy/destination ownership, claim/backing/insurance single-use, exact lien lifecycle, source-realizability, exact-claim, credit-rate transition composition, exact B-booking/account-settlement/carry-normalization partitions, aggregate-versus-one-atom social-loss composition, the source-complete truncating-arithmetic ownership census, odd activation/redirect partitions, oracle, Recovery, terminal, source-lien normalization, selector composition, equivalent-route source composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling; the run observed 1,116,641 CU for max-source liquidation and 1,314,291 CU for max-shape resolved close. The non-exit 28th-lien admission reached 1,398,554 CU and remains an explicit thin-headroom risk. |
...
ratio-change frontier is the non-EWMA backing/claim-rate partition product. These tests do not imply
an unbounded cross-product.
