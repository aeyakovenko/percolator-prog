...
managed-oracle routes, the source-derived matrix now owns 25 epoch-bearing instruction variants as
29 semantic authority cases. Every case retains an old
...
covers 232 public route instances, and the deterministic policy/handoff/resolve product continues
...
adding market-wide state. The follow-on simplification removes the
...
caller-input and boundary rosters now own all 229 public fields across 52 input types, the proof
census owns 182 harnesses across 21 mounted modules, and the public-trace guard owns 33 consumers.
...
The exact rebuilt SBF is 1,230,960 bytes with SHA-256
`3b7c2f16c7c8b6603510d20737650a15638b0ab4b1d79240f616f33ef8342952`. Across the combined
route removal and epoch-binding work, production remains 240 source lines and 15,336 SBF bytes
...
stateful/model tests. The mounted wrapper proof census is 182 harnesses; exact final rerun results
...
routes that reach configured-authority logic: 25 are epoch-bound above; `ClosePortfolio` is
independently portfolio/sequence/episode-bound; and both auxiliary-ledger synchronizers are
deterministic current-state reconciliation. One explicit epoch gap remains:
...
recreation and the one source-rostered lifecycle authority route named in the current checkpoint.
...
all 229 fields across 52 public input types map to exactly one of 20 semantic profiles, one
...
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`9b737fd`/SBF-`3b7c2f16` rerun completed in 301.74 seconds; the 29-case authority-incarnation matrix, every retained-policy landing order, and every prior stateful/model check pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full exact engine-`9b737fd`/SBF-`3b7c2f16` rerun completed in 45.25 seconds; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 890/890 | full exact engine-`9b737fd`/SBF-`3b7c2f16` rerun completed in 199.22 seconds; zero behavioral failures, including the source-complete authority-epoch route/decoder matrices and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse -j 2` | 182/182 | full wrapper-only exact-source rerun with engine `9b737fd`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied. Five solver-hostile decoder relations are decomposed into exact per-route production bodies; the two base-unit proofs preserve every full-width field and reject trailing bytes in under 3.3 seconds. |
...
fee consent remain explicit INV-001/INV-005/INV-014 gaps. The 29 epoch-bearing authority cases now
...
| INV-005 | Independent + P + Direct + F + SVM/CU + Static roster | `public_sbf/inv_005_authority_incarnation_binding.rs`, `stateful/inv_005_authority_incarnation_binding.rs`, `cu/inv_005_authority_incarnation_binding.rs`, and `kani/inv_005_authority_incarnation_binding.rs` own 25 epoch-bearing source variants as 29 semantic authority cases. Every case crosses same-market `A -> B -> A`, rejects the stale retained request with exact rollback, and admits a mutating current-epoch control. Kani proves exact epoch admission, checked handoff advance, migration-floor preservation, and exact generation/epoch wire binding for reserve withdrawals and both base-unit routes. A source-derived call graph classifies all 29 configured-authority routes: 25 epoch-bound, three proof-backed current-state or independently bound routes, and one explicit epoch gap (`UpdateAssetLifecycle`). Whole-market same-pubkey recreation remains an INV-001 dependency because account-local epochs reset. |
...
| AUDIT-005 | OPEN-D | A source-derived call graph classifies all 29 configured-authority routes. The 25 epoch-bearing source variants, represented by 29 semantic authority cases, reject same-market `A -> B -> A` with exact rollback and admit fresh current-epoch controls. `ClosePortfolio` is independently incarnation/sequence/episode-bound, and both auxiliary-ledger synchronizers are deterministic current-state reconciliation. One route remains an explicit epoch gap: `UpdateAssetLifecycle`. The ambiguous market-wide insurance route is removed; its old tag rejects while the scoped route is epoch-bound. Kani proves exact epoch admission, atomic checked handoff, the migration-aware floor, and exact generation/epoch wire binding for reserve withdrawals and both base-unit routes. Persistent market incarnation remains separately open because close/recreate resets account-local epochs. |
