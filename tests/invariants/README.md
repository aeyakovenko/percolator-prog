...
| AUDIT-063 | OPEN-T | Trade consumption has a nonvacuous 4-route x `expiry-1`/`expiry`/`expiry+1` public matrix: every fresh control grows a real lien, fee-capable routes charge real fees, and both expired boundaries roll back while preserving reduction. Released-PnL conversion now has the same three-slot matrix: the fresh control consumes exact backing and withdraws real SPL, while expired cells roll back and preserve senior exit. Retained top-up has a generated expiry regression, and crank/close/payout have focused cases. Release, claim, close, payout, and retire still lack one complete consumer-by-boundary matrix; no proof establishes normalization before every consumer. |
