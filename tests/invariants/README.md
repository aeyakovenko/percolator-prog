...
| AUDIT-079 | OPEN-D | The manifest enforces names/classifications, and a source-derived registry now fails unless every production instruction tag has named public-route/CU evidence or an explicit omission. No formal finding trace-evidence schema records signers, account metas, absence of out-of-band mutation, exact token/lamport deltas, and persistent exit result; add and replay that schema for every qualifying PoC. |
