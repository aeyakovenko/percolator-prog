...
The lifecycle decoder additionally proves every full-width field and exact trailing-byte rejection
without reconstructing the 128-byte authority tuple in a parallel model.

The exact rebuilt SBF is 1,231,696 bytes with SHA-256
`240ac89d414773038941ca5fb626521fc6957c79bd6aaf065bdaa0617490521f`. Across the combined
route removal and epoch-binding work, production remains 208 source lines and 14,600 SBF bytes
smaller than the preceding checkpoint. The
complete exact-artifact gates cover 891 LiteSVM/CU tests, 102 public fuzz regressions, and 202
...
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`9b737fd`/SBF-`240ac89d` rerun completed in 259.25 seconds; the 34-case/272-world authority-incarnation matrix, every retained-policy landing order, and every prior stateful/model check pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full exact engine-`9b737fd`/SBF-`240ac89d` rerun completed in 27.56 seconds; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 891/891 | full exact engine-`9b737fd`/SBF-`240ac89d` rerun completed in 206.44 seconds; zero behavioral failures, including the source-complete authority-epoch route/decoder matrices, canonical permissionless lifecycle epoch, and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse -j 2` | 183/183 | full wrapper-only exact-source rerun with engine `9b737fd`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied. Seven solver-hostile decoder relations use exact per-route production bodies: four trade, two base-unit, and one lifecycle. The lifecycle proof preserves all full-width scalar and authority fields and rejects trailing bytes. |
