...
below; PR259 is now `Certified`, leaving nine executable quarantines.
...
This adds no deployed code. A source-complete search of pinned engine `422893fa` also finds no
...
asset-generation holdout adapters. Its nine focused stateful tests enumerate all 20 retained
...
232 fields across all 49 public instruction variants and the three nested input structs. INV-083
...
all 232 fields across 52 public input types map to exactly one of 20 semantic profiles, one
...
sources: 150 direct harnesses plus 36 macro-generated trade-decoder harnesses, exactly matching the
186 reported by `cargo kani list`. Every direct harness is classified into 85 symbolic-total, 26
...
`422893fa`; approximate-bucket rebucketing is N/A for this profile, while complete-state reachability
remains.
...
20-family generated public replay matrix exercises every retained asset-control family it owns
across retire/reuse with exact rollback and fresh mutation controls. The source roster exposed one
additional generation-bearing route outside that generator: backing-earnings withdrawal. Its
dedicated retained transaction now rejects after slot reuse with exact market, ledger, SPL, and
supply rollback, while a current-generation request passes the generation gate. Existing
value-moving INV-025/028/088 routes prove the same handler remains live. The roster owns all
...
| Focused INV-029 exact-only deployed profile | 208/208 stateful public tests plus 1/1 source lock | after every generated successful public transition, each domain's exact claim equals its positive bound and the complete portfolio attribution; the wrapper exposes no non-exact bound or rebucketing ingress, so approximate-bucket range/rebucketing proofs remain N/A unless that optional profile is introduced |
| Focused INV-030 source-credit transition causality and malformed state | 208/208 stateful public tests plus 3/3 focused CU | every generated public action and successful permissionless crank compares both markets before and after: unchanged formula inputs preserve the rate, every formula-input mutation advances `credit_epoch`, and a live claim's rate rises only with more independently available backing or a smaller claim bound. Twenty malformed relation cases and two exact source-boundary truncations cross both source sides with instruction-error and complete economic rollback; a pin-bound source lock leaves on-chain rate/epoch mutation exclusively in the engine |
...
| `cargo check --tests` | pass | exact engine-`422893fa` build completed on the 2026-08-28 PR135 test head |
| `cargo test --lib` | 7/7 | exact engine-`422893fa` rerun on the 2026-08-28 PR135 production head |
| `PERCOLATOR_FUZZ_CASES=2 cargo test --test v16_program_stateful_fuzz` | 208/208 | full exact engine-`422893fa`/SBF-`10a2de6d` rerun completed in 224.72 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets. The suite includes the finding-blind source-domain single-use and retained source-fee consent campaigns. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
| Registry/manifest checks in the INV-079 module | 12/12 | includes the source-complete 27-oracle evidence roster, sixteen exact terminal-LoF dispositions, complete-route terminal classification, and the recursive 49-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 104/104 | full exact engine-`422893fa`/SBF-`10a2de6d` rerun completed in 27.60 seconds; includes the deterministic source-domain single-use and retained source-fee cap certifications, all other public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 895/895 | full exact engine-`422893fa`/SBF-`10a2de6d` rerun completed in 203.01 seconds; zero behavioral failures across reserve, fee-consent, oracle, Recovery, terminal, lifecycle, and maximum-shape routes. The observed required exits remain below the 1.4M CU ceiling. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 186/186 | full wrapper-only exact-source rerun with engine `422893fa`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied. The three new direct proofs reject both cap-less single-trade schemas and preserve each signed cap through the exact production decoder. |
| Engine runtime/property suites | 187/187 | exact `cargo test --all-targets` rerun on engine `422893fa`. All unit, backing, insolvency, and v16 specification tests pass, including exact source-local claim consumption and atomic funded-backing retry rejection. |
| Focused engine source-conversion guard proofs | 2/2 Kani | the exact production guard blocks both symbolic active source exposure and a symbolic nonzero flat claim with a live lien; both harnesses have constructive covers, and the Kani adapter contains the same short-circuit predicate as production. |
...
branch. The locally rebuilt 1,233,464-byte production SBF used by the 2026-08-28 exact-pin gates has
SHA-256 `10a2de6df9b6ac3229a1fc3c3eb118f4bd9dfe81ad38e511b3b08b707f54abba`.
...
1. Convert the 9 `Quarantined` adapters to `Certified` only after the current pin satisfies their
   positive economic and liveness postconditions. The other 90 entries already have explicit
   executable disposition: 82 fixed-pin certifications and 8 public nonqualifying proofs. Do not
...
| --- | ---: | --- |
| `public_sbf/` | 104 | Deterministic public SBF/LiteSVM regressions, decoder corpora, manifests, and public-trace checks. The retained-intent inventory covers every family, same-transaction duplicates, both insurance entrypoints, and all 16 ordered single/batch CPI/no-CPI trade pairs with exact rollback and economic-state oracles. INV-015 additionally owns the complete persisted byte-domain and alignment boundary; INV-031 owns the fixed-seed cross-domain backing single-use witness; INV-036 owns the fixed-seed retained source-fee cap certification. |
...
reconcile exact provider withdrawal. Both retained backing-provider policy orders in PR339 remain
public counterexamples. Matcher mutations now bind the
...
replay (including PR296/325/326), non-epoch authority-scope retention, and PR339 backing-provider
fee consent remain explicit INV-001/INV-005/INV-014 gaps. The 34 epoch-bearing authority cases now
...
source-local claim/backing single use. The nine remaining quarantines are intentionally still red:
PR339 provider consent and PR293/294/295/296/307/317/325/326 whole-market incarnation replay.
...
| AUDIT-014 | OPEN-D | Same-incarnation sequence supersession is now bidirectionally complete: all fourteen retained control families cross retained-higher/current-lower and retained-lower/current-higher payloads under exact rollback, including both backing sides and every oracle mode. Every sequence-bearing policy route also binds the exact current authority epoch. Independent fee-consent oracles cover the current signed/live route set. Whole-market recreation, remaining non-epoch authority scopes, and durable backing-provider fee consent remain live schema gaps. |
...
82 are `Certified`, 9 remain `Quarantined`, 8 are `Nonqualifying`, and none are `Missing`. Its
...
On engine pin `422893fa0a43a2d07697e88dd2009dcef8dce1d5`, the full 895-test `v16_cu` inventory is
...
owned by INV-028/071/073/077/082; INV-034 additionally owns the public cross-domain-loss
