...
standalone request remains live and bounded, and its duplicate then rejects exactly. Public wire
...
| Focused INV-008 retained-operation retry matrix and contracts | 11/11 public, 10/10 stateful, 4/4 local Kani, 4/4 source/layout CU, plus 3/3 real failed-CPI retry probes; all 11 retained families reject stale retries with exact rollback, every family rejects an identical same-transaction duplicate with bundle-wide rollback before exactly one standalone execution, all 50 public variants have a machine-checked replay disposition, and both direct/domain insurance cross-entrypoint orders remain atomic | rerun on the 2026-08-21 PR135 production head; no live violation in the current retained-operation roster |
...
| `cargo check --tests` | pass | exact engine-`b10b3454` build completed in 6.01 seconds on the 2026-08-21 PR135 production head |
| `cargo test --lib` | 7/7 | exact engine-`b10b3454` rerun on the 2026-08-21 PR135 production head |
| `cargo test --test v16_program_stateful_fuzz` | 197/197 | exact engine-`b10b3454`/SBF-`342f7bb5` rerun in 253.59 seconds, including randomized same-transaction retained-intent duplication, all 2,380 public base words through depth three and their 6,942 independently checked edges, the 32-world live source-lien route-pair ownership matrix, four independent flat-source-lien route worlds with complete terminal observations, source-complete validation of every current public trace consumer, and every prior rank/reference-model, lifecycle, source-lien, terminal, and route-composition check |
...
| `cargo test --test v16_program_fuzz_regressions` | 96/96 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.29 seconds; every retained family passes the same-transaction duplicate matrix, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
