...
| `cargo test --test v16_program_stateful_fuzz` | 203/203 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun completed in 260.42 seconds; the independent exact-receipt/unreceipted-portfolio-bound oracle, public two-stage exact receipt top-ups, six active-payout identity substitutions, nonfinal/terminal close and mode-exclusion checks, 224 bidirectional supersession worlds, and two value-moving underfunded authority/claim worlds join every prior stateful check |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.91 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
...
The current ledger is 9 **CLOSED**, 49 **OPEN-T**, 12 **OPEN-D**, 6 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-013 | OPEN-T | Close consent now binds the exact portfolio ID, shared owner-state sequence, and position epoch; deterministic public funding telemetry, generated deposit/withdraw ABA, failed-deposit rollback, fresh-close liveness, and exact Kani tuple/sequence contracts cover issue 402. Claim and receipt top-up are current-state permissionless transitions with owner-bound destinations, no retained destructive consent, and INV-068's lifecycle exclusion. Shutdown, resolve, liquidation delegation, recovery conversion, and every later retained-consent lifecycle episode still need the same matrix. |
