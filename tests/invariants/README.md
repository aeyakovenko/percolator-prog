...
ADL matrix now repeats liquidation after a later authenticated deficit either on the same selected
leg or, after exact canonical removal of that residual, on the other asset. Larger account
partitions, three-plus episodes, and the remaining maximum-shape cross-products are the next tractable
...
| Focused INV-061/071/073/077/082/086 combined-maxima equal-risk liquidation | 4/4 public persisted-leg/observation-order worlds | all fourteen active legs and twenty-eight source domains become equally adverse; observation order is inert, persisted order selects asset 0 or 13, eleven strict liquidations restore health below 1,155,033 CU, and four owner reductions plus fourteen cleanup/finalization steps lead to exact senior withdrawal. Live unbacked junior conversion rejects with exact rollback; fifteen bounded resolved continuations settle the retained claims and both portfolios close with exact custody. The smaller 32-world two-asset matrix separately repeats liquidation after a second authenticated deficit on either the same or next selected asset and rechecks exact selection, quantity, OI, fees, attribution, and frames. |
...
| INV-061 | Independent + P(engine) + SVM/CU | `stateful/inv_061_deterministic_bounded_liquidation.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `cu/inv_061_deterministic_bounded_liquidation.rs`, `cu/inv_059_fee_fragmentation_bound.rs`, and the issue-408 liquidation world in `cu/inv_027_protected_principal_seniority.rs`. Two public post-ADL prefixes prove pre/post-mark transfer extraction rejects with exact rollback, cannot consume extra backing or create phantom value, and retain bounded owner reduction whose raw-basis result is independently derived from remaining effective OI. Four public dual-ADL transport worlds independently reconstruct the exact liquidation maintenance, target/effective lag, fee/minimum-fee, floor, projected-health, and binary-search selector and require its exact partial close, OI deltas, effective remainder, no-SPL frame, and CU bound. The resolved-ADL matrix proves both terminal landing orders consume only remaining effective OI, pay both funded users exactly, reconcile custody to zero, and close both portfolios. The fractional reset-carry matrix publicly creates two bankruptcies, requires the sole account crank to strictly liquidate its target under the CU cap, then gives every remaining owner a stale raw-basis work budget. Each budget clamps to exact effective OI; all legs and both resets clear in bounded public work. The trace explicitly refills the resulting 11-atom provider receivable, withdraws all restored provider principal, and proves retirement clears only historical spent/source/social/K/F audit state without changing custody. A 32-world two-asset/three-user matrix crosses all four opening transports, both equal-risk target-leg orders, both accrual orders, and same-versus-next-selected-asset follow-up. Every world creates a later authenticated deficit and requires a second exact close, fee, selected-domain budget credit, nonselected frame, and no-SPL frame; next-asset worlds first require exact canonical residual removal and a bounded authenticated time gap. The 48-world successor crosses three unequal losses, all six persisted leg orders, and both accrual orders; every crank closes the independently modeled first live leg, frames all nonselected value/loss domains, stays below 312,341 CU, and converges to exact order-independent terminal payouts. A four-route underfunded bridge now proves one 315,258-CU crank removes exactly 70,000,000 matched OI, finalizes the exact 2,723-atom close, and composes through a 1,000-face partial receipt, a later value-moving payout, and terminal custody. A separate all-route campaign proves two fee-bearing liquidations require two authenticated deficits, same-state/malformed retries cannot manufacture an episode, and owner reduction remains live after episode two. Arbitrary close splitting, three-plus episodes, larger account partitions, and remaining maximum-shape cross-products remain. |
...
The current ledger is 44 **CLOSED**, 21 **OPEN-T**, 10 **OPEN-D**, 0 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-048 | OPEN-T | All four fresh trade routes scan raw OI, and the stateful model keeps an exact independent effective-OI transition ledger across matched/retained trades, crank liquidation, owner rebalance, prior-reset cleanup, and recovery forfeit. Three 32-world public matrices create nonzero ADL through every trade transport and cross `effective - 1`, exact effective, `effective + 1`, and raw basis. Owner reduction crosses both terminal claimant orders; one-sided- and dual-nonunit-ADL Recovery force-close cross both account orders. Each requires the independently derived clamp to match both side OI deltas and recomputed remaining legs before exact terminal value returns. Four exact scaled-liquidation worlds independently reconstruct selector quantity from pre-state and check both OI lanes. Sixteen dual-ADL Recovery-forfeit worlds cross both owner orders and one/max B budgets, frame the opposite OI lane, drain the retained obligation, and converge to the same Live exits. Both resolved-close owner orders now independently census every active leg before resolution and after every accepted close, require deployed nonzero-ADL OI to match the census and decrease monotonically, and end with both OI lanes zero. A 32-world two-asset/three-user ADL matrix derives both assets' effective OI, observes exact selected-only two-lane reduction, frames the nonselected asset, repeats those checks after a later authenticated deficit on either the same selected leg or the other asset after exact residual removal, and drains every residual/reset leg through bounded public exits across all trade transports and landing orders. The 48-world unequal-loss successor verifies selected-only matched-OI removal over three assets and every persisted leg/accrual order. The four-route underfunded bridge removes exactly 70,000,000 matched OI before a partial receipt and terminal custody; the adjacent unattributed-loss world removes a surviving 5,000,000-unit leg without inventing a close domain. A separate 24-world public bankruptcy matrix pins the zero-OI pending-obligation boundary. Transfer/import and caller-sized liquidation are absent from the wrapper; larger partitions/topologies, three-plus episodes, and remaining maximum-shape composition remain. |
...
| AUDIT-061 | OPEN-T | Liquidation safety, fees, progress, and selected generated schedules are covered. The PR250 post-ADL transfer/extraction and phantom-value prefixes reject exactly and retain bounded owner reduction. Four finding-blind dual-ADL route worlds independently reproduce maintenance, target/effective lag, fee/minimum-fee, floor, projected-health, and binary-search sizing from the authenticated certificate and require the selected crank's exact partial close, two-sided OI delta, no-SPL frame, and CU bound. The resolved-ADL close-order violation is fixed on `6c04db7e`: both landing orders advance through effective-OI reduction and reset cleanup to exact funded exits. The independent fractional reset-carry violation terminates on `573c4e90`: the sole public crank strictly liquidates the target below its CU cap, clears eight affected legs and both resets, settles every claim/provider obligation, and retires the asset through one canonical terminal transition. Carry normalization and terminal frames are engine-contract checked. Thirty-two equal-risk public worlds cross all opening transports, target-leg orders, accrual orders, and same-versus-next-selected-asset follow-up in a two-asset/three-user ADL topology. The next-asset branch requires exact canonical residual removal and value/counterparty/SPL frames before a bounded authenticated time gap; both branches require a second exact fee-bearing liquidation with selected-domain attribution and nonselected frames. A 48-world successor crosses three unequal losses, all persisted leg orders, and both accrual orders while framing every nonselected value/loss domain. A four-route underfunded bridge removes exactly 70,000,000 matched OI, preserves the resulting 2,723-atom finalized close through resolution, and composes it into a genuine partial receipt, a value-moving follow-up payout, and terminal custody. A second four-route world proves cross-asset unattributed loss takes the direct bounded-liquidation path rather than creating a conflicting close rank. The INV-059 all-route campaign now composes two fee-bearing liquidations across separate authenticated deficits, exact same-state/malformed rollback, route-identical economics, and a post-episode owner reduction. Caller-sized liquidation is absent and source-locked; add three-plus episodes, larger account partitions, and the remaining maximum-shape cross-products. |
