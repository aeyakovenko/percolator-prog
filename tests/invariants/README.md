...
| Focused INV-010 retained-operation landing orders | 3 fixed matrices, 72 public worlds | all `3!` deposit/withdraw/disable orders at three value boundaries enforce one shared-sequence winner and exact stale rollback; both deposit/reduction orders at three boundaries converge outside the conservative health-certificate cache; all eight market/asset-0 policy lanes cross both authority-handoff orders at low/mid/max values with exact stale rollback, fresh incoming-authority liveness, and complete public capital exits |
...
| `cargo check --tests` | pass | exact engine-`b10b3454` build completed in 6.06 seconds on the 2026-08-21 PR135 production head |
| `cargo test --lib` | 7/7 | exact engine-`b10b3454` rerun on the 2026-08-21 PR135 production head |
| `cargo test --test v16_program_stateful_fuzz` | 199/199 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun passed in 258.24 seconds; the new test contributes 48 authority/policy landing-order worlds, and every prior randomized retry, trade-route, depth-three graph, source-lien, rank/reference-model, lifecycle, terminal, and route-composition check remains green |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.35 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 27 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
...
| AUDIT-010 | OPEN-T | All `3!` landing orders of two same-sequence matcher controls and one retained CPI trade are exhausted with state-derived consent, exact rollback, and a matcher-independent exit. All `3!` retained deposit/withdraw/disable orders at three value boundaries prove exactly one shared-sequence winner and complete capital exit. Deposit and position-episode reduction commute economically in both orders at three boundaries outside the conservative certificate cache. A 48-world matrix crosses market-authority handoff with all eight market/asset-0 policy lanes at low/mid/max values: policy-first preserves authorized state, handoff-first rejects the former authority exactly, every incoming-authority policy remains live, and users exit. Add resolve and claim permutations, then widen the bounded cross-product. |
