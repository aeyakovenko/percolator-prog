...
`202b802f4fb1a8ad46523194473c35d08d0bc16f`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`61f3d4d0dfda182c40bd1a331c74d1114c8303613972d4b113ac949a095a807f`. Resolved-mode
...
| Focused INV-061/069/073/074 fractional-carry terminal lifecycle | 1/1 public liquidation-to-retirement world, 1/1 Recovery-to-fresh-generation world, 2/2 owner exit routes, 3/3 engine whole-body terminal proofs, and 1/1 wrapper Kani gate | engine `cae71267` normalizes fractional social-loss carry; `5b495f33` clears settled reset baselines; `f42650f7` clears spent-only terminal source audit; and `573c4e90` canonicalizes spent-domain/source/social/K/F history inside one restart or retirement while rejecting live receivables. On `202b802f`, the liquidation world additionally applies every latent B delta before reset detachment, proves no stale winner claim or provider debt is manufactured, returns the real unused provider principal despite sticky bankruptcy history, and retires the dynamic asset. The separate Recovery route still settles real claims/provider obligations, restarts asset 0, proves fresh insurance has zero inherited spend, completes a fresh trade round trip, and frames SPL custody below every CU cap. |
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `202b802f` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `202b802f` |
| `cargo test --test v16_program_stateful_fuzz` | 176/176 | full exact-pin rerun in 164.33 seconds, including both active-close/shutdown landing orders and every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full exact-pin rerun in 25.08 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 792/792 | full exact-pin rerun in 199.85 seconds, including maximum-source exits, maximum-shape Recovery obligation cleanup, liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and B-before-claim reset-carry retirement |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 137/137 | full wrapper-only exact-pin rerun on engine `202b802f`; zero harness failures, including all declared nonvacuity covers |
| Engine runtime/property suites | 220/220 | rerun with `cargo test --features fuzz` on engine `202b802f`; all unit, reference-model, arithmetic-discharge, source-credit, insolvency, and v16 specification tests pass, including latent Recovery B settlement |
...
`61f3d4d0dfda182c40bd1a331c74d1114c8303613972d4b113ac949a095a807f`.
...
| INV-061 | Independent + P(engine) + SVM/CU | `stateful/inv_061_deterministic_bounded_liquidation.rs`, `cu/inv_061_deterministic_bounded_liquidation.rs`, and the issue-408 liquidation world in `cu/inv_027_protected_principal_seniority.rs`. Two public PR250 prefixes prove pre/post-mark post-ADL transfers reject with exact rollback, cannot consume extra backing or create phantom value, and retain bounded owner reduction. The resolved-ADL matrix proves both terminal landing orders consume only remaining effective OI, tolerate only exact-rollback payout waits, pay both funded users exactly, reconcile custody to zero, and close both portfolios on engine `6c04db7e`; the shared full-width reduction kernel is contract checked. The fractional reset-carry matrix publicly creates two bankruptcies, requires the sole crank to strictly liquidate the funded target under its CU cap, clears all eight affected legs and both resets, and retires the dynamic asset. On `202b802f`, it also proves latent B settles before detachment, all account PnL and source claims are zero, no provider debt is manufactured, and real unused backing principal remains withdrawable. Engine contracts cover carry normalization, reset-baseline clearing, one-call terminal-history normalization, the live-receivable blocker, and the exact latent-B predicate. Other liquidation selection/order and maximum-shape cross-products remain. |
...
| AUDIT-082 | FRONTIER | The first bounded public transition graph now composes ten public prefixes across two configurations with the deployed mode-aware rank, records only strict lexicographic crank reductions, and proves every observed actionable rank class has a path to zero. The rank independently reconstructs active close residual, canonical per-leg B delta, exact released-obligation eligibility, stale work, and health work in dispatch order rather than trusting cached/global actionability summaries. Public witnesses reduce close, B, obligation, reset, and health classes; the active-close/shutdown pair caught and fixed the old selector's latent-Recovery-B contradiction. The reference model still distinguishes genuine no-permissionless-work Recovery states from owner-exit work. Expand the graph alphabet and state dimensions to all lifecycle, close, B, receipt, oracle-failure, and recovery classes; then connect each abstract node to a public-route reachability witness or a proven unreachability argument. |
...
On engine pin `202b802f4fb1a8ad46523194473c35d08d0bc16f`, the full `v16_cu` inventory is
