...
`6c04db7e01ed2eadc1ea018a521300dc52bcf686`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`4fe6a2bd5f16971510ef59ff2715e4043835cec27c70ff690564a16baa4ebab3`. Resolved-mode
...
engine's 213-test runtime suite and the full-width shared reduction-kernel contract are green. No
...
claim. The asset-local close-drift, unrelated backed-claim lock, and resolved-ADL close-order
branches are now covered and fixed. The next owned public counterexample is INV-061's fractional
reset-carry liquidation lock; the next broader INV-074 target is the remaining
side/domain/receipt locality cross-product, followed by the liveness frontier shared by INV-057,
INV-071, INV-073, INV-078, and INV-082: extend
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `6c04db7e` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `6c04db7e` |
| `cargo test --test v16_program_stateful_fuzz` | 173/173 | full exact-pin rerun in 166.04 seconds; the two-order resolved-ADL public matrix advances both funded users to exact payout and portfolio close through the sole public automatic crank |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full exact-pin rerun in 25.65 seconds; the machine-checked audit census and discovery/special-method rosters name the positive INV-061 route oracle |
| `cargo test --test v16_cu` | 791/791 | full exact-pin rerun in 192.67 seconds, including maximum-source exits, maximum-shape liquidation/resolved close, exhaustive account-role matrices, and the dense zero-delta terminal-shape benchmark |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 136/136 | full wrapper-only exact-pin rerun on engine `6c04db7e`; zero harness failures |
| Engine runtime/property suites | 213/213 | rerun with `cargo test --features fuzz` on engine `6c04db7e`; the new resolved-ADL test proves a basis larger than effective OI advances through bounded reduction, reset, residue detach, and exact capital payout while framing opposite-side OI. The full-width shared reduction-kernel function contract also passes. The historical-bankruptcy account/global H-lock proofs, originating-asset close-staleness contract, and inherited arithmetic/conservation suites remain green |
...
| AUDIT-061 | OPEN-D | Liquidation safety, fees, progress, and selected generated schedules are covered. The PR250 post-ADL transfer/extraction and phantom-value prefixes reject exactly on the integrated engine and retain bounded owner reduction. The public/stateful resolved-ADL close-order violation is fixed on engine `6c04db7e`: both landing orders advance through effective-OI reduction and reset cleanup to exact funded exits. The fractional reset-carry test still asserts an independent violation. Fix that state, then add equal-risk permutations, arbitrary close splitting, normalized loss attribution, and max-shape liquidation coverage. |
...
On engine pin `6c04db7e01ed2eadc1ea018a521300dc52bcf686`, the full `v16_cu` inventory is
