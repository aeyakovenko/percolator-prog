...
`4b23b197d0c130f04aafff486796b292c5bbff21`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`aafca92f5d42951d00022e911a8e975163c96ae543bcf591c4cb283cb11be061`. Resolved-mode
...
claim. The asset-local close-drift and unrelated backed-claim lock branches are now covered and
fixed. The next concrete INV-074 target is the remaining side/domain/receipt locality cross-product,
followed by the liveness frontier shared by INV-057, INV-071, INV-073, INV-078, and INV-082: extend
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `4b23b197` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `4b23b197` |
| `cargo test --test v16_program_stateful_fuzz` | 173/173 | full exact-pin rerun in 164.50 seconds; terminal fixed points require crank/close rejection rather than successful no-op, while claims may reject or remain quiescent; INV-045 includes 64 generated interior cases, 64 ordered route-composition worlds, and 16 repeated four-step/catch-up worlds, while INV-046 adds 64 all-route extreme-price/lifecycle/request-shape worlds |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full exact-pin rerun in 25.42 seconds, including the machine-checked 89-row invariant-audit verdict census |
| `cargo test --test v16_cu` | 791/791 | full exact-pin rerun in 192.64 seconds, including the 28-source conversion/withdraw/close lifecycle, complete canonical single-byte decoder edit census, wrapper-level fee/target-lag lane composition, multi-instruction engine-error propagation, expanded resolved-close and public backing-earnings account-pair matrices, real Token-2022 fee/hook rejection, six primary-decimal raw-atom worlds, all 15 public quote-delta routes, exact-expiry retirement, the 14-asset paid-EWMA full exit, mixed clear/flip/attach/resize slot-plan equivalence, bounded prospective-source expiry progress, the max-source funded refresh/liquidation/owner-exit matrix, the funded unavailable-Pyth terminal lifecycle, the 14-leg/three-feed/full-backlog staggered refresh schedule, the asset-local close-drift public red/green route, and the historical-bankruptcy unrelated-claim conversion/full-exit matrix. One synthetic program-byte-injection test was removed because it violated INV-079 and supplied no public reachability evidence |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 136/136 | full exact-pin rerun on engine `4b23b197` with Kani 0.66; the dedicated target mounts only wrapper-owned proof modules and avoids compiling the LiteSVM runtime suites as proof targets |
| Engine runtime/property suites | 212/212 | rerun with `cargo test --features fuzz` on engine `4b23b197`; an exact source-ledger conversion test retains a historical bankruptcy bit while consuming backing and preserving stock. The account-scoped H-lock Kani harness proves the historical bit alone cannot override an independently checked account, while the unchanged global-lane harness proves the bit still selects global H-max; both targeted harnesses pass with all covers. The originating-asset close-staleness contract and inherited arithmetic/conservation suites also remain green |
...
| INV-074 | Independent + P(engine) + SVM/CU | `cu/inv_074_scope_locality.rs` and `cu/inv_076_close_drift_residual_durability_and_finalization_atomicity.rs`. Asset-local stale/bankruptcy cases preserve unrelated withdrawals and existing-position exits. The public close-drift trace proves unrelated authenticated accrual cannot turn a still-bookable local close into global Recovery; engine `377de75c` scopes staleness to the originating asset and contracts that predicate. The two-/three-cohort backed-claim matrix proves engine `4b23b197` no longer lets permanent bankruptcy history override an unrelated exact source ledger: conversion, full SPL withdrawal, and portfolio close complete while the history remains set, and account/global lane behavior is Kani checked. Broader side/domain/receipt locality remains. |
...
The current ledger is 2 **CLOSED**, 55 **OPEN-T**, 13 **OPEN-D**, 6 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-074 | OPEN-T | The unrelated-accrual close-drift path is fixed and function-contract/public-route covered: a local bookable close no longer gives its owner authority to force global Recovery. The permanent historical-bankruptcy lock on unrelated exact backing is also fixed across two public cohort sizes; the claimant converts once, withdraws fully, and closes while failed portfolios/custody are framed and the history remains set. Complete the side, portfolio, domain, close, receipt, and lifecycle locality cross-product before promotion. |
...
On engine pin `4b23b197d0c130f04aafff486796b292c5bbff21`, the full `v16_cu` inventory is
