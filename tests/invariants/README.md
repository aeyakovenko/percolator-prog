...
872/872 LiteSVM/CU tests, 100/100 public fuzz regressions, and 206/206 stateful/model tests. Static
...
prevents this progress from being overstated: 34 instruction variants have exhaustive current-shape
matrices, four have an explicitly named dynamic-tail/action gap, and twelve remain open. The exact
queue is executable data in `inv_017_account_role_coverage.tsv`; adding or renaming an instruction,
claiming exhaustive evidence without a live test, or dropping a gap fails the CU gate.
...
| Focused INV-017 account-pair and privilege matrices | 78 custody/payout pairs plus 18 downgrades; 126 reserve-custody pairs plus 40 downgrades; 19 core/crank pairs plus 17 downgrades; 6 authority-handoff pairs plus 6 privilege downgrades; 21 initialization/matcher-config pairs plus 7 privilege downgrades; 6 ledger-sync pairs plus 6 privilege downgrades; 7 policy-route pairs plus 14 privilege downgrades; 6 managed-mark/lifecycle pairs plus 13 privilege downgrades | every covered schema starts from a successful public mutation control; every hostile pair or privilege downgrade rejects with exact rollback; the source-complete roster enforces 34 exhaustive, 4 partial, and 12 open variants |
...
| `cargo test --test v16_cu` | 872/872 | full engine-`b10b3454`/SBF-`fb16e50e` exact-source rerun passed in 199.21 seconds on the 2026-08-26 checkpoint; zero failures, including every prior maximum-shape and invariant route plus the source-complete INV-017 account-role roster and exhaustive matrices for authority handoff, initialization, matcher configuration, auxiliary-ledger sync, fee/resolve policy, managed-mark, authority-resolution, drain-only, and stale-resolution routes |
...
| `cu/` | 872 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership and account-role disposition rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, 60 partial-fill route/ratio/maximum-domain worlds, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
| INV-017 | SVM/CU + Partial M | `cu/inv_017_signer_writable_role_and_account_alias_safety.rs`, `stateful/inv_017_signer_writable_role_and_account_alias_safety.rs`, and `inv_017_account_role_coverage.tsv` exhaust all ten direct and all 21 CPI semantic account-pair aliases for single/batch trade; all 15 deposit, 21 withdraw, 21 resolved-close, and 21 value-moving resolved-claim pairs; 126 reserve-custody pairs plus 40 privilege downgrades across market/domain insurance top-ups, backing top-ups, optional ledger tails, insurance/backing withdrawals, and independently generated backing earnings; 19 core/crank pairs plus 17 downgrades across flat close, unilateral reduction, maintenance sync, no-tail crank, and value-moving liquidation-reward crank; all six account-pair aliases plus six privilege downgrades across market and asset-oracle authority handoffs; 21 account-pair aliases plus seven privilege downgrades across portfolio initialization and disabled/enabled matcher configuration; six ledger-sync pairs plus six downgrades; all seven two-account fee/resolve policy pairs plus fourteen downgrades; and six managed-mark/lifecycle pairs plus thirteen privilege downgrades across all four managed-mark routes, authority resolve, drain-only transition, and permissionless stale resolve. Every shape starts from a successful mutating fixture and hostile cases require exact rollback. The source-locked roster prevents drift and currently classifies 34 variants exhaustive, four partial, and twelve open with exact remaining shapes. |
...
| AUDIT-017 | OPEN-T | A source-complete roster binds all 50 production variants to an account-role disposition and live evidence. Thirty-four variants are exhaustive; four remain partial (`PermissionlessCrank` authenticated-oracle tails, `UpdateAssetLifecycle` permissionless activation/actions, and the optional ledger withdrawal tails for `WithdrawBackingBucket` and `WithdrawInsuranceAsset`); twelve remain open (`InitMarket`, `CloseSlab`, `ConvertReleasedPnl`, `ConfigureHybridOracle`, `WithdrawInsurance`, `CureAndCancelClose`, `ForfeitRecoveryLeg`, `FinalizeResetSide`, `UpdateBaseUnitMints`, `SwapSecondaryForPrimary`, `ForceCloseAbandonedAsset`, and `RestartAssetOracle`). Covered shapes begin with successful mutating public controls and hostile aliases/privilege downgrades require exact rollback. The executable queue is `inv_017_account_role_coverage.tsv`; closure requires exhausting those sixteen remaining schemas and documenting intentional safe aliases. |
...
On engine pin `b10b3454dd03dcf4c04a020dc1a90381ff179200`, the full 872-test `v16_cu` inventory is
