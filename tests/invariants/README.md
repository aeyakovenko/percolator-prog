...
| AUDIT-063 | OPEN-T | Trade consumption now has a nonvacuous 4-route x `expiry-1`/`expiry`/`expiry+1` public matrix: every fresh control grows a real lien, fee-capable routes charge real fees, and both expired boundaries roll back while preserving reduction. Conversion and retained top-up have generated expiry regressions, and crank/close/payout have focused cases. Release, claim, close, payout, and retire still lack one complete consumer-by-boundary matrix; no proof establishes normalization before every consumer. |
