...
| `stateful/` | 107 | Proptest-generated public routes, including generalized active-leg/currentness, source-claim attribution, source-credit-rate, authenticated-expiry route matrices, state-indexed liveness witnesses, and reference-model/deployed-transition equivalence |
...
| INV-081 | F + Direct + SVM/CU | `public_sbf/inv_081_success_state_validity_over_complete_public_routes.rs`, `stateful/inv_081_success_state_validity_over_complete_public_routes.rs`, `cu/inv_081_success_state_validity_over_complete_public_routes.rs` (twelve generated operation classes share success/rollback/global-state oracles; all five added classes have successful witnesses; terminal exits include the designated liquidity provider after unilateral reduction) |
...
| INV-086 | Direct + F + Partial M | `public_sbf/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs` (twelve generated public operation classes, exact custody/account frames, OI/position attribution, and terminal exits; identity, liens, payout, authority, resolution, and lifecycle remain) |
...
1. The charter requests `P` for 76 invariants, `F` for 85, `I` for 66, `M` for 32, `R` for 22,
   ...
   obligations: all 32 `M` and both `C` rows have partial named evidence, while all 22 exhaustive
   `R` obligations remain explicitly omitted.
2. The deployed decoder has 50 public instruction variants. The stateful public-interface model
   generates twelve direct operation classes (trade, EWMA configuration, mark push, crank, deposit,
   withdraw, maintenance sync, matcher configuration, insurance top-up, backing top-up,
   released-PnL conversion, and rebalance reduction) plus replay/substitution meta-actions. The five
   added classes now share exact rollback, token/account frame, ghost-position, and global-state
...
| AUDIT-073 | OPEN-D | Many bounded exits exist, and the stateful terminal campaign now includes a public exit for the designated liquidity provider after unilateral reduction, but multiple owned tests still assert publicly reachable funded locks. Fix those locks, then build a small public state graph plus long sequences requiring every funded nonterminal node to reach principal return, a receipt, or authorized junior forfeit. |
...
| AUDIT-081 | FRONTIER | The stateful model now covers twelve direct operation classes, including matcher configuration, insurance/backing top-ups, released-PnL conversion, and rebalance reduction under the shared success/rollback oracle. Deterministic witnesses require all five added classes to succeed, and the terminal campaign now exits every modeled portfolio, including the designated liquidity provider after unilateral reduction. Lifecycle, resolution, claim, authority, and close transitions remain outside the generated alphabet, and the runner still does not assert the full invariant suite after every success. Expand those classes and the executable invariant registry, then use typed-transition composition for routes whose whole-body proof remains intractable. |
...
| AUDIT-086 | OPEN-T | The reference runner now differentially checks five additional public classes with exact custody deltas, account/token frames, position ghosts, and global-state postconditions. It still does not model identity, all balances, liens, payout, authority, resolution, and lifecycle across every public action. Expand those state components and add bounded state-graph differential checking. |
