...
tests, 111 public regressions, 216 stateful tests, 916 LiteSVM/CU tests, and 193 wrapper Kani
...
INV-086 now owns the owner-rebalance portion of the unilateral-close frontier. Thirty-two public
worlds cross all four trade transports, `effective - 1`, exact-effective, `effective + 1`, and raw-
basis work requests, plus both terminal claimant orders. The oracle derives the allowed reduction
from the authenticated pre-state request, account effective quantity, and both side OI counters;
it independently recomputes the remaining effective quantity and requires exact two-sided OI
deltas, zero SPL movement during reduction, safe acceptance of oversize work, exact terminal
payouts, and portfolio closure. The next tractable INV-086 work is to apply the same pre-state clamp
oracle to permissionless liquidation, Recovery forfeit, and delayed force-close across nonzero ADL
rounding boundaries; it must not trust a deployed post-state counter.
...
| Focused INV-086 terminal reference and ADL-clamp composition | 12/12 public terminal worlds, 32/32 all-route ADL owner-reduction worlds, one post-claim backing recovery edge, one active-close-to-partial-receipt-to-terminal bridge, plus all 2,380 base words through depth three | every seed is rebuilt through public top-up, trade, mark, crank, resolve, and close instructions; all 6,942 base-word edges plus the recovery edge run the independent state/rollback oracles, every action produces a real third-position transition, depth three adds normalized states and edges beyond the complete depth-two frontier, all claim-priority worlds move real SPL value, exact/late expiry normalize on a recorded edge, route/claimant orders converge under independent position/OI/source-credit/encumbrance/stock/custody oracles, and the separate three-asset bridge preserves an active value-bearing close through resolution, finalizes it into a genuine partial receipt, moves additional SPL value, and terminates all five funded portfolios. The ADL matrix crosses all four opening transports, four reduction boundaries, and both terminal orders; its pre-state oracle derives the exact clamp and requires two-sided OI agreement, one-atom remainder fidelity, accepted overshoot, no reduction-time SPL movement, and exact terminal payouts. |
...
| AUDIT-048 | OPEN-T | All four fresh trade routes scan raw OI, and the stateful model keeps an exact independent effective-OI transition ledger across matched/retained trades, crank liquidation, owner rebalance, prior-reset cleanup, and recovery forfeit. A 32-world public matrix creates nonzero ADL through every trade transport, crosses owner requests at `effective - 1`, exact effective, `effective + 1`, and raw basis, and requires the independently derived clamp to match both side OI deltas and the recomputed remaining leg before both terminal claimant orders return exact funded value. A separate 24-world public bankruptcy matrix pins the zero-OI pending-obligation boundary: the final matched reduction clears effective OI while retaining exactly one zero-basis stored leg and one obligation, and terminal payout clears both without resurrecting OI. Directed nonzero-ADL liquidation/Recovery-forfeit/force-close schedules and larger multi-account ADL topologies remain. |
...
| AUDIT-051 | FRONTIER | Zero-effective-OI directed matrices and the stateful transition ledger cover resize, matched trade, rebalance, liquidation, reset clear, and recovery forfeit without collapsing raw basis into effective OI. The 32-world owner-reduction matrix adds nonzero ADL, all four opening routes, the complete one-atom/exact/overshoot request boundary, independently recomputed remaining effective quantity, and both resolved terminal orders. The bankruptcy matrix carries zero effective OI through a nonzero pending-obligation epoch and terminal close. Transfer, nonzero-ADL liquidation/Recovery-forfeit/force-close and retirement compositions, and a pure whole-transition equivalence proof remain. |
