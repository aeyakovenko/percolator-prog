...
| `cargo test --test v16_program_stateful_fuzz` | 190/190 | exact engine-`592d538c`/SBF-`909c24cc` rerun in 175.11 seconds, including both active-close barrier orientations across every trade route, the independent raw-basis/effective-OI ledger, sub-minimum-ADL and cross-domain fractional-source progress, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, concurrent partial receipts, disjoint- and shared-portfolio lifecycle-local reset/shutdown ordering, retained-reduction/shutdown landing orders, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
...
| INV-065 | F + SVM/CU + Partial R | `stateful/inv_065_reset_recovery_and_retired_state_isolation.rs`, `stateful/inv_081_success_state_validity_over_complete_public_routes.rs`, and `cu/inv_065_reset_recovery_and_retired_state_isolation.rs` cover generated public policy-to-shutdown transitions, exact owner recovery forfeits, a post-delay permissionless two-account force close, pre-delay atomic rejection, a complete empty-Recovery restart followed by fresh-generation trading, and the public Active-to-DrainOnly-to-empty-Retired path. Four no-injection matrices now cover 64 lifecycle worlds: base/dynamic assets, all four trade routes, both reducer sides, shutdown over a live reset obligation with stale hint absent/present, shutdown after stale-leg cleanup on either side of reset finalization, and a retained unilateral reduction landing before/after shutdown. The post-shutdown request rejects exactly, then both owner forfeits and one real crank converge to the same principal return as the pre-shutdown reduction. Every world reaches bounded cleanup/finalization, monotonic restart, fresh route liveness, complete owner exit, and exact stock/encumbrance reconciliation; post-restart retained old-generation trades reject exactly. The exhaustive reset/recovery/retirement graph remains. |
...
| AUDIT-074 | OPEN-T | The unrelated-accrual close-drift path is fixed and function-contract/public-route covered. Eight same-asset worlds preserve complete unrelated reductions across every route/orientation; two asset-local closes advance independently; shutdown/close ordering converges through canonical B discovery. Historical bankruptcy no longer blocks unrelated exact backing, provider principal, or remaining insurance after active blockers clear. Twelve underfunded worlds preserve unrelated flat principal across expiry, claimant order, and payout-route priority. INV-075 already exhausts both landing orders for same-domain close contenders and proves rejected-contender terminal liveness. The split-claim composition now covers all sixteen route pairs with two simultaneous partial receipts: destination substitution rejects exactly and a nonzero canonical claim frames the other receipt. Sixteen disjoint-portfolio worlds prove `ResetPending`/Recovery shutdown on asset 0 and a bilateral exit on asset 1 commute economically across every trade route and reset side while framing each other's scoped state. Sixteen shared-portfolio worlds add the account-local prerequisite case: exact early rollback is accepted only when a real permissionless crank makes the identical exit live, and both schedules converge. Complete the remaining portfolio/domain and lifecycle locality cross-product before promotion. |
