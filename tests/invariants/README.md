...
| `stateful/` | 117 | Proptest-generated public routes plus bounded lifecycle models for scarce-backing pair/chunk allocation orders, a 16-cell positive-claim boundary partition, all 3! matcher-control/trade landing orders, 20 user-operation/admission cells, 12 caller-priced boundary-exit cells, the four-state retirement-obligation lattice, a four-state Recovery resource-failure lattice, a Recovery crank/owner-exit classifier boundary, and all 5! claimant orders, including generalized active-leg/currentness, source-claim attribution, source-credit-rate, authenticated-expiry, state-indexed liveness witnesses, and reference-model/deployed-transition equivalence |
| `cu/` | 712 | Full `v16_cu` public-route, metamorphic, rollback, liveness, arithmetic-differential, and max-shape CU inventory, with no standalone top-level tests |
...
| INV-079 | Direct + Static rosters | `public_sbf/inv_079_public_reachability_evidence.rs` enforces the finding manifest, all 50 production instruction tags, and all charter-required metamorphic/reachability/compute method classifications |
...
| INV-082 | F route witness + SVM/CU + Model/proof gap | `stateful/inv_082_state_indexed_liveness_theorem.rs`, `cu/inv_082_state_indexed_liveness_theorem.rs` require fixed public sequences to expose rank-decreasing permissionless progress, normal exits, liquidation progress, retained no-CPI execution under the state oracle, exact rollback of account substitutions, bad-hint noise, and no known-blocker quarantine. A public Recovery-only regression separates the engine's non-dispatchable crank state from the still-live owner exit; exhaustive bounded reachable-state liveness remains proof/model work. |
...
| INV-084 | P + Assumption inventory + Proof-harness gap | `kani/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` statically inventories every current `kani::assume` and proves concrete witnesses for decoder indices, engine errors, matcher flags, positive marks, and the bounded arithmetic domain; mutation and public-route establishment remain open |
...
| INV-086 | Direct + F + Partial M | `public_sbf/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs` (fifteen generated public operation classes, exact custody/account frames, lifecycle-aware normal-live OI and crank-dispatch ranks, OI/position attribution, authority/policy/lifecycle wrapper transitions, and terminal exits; the saved shutdown/recovery corpus guards the distinction between Recovery owner exits and crank work; independent identity, lien, payout, authority-epoch, terminal-resolution, and full-lifecycle models remain) |
...
1. The charter requests `P` for 76 invariants, `F` for 85, `I` for 66, `M` for 32, `R` for 22,
   ...
   obligations: all 32 `M` and both `C` rows have partial named evidence; 15 `R` rows now have
   bounded generated or exhaustive-topology evidence and the other 7 remain explicitly omitted.
...
4. No general bounded BFS/model checker enumerates the reachable lifecycle graph required by
   ...
   INV-071, INV-073, INV-079, INV-082, INV-084, or INV-086. INV-066,
   ...
   starts and demonstrates first-landed exclusion rather than priority preemption; INV-078 crosses
...
| AUDIT-022 | FRONTIER | Split Kani and exhaustive host/SVM decoder rosters backstop several solver cliffs. A deterministic 4,096-payload host corpus now checks totality/canonicality, a canonical corpus locks all 50 tags, and curated prior schemas plus vector-length boundaries reject. Duplicate-field N/A documentation, stateful byte mutation, and per-tag proof decomposition for the remaining solver-cliff payloads remain. |
...
| AUDIT-079 | OPEN-D | The manifest enforces names/classifications; source-derived registries now fail unless every production instruction tag and every charter-required M/R/C method has named evidence or an explicit omission. No formal finding trace-evidence schema records signers, account metas, absence of out-of-band mutation, exact token/lamport deltas, and persistent exit result; add and replay that schema for every qualifying PoC. |
...
| AUDIT-082 | FRONTIER | Fixed route witnesses plus a saved generated shutdown/recovery trace are not a theorem over reachable states. The reference model now distinguishes Active/DrainOnly auto-crank work from Recovery owner-exit work and proves the latter remains live, while preserving the classifier/dispatch contradiction as a deterministic public regression. Build a bounded public transition graph, define mode-specific ranks, and prove every abstract transition either terminates or has a constructible rank-decreasing call. |
...
| AUDIT-084 | FRONTIER | A compile-time inventory now classifies every current `kani::assume` across all nine mounted Kani owners and adds concrete boundary witnesses for decoder indices, engine-error tags, mark positivity, matcher flags, and the arithmetic solver bound. Mutation tests and public-route establishment or a named unreachability proof for every precondition remain. |
...
| AUDIT-086 | OPEN-T | The reference runner now checks fifteen public classes with exact custody deltas, account/token frames, position ghosts, lifecycle-aware normal-live OI/crank ranks, and global-state postconditions, including wrapper policy, authority, shutdown, Recovery owner exits, and a persisted generated shutdown/recovery corpus. It still lacks independent identity, all-balance, lien, payout, authority-epoch, terminal-resolution, and complete lifecycle state components. Expand those components and add bounded state-graph differential checking. |
