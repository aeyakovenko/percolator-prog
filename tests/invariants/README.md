...
backing-earnings, and asset-scoped insurance withdrawals. Combined with terminal slab close, the
three value-bearing reserve top-ups, prior authority handoff, authenticated resolution,
sequence-bearing policy, and managed-oracle routes, the source-derived matrix now owns 23
epoch-bearing instruction variants as 27 semantic authority cases. Every case retains an old
...
covers 216 public route instances, and the deterministic policy/handoff/resolve product continues
...
backing-fee floor, and centralizes fresh-request binding in the transaction builder. This tranche
adds the explicit epoch wire field and canonical handler check to three routes, so it is a necessary
schema extension rather than a raw source-line reduction. Local backing/insurance authority uses
the target asset epoch; the shutdown market-authority override uses asset 0; and a caller matching
both roles is bound to the target asset. A source-derived decoder roster, one-byte
truncation/trailing-byte proofs, and exact field-preservation proofs prevent route drift. The
caller-input and boundary rosters now own all 228 public fields, the proof census owns 180
harnesses across 21 mounted modules, and the public-trace guard owns 33 consumers.
...
the solver's wide state-space wall was replaced by three exact per-route proofs; each now solves in
under 3.1 seconds instead of exceeding fifteen minutes without a result. No deployed validation was
abstracted or weakened.

The exact rebuilt SBF is 1,246,296 bytes with SHA-256
`5b8c58351f0e07eaa9084c3719b2fd3bd89332ec5717084156ac720d440bc12b`. The complete exact-artifact
gates pass 891/891 LiteSVM/CU tests, 102/102 public fuzz regressions, and 202/202 stateful/model
tests. The mounted wrapper proof census is 180 harnesses; its exact-source result is recorded in
the verification table below.

This is still not INV-005 closure. A source-derived call-graph census classifies all 30 public
routes that reach configured-authority logic: 23 are epoch-bound above; `ClosePortfolio` is
independently portfolio/sequence/episode-bound; and both auxiliary-ledger synchronizers are
deterministic current-state reconciliation. Four explicit epoch gaps remain: the legacy multi-domain
`WithdrawInsurance`, `UpdateAssetLifecycle`, `UpdateBaseUnitMints`, and `SwapSecondaryForPrimary`.
Full same-pubkey market recreation is separately blocked by INV-001:
...
roster over all 50 public variants: 20 have no mixed-instance role, 30 exhaust every current
...
an explicit evidence class. Six oracles now carry classifier-bound public LoF evidence. In
...
The remaining 22 oracles retain narrower replay, local-safety, or economic-delta claims until
...
its executable roster contains 51 owner/method classes covering 63 production transition calls.
...
authority routes, including terminal `CloseSlab` and the three reserve withdrawals, reject
same-market `A -> B -> A`; remaining destructive-consent gaps are same-pubkey whole-market
recreation and the four source-rostered non-epoch authority routes named in the current checkpoint.
Those remain owned by INV-001/005.
...
INV-088 now has a source-complete disposition roster for 51 wrapper-to-engine owner/method
classes covering 63 production `*_not_atomic` calls. Every class is assigned to an aggregate-summary family
...
222 fields across all 50 public instruction variants and the three nested input structs. INV-083
maps every field to one of 20 locked boundary profiles and executable field/profile witnesses;
INV-017's 50-row roster is entirely `EXHAUSTIVE` over account aliases and signer/writable roles;
...
all 228 fields across 53 public input types map to exactly one of 20 semantic profiles, one
...
sources: 141 direct harnesses plus 36 macro-generated trade-decoder harnesses, exactly matching the
177 reported by `cargo kani list`. Every direct harness is classified into 77 symbolic-total, 26
...
SPL, and program-lamport rollback. A recursive source-complete guard requires all 32 current
...
`is_violation` oracles: authority-epoch terminal replay, terminal-generation replay, terminal dust,
cross-domain backing double-consume, and retained source-fee consent are classifier-bound public LoF evidence, while the other 22 are mechanically limited to economic-delta, local-safety, or
...
contracts. The 172-byte lifecycle schema exceeds the current Kani decoder budget, so exhaustive
canonical-prefix and legacy-schema rejection is host-tested while whole-route composition is
tested through the deployed SBF. Resolved claims are permissionless current-state transitions;
...
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`9b737fd`/SBF-`5b8c5835` rerun completed in 258.86 seconds; the 27-case authority-incarnation matrix, every retained-policy landing order, and every prior stateful/model check pass |
| Registry/manifest checks in the INV-079 module | 12/12 | includes the source-complete 28-oracle evidence roster, complete-route terminal classification, and the recursive 33-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full engine-`9b737fd`/SBF-`5b8c5835` exact-source rerun completed in 27.50 seconds; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 891/891 | full engine-`9b737fd`/SBF-`5b8c5835` exact-source rerun passed in 198.58 seconds on 2026-08-28; zero failures, including the source-complete authority-epoch route/decoder matrices and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse -j 2` | 180/180 | full wrapper-only exact-source rerun with engine `9b737fd`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied. The formerly combined five-route decoder query is now three exact per-route proofs that each solve in under 3.1 seconds. |
...
fee consent remain explicit INV-001/INV-005/INV-014 gaps. The 27 epoch-bearing authority cases now
...
| INV-005 | Independent + P + Direct + F + SVM/CU + Static roster | `public_sbf/inv_005_authority_incarnation_binding.rs`, `stateful/inv_005_authority_incarnation_binding.rs`, `cu/inv_005_authority_incarnation_binding.rs`, and `kani/inv_005_authority_incarnation_binding.rs` own 23 epoch-bearing source variants as 27 semantic authority cases. Every case crosses same-market `A -> B -> A`, rejects the stale retained request with exact rollback, and admits a mutating current-epoch control. Kani proves exact epoch admission, checked handoff advance, migration-floor preservation, and exact generation/epoch wire binding for reserve withdrawals. A source-derived call graph classifies all 30 configured-authority routes: 23 epoch-bound, three proof-backed current-state or independently bound routes, and four explicit epoch gaps. Whole-market same-pubkey recreation remains an INV-001 dependency because account-local epochs reset. |
...
| AUDIT-005 | OPEN-D | A source-derived call graph classifies all 30 configured-authority routes. The 23 epoch-bearing source variants, represented by 27 semantic authority cases, reject same-market `A -> B -> A` with exact rollback and admit fresh current-epoch controls. `ClosePortfolio` is independently incarnation/sequence/episode-bound, and both auxiliary-ledger synchronizers are deterministic current-state reconciliation. Four routes remain explicit epoch gaps: legacy multi-domain insurance withdrawal, lifecycle, base-unit mint replacement, and secondary-to-primary swap. Kani proves exact epoch admission, atomic checked handoff, the migration-aware floor, and exact generation/epoch wire binding for reserve withdrawals. Persistent market incarnation remains separately open because close/recreate resets account-local epochs. |
