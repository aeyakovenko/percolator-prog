...
| Focused INV-089 activation/reactivation equivalence | 13/13 public CU tests | both permissionless and privileged reuse reset an old generation's `u64::MAX` replay watermark; a byte-complete persisted-slot differential after public trade/oracle/backing history matches fresh activation after normalizing only generation IDs, and fresh same-generation trading plus full withdrawal remains live |
...
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`592d538c`/SBF-`beeea60a` rerun in 27.23 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 814/814 | full engine-`592d538c`/SBF-`beeea60a` exact-source rerun in 193.69 seconds, including all maximum-source and maximum-shape exits, stale-Hybrid 14-leg delegated-CPI compositions through Resolved and Recovery with exact terminal value, full persisted-slot activation/reuse equivalence after public economic history, exact maximum cross-zero quantity boundaries, all-route DrainOnly/Recovery admission, post-ADL effective conversion, liquidation/resolved close, exhaustive account-role matrices, the 7,183-word parser-delegation corpus, 726 independently modeled parser boundaries, 15,552 structural combinations, 12,288 seeded full-width layouts, the exact non-saturating elapsed-time boundary, 1,310,720 wide confidence comparisons, all-provider composite epoch/rewind/freshness boundaries, composite liquidation and shutdown/restart composition, nine single-provider and twenty-four multi-provider lifecycle worlds, explicit invert/unit-scale lifecycle histories, the dense zero-delta terminal-shape benchmark, and every cross-invariant progress/rollback audit |
...
8. Extend INV-089's byte-complete fresh/reuse differential beyond matched trade, mark, and backing
   history to publicly retired generations with spent insurance, prior claims, current certificates,
   and maximum supported account shape. Replay-watermark poisoning is closed for both reuse routes;
   do not duplicate that cell with field-by-field assertions.
...
| AUDIT-089 | OPEN-T | Fresh/reuse authority and price checks are broad. The shared model proves both a public Active-to-DrainOnly-to-empty-Retired lifecycle and an empty-Recovery restart that advances generation, preserves authority/fee/insurance scope, frames unrelated assets/accounts/custody, and admits a fresh-generation trade under independent leg/OI checks. A new byte-complete dynamic-slot differential independently found that wrapper replay watermarks survived engine/profile reset: an old generation could set `oracle_observation = u64::MAX` and permanently poison the replacement authority. Both privileged and permissionless reuse now reset all eleven per-generation lanes. After public matched trade, mark movement, backing top-up/withdrawal, retirement, and reuse, every wrapper and engine byte equals fresh activation after normalizing only generation IDs, and fresh trading/withdrawal remains live. Spent-insurance/prior-claim history, current-certificate invalidation, and unsupported-shape permutations remain outside this single differential. |
