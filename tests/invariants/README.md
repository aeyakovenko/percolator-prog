...
predicate, or supported shape reopens it. The next adjacent target is INV-084's public-route
establishment of every explicit proof assumption.
...
| `cargo test --test v16_cu` | 828/828 | full engine-`592d538c`/SBF-`53c07731` exact-source rerun in 190.88 seconds, including INV-083's 206-field/53-type/20-profile boundary inventory and 25 public initialization rejection partitions; the complete 53-call-site INV-088 transition roster; all backing, insurance, source-claim, earnings, and resolved-claimant order matrices; all maximum-source and maximum-shape exits; stale-Hybrid 14-leg delegated-CPI terminal compositions; byte-complete persisted-slot activation/reuse equivalence; exact maximum cross-zero boundaries; all-route lifecycle admission; exhaustive account-role matrices; parser, layout, confidence, oracle, liquidation, shutdown/restart, backing/insurance, and terminal-shape campaigns; and every cross-invariant progress/rollback audit |
