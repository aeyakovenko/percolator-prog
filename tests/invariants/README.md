...
`c0dec8ce1e0f193d428f33f413c3116139fede55` (engine PR
...
This adds no deployed code. A source-complete search of pinned engine `c0dec8ce` also finds no
...
`c0dec8ce`; approximate-bucket rebucketing is N/A for this profile, while complete-state reachability
remains.
...
| Focused INV-029 exact-only deployed profile | 210/210 stateful public tests plus 1/1 source lock | after every generated successful public transition, each domain's exact claim equals its positive bound and the complete portfolio attribution; the wrapper exposes no non-exact bound or rebucketing ingress, so approximate-bucket range/rebucketing proofs remain N/A unless that optional profile is introduced |
| Focused INV-030 source-credit transition causality and malformed state | 210/210 stateful public tests plus 3/3 focused CU | every generated public action and successful permissionless crank compares both markets before and after: unchanged formula inputs preserve the rate, every formula-input mutation advances `credit_epoch`, and a live claim's rate rises only with more independently available backing or a smaller claim bound. Twenty malformed relation cases and two exact source-boundary truncations cross both source sides with instruction-error and complete economic rollback; a pin-bound source lock leaves on-chain rate/epoch mutation exclusively in the engine |
...
| `cargo check --tests` | pass | exact engine-`c0dec8ce` build completed on commit `5c08e400` |
| `cargo test --lib` | 7/7 | exact engine-`c0dec8ce` rerun on commit `5c08e400` |
| `PERCOLATOR_FUZZ_CASES=2 cargo test --test v16_program_stateful_fuzz` | 210/210 | full exact engine-`c0dec8ce`/SBF-`9ad95c99` rerun completed in 225.70 seconds on `5c08e400`; every generated public action and successful crank composes the shared invariant oracles for both markets. The suite includes the 56-world one-domain and 48-world two-domain INV-052 source-lien matrices, the asymmetric-fee INV-041 order regression, INV-076 rollback frames, the finding-blind 21-family retained asset-generation matrix, source-domain single-use, retained source-fee consent, and backing-provider landing-order campaigns. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
...
| `cargo test --test v16_program_fuzz_regressions` | 106/106 | full exact engine-`c0dec8ce`/SBF-`9ad95c99` rerun completed in 27.99 seconds on `5c08e400`; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 895/895 | full exact engine-`c0dec8ce`/SBF-`9ad95c99` rerun completed in 199.09 seconds on `5c08e400`; zero behavioral failures across reserve, fee-consent, oracle, Recovery, terminal, lifecycle, and maximum-shape routes. The canonicalization runs only for persisted inversions, and every observed required exit remains below the 1.4M CU ceiling. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 187/187 | full wrapper-only exact-source rerun on commit `5c08e400` with engine `c0dec8ce`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied |
| Engine runtime/property suites | 188/188 | exact `cargo test --all-targets` rerun on engine `c0dec8ce`: 50 library, 13 backing, 2 resolved-insolvency, and 123 v16 specification tests pass |
| Focused engine source-domain canonicalization | 1/1 Kani | `proof_v16_mutable_view_canonicalizes_source_domain_order` passes with 0/3,045 failed checks, 145 unreachable checks, and its constructive cover satisfied |
| Focused INV-033 engine counterparty-lien contracts | 5/5 Kani | create, live release, terminal release, impairment, and consume contracts all pass on exact engine `c0dec8ce`, so the wrapper's deliberate public-unreachability composition does not inherit stale proof evidence from the old pin |
...
exists. The locally rebuilt 1,236,048-byte production SBF used by the 2026-08-29 exact-pin gates has
SHA-256 `9ad95c99862c7bd2b14ec5b939babb0ff8e7c83268ad7e06e571bad5c6273f31`.
The 680-byte increase from the preceding artifact is the bounded engine source-domain
canonicalization; wrapper production code is unchanged, and the complete 895-test CU suite remains
green.
...
1. Convert the 8 `Quarantined` adapters to `Certified` only after the current pin satisfies their
...
4. Apply INV-052's split/merge oracle to multi-asset or larger-account liquidation and to mixed
   fresh/expired source domains, cooldowns, rates, and policy limits. The source-lien expiry route
   now compares aggregate, domain-isolated, equal two-account, asymmetric three-account, and
   asymmetric four-account shapes across all four trade families, both source orders, and both exit
   orders at exact/late expiry. Four targets plus one shared counterparty exhaust the current
   five-actor fixture; testing five or more target partitions requires a larger public topology
   rather than another permutation of this fixture.
...
5. Complete INV-076's fallible-phase decomposition. Public tests now own rollback after mutating
...
| AUDIT-033 | CLOSED | The wrapper exposes counterparty-backed liens but intentionally exposes no insurance-credit reservation route. The public control creates a real counterparty lien and proves all insurance categories stay zero; a separately funded insurance-only attempt rejects with exact rollback and cannot silently reserve or consume insurance. A source-complete guard proves the wrapper calls none of the five engine reservation/lien mutators and binds the claim to exact engine pin `c0dec8ce1e0f193d428f33f413c3116139fede55`. That pin's create, live-release, terminal-release, and impairment contracts plus consume ledger-closure proof were rerun and establish lockstep source/reservation classification while leaving counterparty backing untouched. This is closed by public unreachability plus engine proof, not by adding a redundant wrapper API; any new callsite or pin reopens it. |
...
On engine pin `c0dec8ce1e0f193d428f33f413c3116139fede55`, the full 895-test `v16_cu` inventory is
