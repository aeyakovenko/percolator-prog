...
866/866 LiteSVM/CU tests, 100/100 public fuzz regressions, and 206/206 stateful/model tests. Static
...
configuration. Every matrix starts from a valid mutation control. That narrows but does not close
its remaining instruction-schema matrix.
...
| Focused INV-017 account-pair and privilege matrices | 78 custody/payout pairs plus 18 downgrades; 126 reserve-custody pairs plus 40 downgrades; 19 core/crank pairs plus 17 downgrades; 6 authority-handoff pairs plus 6 privilege downgrades; 21 initialization/matcher-config pairs plus 7 privilege downgrades | resolved close, a genuinely partial value-moving resolved claim, independently generated backing-provider earnings, nonzero liquidation-reward crank, both two-signer authority handoffs, portfolio initialization, and both matcher-config shapes start from successful public controls; every hostile pair or privilege downgrade rejects with exact rollback |
...
| `cargo test --test v16_cu` | 866/866 | full engine-`b10b3454`/SBF-`fb16e50e` exact-source rerun passed in 203.93 seconds on the 2026-08-26 checkpoint; zero failures, including every prior maximum-shape and invariant route plus exhaustive account-pair and required-privilege matrices for both authority handoffs, portfolio initialization, and disabled/enabled matcher configuration |
...
| `cu/` | 866 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, 60 partial-fill route/ratio/maximum-domain worlds, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
On engine pin `b10b3454dd03dcf4c04a020dc1a90381ff179200`, the full 866-test `v16_cu` inventory is
