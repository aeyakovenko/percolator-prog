...
ADL matrix now repeats liquidation after a later authenticated deficit on the selected leg. Larger
account partitions, repeated episodes that advance to another selected asset, and the remaining
maximum-shape cross-products are the next tractable
...
| Focused INV-061/071/073/077/082/086 combined-maxima equal-risk liquidation | 4/4 public persisted-leg/observation-order worlds | all fourteen active legs and twenty-eight source domains become equally adverse; observation order is inert, persisted order selects asset 0 or 13, eleven strict liquidations restore health below 1,155,033 CU, and four owner reductions plus fourteen cleanup/finalization steps lead to exact senior withdrawal. Live unbacked junior conversion rejects with exact rollback; fifteen bounded resolved continuations settle the retained claims and both portfolios close with exact custody. The smaller 16-world two-asset matrix separately repeats liquidation after a second authenticated deficit and rechecks exact selection, quantity, OI, fees, attribution, and frames. |
...
| INV-061 | Independent + P(engine) + SVM/CU | `stateful/inv_061_deterministic_bounded_liquidation.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `cu/inv_061_deterministic_bounded_liquidation.rs`, `cu/inv_059_fee_fragmentation_bound.rs`, and the issue-408 liquidation world in `cu/inv_027_protected_principal_seniority.rs`. Two public post-ADL prefixes prove pre/post-mark transfer extraction rejects with exact rollback, cannot consume extra backing or create phantom value, and retain bounded owner reduction whose raw-basis result is independently derived from remaining effective OI. Four public dual-ADL transport worlds independently reconstruct the exact liquidation maintenance, target/effective lag, fee/minimum-fee, floor, projected-health, and binary-search selector and require its exact partial close, OI deltas, effective remainder, no-SPL frame, and CU bound. The resolved-ADL matrix proves both terminal landing orders consume only remaining effective OI, pay both funded users exactly, reconcile custody to zero, and close both portfolios. The fractional reset-carry matrix publicly creates two bankruptcies, requires the sole account crank to strictly liquidate its target under the CU cap, then gives every remaining owner a stale raw-basis work budget. Each budget clamps to exact effective OI; all legs and both resets clear in bounded public work. The trace explicitly refills the resulting 11-atom provider receivable, withdraws all restored provider principal, and proves retirement clears only historical spent/source/social/K/F audit state without changing custody. A sixteen-world two-asset/three-user matrix crosses all four opening transports, both equal-risk target-leg orders, and both accrual orders; every world then creates a later authenticated deficit on the selected leg and requires a second exact close, fee, selected-domain budget credit, nonselected frame, and no-SPL frame. The 48-world successor crosses three unequal losses, all six persisted leg orders, and both accrual orders; every crank closes the independently modeled first live leg, frames all nonselected value/loss domains, stays below 312,341 CU, and converges to exact order-independent terminal payouts. A four-route underfunded bridge now proves one 315,258-CU crank removes exactly 70,000,000 matched OI, finalizes the exact 2,723-atom close, and composes through a 1,000-face partial receipt, a later value-moving payout, and terminal custody. A separate all-route campaign proves two fee-bearing liquidations require two authenticated deficits, same-state/malformed retries cannot manufacture an episode, and owner reduction remains live after episode two. Arbitrary close splitting, cross-selected-asset repeated episodes, larger account partitions, and remaining maximum-shape cross-products remain. |
