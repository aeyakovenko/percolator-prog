...
`e914dbcff8482416bcea4d5ab827cda2dbb3f5cb`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`6019f009295f7785d2a04217319490612b28b842dcc652439786255ad76175c7`. Resolved-mode
...
healthy pair holds exposure on the same asset. The healthy pair's full risk-reducing trade lands,
removes exactly its own long and short effective OI, frames both close participants and the complete
close ledger byte-for-byte, and moves no internal or SPL custody. This covers the same-asset
risk-reduction cell while preserving the intended rule that new risk may remain blocked by the
affected domain barrier.
...
released zero-basis obligation cleanup are also covered and fixed on `e914dbcf`. The next broader
INV-074 target is the remaining side/domain/receipt
locality cross-product, followed by the liveness frontier shared by INV-057, INV-071, INV-073,
INV-078, and INV-082: extend
...
| Focused INV-061/069/073/074 fractional-carry terminal lifecycle | 1/1 public liquidation-to-retirement world, 1/1 Recovery-to-fresh-generation world, 2/2 owner exit routes, 3/3 engine whole-body terminal proofs, and 1/1 wrapper Kani gate | engine `cae71267` normalizes fractional social-loss carry; `5b495f33` clears settled reset baselines; `f42650f7` clears spent-only terminal source audit; and `573c4e90` canonicalizes spent-domain/source/social/K/F history inside one restart or retirement while rejecting live receivables. Public routes clear eight legs and both resets, settle every claim/provider obligation, withdraw provider principal and remaining insurance despite sticky bankruptcy history, retire the dynamic asset, restart asset 0, prove fresh insurance has zero inherited spend, complete a fresh trade round trip, and frame SPL custody below every CU cap |
| Focused INV-039/041/067/073/077 Recovery forfeit-order durability | 2/2 complete public landing orders, 2/2 prior-claim prerequisite worlds, 1/1 maximum-shape route, 219/219 engine runtime/property tests, and 3/3 focused engine contracts | engine `e914dbcf` retains loss weight in a zero-basis obligation until the opposite real position exits, routes released cleanup through canonical K/F/B settlement, and commits terminal Recovery instead of returning a rollback-only error. Both orders preserve identical payouts, 8,424 atoms of social loss, provider attribution, and terminal custody; maximum observed CU is 931,870 |
...
| `cargo check --tests` | pass | rerun on the 2026-08-21 PR135 production head pinned to engine `e914dbcf` |
| `cargo test --lib` | 7/7 | rerun on the 2026-08-21 PR135 production head pinned to engine `e914dbcf` |
| `cargo test --test v16_program_stateful_fuzz` | 173/173 | full exact-pin rerun in 165.78 seconds |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full exact-pin rerun in 25.63 seconds; the sealed benchmark census accepts the independently expanded post-restart insurance/trading route |
| `cargo test --test v16_cu` | 791/791 | full exact-pin rerun in 194.84 seconds, including maximum-source exits, maximum-shape Recovery obligation cleanup, liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and fresh-generation insurance/restart progress |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 137/137 | full wrapper-only exact-pin rerun on engine `e914dbcf`; zero harness failures, including all declared nonvacuity covers |
| Engine runtime/property suites | 219/219 | rerun with `cargo test --features fuzz` on engine `e914dbcf`; all unit, reference-model, arithmetic-discharge, source-credit, insolvency, and v16 specification tests pass |
...
`4fe6a2bd5f16971510ef59ff2715e4043835cec27c70ff690564a16baa4ebab3`.
...
| INV-039 | Independent + Direct + SVM/CU | `public_sbf/inv_039_pending_loss_obligation_durability.rs`, `stateful/inv_039_pending_loss_obligation_durability.rs`, and `cu/inv_041_deterministic_allocation_and_caller_order_independence.rs`; the four-party Recovery matrix exhausts all landing orders while independently checking retained weight and pending counts. INV-027 owns the terminal owner-attribution assertion for the cross-cutting stale-cohort novation counterexample. |
...
| INV-061 | Independent + P(engine) + SVM/CU | `stateful/inv_061_deterministic_bounded_liquidation.rs`, `cu/inv_061_deterministic_bounded_liquidation.rs`, and the issue-408 liquidation world in `cu/inv_027_protected_principal_seniority.rs`. Two public PR250 prefixes prove pre/post-mark post-ADL transfers reject with exact rollback, cannot consume extra backing or create phantom value, and retain bounded owner reduction. The resolved-ADL matrix proves both terminal landing orders consume only remaining effective OI, tolerate only exact-rollback payout waits, pay both funded users exactly, reconcile custody to zero, and close both portfolios on engine `6c04db7e`; the shared full-width reduction kernel is contract checked. The fractional reset-carry matrix publicly creates two bankruptcies, requires the sole crank to strictly liquidate the funded target under its CU cap, clears all eight affected legs and both resets, settles all claims/provider principal, and retires the dynamic asset on `573c4e90`. Engine contracts cover carry normalization, reset-baseline clearing, one-call terminal-history normalization, and the live-receivable blocker. Other liquidation selection/order and maximum-shape cross-products remain. |
...
| INV-073 | Independent + P(engine) + F + SVM/CU + Partial R | `stateful/inv_065_reset_recovery_and_retired_state_isolation.rs`, `stateful/inv_066_resolved_payout_fairness_and_order_independence.rs`, `stateful/inv_071_crank_progress.rs`, `stateful/inv_081_success_state_validity_over_complete_public_routes.rs`, `cu/inv_073_no_permanent_user_lock.rs`, the terminal route in `cu/inv_061_deterministic_bounded_liquidation.rs`, and the shared-expiry lifecycle in `cu/inv_028_source_domain_realizability_cap.rs` cover an owner-executable DrainOnly reduction through empty retirement, generated Recovery exits through both owner forfeits and a noncooperative-owner force-close witness, stale-market terminal disposition, all basic claimant orders, publicly booked bankruptcy obligations, and provider-expiry/prospective-loss composition. The fractional-carry fixture now forks across owner reduction and bilateral trade, requires both to clear the leg and return senior capital, then separately drives automatic liquidation through reset finalization, every claim/provider settlement, and retirement. The Recovery-order matrix executes winner-first and loser-first from the same public state, requires the first exit to retain a zero-basis obligation, the second exit to release it, and a permissionless crank to settle and remove it before exact terminal payout. Other owned public locks and exhaustive funded-state reachability remain. |
| INV-074 | Independent + P(wrapper) + P(engine) + F + SVM/CU | `kani/inv_074_scope_locality.rs`, `cu/inv_074_scope_locality.rs`, `stateful/inv_074_scope_locality.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `cu/inv_061_deterministic_bounded_liquidation.rs`, and `cu/inv_076_close_drift_residual_durability_and_finalization_atomicity.rs`. Asset-local stale/bankruptcy cases preserve unrelated withdrawals and existing-position exits. The public close-drift trace proves unrelated authenticated accrual cannot turn a still-bookable local close into global Recovery; engine `377de75c` scopes staleness to the originating asset and contracts that predicate. A same-asset model proves one pair's active bankruptcy close does not block another healthy pair's full reduction or alter the close ledger/custody. The two-/three-cohort backed-claim matrix proves engine `4b23b197` no longer lets permanent bankruptcy history override an unrelated exact source ledger. The terminal provider route additionally proves wrapper backing withdrawal succeeds while that history bit remains set only after the active negative-account count and selected-domain barrier are zero; an assumption-free wrapper proof exhausts all live blocker inputs. Twelve genuine partial-receipt worlds require an unrelated flat portfolio's exact 777-atom principal to exit before snapshot capture across expiry, claimant-order, and route-priority permutations. Broader side/domain/receipt/lifecycle locality remains. |
...
| INV-076 | P(engine) + SVM/CU + Model gap | `cu/inv_076_close_drift_residual_durability_and_finalization_atomicity.rs` covers stale-cure and public-created zero-cure exact rollback plus a public two-asset close-drift ordering trace. The latter advances the global slot through an unrelated authenticated accrual while the originating asset remains frozen, then requires strict local residual progress, exact custody/foreign-account frames, a Live market, and unrelated owner exits. Engine `377de75c` contracts the asset-local stale predicate. Same-asset price/funding drift, complete close-phase fault injection, and atomic OI/basis-clear composition remain. |
...
The current ledger is 2 **CLOSED**, 56 **OPEN-T**, 12 **OPEN-D**, 6 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-039 | OPEN-D | Many accrual-before-weight-removal routes are covered, while stale-cohort novation remains an expected cross-cutting violation whose terminal principal-attribution owner is INV-027. The public four-party Recovery matrix exhausts all owner landing orders and checks retained loss weight and pending counts after every removal. Transfer, reset, account close, and partial liquidation still need the common obligation-before-removal state machine. |
...
| AUDIT-046 | OPEN-T | A public 12-cell caller-priced model covers raw `0/1/MAX`; a second 64-world model crosses all four trade routes, raw `1/MAX`, strict-reduction/cross-zero shapes, and Active/DrainOnly/Recovery/Resolved. It proves exact rejected rollback, authenticated-mark/value preservation, canonical reduction in both wind-down modes, full owner withdrawals, and exact terminal payouts. A real same-asset active-close world additionally preserves an unrelated pair's full reduction, close frames, and custody. Add stale/pending authenticated-oracle compositions and bounded reachability over the remaining lifecycle transitions. |
...
| AUDIT-073 | OPEN-D | The stateful campaigns exit the designated liquidity provider after unilateral reduction, reduce a real bilateral position through DrainOnly and retire the empty asset, exercise both owners' exact junior-value forfeits in Recovery, prove a third-party cranker can clear an abandoned opposite-side pair after public asset shutdown, take a funded stale market through permissionless resolution to terminal disposition, and settle the bounded claimant schedules. The fractional-carry owner routes, automatic liquidation/reset/provider-retirement route, and asset-0 provider/insurance/restart/fresh-trade route terminate publicly. Engine `e914dbcf` also closes the provider-backed forfeit-order lock: both public landing orders retain and settle the exact loss obligation, converge economically, and finish all funded accounts through bounded cranks. Multiple owned tests still assert other publicly reachable funded locks. Fix those locks, then expand the public state graph so every funded nonterminal node reaches principal return, a receipt, or authorized junior forfeit. |
| AUDIT-074 | OPEN-T | The unrelated-accrual close-drift path is fixed and function-contract/public-route covered: a local bookable close no longer gives its owner authority to force global Recovery. A public same-asset world now proves the active close does not block another pair's complete risk reduction or rewrite the close/custody state. The permanent historical-bankruptcy lock on unrelated exact backing is fixed across two public cohort sizes. The provider-settlement route proves both backing-principal and remaining-insurance withdrawals are not blocked by audit history after active loss is gone; the shared predicate and all blocker partitions are covered by one assumption-free wrapper proof. Twelve public underfunded worlds prove a genuine partial receipt cannot delay an unrelated flat portfolio's exact principal across expiry, claimant-order, and payout-route permutations. Complete the remaining side, portfolio, domain, concurrent-close, receipt-substitution/concurrency, and lifecycle locality cross-product before promotion. |
...
On engine pin `e914dbcff8482416bcea4d5ab827cda2dbb3f5cb`, the full `v16_cu` inventory is
