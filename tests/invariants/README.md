...
`23db9e77a56a26d841963d72f004694f761b4c102671dd70cad4abb646d2ec14`. Resolved-mode
...
SPL, and program-lamport rollback. A recursive source-complete guard requires all 24 current
...
exit, atomic rejection, and nonterminal progress; a real public full withdrawal and subsequent
over-withdrawal prove the positive exit and rejection cells, while negative controls prevent one
rejection from being mislabeled as LoF or persistent DoS. Qualifying counterexamples still need
independently measured terminal observations before INV-079 can close.
...
The next INV-079 increment is to attach independent terminal-value and terminal-liveness
observations to the finding-blind public counterexample runners, then require every qualifying
result to pass the normalized classifier. Loss observations must come from independent SPL,
lamport, stock, and attribution deltas; persistent-lock observations must attempt the complete
owner-exit and honest-continuation set and retain funded value plus an unresolved obligation. The
sealed holdout remains unopened while this generic evidence path is built.
...
| `cargo test --test v16_program_stateful_fuzz` | 195/195 | exact engine-`592d538c`/SBF-`23db9e77` rerun in 174.68 seconds, including source-complete validation of every current public trace consumer and the persisted quantity-ADL unilateral-close clamp boundary, plus every prior rank/reference-model, lifecycle, source-lien, terminal, and route-composition check |
| Registry/manifest checks in the INV-079 module | 11/11 | rerun on the 2026-08-21 PR135 exact-pin test head; includes normalized terminal classification and the recursive 24-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 95/95 | full engine-`592d538c`/SBF-`23db9e77` exact-source rerun in 26.57 seconds; every current public trace consumer validates normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
| `cargo test --test v16_cu` | 844/844 | full engine-`592d538c`/SBF-`23db9e77` exact-source rerun passed in 191.63 seconds on the 2026-08-21 checkpoint; zero failures, including the source-complete INV-085 arithmetic census, bigint boundary/provider references, exact host-to-SBF comparisons, and INV-080 error-disposition/dispatcher/entrypoint/later-consumer composition |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 171/171 | full wrapper-only exact-source rerun on the 2026-08-21 checkpoint with engine `592d538c`; zero failures, including all 26 branch-witnessed proof classes with constructive covers, all 13 source-inventoried assumptions, the twelve INV-085 arithmetic relations, 16 provider-parser decomposition harnesses, and every prior decoder and rollback proof |
