...
`fc80768eae5b344fcc4d812aed47fcdad565819ebd5a2eaf9b4e8ef5a22e4fc4`. Resolved-mode
...
| --- | ---: | --- |
| Focused INV-020 authenticated timestamp, composite epoch, and lifecycle matrix | 1/1 direct public, 2/2 stateful, 2/2 full-width Kani, 4 deterministic CU matrices, plus the complete 803-test CU gate | 126 provider/transform configurations reject 114 selected-leg skews and 126 coherent rewinds; 39 provider words cross freshness ages 59/60/61; three provider-role worlds reach a genuine coherent liquidation then exact effective exit; one composite shutdown/force-close/restart world clears old provenance and resumes trading; mixed-time controls preserve value and terminal payout |
...
`fc80768eae5b344fcc4d812aed47fcdad565819ebd5a2eaf9b4e8ef5a22e4fc4`.
...
2. Decompose each Pyth, Switchboard, and Chainlink `AccountInfo` parser into a pure observation
   function and prove parser-to-observation equivalence. The public matrix now includes ordinary
   trade exit, a real liquidation, shutdown, force close, restart, and terminal resolution, but it
   is an orthogonal provider-role sample rather than the full provider-by-lifecycle Cartesian
   product. Do not treat those routes or full-width timestamp equality as a parser proof.
