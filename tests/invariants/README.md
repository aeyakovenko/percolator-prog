...
and claim ordering then add 58 worlds: all eight market/asset-0 policy lanes cross both authority
...
third matrix exhausts all `3!` orders of retained fee redirect, authority handoff, and resolution in
that same value-moving world. Exactly three orders stale each old-authority operation; all six keep
fresh incoming-authority policy and resolve paths, and their terminal economics are identical.
Higher-arity products for the other policy lanes and larger topologies remain.
...
| Focused INV-010 retained-operation landing orders | 6 fixed matrices, 82 public worlds | all `3!` deposit/withdraw/disable orders at three value boundaries enforce one shared-sequence winner and exact stale rollback; both deposit/reduction orders converge outside the conservative health-certificate cache; all eight market/asset-0 policy lanes cross both authority-handoff orders at low/mid/max values; both full-funded authority/resolve orders pay and close all five users; both underfunded authority/resolve orders and all six representative policy/handoff/resolve orders create genuine partial receipts, execute value-moving claims, and converge under the independent terminal oracles |
...
| `cargo test --test v16_program_stateful_fuzz` | 206/206 | full exact engine-`b10b3454`/SBF-`342f7bb5` rerun completed in 258.89 seconds; all six underfunded policy/handoff/resolve orders create value-moving partial claims and converge alongside every prior stateful check |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun in 27.83 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 28 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
| `cargo test --test v16_cu` | 850/850 | full engine-`b10b3454`/SBF-`342f7bb5` exact-source rerun passed in 188.66 seconds on the 2026-08-21 checkpoint; zero failures, including 60 partial-fill route/ratio/maximum-domain worlds with independent rounding arithmetic, exact/late impaired-lien terminal disposition, all-28 simultaneous source-lien chunked release and terminal exit, every prior maximum-shape route, the source-complete INV-085 arithmetic census, bigint boundary/provider references, exact host-to-SBF comparisons, and INV-080 error composition |
...
| `stateful/` | 206 | Generated public routes with exact rollback on rejection and shared state, custody, OI, source-credit, and liveness oracles on success. Coverage includes randomized retained retries and trade-route switching, exact normalized nonzero-fee equivalence across all four trade route classes, lifecycle and terminal compositions, all 2,380 public action words through depth three, 20 underfunded terminal worlds including authority/resolve and every representative policy/handoff/resolve order, one public two-stage exact receipt-top-up lifecycle, and all 5! basic claimant orders. Per-invariant rows below are the source of truth for uncovered dimensions. |
| `cu/` | 850 | Public-route, metamorphic, rollback, liveness, arithmetic-differential, and maximum-shape LiteSVM coverage. It includes source-complete ownership rosters for all 50 instructions, real SPL/CPI boundaries, retained-intent failures and retries, 60 partial-fill route/ratio/maximum-domain worlds, lifecycle and terminal exits, hostile hint/oracle/matcher inputs, full supported shapes, and exact custody/accounting checks. |
...
The current ledger is 9 **CLOSED**, 47 **OPEN-T**, 14 **OPEN-D**, 6 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-010 | OPEN-T | Matcher/control/trade, deposit/withdraw/control, and deposit/reduction orders are exhausted at their documented boundaries. A 48-world matrix crosses market-authority handoff with all eight market/asset-0 policy lanes at low/mid/max values. Two full-funded and two underfunded authority/resolve worlds enforce exact stale rollback and fresh-authority resolve; the latter create genuine partial receipts and value-moving claims. Six additional worlds exhaust the `3!` orders of retained fee redirect, authority handoff, and resolution in that same underfunded model: exactly three stale each old-authority operation, every incoming-authority continuation remains live, and all terminal outcomes converge. Remaining tractable expansion is the same higher-arity terminal product over the other seven policy lanes and larger economic topology, not a missing retained route. |
