...
| --- | ---: | --- |
| Focused INV-020 authenticated timestamp and composite-epoch matrix | 1/1 direct public, 2/2 stateful, 2/2 full-width Kani, plus the complete 799-test CU gate | selected-Switchboard freshness remains covered; numerator-only, denominator-only, and all-fresh-but-cross-epoch composite words reject exactly; mixed-time Hybrid configuration rejects exactly; ignored crank hints preserve oracle/value state; coherent controls retain owner exit and exact terminal payout |
...
2. Extend INV-020's exact composite-epoch rule across Pyth and Switchboard, one/two/three-leg
   multiply/divide/invert modes, stored-time rewind, freshness `-1/0/+1`, configuration, crank,
   trade, liquidation, shutdown, resolve, and restart consumers. Add a pure parser-to-observation
   equivalence decomposition; do not treat the full-width timestamp-equality proof as proof of
   authenticated account parsing or every lifecycle composition.
...
| INV-020 | Independent + P + Direct + F + SVM/CU | `public_sbf/inv_020_authenticated_clock_slot_and_oracle_provenance.rs`, `stateful/inv_020_authenticated_clock_slot_and_oracle_provenance.rs`, `cu/inv_020_authenticated_clock_slot_and_oracle_provenance.rs`, and `kani/inv_020_authenticated_clock_slot_and_oracle_provenance.rs`. Selected Switchboard freshness uses `CurrentResult.submission_idx`; stale selected results reject exactly and account-write churn cannot refresh liveness. The finding-blind composite matrix covers both one-leg-fresh directions, all-fresh cross-epoch reports, mixed-time Hybrid initialization, an ignored mixed-time crank hint, coherent update, healthy owner exit, and exact terminal payout. The production predicate is exhaustive over all `i64` pairs/triples. Stored-slot rewind and expiry boundaries are not yet crossed through every provider mode and public consumer. |
...
| AUDIT-020 | OPEN-T | Issue 405's account-write/selected-result timestamp split is closed on configuration and crank routes. The composite cross-epoch LoF is also fixed: one-leg-fresh and all-fresh-but-different-epoch reports cannot mutate composite provenance or user value, coherent controls remain live through exit and terminal payout, and Kani exhausts the exact production equality predicate over full-width timestamps. Stored-slot rewind and expiry `-1/0/+1` are not yet crossed with every oracle provider mode and public consumer; no whole-parser `AccountInfo` proof or complete clock/observation/lifecycle matrix exists. |
