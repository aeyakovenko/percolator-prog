...
| `cargo test --test v16_program_stateful_fuzz` | 196/196 | exact engine-`592d538c`/SBF-`909c24cc` rerun in 176.24 seconds, including the public three-asset active-close-to-partial-receipt-to-terminal bridge, both active-close barrier orientations, the 32-world cross-asset active-close admission matrix, the inverse 40-world direct/prior-leg terminal-liability matrix, and paired 32-world active-close/ResetPending and active-close/Recovery-reset rank compositions across every trade route, the independent raw-basis/effective-OI ledger, sub-minimum-ADL and cross-domain fractional-source progress, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, concurrent partial receipts, disjoint- and shared-portfolio lifecycle-local reset/shutdown ordering, retained-reduction/shutdown landing orders, simultaneous two-asset lifecycle ordering, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
