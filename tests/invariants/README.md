...
| `cargo test --test v16_program_stateful_fuzz` | 206/206 | full exact engine-`b10b3454`/SBF-`02c40bcb` rerun completed in 253.02 seconds; the expanded three-kind same-portfolio episode generator and every prior stateful check pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 97/97 | full engine-`b10b3454`/SBF-`02c40bcb` exact-source rerun in 27.19 seconds; every retained family passes the same-transaction duplicate matrix, all 16 ordered retained-trade route pairs are exact-once, all 28 public trace consumers validate normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
| `cargo test --test v16_cu` | 851/851 | full engine-`b10b3454`/SBF-`02c40bcb` exact-source rerun passed in 195.14 seconds on the 2026-08-21 checkpoint; zero failures, including the three-kind INV-013 episode matrix, INV-012 route roster, 60 partial-fill route/ratio/maximum-domain worlds with independent rounding arithmetic, exact/late impaired-lien terminal disposition, all-28 simultaneous source-lien chunked release and terminal exit, every prior maximum-shape route, the source-complete INV-085 arithmetic census, bigint boundary/provider references, exact host-to-SBF comparisons, and INV-080 error composition |
...
The current ledger is 9 **CLOSED**, 45 **OPEN-T**, 16 **OPEN-D**, 6 **PARTIAL**, 12 **FRONTIER**,
