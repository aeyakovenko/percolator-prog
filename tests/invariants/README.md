...
| INV-071 | Independent + P(engine) + source composition + F + SVM/CU + R + C | `cu/inv_071_crank_progress.rs` partitions the complete current crank surface into nine liveness strata and binds twenty-nine exact-pin engine contracts/proofs to twenty-two executable public witnesses. INV-072 source-locks all ten plan shapes, the sole wrapper ingress, and every parser/dispatch stratum. Public bounded frontiers cover Recovery, explicit B, active close, lien impairment, receipt conflict, oracle failure, and ResetPending; each accepted crank strictly decreases an independent mode-aware rank ordered by the production selector, including selected-leg and flat-account fallback accrual distance, or enters a lower terminal mode, while fixed points reject exactly. Maximum-shape routes cover fourteen active legs, twenty-eight source domains, and forty-two external-feed accounts. Closure assumes authenticated time/oracle inputs, fair submission, SVM rollback, and the named engine arithmetic boundary. A new plan, actionable flag, engine pin, wrapper ingress, failure class, or supported shape reopens this row. |
...
| AUDIT-071 | REOPENED | The current public crank surface is source-complete by composition. Nine liveness classes bind twenty-nine exact-pin engine contracts/proofs to twenty-two public witnesses; all ten selector result shapes and every wrapper parser/dispatch stratum are owned. Independent ranks and bounded public frontiers cover stale K/F, ResetPending, B, active/expired close, liquidation, source-lien and released-obligation cleanup, Recovery, resolved payout, fixed points, and terminal slab progress. Recovery, explicit-B, lien-impairment, receipt-conflict, oracle-failure, active-close, and ResetPending products exhaust their bounded orderings; fourteen-leg/twenty-eight-source/forty-two-feed routes cover the supported maxima. Closure is conditional on authenticated inputs, fair submission, SVM rollback, and the pinned arithmetic boundary; a new class, plan, ingress, pin, or shape reopens it. |
