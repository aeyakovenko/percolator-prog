...
caps, and both exit-only lifecycle modes; its next tractable partition is two simultaneous barriers
across two assets, followed by explicit reachability/unreachability arguments for ResetPending,
Resolved, and Retired exposure. The next broader
...
| Focused INV-050 scalar, lifecycle, and active-close barrier matrix | 4/4 scalar route worlds, 8/8 DrainOnly/Recovery route worlds, and 8/8 stateful route/orientation worlds | zero rejects exactly; one-atom reduction/flip/close and exact maximum open/close land below route CU caps; max+1 rejects exactly; both exit-only modes reject flips while exact closes remain live; both long and short active-close barriers reject reissue, frame the close, retain the matching loss obligation, and release it through the actual owner account's automatic crank |
