...
and custody conservation. A second public matrix exhausts all `4!` landing orders for two real
positions per side, independently reconstructs effective OI, stored counts, pending-obligation
counts, and loss weights after every instruction, then requires bounded permissionless cleanup and
exact return of all four principals. At the maximum supported 14-leg/28-source shape, the retaining
exit, opposite exit, and permissionless cleanup consume 202,299, 931,870, and 428,232 CU
respectively.
...
| AUDIT-041 | OPEN-T | A public scarce-backing topology exhausts both equal-priority pair orders crossed with one-shot/dust force-close schedules and compares per-user claims plus domain classifications; observation order is also covered. A separate public model exhausts all `4!` Recovery landing orders for two positions per side, reconstructs effective OI, stored/pending counts, and loss weights after every step, and requires identical terminal principal and custody. Extend the model to liquidation, insurance, lien, residual, payout, claim, and close-preemption ordering. |
...
| AUDIT-074 | OPEN-T | The unrelated-accrual close-drift path is fixed and function-contract/public-route covered: a local bookable close no longer gives its owner authority to force global Recovery. The permanent historical-bankruptcy lock on unrelated exact backing is fixed across two public cohort sizes. The provider-settlement route proves both backing-principal and remaining-insurance withdrawals are not blocked by audit history after active loss is gone; the shared predicate and all blocker partitions are covered by one assumption-free wrapper proof. Twelve public underfunded worlds prove a genuine partial receipt cannot delay an unrelated flat portfolio's exact principal across expiry, claimant-order, and payout-route permutations. Complete the remaining side, portfolio, domain, close, receipt-substitution/concurrency, and lifecycle locality cross-product before promotion. |
