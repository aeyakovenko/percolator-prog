...
caller matching both roles is bound to the target asset. The follow-on simplification removes the
...
caller-input and boundary rosters now own all 227 public fields across 52 input types, the proof
census owns 180 harnesses across 21 mounted modules, and the public-trace guard owns 33 consumers.
...
The exact rebuilt SBF is 1,230,120 bytes with SHA-256
`38179468b9a04a04c49fe8b7452e17d06e86417d469d8bbe2079b9b34d8abc72`. Removing the aggregate
route cuts 293 production-source lines and 16,176 SBF bytes from the preceding checkpoint. The
complete exact-artifact gates cover 890 LiteSVM/CU tests, 102 public fuzz regressions, and 202
stateful/model tests. The mounted wrapper proof census is 180 harnesses; exact final rerun results
are recorded in the verification table below.

This is still not INV-005 closure. A source-derived call-graph census classifies all 29 public
...
deterministic current-state reconciliation. Three explicit epoch gaps remain:
`UpdateAssetLifecycle`, `UpdateBaseUnitMints`, and `SwapSecondaryForPrimary`.
...
recreation and the three source-rostered non-epoch authority routes named in the current checkpoint.
...
221 fields across all 49 public instruction variants and the three nested input structs. INV-083
...
all 227 fields across 52 public input types map to exactly one of 20 semantic profiles, one
...
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`9b737fd`/SBF-`38179468` rerun completed in 3,299.87 seconds; the 27-case authority-incarnation matrix, every retained-policy landing order, and every prior stateful/model check pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full exact engine-`9b737fd`/SBF-`38179468` rerun completed in 219.46 seconds; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 890/890 | full exact engine-`9b737fd`/SBF-`38179468` rerun completed in 1,321.38 seconds; zero behavioral failures, including the source-complete authority-epoch route/decoder matrices and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
...
| AUDIT-005 | OPEN-D | A source-derived call graph classifies all 29 configured-authority routes. The 23 epoch-bearing source variants, represented by 27 semantic authority cases, reject same-market `A -> B -> A` with exact rollback and admit fresh current-epoch controls. `ClosePortfolio` is independently incarnation/sequence/episode-bound, and both auxiliary-ledger synchronizers are deterministic current-state reconciliation. Three routes remain explicit epoch gaps: lifecycle, base-unit mint replacement, and secondary-to-primary swap. The ambiguous market-wide insurance route is removed; its old tag rejects while the scoped route is epoch-bound. Kani proves exact epoch admission, atomic checked handoff, the migration-aware floor, and exact generation/epoch wire binding for reserve withdrawals. Persistent market incarnation remains separately open because close/recreate resets account-local epochs. |
