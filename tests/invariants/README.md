...
theorem. The exact current artifact passes 912/912 CU tests; the full Kani result is recorded below.
...
| `cargo test --test v16_cu` | 912/912 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 208.26 seconds; zero behavioral failures across market tombstone/rent/fresh-address behavior, matcher identity/incarnation and stateless-PDA composition, reserve, fee-policy/destination ownership, claim/backing/insurance single-use, exact lien lifecycle, source-realizability, exact-claim, credit-rate transition composition, exact B-booking/account-settlement/carry-normalization partitions, oracle, Recovery, terminal, source-lien normalization, selector composition, equivalent-route source composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling; the run observed 1,116,641 CU for max-source liquidation and 1,314,291 CU for max-shape resolved close. The non-exit 28th-lien admission reached 1,398,554 CU and remains an explicit thin-headroom risk. |
...
social-loss-clearing examples. A source-complete wrapper rounding-site inventory and the broader
ratio-change/split matrix remain open; these tests do not imply them.
...
| AUDIT-038 | OPEN-T | Fractional price movement carries exact sub-basis-point residue, reaches the target in finite public cranks, and preserves exact terminal payout; denominator boundaries and reserved bytes fail closed. Independent public oracles now reconstruct both resolved-receipt top-up floors from immutable face and the exact market B-booking, account B-settlement, and zero-OI carry-normalization quotient/remainder equations, including side-local dust and explicit loss. Build a source-complete semantic inventory for every wrapper-origin value-affecting division and cover the unenumerated ratio-change and split/merge products before closure. |
