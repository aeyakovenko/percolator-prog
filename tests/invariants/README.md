...
tests, 101 public regressions, 207 stateful tests, 916 LiteSVM/CU tests, and 193 wrapper Kani
...
terminal-generation classification, and source-locks all 30 finding-blind `is_violation` oracles to
an explicit evidence class. Twenty oracles now carry classifier-bound public LoF evidence. In
...
payouts show both victim loss and net coalition gain. The remaining 10
...
therefore attribution-only, not terminal LoF. Five oracle-control families remain explicit
terminal-value candidates; the other six remain constrained to signed economic bounds,
attribution, liveness, or provider-consent claims until stronger evidence exists.
...
remaining finding-blind public counterexample runner. A source-complete roster now covers all 30
...
activation-fee debit, and the revoked-matcher terminal world raise the classifier-bound set to 20.
Source-complete retry and supersession disposition tables prevent weaker discoveries from
inheriting that label; the supersession table keeps five terminal-value candidates explicit and
closes both cranker-share controls as attribution-only. The
...
mutation moves the exact inverse amount between cranker payout and retained insurance. The five
mark/configuration families remain named terminal-value candidates for the next paired-world
tranche.
...
| `cargo test --test v16_program_stateful_fuzz` | 212/212 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 257.86 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets, including generated route selection for all four terminal retained-trade retry worlds, both exact external-debit retry families, the revoked-matcher paired terminal world with a fresh-grant nonvacuity witness, and fee-bearing maintenance/liquidation share worlds. The latter prove exact fee and affected-user terminal-payout invariance while the cranker/insurance allocation moves nonvacuously. The suite also owns the complete pending-obligation and close-ledger encumbrance census, direct per-domain `usable_positive_credit_num <= available_backing_num`, exact/bound claim transitions, credit-rate edge causality, and the independent resolved-payout quotient/remainder oracle. The corrected INV-030 matrix additionally crosses all four trade routes and both source sides while proving one-for-one face settlement, zero impaired-domain credit, exact rollback of favorable admission, and live bilateral plus owner-only reduction. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
| Registry/manifest checks in the INV-079 module | 14/14 | includes the source-complete 30-oracle evidence roster, twenty exact terminal-LoF dispositions, all 11 retained-retry dispositions, all 14 retained-control supersession dispositions with two attribution-only fee-share rows and five explicit terminal-value candidates, complete-route terminal classification, and the recursive 54-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 108/108 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 28.36 seconds; includes the generic 11-operation whole-market no-reuse matrix, deterministic terminal retained-trade, exact external-debit retry, revoked-matcher, and both fee-share attribution worlds, the source-complete 11-kind retry and 14-kind supersession disposition censuses, the source-complete account-incarnation and detached-signature censuses, all public regressions, 54 trace consumers, executable registries, and the 30-oracle INV-079 evidence roster |
...
| AUDIT-079 | OPEN-T | The LiteSVM trace schema records actual transaction signers, compiled account metas, exact authority-attributed tracked token/lamport deltas, rejected writable-account rollback with the fee-payer network charge separated from program effects, and between-transaction economic mutation. The shared validator requires an allowlisted public construction sequence containing a real wrapper call and rejects malformed success, rollback, signer, account, payload, program, CU, token-owner, vault-participation, and quote-balance evidence. A recursive scan source-locks all 54 current trace consumers to validate or classify immediately. The normalized terminal classifier distinguishes exact loss/unauthorized gain, persistent funded lock, bounded exit, atomic rejection, and progress; real public exit/rejection witnesses plus negative anti-overclaim controls pass. Required, attempted, and progressing route masks prevent incomplete searches or successful no-op continuations from being labeled persistent DoS. Twenty of 30 finding-blind violation oracles carry classifier-bound exact LoF evidence, including all eight signed fee-consent families, four retained trade transports, the exact nonrecoverable activation-fee retry, fractional settlement with a complete progressing-route observation, stale pending-mark admission and inheritance matrices, and revoked matcher consent with a fresh-grant nonvacuity witness; the paid mark-reserve and paired pending-target oracles separately carry exact terminal evidence while retaining narrower labels for local rollback branches. All 11 retained-retry kinds and all 14 same-incarnation supersession kinds have source-complete dispositions. Recoverable insurance principal remains non-LoF; real fee-bearing terminal worlds classify liquidation and maintenance shares as attribution-only; five oracle controls remain explicit terminal-value candidates. The enforced roster limits the remaining replay, semantic, privileged, or local discoveries to their actual claims. Attach independently measured terminal observations and complete exit-route attempts to every remaining qualifying finding-blind counterexample before promotion. |
