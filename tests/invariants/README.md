...
duplicating the matrices owned by adjacent invariants. Its source-derived roster still owns all
206 fields across all 50 public instruction variants and the three nested input structs. INV-083
...
| `cargo test --test v16_program_stateful_fuzz` | 205/205 | full exact engine-`9b737fd`/SBF-`320e8162` rerun completed in 258.89 seconds; the independently derived INV-034 cross-domain loss world and every prior stateful/model check pass. One duplicate finding-specific INV-034 adapter was removed. |
| Registry/manifest checks in the INV-079 module | 12/12 | includes the source-complete 28-oracle evidence roster, complete-route terminal classification, and the recursive 32-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full engine-`9b737fd`/SBF-`320e8162` exact-source rerun; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 889/889 | full engine-`9b737fd`/SBF-`320e8162` exact-source rerun passed in 196.50 seconds on 2026-08-28; zero failures, including the source-complete INV-034 role roster, all 50 account-role classifications, reserve, Hybrid-oracle, public Recovery, terminal slab, lifecycle, maximum-shape, and prior invariant routes. The count decreases by one because a byte-seeded resolved-claim duplicate was removed in favor of the stronger all-public stateful lifecycle. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 174/174 | full wrapper-only exact-source rerun with engine `9b737fd`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied |
