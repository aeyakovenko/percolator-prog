...
`d604ca09b7e584d3875ce4516bab1186346bf4a6` (engine PR
[#191](https://github.com/aeyakovenko/percolator/pull/191), stacked on canonical-allocation PR
[#190](https://github.com/aeyakovenko/percolator/pull/190)). Commit `6c8d94bc` is the deployed
source-lien progress fix; `d604ca0` adds proof and documentation only and does not change engine
production code.
...
`Missing`. This closes same-address wrapper-market reuse; INV-006 still owns explicit genesis/chain
domain binding for detached retained-message formats.
...
| Focused INV-029 exact-only deployed profile | 211/211 stateful public tests plus 1/1 source lock | after every generated successful public transition, each domain's exact claim equals its positive bound and complete portfolio attribution; in Live mode, each portfolio's aggregate source-claim face also equals its exact positive-PnL face. The wrapper exposes no non-exact bound or rebucketing ingress, so approximate-bucket range/rebucketing proofs remain N/A unless that optional profile is introduced |
...
| `cargo check --tests` | pass | exact engine-`d604ca0` build completed at the current checkpoint |
| `cargo test --lib` | 7/7 | exact engine-`d604ca0` rerun at the current checkpoint |
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 259.94 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets. The suite includes the finding-blind 11-operation whole-market no-reuse property, 56-world one-domain, 48-world two-domain, and 48-world mixed-expiry INV-052 source-lien matrices, exact per-portfolio source attribution and close exclusion, the asymmetric-fee INV-041 order regression, INV-076 rollback frames, the finding-blind 21-family retained asset-generation matrix, source-domain single-use, retained source-fee consent, and backing-provider landing-order campaigns. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
...
| `cargo test --test v16_program_fuzz_regressions` | 99/99 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 28.08 seconds; includes the generic 11-operation whole-market no-reuse matrix, all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 900/900 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 198.73 seconds; zero behavioral failures across market tombstone/rent/fresh-address behavior, reserve, fee-consent, oracle, Recovery, terminal, lifecycle, source-lien normalization, selector composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling; the run observed 1,116,641 CU for max-source liquidation and 1,314,291 CU for max-shape resolved close. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 188/188 | full wrapper-only exact-source rerun with engine `d604ca0` completed in 2,345 seconds; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, deployed-arithmetic, and arbitrary-prestate tombstone harnesses, with required constructive covers satisfied |
| Engine runtime/property suites | 189/189 | exact `cargo test --all-targets` rerun on engine `d604ca0`: 50 library, 13 backing, 2 resolved-insolvency, and 124 v16 specification tests pass |
...
exists. The locally rebuilt 1,240,192-byte production SBF used by the 2026-08-30 exact-pin gates has
SHA-256 `e52a9cfb0412d07b1cf4e7699296ed225815bdf55d931374818123c1dcc79d4f`.
...
engine branch. The complete 900-test CU suite remains green.
...
2. Extend INV-001/007 beyond the now-closed wrapper-market address class without weakening the
   strict no-reuse policy. Add explicit cross-program and detached chain-domain controls under
   INV-006, and inventory any closable wrapper-owned receipt, delegate, capability, or auxiliary
   account whose pubkey can outlive its economic incarnation. Portfolio and asset reuse already
   have independent program-assigned IDs under INV-002/003.
...
| INV-019 | P + generated F + SVM/CU | `kani/inv_019_cpi_invocation_and_return_data_binding.rs` and `cu/inv_019_cpi_invocation_and_return_data_binding.rs` prove full-width matcher field/flag binding, request freshness, single-context and batch return replay rejection, tail isolation, and current-producer provenance. A distinct nested program's return is harmless before the configured matcher overwrites it but rejects with exact rollback when emitted afterward. A deployed external matcher publicly controls every hostile mode and closes/recreates the same context while the wrapper publicly closes/reinitializes the same LP address; a publicly written stale response rejects exactly before a fresh response succeeds. An eight-world stateful matrix varies route order, asset, size, and three same-address context incarnations while preserving rollback, exit, OI, custody, and CU invariants. Oracle routes consume authenticated account data rather than CPI return data. INV-001/007 now prevent the market-local request sequence from resetting under the same market pubkey. |
...
The current ledger is 19 **CLOSED**, 38 **OPEN-T**, 13 **OPEN-D**, 5 **PARTIAL**, 12 **FRONTIER**,
...
| AUDIT-007 | OPEN-T | A bounded public model exhausts all 11 retained market-request kinds across the close/reuse boundary with zero state injection and exact trace frames. The current pin permanently tombstones the retired address, so reinitialization and every retained request reject exactly while fresh-address initialization remains live. The executable 99-finding manifest has zero quarantines. Extend the account-class census to receipts, delegates, capabilities, and auxiliary accounts; asset and portfolio reuse already use program-assigned IDs under INV-002/003. |
...
| AUDIT-019 | OPEN-T | Matcher return fields, stale data, req_id, tails, local validation, and nested-CPI producer ordering are covered. Every hostile matcher mode now uses the external fixture's public control path. A second program's return before the configured matcher is superseded and live; replacement after the matcher rejects with exact writable-state rollback. The former injected ABA setup is replaced by public external-context and wrapper-portfolio close/recreate lifecycles. An eight-world stateful campaign composes both CPI routes in both orders through repeated same-address context incarnations, exact stale/no-write rollback, fresh retries, inverse exits, zero OI, exact custody, and bounded CU. Oracle paths read authenticated accounts rather than CPI return data. INV-001/007 prevent the market-local request sequence from resetting at the same wrapper-market pubkey. A complete detached matcher-capability/account-class ABA census remains before closure. |
...
On engine pin `d604ca09b7e584d3875ce4516bab1186346bf4a6`, the full 900-test `v16_cu` inventory is
