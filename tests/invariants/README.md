...
| `cargo test --test v16_program_stateful_fuzz` | 180/180 | full engine-`3b76b794`/SBF-`97eb59e9` rerun in 166.63 seconds, including generated aggregate/split/reversed terminal-insurance withdrawal, the 16-route resolved-claim split matrix, Recovery K/F refresh, same-asset close drift with real funding, malformed-hint rollback and exact OI framing, success-must-mutate/fixed-point rollback checks, both active-close/shutdown landing orders, and every shared rank/reference-model check |
...
3. Apply INV-052's split/merge oracle to liquidation, multi-domain/expiry lien consumption,
   cooldowns, rates, and policy limits. Live asset-insurance withdrawal, terminal market-wide
   insurance withdrawal, atomic live backed-claim conversion, and public resolved-claim splitting
   now have generated or exhaustive route-partition coverage. Keep each remaining operation's
...
| AUDIT-052 | OPEN-T | The current-anchor compounding and endpoint-funding sampling violations are fixed by one bounded canonical accrual path. Eleven CU and eleven stateful public tests now include generated target replacement plus live, resolved, shutdown/Recovery, owner-reduction, live and terminal insurance, and backed-claim conversion partitions, exact normalized/SPL outcomes, both resolved claimant orders, full 14-leg/32-step CU coverage, exact post-ADL zero-sum settlement, five engine proofs, and a wrapper carry-domain proof. Owner-reduction split/reversed rounding is independently specified and conservatively bounded. The live insurance model crosses both asset domains in aggregate, split, and reversed order, checks exact per-call value flow, rejects one-atom overspend atomically, and converges every economic byte. The terminal model resolves and pays all claimants, dematerializes all portfolios, and proves aggregate/split/reversed market-wide insurance drains converge before one exact-rollback exhausted retry. The all-four-route claim model proves conversion is deliberately atomic: strict partition caps roll back rather than partially consume claim/lien atoms, then one full conversion and one reuse rejection converge exactly. A separate public matrix crosses all 16 open/close route pairs and proves one exact-expiry underfunded claim cannot gain by splitting across two portfolios: both worlds create real partial receipts and nonzero payouts, routes converge, and the only allowed delta is one conservative floor atom. A direction-reversal witness proves gross paid/received funding telemetry is cadence-dependent and unsafe as a paid-only reward basis; the wrapper has no such consumer. Add exact stale/fresh and mixed-oracle histories plus arbitrary partition/permutation fuzz for liquidation, multi-domain/expiry lien consumption, cooldowns, rates, and policy limits. |
