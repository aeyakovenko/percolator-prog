...
The current INV-005 tranche extends the canonical per-asset authority epoch from authority handoff
and authenticated resolution to every sequence-bearing policy and managed-oracle route. Thirteen
additional source variants now carry and require the exact current epoch: all six fee-policy
updates, permissionless-resolve policy, Hybrid/EWMA/Auth mark configuration and publication, and
oracle restart. Together with market handoff and the five asset-role handoffs, the source-derived
matrix owns 16 epoch-bearing instruction variants as 20 semantic authority cases. Every case
retains an old request across `A -> B -> A`, requires exact byte/SPL rollback for the stale request,
and proves a fresh current-epoch control both lands and mutates the intended state. The generated
campaign covers 160 public route instances, and the deterministic policy/handoff/resolve product
continues to cover every landing order in funded and underfunded terminal worlds.

The implementation adds no persisted bytes, mirror field, or parallel epoch store. It reuses the
former short-side backing-fee watermark as the one authority-epoch lane, keeps one migration-aware
backing-fee floor, and centralizes fresh-request binding in the transaction builder. This tranche
does add the explicit epoch wire field and handler check to 13 routes, so it is a canonicalization
and schema-hardening change rather than a net source-line reduction. A source-derived decoder
roster, one-byte truncation/trailing-byte proofs, and exact field-preservation proofs prevent route
drift. The caller-input and boundary rosters now own all 222 public fields, the proof census owns
177 harnesses across 21 mounted modules, and the public-trace guard owns 33 consumers.

The exact rebuilt SBF is 1,244,792 bytes with SHA-256
`38dac2fd3d378e32703a4f7772a1b7ec806a4413c210e4ac1eb4d1dbd468ff4c`. The complete exact-artifact
gates pass 890/890 LiteSVM/CU tests, 102/102 public fuzz regressions, 202/202 stateful/model tests,
and 177/177 wrapper Kani harnesses. The two initially stale INV-010 fixtures were corrected to read
the real post-handoff epoch/floor; production admission was not weakened.

This is still not INV-005 closure. Reserve funding/withdrawal, auxiliary-ledger synchronization,
lifecycle, mint replacement, collateral swap, and terminal-cleanup authority routes still require
a source-complete classification: bind the exact epoch when the operation can be retained, or prove
that it is immediate, idempotent/current-state-only, or independently incarnation-bound. Full
same-pubkey market recreation is separately blocked by INV-001: the market account currently resets
asset IDs, authority epochs, and local replay watermarks when it is closed and initialized again,
so no account-local epoch can establish market-incarnation identity by itself.
...
conversion moves the replacement PnL into capital. Existing INV-004 close/cure and route/writer
  ...
  authority routes reject same-market `A -> B -> A`; remaining destructive-consent gaps are
  same-pubkey whole-market recreation, unclassified non-epoch authority routes, and payload-free
  terminal `CloseSlab`. Those remain owned by INV-001/005.
...
all 222 fields across 53 public input types map to exactly one of 20 semantic profiles, one
...
| `cargo test --test v16_program_stateful_fuzz` | 202/202 | full exact engine-`9b737fd`/SBF-`38dac2fd` rerun completed in 256.13 seconds; the 20-case authority-incarnation matrix, every retained-policy landing order, and every prior stateful/model check pass |
...
| `cargo test --test v16_program_fuzz_regressions` | 102/102 | full engine-`9b737fd`/SBF-`38dac2fd` exact-source rerun; includes all public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 890/890 | full engine-`9b737fd`/SBF-`38dac2fd` exact-source rerun passed in 202.72 seconds on 2026-08-28; zero failures, including the source-complete authority-epoch route/decoder matrices and all prior reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 177/177 | full wrapper-only exact-source rerun with engine `9b737fd`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied |
...
fee consent remain explicit INV-001/INV-005/INV-014 gaps. The 20 epoch-bearing authority cases now
...
| INV-005 | Independent + P + Direct + F + SVM/CU + Static roster | `public_sbf/inv_005_authority_incarnation_binding.rs`, `stateful/inv_005_authority_incarnation_binding.rs`, `cu/inv_005_authority_incarnation_binding.rs`, and `kani/inv_005_authority_incarnation_binding.rs` own the 16 epoch-bearing source variants as 20 semantic authority cases. Every case crosses same-market `A -> B -> A`, rejects the stale retained request with exact rollback, and admits a mutating current-epoch control. Kani proves exact epoch admission, checked handoff advance, and migration-floor preservation. Reserve funding/withdrawal, auxiliary-ledger synchronization, lifecycle, mint replacement, swap, and terminal-cleanup authority routes still need exact-epoch binding or a proof-backed non-retained classification. Whole-market same-pubkey recreation remains an INV-001 dependency because account-local epochs reset. |
...
| AUDIT-005 | OPEN-D | The 16 epoch-bearing source variants, represented by 20 semantic authority cases, now reject same-market `A -> B -> A` with exact rollback and admit fresh current-epoch controls. Three Kani harnesses prove exact epoch admission, atomic checked handoff, and the migration-aware floor. Closure still requires a source-complete disposition for every remaining non-epoch authority route and a persistent market-incarnation primitive so close/recreate cannot reset account-local epochs. |
