...
theorem. The exact current artifact passes 911/911 CU tests; the full Kani result is recorded below.
...
| `cargo test --test v16_cu` | 911/911 | full exact engine-`d604ca0`/SBF-`e52a9cfb` rerun completed in 200.07 seconds; zero behavioral failures across market tombstone/rent/fresh-address behavior, matcher identity/incarnation and stateless-PDA composition, reserve, fee-policy/destination ownership, claim/backing/insurance single-use, exact lien lifecycle, source-realizability, exact-claim, and credit-rate transition composition, oracle, Recovery, terminal, source-lien normalization, selector composition, equivalent-route source composition, and maximum-shape routes. Every observed required exit remains below the 1.4M CU ceiling; the run observed 1,116,641 CU for max-source liquidation and 1,314,291 CU for max-shape resolved close. The non-exit 28th-lien admission reached 1,398,554 CU and remains an explicit thin-headroom risk. |
