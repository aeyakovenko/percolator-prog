...
`Certified`; the executable manifest now contains 72 certified, 19 quarantined, 8 nonqualifying,
...
1. Convert the 19 `Quarantined` adapters to `Certified` only after the current pin satisfies their
   positive economic and liveness postconditions. The other 80 entries already have explicit
   executable disposition: 72 fixed-pin certifications and 8 public nonqualifying proofs. Do not
...
The executable 99-finding manifest currently contains 72 `Certified`, 19 `Quarantined`, 8
...
72 are `Certified`, 19 remain `Quarantined`, 8 are `Nonqualifying`, and none are `Missing`. Its
