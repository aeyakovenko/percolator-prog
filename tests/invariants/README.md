...
| Focused INV-071 wrapper fixed-point composition | 1/1 public first-observation/duplicate pair plus every generated crank schedule | the pre-fix duplicate succeeded at 38,437 CU without changing persistent state; the fixed wrapper accepts the first real accrual, rejects the same-slot fixed point as `EngineNonProgress`, and requires exact program/token/custody rollback. The shared oracle rejects any successful crank whose economic snapshot is unchanged. |
...
| `cargo test --test v16_program_stateful_fuzz` | 177/177 | full engine-`3b76b794`/SBF-`97eb59e9` rerun in 165.76 seconds, including Recovery K/F refresh, success-must-mutate/fixed-point rollback checks, both active-close/shutdown landing orders, and every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`3b76b794`/SBF-`97eb59e9` rerun in 26.72 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 793/793 | full engine-`3b76b794`/SBF-`97eb59e9` rerun in 191.99 seconds, including duplicate-observation fixed-point rejection, maximum-source exits, maximum-shape Recovery obligation and K/F cohort cleanup, liquidation/resolved close, exhaustive account-role matrices, the dense zero-delta terminal-shape benchmark, and B-before-claim reset-carry retirement |
