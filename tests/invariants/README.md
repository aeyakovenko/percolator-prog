...
| --- | ---: | --- |
| Focused INV-020 authenticated timestamp and composite-epoch matrix | 1/1 direct public, 2/2 stateful, 2/2 full-width Kani, 2 deterministic CU matrices, plus the complete 801-test CU gate | selected-Switchboard freshness remains covered; 126 one/two/three-leg provider/transform configurations reject 114 selected-leg skews and 126 coherent rewinds exactly; 39 provider words cross configuration and crank freshness at ages 59/60/61; mixed-time Hybrid configuration and ignored crank hints preserve oracle/value state; coherent controls retain owner exit and exact terminal payout |
...
2. Extend INV-020's now-complete configuration/crank provider matrix through trade, liquidation,
   shutdown, resolve, and restart consumers. Decompose each Pyth, Switchboard, and Chainlink
   `AccountInfo` parser into a pure observation function and prove parser-to-observation
   equivalence; do not treat full-width timestamp equality or the 126/39 public matrices as proof
   of authenticated account parsing or every lifecycle composition.
