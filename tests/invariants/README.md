...
`53c07731a9335d6f2b3bc87843d04d6c6bdb791fc0b964b0ef2de9f70895c7c7`. Resolved-mode
...
roster passes 164/164. Arbitrary relational wide scale division remains behind the arithmetic wall
...
sources: 128 direct harnesses plus 36 macro-generated trade-decoder harnesses, exactly matching the
164 reported by `cargo kani list`. Every direct harness is classified into 67 symbolic-total, 23
...
The next INV-085 tranche adds no production code. Three assumption-free Kani harnesses compare the
...
was found. INV-085 remains `FRONTIER`: full relational oracle scaling, private processor fee/notional
adapters, host-to-BPF boundary equivalence, and wrapper-versus-bigint coverage are still incomplete;
...
| Focused INV-083 field-complete boundary inventory | 10/10 CU plus 164/164 wrapper Kani composition | all 206 fields across 53 public input types are source-locked into 20 semantic boundary profiles with per-field and profile-level executable evidence; the class roster retains all eleven required classes, and 25 public invalid `InitMarket` partitions reject with pristine-account rollback before a valid retry creates a usable portfolio |
| Focused INV-084 proof-harness nonvacuity inventory | 3/3 CU plus 13/13 assumption-focused Kani and 164/164 wrapper Kani composition | a source parser derives all 128 direct and 36 generated harnesses and locks 67 symbolic-total, 23 branch-witnessed, 10 explicitly constrained, 28 concrete-exact, and 36 generated-symbolic dispositions; all 13 explicit assumptions across 19 modules retain exact ownership and public evidence; public routes reach admitted domains, reject invalid partitions with exact rollback, and terminate with exact custody |
| Focused INV-085 deployed arithmetic differential | 7/7 host/CU plus 5/5 Kani and 126/126 public composite worlds | five wrapper policy/oracle primitives match independent widened formulas over fixed boundaries and 16,384 deterministic full-width words; premium funding exhausts an 8-bit symbolic product, EWMA and fee-supported movement exhaust 3-bit products, and 512 generated dynamic-fee words match an exhaustive fee scan. Every legal three-provider composition topology lands its independently calculated rational E6 price through the deployed SBF route. Full relational scaling, private processor adapters, host/BPF boundaries, and bigint equivalence remain frontier work. |
...
| `cargo test --test v16_program_stateful_fuzz` | 195/195 | exact engine-`592d538c`/SBF-`53c07731` rerun in 180.28 seconds, including the 40-world aggregate/equal-two/asymmetric-three source-lien partition matrix, public composite-epoch rejection/ignore/coherent-control matrix, Hybrid terminal payout, three-asset active-close-to-partial-receipt-to-terminal bridge, both active-close barrier orientations, the 32-world cross-asset active-close admission matrix, the inverse 40-world direct/prior-leg terminal-liability matrix, and paired 32-world active-close/ResetPending and active-close/Recovery-reset rank compositions across every trade route, plus every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`592d538c`/SBF-`53c07731` exact-source rerun in 165.10 seconds; the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green after INV-084 closure |
| `cargo test --test v16_cu` | 833/833 | full engine-`592d538c`/SBF-`53c07731` exact-source rerun passed in 194.09 seconds on the 2026-08-21 checkpoint; zero failures, including the new INV-085 host differential corpus, exact public composite-price oracle, and updated INV-084 proof roster |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 164/164 | full wrapper-only exact-pin rerun on the 2026-08-21 checkpoint with engine `592d538c`; zero failures, including all 23 branch-witnessed proof classes with constructive covers, all 13 source-inventoried assumptions, the five INV-085 arithmetic relations, 16 provider-parser decomposition harnesses, and every prior decoder and rollback proof |
...
`53c07731a9335d6f2b3bc87843d04d6c6bdb791fc0b964b0ef2de9f70895c7c7`.
...
| INV-084 | P + Source audit + SVM/CU | `cu/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` derives all 164 mounted harnesses, locks their five nonvacuity categories, follows local assertion helpers, requires constructive covers for every branch-limited claim, and binds every concrete-fixture module to public evidence. It separately discovers all 13 explicit `kani::assume` sites across all 19 modules and requires an exact row naming the owning proof, constructive witness, classification, and public evidence. `kani/inv_084_proof_assumptions_are_reachable_and_nonvacuous.rs` exhausts every finite admitted/excluded partition with boundary mutation killers. Public LiteSVM reaches the valid identity, sequence, episode, fee, enable, and signed-size domains and proves invalid partitions roll back exactly before terminal exit. Any proof-roster or category drift reopens this current-surface closure. |
| INV-085 | P + SVM/CU arithmetic differential + Proof gap | `kani/inv_085_proven_arithmetic_equals_deployed_arithmetic.rs` has five assumption-free bounded relational proofs for deployed price movement, dt clamping, premium funding, fee-weighted EWMA, and fee-supported mark movement. `cu/inv_085_proven_arithmetic_equals_deployed_arithmetic.rs` compares the same primitives over fixed edges and 16,384 deterministic full-width words, then compares 512 dynamic-fee words with an exhaustive fee scan. INV-020's 126-world public provider matrix now requires the exact independent rational E6 result for every legal multiply/divide/invert/unit-scale topology. Full relational scale arithmetic, private processor adapters, host/BPF boundary equivalence, and bigint equivalence remain. Engine arithmetic is excluded as engine-owned. |
...
| AUDIT-084 | CLOSED | A host source audit derives all 128 direct and 36 macro-generated harnesses from all 19 mounted modules, exactly matching `cargo kani list`, and locks 67 symbolic-total, 23 branch-witnessed, 10 explicitly constrained, 28 concrete-exact, and 36 generated-symbolic dispositions. Every branch-limited claim has a satisfiable cover; every concrete-fixture module has assertion-bearing proofs and named public evidence; every generated proof is tied to the symbolic macro template and assertion helper. All 13 explicit assumptions retain exact file/line/predicate ownership, constructive Kani witnesses, classification, and public-route evidence. Full-width partitions pin off-by-one, widening, dropped-mark-clause, cap, epoch, identity, sequence, zero-size, premium-sign, EWMA-weight, and asymmetric-fee mutations. A new mounted module, harness, macro generator, assumption, branch-limited claim, or concrete-fixture module reopens this row. |
| AUDIT-085 | FRONTIER | Five deployed price/funding/fee primitives match widened references under assumption-free bounded Kani, fixed edges, and 16,384 deterministic full-width host words. A 512-word dynamic-fee differential matches an exhaustive fee scan. The 126-world public composite-provider matrix now checks exact independent rational output across every legal multiply/divide/invert/unit-scale topology. The attempted 8-bit seven-axis EWMA relation crossed a five-minute isolated budget; the complete 3-bit product passes. Remaining work is an exact source roster and differential model for private processor fee/notional/reward adapters, relational full-width Pyth/Switchboard/Chainlink scaling, explicit host-versus-SBF boundary words, and bigint/equivalence discharge. Engine arithmetic is engine-owned and must not be duplicated. |
| AUDIT-086 | OPEN-T | The shared runner checks 27 semantic action classes spanning 25 decoder variants and includes deployed authority/permissionless resolution, payout, live insurance/backing withdrawal, owner recovery-forfeit, abandoned-asset force-close, restart, fresh-generation trade, DrainOnly reduction, empty retirement, and terminal-convergence edges. The base graph exhausts 183 words through depth two over thirteen actions. Its normalized state now includes every portfolio's PnL, escrow, status, close ledger, and payout receipt, the market payout snapshot/ledger, and every source-credit, backing-bucket, and insurance-reservation domain. A second graph independently reconstructs 12 public partial-receipt worlds across expiry, claimant order, and route priority; all six claim-priority schedules move SPL value, all eight exact/late schedules normalize backing on a recorded edge, an unrelated flat portfolio receives its exact 777-atom principal before snapshot capture, and all schedules converge economically under exact custody/account frames and independent position/effective-OI/source-credit/encumbrance/stock oracles. A separate public three-asset prefix now composes a nonzero active-close residual through exact `ResolveMarket` framing and resolved finalization into a genuinely partial receipt while three independent source-claim domains remain live. The new routes exposed and corrected a test-oracle defect: atomic receipt clearing is valid only when the same route payout covers the remaining entitlement. Add identity/all-balance/authority-epoch/recovery/prior-insurance dimensions and deeper public sequences without treating these finite graphs as universal equivalence. |
