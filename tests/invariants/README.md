...
source exposure. PR267 is therefore `Certified`; ten executable quarantines remain.
...
The current source-attribution tranche pins engine `422893fa`, whose final commit adds proof code
...
230 fields across all 49 public instruction variants and the three nested input structs. INV-083
...
all 230 fields across 52 public input types map to exactly one of 20 semantic profiles, one
...
sources: 147 direct harnesses plus 36 macro-generated trade-decoder harnesses, exactly matching the
183 reported by `cargo kani list`. Every direct harness is classified into 83 symbolic-total, 26
...
| Focused INV-083 field-complete boundary inventory | 10/10 CU plus 183/183 wrapper Kani composition | all 230 fields across 52 public input types are source-locked into 20 semantic boundary profiles with per-field and profile-level executable evidence; the class roster retains all eleven required classes, and 25 public invalid `InitMarket` partitions reject with pristine-account rollback before a valid retry creates a usable portfolio |
| Focused INV-084 proof-harness nonvacuity inventory | 3/3 CU plus 13/13 assumption-focused Kani and 183/183 wrapper Kani composition | a source parser derives all 147 direct and 36 generated harnesses and locks 83 symbolic-total, 26 branch-witnessed, 10 explicitly constrained, 28 concrete-exact, and 36 generated-symbolic dispositions; all 13 explicit assumptions across 21 modules retain exact ownership and public evidence; public routes reach admitted domains, reject invalid partitions with exact rollback, and terminate with exact custody |
...
| `PERCOLATOR_FUZZ_CASES=2 cargo test --test v16_program_stateful_fuzz` | 208/208 | full exact engine-`422893fa`/SBF-`d6ac2697` rerun completed in 224.61 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets. The suite includes the finding-blind source-domain single-use campaign, which leaves the unfunded claim unchanged, consumes the funded claim and backing together, and requires exact rollback on retry. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
...
| `cargo test --test v16_program_fuzz_regressions` | 103/103 | full exact engine-`422893fa`/SBF-`d6ac2697` rerun completed in 28.04 seconds; includes the deterministic source-domain single-use regression, all other public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 895/895 | full exact engine-`422893fa`/SBF-`d6ac2697` rerun completed in 199.76 seconds; zero behavioral failures across reserve, oracle, Recovery, terminal, lifecycle, and maximum-shape routes. The observed required exits remain below the 1.4M CU ceiling. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse -j 2` | 183/183 | full wrapper-only exact-source rerun with engine `422893fa`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied. Nine solver-hostile decoder relations share one tag-directed adapter over exact production bodies: InitMarket, hybrid oracle, four trade, two base-unit, and lifecycle. |
...
1. Convert the 10 `Quarantined` adapters to `Certified` only after the current pin satisfies their
   positive economic and liveness postconditions. The other 89 entries already have explicit
   executable disposition: 81 fixed-pin certifications and 8 public nonqualifying proofs. Do not
...
| --- | ---: | --- |
| `public_sbf/` | 103 | Deterministic public SBF/LiteSVM regressions, decoder corpora, manifests, and public-trace checks. The retained-intent inventory covers every family, same-transaction duplicates, both insurance entrypoints, and all 16 ordered single/batch CPI/no-CPI trade pairs with exact rollback and economic-state oracles. INV-015 additionally owns the complete persisted byte-domain and alignment boundary; INV-031 owns the fixed-seed cross-domain backing single-use witness. |
...
The executable 99-finding manifest currently contains 81 `Certified`, 10 `Quarantined`, 8
...
live base fee by its signed matcher policy. Those four rows are fixed-pin certifications backed by
deterministic and generated public routes. The no-CPI source-fee debit in PR259 and both retained
backing-provider policy orders in PR339 remain public counterexamples. Matcher mutations now bind the
...
source-local claim/backing single use. The ten remaining quarantines are intentionally still red:
PR259 and PR339 fee/provider consent, and PR293/294/295/296/307/317/325/326 whole-market
incarnation replay.
...
81 are `Certified`, 10 remain `Quarantined`, 8 are `Nonqualifying`, and none are `Missing`. Its
