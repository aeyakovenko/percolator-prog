...
`7513b6cd955a0ebf0f5164c437cdc806aa255d0c1ef72f6109959c81492a15d5`. Resolved-mode
...
theorem. The exact current artifact passes 812/812 CU tests; the full Kani result is recorded below.
...
INV-045 now has a second maximum-shape whole-route composition. All 14 assets are configured with
...
atoms per asset, and all `189_476` movement-fee atoms remain nonwithdrawable protocol stock. The
same funded cohort then crosses `ResolveMarket`; bounded public resolved continuations pay every
non-fee atom and leave engine vault accounting equal to SPL custody. Observed peaks are `958_147`
CU for the value-bearing batch CPI and `665_642` CU for a maximum-shape resolved close. This closes
one distinct Hybrid/CPI/Resolved maximum-shape cell without changing production code. It does not
close the remaining Recovery and route/lifecycle maximum-shape cross-product.
...
`7513b6cd955a0ebf0f5164c437cdc806aa255d0c1ef72f6109959c81492a15d5`.
...
3. Extend INV-045 beyond its fixed 80-cell boundary matrix, generated interior worlds, complete
   ...
   maximum shape, and stale-Hybrid/batch-CPI/Resolved maximum shape. Cross the remaining Recovery
   and route/lifecycle maximum-shape cells while retaining exact fee attribution, terminal supply,
   owner-exit, and CU oracles. Whole-domain arithmetic composition remains behind the deployed
