...
| INV-009 | P + SVM/CU + M | `cu/inv_009_partial_fill_and_retry_accounting.rs` and `kani/inv_009_partial_fill_and_retry_accounting.rs` prove deployed single-CPI partial-fill accounting, 12 repeated partitions, the complete 16-pair half-fill matrix, 32 signed integral/non-integral ratio worlds, and 12 maximum-domain worlds. Quantity, OI, fees, epochs, rollback, custody, CU, and conservative rounding are checked independently. A successful single-CPI partial consumes the whole one-shot authorization; an owner must sign a new residual request against the advanced episodes. Batch CPI is exact-fill-only and rejects uniform or asymmetric partials atomically. A source-complete composition locks those semantics and the aggregate batch fee/slippage fields. Persistent remaining authorization or durable-nonce economic consent reopens this row. |
...
| INV-011 | P + SVM/CU + M | `cu/inv_011_signed_aggregate_economic_bounds.rs` source-composes every trade request and both shared executors. A two-leg public batch-CPI route rejects exact-minus-one aggregate adverse-slippage and taker-fee caps with complete matcher/wrapper rollback, accepts both exact limits, and installs exactly the signed mixed-direction position/OI vector. Four full-width Kani harnesses prove checked adverse-slippage accumulation and exact preservation of both aggregate wire fields. Single-leg requests are intrinsically aggregate; exact-fill batches make the signed leg vector the aggregate quantity and final-position bound. The signed transaction recent blockhash supplies the deadline under INV-006. Adding persistent partial fills, durable-nonce economic intents, or collateral/PnL-credit spending to trade reopens this row. |
