...
| `PERCOLATOR_FUZZ_CASES=2 cargo test --test v16_program_stateful_fuzz` | 208/208 | full exact engine-`422893fa`/SBF-`4314e6b0` rerun completed in 225.80 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets. The suite includes the finding-blind 21-family retained asset-generation matrix with nonvacuous backing earnings in both generations, source-domain single-use, retained source-fee consent, and backing-provider landing-order campaigns. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
...
| `cargo test --test v16_program_fuzz_regressions` | 106/106 | full exact engine-`422893fa`/SBF-`4314e6b0` rerun completed in 28.17 seconds; includes the nonvacuous same-world backing-earnings generation replay, deterministic source-domain single-use, retained source-fee cap, and both backing-provider consent landing-order certifications, plus all other public regressions, trace consumers, executable registries, and the source-complete INV-079 evidence roster |
| `cargo test --test v16_cu` | 895/895 | full exact engine-`422893fa`/SBF-`4314e6b0` rerun completed in 209.95 seconds; zero behavioral failures across reserve, fee-consent, oracle, Recovery, terminal, lifecycle, and maximum-shape routes. The observed required exits remain below the 1.4M CU ceiling. |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 187/187 | full wrapper-only exact-source rerun on commit `68254277` with engine `422893fa`; zero failures after all decoder, identity-binding, assumption-nonvacuity, provider-parser, and deployed-arithmetic harnesses, with required constructive covers satisfied. The current proofs establish the exact five-lane funded-provider policy predicate, reject the old intentless top-up schema, and preserve both signed provider terms through the exact 55-byte production decoder. |
