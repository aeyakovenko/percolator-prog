...
`7be350fd684161a228b098cd73c68596a17f259beeb2a94409ccaeaba19a8306`. Resolved-mode
...
An orthogonal 18-world SBF matrix carries six two/three-leg formulas through the same three
lifecycle modes. The formulas place every provider in numerator and denominator roles and cover
both two-leg operations plus all four three-leg multiply/divide shapes. Every world first rejects a
malformed selected provider with exact market, portfolio, keeper, and vault rollback, accepts all
legs at the exact 60-second freshness boundary, rejects them one second later, and accepts a fresh
coherent retry. DrainOnly and Recovery reconcile owner withdrawals plus remaining vault exactly;
Resolved reconciles payouts plus remaining vault exactly. Oracle ingestion peaks at `50,738` CU.
This is representative role/formula composition, not every provider-order/invert/scale/lifecycle
permutation, every byte string, or the solver-bound relational wide-product theorem. The exact
current artifact passes 811/811 CU tests; the full Kani result is recorded below.

The parser-proof decomposition then simplified the production Switchboard reader into two typed
seams: a bounded selected-submission timestamp lookup and one observation validator. Fourteen new
assumption-free Kani harnesses compose canonical Pyth and Chainlink bytes through symbolic
...
for all three providers. The complete wrapper roster passes 158/158. The full monolithic
3,208-byte Switchboard byte-to-typed query remains beyond the current solver's state-size budget,
and arbitrary relational wide scale division remains behind the arithmetic wall. Those gaps are
backstopped by the independent host models and public parser/lifecycle corpora above, but are not
claimed as formal closure.
...
| --- | ---: | --- |
| Focused INV-020 authenticated timestamp, parser, composite epoch, and lifecycle matrix | 1/1 direct public, 2/2 stateful, 21/21 Kani, 58/58 CU/host, plus the complete 811-test CU gate | full-width timestamp/freshness/owner/key/confidence-totality predicates plus all-provider short-data rejection; canonical Pyth/Chainlink bytes compose symbolic fields through production, while the complete Switchboard selected-timestamp table and typed validator are separately proven without assumptions; concrete scale boundaries cover floor/min/max/overmax/overflow; 7,183 provider words prove the thin `AccountInfo` entrypoint delegates exactly to the pure production parser; independent references cover 726 boundary words, 15,552 structural/semantic combinations, 12,288 seeded full-width layouts, the exact non-saturating elapsed-time boundary, and 1,310,720 wide confidence comparisons; 126 provider/transform configurations reject 114 selected-leg skews and 126 coherent rewinds; 39 provider words cross freshness ages 59/60/61; three provider-role worlds reach a genuine coherent liquidation then exact effective exit; one composite shutdown/force-close/restart world clears old provenance; nine single-provider and eighteen multi-provider lifecycle worlds preserve bounded exits, exact rollback, and custody through DrainOnly, Recovery/restart, and Resolved settlement |
...
| `cargo test --test v16_program_stateful_fuzz` | 195/195 | exact engine-`592d538c`/SBF-`7be350fd` rerun in 174.60 seconds, including the public composite-epoch rejection/ignore/coherent-control matrix, Hybrid terminal payout, three-asset active-close-to-partial-receipt-to-terminal bridge, both active-close barrier orientations, the 32-world cross-asset active-close admission matrix, the inverse 40-world direct/prior-leg terminal-liability matrix, and paired 32-world active-close/ResetPending and active-close/Recovery-reset rank compositions across every trade route, plus every shared rank/reference-model check |
| Registry/manifest checks in the INV-079 module | 9/9 | rerun on the 2026-08-21 PR135 exact-pin test head |
| `cargo test --test v16_program_fuzz_regressions` | 93/93 | full engine-`592d538c`/SBF-`7be350fd` rerun in 26.81 seconds; the sealed benchmark census and invariant registries remain green |
| `cargo test --test v16_cu` | 811/811 | full engine-`592d538c`/SBF-`7be350fd` exact-source rerun in 191.14 seconds, including all maximum-source and maximum-shape exits, exact maximum cross-zero quantity boundaries, all-route DrainOnly/Recovery admission, post-ADL effective conversion, liquidation/resolved close, exhaustive account-role matrices, the 7,183-word parser-delegation corpus, 726 independently modeled parser boundaries, 15,552 structural combinations, 12,288 seeded full-width layouts, the exact non-saturating elapsed-time boundary, 1,310,720 wide confidence comparisons, all-provider composite epoch/rewind/freshness boundaries, composite liquidation and shutdown/restart composition, nine single-provider and eighteen multi-provider lifecycle worlds, the dense zero-delta terminal-shape benchmark, and every cross-invariant progress/rollback audit |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 158/158 | full wrapper-only exact-pin rerun on engine `592d538c`; zero harness failures, including 14 new assumption-free provider-parser decomposition harnesses for canonical Pyth/Chainlink bytes, the complete Switchboard timestamp table and typed validator, invalid-domain partitions, confidence routing, and concrete scale boundaries, plus all prior decoder, arithmetic, rollback, and nonvacuity proofs |
...
`7be350fd684161a228b098cd73c68596a17f259beeb2a94409ccaeaba19a8306`.
...
2. Finish the remaining INV-020 composition frontier. Pyth and Chainlink canonical-byte fields and
   Switchboard's selected-table/typed-validation seams are now formally decomposed; do not retry
   the intractable monolithic 3,208-byte Switchboard query unchanged. Instead add a mechanically
   checked byte-offset-to-typed-field equivalence layer, then compose that layer with the proven
   validator. Extend the finite runtime matrix to invert/unit-scale lifecycle pairs only where
   they add behavior beyond the existing 126-case transform matrix. The all-symbolic relational
   wide-scale division theorem remains solver-bound and requires a named arithmetic equivalence
   assumption or differential boundary corpus, not a claim that finite generation covers every
   provider byte word.
...
| AUDIT-020 | OPEN-T | Issue 405's account-write/selected-result timestamp split is closed on configuration and crank routes. The composite cross-epoch LoF is also fixed: one-leg-fresh and all-fresh-but-different-epoch reports cannot mutate composite provenance or user value, coherent controls remain live through exit and terminal payout, and Kani exhausts the exact production equality predicate over full-width timestamps. Public configuration/crank tests cross all Pyth/Switchboard/Chainlink one/two/three-leg orders, every legal composition transform, coherent stored-time rewind, and freshness ages 59/60/61. Orthogonal public compositions cover ordinary trade exit, real liquidation and reward, shutdown, force close, restart, and terminal resolution with exact rollback and liveness. Nine single-provider and eighteen multi-provider worlds cross DrainOnly/Recovery/Resolved; the latter cover every provider in numerator/denominator roles, all legal multiply/divide shapes, exact expiry, malformed selected-account rollback, and exact terminal value reconciliation. The `AccountInfo` boundary delegates by construction to one pure production parser; 7,183 host corpus words compare both paths exactly. Independent models agree on 726 boundary words, 15,552 structural/semantic combinations, 12,288 seeded full-width layouts, an exact non-saturating elapsed-time boundary, and 1,310,720 all-BPS wide confidence cases. Fourteen assumption-free Kani harnesses now compose canonical Pyth/Chainlink byte fields, the full Switchboard selected-time table and typed validator, invalid-domain partitions, confidence routing, and concrete scale boundaries. Remaining closure is narrower: mechanically prove Switchboard byte offsets populate the typed observation exactly, add nonredundant invert/unit-scale lifecycle pairs, and discharge or explicitly axiomatize relational wide-scale division; the unchanged monolithic 3,208-byte query remains a documented solver/state-size frontier. |
