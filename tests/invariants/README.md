...
terminal-generation classification, and source-locks all 29 finding-blind `is_violation` oracles to
an explicit evidence class. Nineteen oracles now carry classifier-bound public LoF evidence. In
...
SPL, and program-lamport rollback. A recursive source-complete guard requires all 53 current
...
remaining finding-blind public counterexample runner. A source-complete roster now covers all 29
...
exact affected-signer loss plus caller-fee gain. Paired terminal trade retries and the exact
activation-fee debit raise the classifier-bound set to 19, while a source-complete disposition table
prevents the six non-LoF retry families, including recoverable insurance principal, from inheriting
that label. The other 10 retain their mechanically enforced
...
| `cargo test --test v16_program_stateful_fuzz` | 209/209 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 253.22 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets, including generated route selection for all four terminal retained-trade retry worlds and both exact external-debit retry families, the complete pending-obligation and close-ledger encumbrance census, direct per-domain `usable_positive_credit_num <= available_backing_num`, exact/bound claim transitions, credit-rate edge causality, and the independent resolved-payout quotient/remainder oracle. The corrected INV-030 matrix additionally crosses all four trade routes and both source sides while proving one-for-one face settlement, zero impaired-domain credit, exact rollback of favorable admission, and live bilateral plus owner-only reduction. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
| Registry/manifest checks in the INV-079 module | 13/13 | includes the source-complete 29-oracle evidence roster, nineteen exact terminal-LoF dispositions, all 11 retained-retry dispositions, complete-route terminal classification, and the recursive 53-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 104/104 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 28.57 seconds; includes the generic 11-operation whole-market no-reuse matrix, deterministic terminal retained-trade and exact external-debit retry worlds, the source-complete 11-kind retry disposition census, the source-complete account-incarnation and detached-signature censuses, all public regressions, 53 trace consumers, executable registries, and the 29-oracle INV-079 evidence roster |
...
| AUDIT-079 | OPEN-T | The LiteSVM trace schema records actual transaction signers, compiled account metas, exact authority-attributed tracked token/lamport deltas, rejected writable-account rollback with the fee-payer network charge separated from program effects, and between-transaction economic mutation. The shared validator requires an allowlisted public construction sequence containing a real wrapper call and rejects malformed success, rollback, signer, account, payload, program, CU, token-owner, vault-participation, and quote-balance evidence. A recursive scan source-locks all 53 current trace consumers to validate or classify immediately. The normalized terminal classifier distinguishes exact loss/unauthorized gain, persistent funded lock, bounded exit, atomic rejection, and progress; real public exit/rejection witnesses plus negative anti-overclaim controls pass. Required, attempted, and progressing route masks prevent incomplete searches or successful no-op continuations from being labeled persistent DoS. Nineteen of 29 finding-blind violation oracles carry classifier-bound exact LoF evidence, including all eight signed fee-consent families, four retained trade transports, the exact nonrecoverable activation-fee retry, fractional settlement with a complete progressing-route observation, and stale pending-mark admission and inheritance matrices; the paid mark-reserve and paired pending-target oracles separately carry exact terminal evidence while retaining narrower labels for local rollback branches. All 11 retained-retry kinds have source-complete terminal or explicit non-LoF dispositions, including recoverable insurance principal. The enforced roster limits the remaining replay, semantic, privileged, or local discoveries to their actual claims. Attach independently measured terminal observations and complete exit-route attempts to every remaining qualifying finding-blind counterexample before promotion. |
