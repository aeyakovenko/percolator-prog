...
more than two assets, and close compositions with Recovery, receipts, or multiple simultaneous
lifecycle classes remain in the wider bounded reachability model. A 32-world public matrix now
owns the two-class active-close plus `ResetPending` cell in both landing orders and proves close
priority, reset finalization, order-independent terminal economics, and every funded exit.
INV-028/071/073/082
...
| Focused INV-071/074/082 close-plus-ResetPending composition | 32/32 public worlds | all four trade routes, both close orientations, both reset orientations, and both transition landing orders create simultaneous independent close/reset work; the first auto-crank strictly reduces the higher-priority close while framing the reset asset, bounded continuations clear and finalize reset work, both orders converge to identical owner-level economics, and every funded owner exits. The independent rank excludes permanent `bankruptcy_hlock_active` audit history after concrete work reaches its fixed point |
...
| `cargo test --test v16_program_stateful_fuzz` | 194/194 | exact engine-`592d538c`/SBF-`909c24cc` rerun in 174.58 seconds, including both active-close barrier orientations, the 32-world cross-asset active-close admission matrix, the inverse 40-world direct/prior-leg terminal-liability matrix, and the 32-world active-close/ResetPending rank composition across every trade route, the independent raw-basis/effective-OI ledger, sub-minimum-ADL and cross-domain fractional-source progress, pending-target/funding/withdrawal composition, clock-first mark discovery, source-lien partition/expiry/exit, concurrent partial receipts, disjoint- and shared-portfolio lifecycle-local reset/shutdown ordering, retained-reduction/shutdown landing orders, simultaneous two-asset lifecycle ordering, terminal-insurance withdrawal, resolved-claim and liquidation partitions, Recovery K/F refresh, close drift, exact rollback, and every shared rank/reference-model check |
