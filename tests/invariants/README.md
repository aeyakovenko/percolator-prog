...
| INV-061 | Independent + P(engine) + SVM/CU | `stateful/inv_061_deterministic_bounded_liquidation.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `cu/inv_061_deterministic_bounded_liquidation.rs`, `cu/inv_059_fee_fragmentation_bound.rs`, and the issue-408 liquidation world in `cu/inv_027_protected_principal_seniority.rs`. Two public post-ADL prefixes prove pre/post-mark transfer extraction rejects with exact rollback, cannot consume extra backing or create phantom value, and retain bounded owner reduction whose raw-basis result is independently derived from remaining effective OI. Four public dual-ADL transport worlds independently reconstruct the exact liquidation maintenance, target/effective lag, fee/minimum-fee, floor, projected-health, and binary-search selector and require its exact partial close, OI deltas, effective remainder, no-SPL frame, and CU bound. The resolved-ADL matrix proves both terminal landing orders consume only remaining effective OI, pay both funded users exactly, reconcile custody to zero, and close both portfolios. The fractional reset-carry matrix publicly creates two bankruptcies, requires the sole account crank to strictly liquidate its target under the CU cap, then gives every remaining owner a stale raw-basis work budget. Each budget clamps to exact effective OI; all legs and both resets clear in bounded public work. The trace explicitly refills the resulting 11-atom provider receivable, withdraws all restored provider principal, and proves retirement clears only historical spent/source/social/K/F audit state without changing custody. A 32-world two-asset/three-user matrix crosses all four opening transports, both equal-risk target-leg orders, both accrual orders, and same-versus-next-selected-asset follow-up plus a third same-selected-asset episode. Every world creates two later authenticated deficits and requires exact closes, fees, selected-domain budget credits, nonselected frames, and no-SPL frames for both; next-asset worlds first require exact canonical residual removal and a bounded authenticated time gap, then repeat that selected leg at a fresh bounded slot. The 48-world successor crosses three unequal losses, all six persisted leg orders, and both accrual orders; every crank closes the independently modeled first live leg, frames all nonselected value/loss domains, stays below 312,341 CU, and converges to exact order-independent terminal payouts. A four-route underfunded bridge now proves one 315,258-CU crank removes exactly 70,000,000 matched OI, finalizes the exact 2,723-atom close, and composes through a 1,000-face partial receipt, a later value-moving payout, and terminal custody. A separate all-route campaign proves two fee-bearing liquidations require two authenticated deficits, same-state/malformed retries cannot manufacture an episode, and owner reduction remains live after episode two. Arbitrary close splitting, four-plus episodes, larger account partitions, and remaining maximum-shape cross-products remain. |
...
| AUDIT-061 | OPEN-T | Liquidation safety, fees, progress, and selected generated schedules are covered. The PR250 post-ADL transfer/extraction and phantom-value prefixes reject exactly and retain bounded owner reduction. Four finding-blind dual-ADL route worlds independently reproduce maintenance, target/effective lag, fee/minimum-fee, floor, projected-health, and binary-search sizing from the authenticated certificate and require the selected crank's exact partial close, two-sided OI delta, no-SPL frame, and CU bound. The resolved-ADL close-order violation is fixed on `6c04db7e`: both landing orders advance through effective-OI reduction and reset cleanup to exact funded exits. The independent fractional reset-carry violation terminates on `573c4e90`: the sole public crank strictly liquidates the target below its CU cap, clears eight affected legs and both resets, settles every claim/provider obligation, and retires the asset through one canonical terminal transition. Carry normalization and terminal frames are engine-contract checked. Thirty-two equal-risk public worlds cross all opening transports, target-leg orders, accrual orders, and same-versus-next-selected-asset follow-up plus a third same-selected-asset episode in a two-asset/three-user ADL topology. The next-asset branch requires exact canonical residual removal and value/counterparty/SPL frames before a bounded authenticated time gap; both branches require exact fee-bearing second and third liquidations with selected-domain attribution, nonselected frames, and fresh bounded authenticated slots. A 48-world successor crosses three unequal losses, all persisted leg orders, and both accrual orders while framing every nonselected value/loss domain. A four-route underfunded bridge removes exactly 70,000,000 matched OI, preserves the resulting 2,723-atom finalized close through resolution, and composes it into a genuine partial receipt, a value-moving follow-up payout, and terminal custody. A second four-route world proves cross-asset unattributed loss takes the direct bounded-liquidation path rather than creating a conflicting close rank. The INV-059 all-route campaign now composes two fee-bearing liquidations across separate authenticated deficits, exact same-state/malformed rollback, route-identical economics, and a post-episode owner reduction. Caller-sized liquidation is absent and source-locked; add four-plus episodes, larger account partitions, and the remaining maximum-shape cross-products. |
