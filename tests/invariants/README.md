...
depth three and records 551 exact authenticated tracked wrapper states at that frontier. Each key
...
adding 7,163 depth-four words and 28,652 public transitions. All 9,543 words and 35,594 base-graph
...
owner-exit campaign that moves nonzero funded SPL value. This closes the first seeded lifecycle
frontier; active-close, lien-impairment, receipt, and oracle-failure seeds remain. No production
source or persisted state changed.
...
The current SBF is 1,255,960 bytes with SHA-256
`d70956aa6559f9e3cdd5dde9553866227cea53b8df560e8d2da30b7f4252d837`. The last unchanged broad
run on the exact pin passed
...
The Recovery extension adds a 233rd stateful/model test; that focused test passes 1/1 in 27.56
seconds. The explicit-B extension adds a 234th test; it passes 1/1 in 43.79 seconds. Its shared
liveness-oracle change directly affects the bounded crank graph and Recovery frontier, which pass
1/1 in 8.17 seconds and 1/1 in 27.56 seconds respectively. The other unchanged tests were not
redundantly rerun for this proof-only addition.
...
it exhausts 2,380 words through depth three, then extends all 551 exact authenticated tracked wrapper
states with every public graph action for 9,543 total words, plus twelve underfunded terminal schedules.
...
shared deployed graph now applies an equivalent independent edge oracle to all 35,595 bounded live and
...
exact scaled-liquidation worlds, sixteen dual-ADL Recovery-forfeit worlds, sixteen multi-asset
...
oracle-discovery hints and INV-059 fails closed if a sized liquidation route appears. Multi-asset
or larger-account liquidation partitions, repeated liquidation episodes across changing
authenticated state, and the remaining maximum-shape cross-products are the next tractable
whole-route products. Retirement already has a separate public liquidation composition, but the
...
from every result. Extend the same public transition method across active-close, explicit
B-settlement, receipt, lien-impairment, and oracle-failure seeds; require every funded nonterminal
node to expose a constructible bounded rank-decreasing or terminal action; and replay each witness
through the deployed SBF at maximum relevant shape. The
...
| Focused INV-029 exact-only deployed profile | 232 prior stateful public tests plus 1/1 focused Recovery frontier, 9,543 bounded public words/35,595 transitions, twelve terminal schedules, and 1/1 source-complete composition | after every generated or bounded successful public transition, each domain's exact claim equals its positive bound and complete portfolio attribution; in Live mode, each portfolio's aggregate source-claim face also equals its exact positive-PnL face. The graph exhausts depth three, then applies every action to all 551 exact authenticated tracked wrapper states at that frontier; each key includes byte-identical tracked account/balance state and authenticated Clock. The new Recovery frontier adds 366 fresh/exact-expiry worlds and 702 checked transitions, each with the same source-credit and claim oracles. It must traverse nonzero claim-changing edges and every partial receipt must have an observed exact bound replacement. The wrapper exposes no non-exact bound or rebucketing ingress, so approximate-bucket range/rebucketing proofs remain N/A unless that optional profile is introduced |
| Focused INV-030 source-credit transition causality and malformed state | 232 prior stateful public tests plus 1/1 focused Recovery frontier, 9,543 bounded public words/35,595 transitions, and 4/4 focused CU | every generated public action, successful permissionless crank, and bounded live/terminal edge enforces the independent transition relation: unchanged formula inputs preserve the rate, every formula-input mutation advances `credit_epoch`, and rate improvements require more backing or a smaller claim. The reduced depth-four frontier extends every one of 551 exact authenticated tracked depth-three wrapper states by all 13 actions. The separate Recovery frontier applies the same transition oracle to 702 edges from fresh and exact-expiry backing seeds while requiring a funded bounded exit after every word. The graphs exercise 93+ input mutations, both rate directions, claim-reduction recovery, and a dedicated public post-claim backing recovery edge. Twenty malformed relation cases and two exact source-boundary truncations reject with complete economic rollback; a source-complete gate binds the pin-owned writer lock and INV-088 transition roster |
...
| Focused INV-086 terminal reference and ADL-clamp composition | 12/12 public terminal worlds, 32/32 all-route ADL owner-reduction worlds, 32/32 one-sided-ADL Recovery force-close worlds, one post-claim backing recovery edge, one active-close-to-partial-receipt-to-terminal bridge, 9,543 bounded public words, plus 366 Recovery words/702 transitions | every seed is rebuilt through public top-up, trade, mark, crank, resolve, and close instructions. The base graph exhausts all 2,380 words through depth three, preserves one complete prefix for each of 551 exact authenticated tracked wrapper states, and applies all 13 actions to every such state. All 35,594 base-graph edges plus the recovery edge run the independent state/rollback oracles; every action produces real third- and fourth-position economic-state transitions, and both frontiers add normalized states and edges. The separate Recovery graph starts from public committed nonflat positions with funded backing and insurance, crosses fresh versus exact authenticated backing expiry, exhausts every one- and two-action order, and requires a nonzero value-moving bounded owner exit from every reached state. Recovery rebalance is retained as an exact-rollback admission control rather than mislabeled as progress. All claim-priority worlds move real SPL value, exact/late expiry normalize on a recorded edge, route/claimant orders converge under independent position/OI/source-credit/encumbrance/stock/custody oracles, and the separate three-asset bridge preserves an active value-bearing close through resolution, finalizes it into a genuine partial receipt, moves additional SPL value, and terminates all five funded portfolios. Both ADL matrices cross all four opening transports and four reduction boundaries. Owner reduction crosses both terminal orders; force-close crosses both account orders. Their pre-state oracle derives the exact clamp and requires two-sided OI agreement, one-atom remainder fidelity, no reduction-time SPL movement, and exact terminal payouts. This remains finite reachability evidence, not universal equivalence. |
...
| `cargo check --tests` | pass | exact engine-`6f3c5c12` build completed at the current checkpoint |
| `cargo test --lib` | 7/7 | exact engine-`6f3c5c12` rerun at the current checkpoint |
| `cargo test --test v16_program_stateful_fuzz` | 231 unchanged tests from the prior full pass; changed graph 1/1 focused | the full-Clock authenticated-state graph was rerun alone against exact engine-`6f3c5c12`/SBF-`d70956aa` and passed in 599.28 seconds; all 231 unrelated tests were filtered out. It covers 9,543 public words and 35,594 base-graph edges. The prior full pass covers the unchanged 64-world aggregate/two-way/three-way/four-way receipt route matrix, 24-world unequal three-receipt and 120-world unequal four-receipt claimant/release products, second 24-world prior-insurance composition through gated drain and slab closure, 16-world partial-receipt release/claim matrix, four-world independent-Recovery/receipt product, four-route terminal-expiry branch, and all prior authority, policy, liquidation, and payout campaigns. |
...
8. The base graph now extends every exact authenticated tracked depth-three wrapper state by every
   action. Recovery and explicit side-local B seeds each exhaust all 366 empty/one/two-action words
   and 702 transitions, and every result retains a constructible value-moving owner exit. The B
   frontier additionally crosses both side orientations, both honest hint shapes, owner reduction,
   matcher disable, mark movement, shutdown rollback, and permissionless stale resolution. Next add
   public-prefix seeds for active close, lien impairment, receipt conflict, and oracle failure, then
   extend those frontiers with the same exact-state reduction. Keep the graph explicitly
...
| INV-061 | Independent + P(engine) + SVM/CU | `stateful/inv_061_deterministic_bounded_liquidation.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `cu/inv_061_deterministic_bounded_liquidation.rs`, and the issue-408 liquidation world in `cu/inv_027_protected_principal_seniority.rs`. Two public post-ADL prefixes prove pre/post-mark transfer extraction rejects with exact rollback, cannot consume extra backing or create phantom value, and retain bounded owner reduction whose raw-basis result is independently derived from remaining effective OI. Four public dual-ADL transport worlds independently reconstruct the exact liquidation maintenance, target/effective lag, fee/minimum-fee, floor, projected-health, and binary-search selector and require its exact partial close, OI deltas, effective remainder, no-SPL frame, and CU bound. The resolved-ADL matrix proves both terminal landing orders consume only remaining effective OI, pay both funded users exactly, reconcile custody to zero, and close both portfolios. The fractional reset-carry matrix publicly creates two bankruptcies, requires the sole account crank to strictly liquidate its target under the CU cap, then gives every remaining owner a stale raw-basis work budget. Each budget clamps to exact effective OI; all legs and both resets clear in bounded public work. The trace explicitly refills the resulting 11-atom provider receivable, withdraws all restored provider principal, and proves retirement clears only historical spent/source/social/K/F audit state without changing custody. A sixteen-world two-asset/three-user matrix crosses all four opening transports, both equal-risk target-leg orders, and both accrual orders. The 48-world successor crosses three unequal losses, all six persisted leg orders, and both accrual orders; every crank closes the independently modeled first live leg, frames all nonselected value/loss domains, stays below 312,341 CU, and converges to exact order-independent terminal payouts. A four-route underfunded bridge now proves one 315,258-CU crank removes exactly 70,000,000 matched OI, finalizes the exact 2,723-atom close, and composes through a 1,000-face partial receipt, a later value-moving payout, and terminal custody. Arbitrary close splitting and remaining maximum-shape cross-products remain. |
...
| INV-066 | SVM/CU + M + Partial R | `stateful/inv_066_resolved_payout_fairness_and_order_independence.rs` and `cu/inv_066_resolved_payout_fairness_and_order_independence.rs`. All 5! basic claimant orders complete the same two-asset lifecycle with identical payouts. A separate underfunded public model creates one immutable partial receipt and two independently expiring backing domains, then exhausts all 16 choices of landing the claim route before and/or after each release. The route is classified from observed state as exact payout, zero-payout cleanup progress, or exact no-op; every value-moving scheduled claim additionally matches an independent long-division quotient/remainder oracle with nonzero-remainder coverage and a stable pre-existing payout rate. A third three-asset model preserves both claim-producing pairs while an independent pair enters Recovery, then crosses both fair request orders with claim-before/claim-after backing release. Larger public models create unequal `350/650/1000` and `150/350/550/950` receipt sets. They exhaust all 24 three-receipt and 120 four-receipt claimant/release orders with nonzero full-width residue and identical claimant-local payouts; the adjacent 64-world route matrix bounds every one/two/three/four-way partition by N-1 floor atoms. A second 24-world product carries 37 atoms of prior insurance through the three-receipt schedules: claimant entitlements remain identical to the uninsured control, all eight premature withdrawal prefixes reject with exact rollback, the reserve drains exactly only after all portfolios dematerialize, and bounded exact-expiry cleanup reaches the slab tombstone. Attempted authority refinement is closed by surface exclusion: commit `392a3768` removed that public instruction, while the decoder Kani proof and exact-SVM raw-tag regression require tag 47 to reject before mutation. Unbounded claimant-count induction remains; adding factorial tests above four is not a substitute for that theorem. |
| INV-067 | Independent + Direct + P(engine) + F + SVM/CU + Partial R | `public_sbf/inv_067_terminal_payout_completeness_and_exact_once_settlement.rs`, `stateful/inv_066_resolved_payout_fairness_and_order_independence.rs`, `stateful/inv_067_terminal_payout_completeness_and_exact_once_settlement.rs`, `stateful/inv_071_crank_progress.rs`, `stateful/inv_081_success_state_validity_over_complete_public_routes.rs`, and `cu/inv_067_terminal_payout_completeness_and_exact_once_settlement.rs`. The finding-blind four-route matrix varies a flattened one-quantum source-backed position and now proves the unrelated victim receives its full claim, the sole terminal residue equals the coalition's one-atom rounding loss, SPL supply is unchanged, and every close/claim retry is quiescent. Engine `4c4dfb20` retains a bounded first-source terminal haircut in existing `reserved_pnl`, excludes it from later source realization, scans past occupied zero-claim entries, and clears it into the final receipt; the full-width production-kernel contract proves the source-face partition. Both terminal payout routes are retried at the fixed point after each claimant in all 5! basic orders. The 16-schedule partial-receipt model places claims on both sides of two real backing-release frontiers; every paying edge equals the receipt and SPL-vault delta and independently recomputed rational entitlement, every zero-payout edge is strict cleanup progress or an exact no-op, cumulative payment never exceeds immutable face, and all schedules terminate with the same actual haircut entitlement and external balances. A separate 24-world matrix reaches resolution through a publicly booked bankruptcy obligation and varies all four trade routes, three claimant orders, and both payout-route priorities with exact per-owner payout equivalence. The shared runner resolves a funded stale market permissionlessly and requires every portfolio to reach the terminal fixed point. The public Recovery-order matrix on `e914dbcf` additionally proves a first exit cannot erase loss weight or strand a later claimant: both landing orders converge to identical exact payouts, provider attribution, social loss, and terminal custody after bounded permissionless cleanup. The four-route liquidation bridge adds a real finalized 2,723-atom close before resolution, then requires a genuine 1,000-face/125-paid receipt, a 126-atom follow-up payout, and terminal disposition for all five actors with exact engine/SPL custody. Its insurance-bearing counterpart spends the exact 123-atom source-domain budget before resolution and still reaches an exact 1,125-face/198-paid receipt, a 176-atom follow-up payout, and all five terminal actors. |
...
| INV-079 | Direct + Static rosters + Partial R | `public_sbf/inv_079_public_reachability_evidence.rs`, `public_sbf/inv_007_no_aba_reuse.rs` enforce the finding manifest and production/method rosters, mutation-test the public trace recorder, and replay all 11 whole-market ABA request classes with actual transaction signers, compiled account metas, exact token/lamport deltas, mint-supply deltas, rejected-call rollback with the network fee classified separately, and zero out-of-band economic mutation. The shared validator rejects malformed provenance, checked quote imbalance, wrong token ownership, and malformed vault participation; a recursive source guard requires all 59 current trace consumers to validate or classify immediately. A normalized terminal classifier has real bounded-exit and atomic-rejection witnesses plus anti-overclaim controls for LoF and persistent DoS. Persistent-lock evidence requires complete required/attempted masks with zero progressing bits, so successful no-op routes cannot masquerade as liveness. The flat-source-lien and cross-domain rounding campaigns carry route-level progress evidence; authority-epoch terminal replay, authority-incarnation backing-principal extraction, terminal-generation replay, terminal dust, prospective-accrual ordering, accrual/removal ordering, pending-mark resolve ordering, zero-move terminal funding ordering, shutdown-funding ordering, retained source-fee consent, signed fee consent across all eight fee families, retained trade retries, direct external-debit retries, bilateral mark-fee extraction, composite-oracle rounding, trade-driven liquidation, fractional target settlement, stale pending-mark admission, retained pending-mark inheritance, revoked matcher consent, all five oracle supersession families, and fee-redirection supersession bind exact public victim loss to the classifier. INV-031 separately certifies cross-domain backing single use with zero loss. The fractional campaign also records progressing crank and stale-resolution routes, distinguishing settlement LoF from persistent DoS. The paid mark-reserve campaign proves exact terminal coalition nonextraction, and the pending-target override campaign compares both landing orders through classifier-bound terminal payouts, without promoting their local rollback branches to LoF. A source-complete 32-oracle roster prevents the remaining 10 replay, semantic, privileged, or local discoveries from being promoted to terminal LoF/DoS evidence. All 11 retry kinds and all 14 same-incarnation supersession kinds have explicit dispositions; seven supersession controls have terminal LoF evidence, real fee-bearing terminal worlds classify liquidation and maintenance shares as attribution-only, resolve-policy timing has complete finite funded liveness evidence, and no terminal-value or liveness candidate remains. |
...
| INV-086 | Direct + F + Partial M + Partial R | `public_sbf/inv_086_reference_model_and_deployed_transition_equivalence.rs`, `stateful/inv_086_reference_model_and_deployed_transition_equivalence.rs` (the shared runner exercises 27 semantic action classes spanning 25 decoder variants, including authority and permissionless-stale resolution, payout, live insurance/backing withdrawal, owner recovery-forfeit, abandoned-asset force-close, restart, DrainOnly, and retirement routes. It independently checks terminal position/OI/receipt/custody edges, exact withdrawal custody/ledger deltas, strict Recovery and DrainOnly exit transitions, monotonic restart, fresh-generation trade attachment, empty-asset retirement, and terminal convergence after stale resolution. Its permissionless-drain bound is derived from the generated public action history, including mark pushes and every clock-advancing lifecycle action; a persisted minimized long-catch-up trace prevents regressions to the former empty-action bound. The OI ledger derives owner-rebalance matching reduction from the exact public request/effective/both-side clamp rather than substituting a rounded public pre/post delta. Three 32-world deployed matrices cross all four opening transports and four clamp boundaries. Owner reduction crosses both terminal claimant orders; one-sided- and dual-nonunit-ADL Recovery force-close cross both account orders. All independently derive the reduction from authenticated pre-state, check both side OI deltas and remaining effective quantities, move no SPL value during reduction, and return exact funded payouts. A four-world liquidation product independently reconstructs maintenance, target/effective lag, fees, floor, projected health, and the selector's binary search from the pre-liquidation certificate, then requires the exact partial close across every opening transport. A 16-world Recovery-forfeit product crosses both owner orders and one/max B work budgets, requires exact own-side effective-OI retirement and opposite-side framing, drains the retained zero-basis obligation through one bounded public crank, and proves order- and budget-independent Live owner exits with exact maintenance-fee/SPL/custody partition. A second persisted seed proves zero-amount public withdrawal is an admitted economic no-op: it may charge only exact maintenance, pays no SPL value, leaves the vault unchanged, and cannot increment value-withdrawal coverage. The base graph exhausts all 2,380 words through depth three over thirteen value, trade, crank, policy, authority, backing, lifecycle, authority-resolution, and resolved-close actions. It records 551 exact authenticated tracked wrapper states at that frontier and extends each with all thirteen actions, for 9,543 total words and 35,594 checked base-graph edges. Each reduction key includes byte-identical tracked account/balance state and authenticated Clock. Every action changes economic state in both a third and fourth position; both frontiers strictly extend their predecessor's normalized node and edge sets. A second graph independently rebuilds 12 public underfunded partial-receipt worlds across pre/exact/post expiry, two claimant orders, and both close/claim priorities, plus one post-claim backing recovery edge. Every claim-priority world moves SPL value; exact/late worlds record a backing-normalization edge; the recovery edge requires an unchanged discounted claim to gain rate only after new backing; an unrelated flat portfolio receives its exact 777-atom principal before snapshot capture; all terminal schedules converge economically. The normalized node includes per-domain source credit, backing buckets, and insurance reservations in addition to portfolio and payout state. Every edge runs exact custody/account frames and independent position/effective-OI/source-credit/encumbrance/stock oracles against one production SBF hash. The four-route liquidation bridge is paired with a source-domain insurance world: 123 funded atoms are consumed once, B takes only the 2,600 remainder, resolution and a partial receipt preserve attribution, a later payout moves value, and all five portfolios terminate with exact custody. The same public prefix now branches before and at backing expiry and independently proves the exact `751/751/0/0` versus `751/1/123/627` backing/provider/insurance/retirement partitions across all four transports. A dedicated explicit-B graph adds 366 worlds and 702 transitions from exact side-local target/snapshot gaps in both orientations, exercises owner and unrelated cranks, value movement, owner reduction, policy, mark, matcher, lifecycle-rejection, and terminal-fallback orderings, and requires a value-moving bounded owner exit from every world. Larger multi-account/multi-asset ADL, identity, authority epochs, insurance impairment, active-close/lien/receipt/oracle-failure seeds, deeper sequences, maximum-shape composition, and complete lifecycle models remain) |
...
The current ledger is 38 **CLOSED**, 25 **OPEN-T**, 10 **OPEN-D**, 0 **PARTIAL**, 14 **FRONTIER**,
...
4. No general bounded BFS/model checker enumerates the reachable lifecycle graph required by
   ...
   classes before extending every one of 551 exact authenticated tracked states with all thirteen
...
| AUDIT-044 | OPEN-T | A finding-blind public two-asset order oracle independently exposed a 200-atom source-claim burn: the first parent engine returned 3,100 versus 3,300 depending only on permissionless crank order. The expanded four-way oracle then exposed persisted leg-slot order as a second value-bearing input: newly observed gains could erase losses before source reservation. Engine `a6e3c79` burns face one-for-one after support and applies one bounded canonical whole-account K/F plan in negative/nonnegative, source-domain, leg-slot order, with live target/phase/domain validation before mutation. All account-order and leg-slot-order combinations now agree on capital, PnL, source face, certified equity, source stock, withdrawals, terminal payout, and zero terminal custody residue. Three exact Kani proofs own the face equation and canonical plan. Selected B and parked-PnL cases plus cross-owner stock tests also exist. Exercise the remaining A/K/F/B index, certificate, reservation, lien, tag, and soft-credit durable-use paths through public transitions with token/encumbrance balance checks. |
...
| AUDIT-048 | OPEN-T | All four fresh trade routes scan raw OI, and the stateful model keeps an exact independent effective-OI transition ledger across matched/retained trades, crank liquidation, owner rebalance, prior-reset cleanup, and recovery forfeit. Three 32-world public matrices create nonzero ADL through every trade transport and cross `effective - 1`, exact effective, `effective + 1`, and raw basis. Owner reduction crosses both terminal claimant orders; one-sided- and dual-nonunit-ADL Recovery force-close cross both account orders. Each requires the independently derived clamp to match both side OI deltas and recomputed remaining legs before exact terminal value returns. Four exact scaled-liquidation worlds independently reconstruct selector quantity from pre-state and check both OI lanes. Sixteen dual-ADL Recovery-forfeit worlds cross both owner orders and one/max B budgets, frame the opposite OI lane, drain the retained obligation, and converge to the same Live exits. A sixteen-world two-asset/three-user ADL matrix derives both assets' effective OI, observes exact selected-only two-lane reduction, frames the nonselected asset, and drains every residual/reset leg through bounded public exits across all trade transports and landing orders. The 48-world unequal-loss successor verifies selected-only matched-OI removal over three assets and every persisted leg/accrual order. The four-route underfunded bridge removes exactly 70,000,000 matched OI before a partial receipt and terminal custody; the adjacent unattributed-loss world removes a surviving 5,000,000-unit leg without inventing a close domain. A separate 24-world public bankruptcy matrix pins the zero-OI pending-obligation boundary. Transfer/import and caller-sized liquidation are absent from the wrapper; larger partitions/topologies, repeated episodes, and remaining maximum-shape composition remain. |
...
| AUDIT-061 | OPEN-T | Liquidation safety, fees, progress, and selected generated schedules are covered. The PR250 post-ADL transfer/extraction and phantom-value prefixes reject exactly and retain bounded owner reduction. Four finding-blind dual-ADL route worlds independently reproduce maintenance, target/effective lag, fee/minimum-fee, floor, projected-health, and binary-search sizing from the authenticated certificate and require the selected crank's exact partial close, two-sided OI delta, no-SPL frame, and CU bound. The resolved-ADL close-order violation is fixed on `6c04db7e`: both landing orders advance through effective-OI reduction and reset cleanup to exact funded exits. The independent fractional reset-carry violation terminates on `573c4e90`: the sole public crank strictly liquidates the target below its CU cap, clears eight affected legs and both resets, settles every claim/provider obligation, and retires the asset through one canonical terminal transition. Carry normalization and terminal frames are engine-contract checked. Sixteen equal-risk public worlds cross all opening transports, target-leg orders, and accrual orders in a two-asset/three-user ADL topology. A 48-world successor crosses three unequal losses, all persisted leg orders, and both accrual orders while framing every nonselected value/loss domain. A four-route underfunded bridge removes exactly 70,000,000 matched OI, preserves the resulting 2,723-atom finalized close through resolution, and composes it into a genuine partial receipt, a value-moving follow-up payout, and terminal custody. A second four-route world proves cross-asset unattributed loss takes the direct bounded-liquidation path rather than creating a conflicting close rank. Caller-sized liquidation is absent and source-locked by INV-059; add repeated authenticated liquidation episodes, larger account partitions, and the remaining maximum-shape cross-products. |
...
| AUDIT-066 | OPEN-T | A public two-asset lifecycle exhausts all 5! basic claimant orders with exact payout/vault reconciliation and identical outcomes. Every trade, resolution, and claimant transition additionally passes an independent all-portfolio/domain encumbrance census and decoded/raw-header/SPL stock reconciliation. The adjacent underfunded model exhausts all 16 placements of a claim before/after two independent backing-release frontiers, with every value-moving edge checked by an independent long-division quotient/remainder oracle. A separate four-world composition keeps the claim-producing pairs intact while a third asset enters Recovery. Larger-topology models create three and four unequal concurrent receipts, exhaust all 24 and 120 claimant/release orders, and cross aggregate, equal two-way, unequal three-way, and unequal four-way claims over all 16 open/close route pairs. Every world has nonzero arithmetic residue, loses at most N-1 designated floor atoms, and converges to identical claimant-local payouts and custody. The complete three-receipt product is repeated with prior insurance through eight exact rejected withdrawal prefixes, final exact drain, expired-backing normalization, and slab closure. INV-029 proves exact bound replacement, and INV-068 composes one receipt through two exact public top-ups. Authority refinement is surface-excluded by commit `392a3768`, decoder Kani, and exact-SVM raw-tag rejection. Unbounded claimant-count induction remains; additional factorial examples do not close it. |
| AUDIT-067 | OPEN-T | Both payout routes are retried at a byte- and token-stable fixed point after every claimant across all 5! basic orders. A second matrix reaches terminal settlement from a publicly booked bankruptcy obligation and proves exact per-owner payouts across all four trade routes, three claimant orders, and both payout-route priorities. The shared runner also takes a funded live market through permissionless stale resolution and requires every portfolio to reach that terminal fixed point. INV-063 creates a genuinely partial receipt and value-moving top-up through public instructions with route/order and expiry equivalence; INV-068 adds two successive exact top-ups, exact no-op retries, nonfinal close rejection, terminal close, and proves Recovery/asset/portfolio episode reuse is excluded while Resolved. The 16-schedule release/claim matrix proves that claims landing before or after either backing release cannot double-pay, skip terminal cleanup, or change any final claimant balance. A separate four-world product composes an independent Recovery pair with both owner/crank request orders and claim-before/claim-after backing release. The 24-world three-receipt and 120-world four-receipt products cover every claimant order and exact-expiry release insertion with identical per-claimant payout and terminal custody; the prior-insurance twin additionally proves eight premature withdrawal prefixes reject before an exact post-dematerialization drain and slab closure. The terminal source-haircut counterexample is fixed and certified across all four trade routes, while engine `e914dbcf` preserves social-loss attribution across both Recovery forfeit orders and exposes bounded cleanup before exact payout. A claimant-count-independent exact-once induction remains. |
...
| AUDIT-079 | OPEN-T | The LiteSVM trace schema records actual transaction signers, compiled account metas, exact authority-attributed tracked token/lamport deltas, rejected writable-account rollback with the fee-payer network charge separated from program effects, between-transaction economic mutation, and exact mint-supply deltas for terminal burns. The shared validator requires an allowlisted public construction sequence containing a real wrapper call and rejects malformed success, rollback, signer, account, payload, program, CU, token-owner, vault-participation, and quote-balance evidence. A recursive scan source-locks all 59 current trace consumers to validate or classify immediately. The normalized terminal classifier distinguishes exact loss/unauthorized gain, persistent funded lock, bounded exit, atomic rejection, and progress; real public exit/rejection witnesses plus negative anti-overclaim controls pass. Required, attempted, and progressing route masks prevent incomplete searches or successful no-op continuations from being labeled persistent DoS. Twenty-two of 32 finding-blind violation oracles carry classifier-bound exact LoF evidence, including all eight signed fee-consent families, four retained trade transports, the exact nonrecoverable activation-fee retry, fractional settlement with a complete progressing-route observation, stale pending-mark admission and inheritance matrices, revoked matcher consent, all five oracle supersession families, and fee-redirection supersession; the paid mark-reserve and paired pending-target oracles separately carry exact terminal evidence while retaining narrower labels for local rollback branches. All 11 retained-retry kinds and all 14 same-incarnation supersession kinds have source-complete dispositions. Recoverable insurance principal remains non-LoF; real fee-bearing terminal worlds classify liquidation and maintenance shares as attribution-only; resolve-policy timing is finite bounded liveness rather than persistent DoS; no terminal-value or liveness candidate remains. The enforced roster limits the remaining replay, semantic, privileged, or local discoveries to their actual claims. |
