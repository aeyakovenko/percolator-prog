...
| Focused INV-038 wrapper truncation ownership and odd partitions | 914/914 CU; 36/36 production functions, 62/62 truncating operations, and 2/2 odd public value partitions | one source-derived semantic census includes plain division, modulo, and checked division rather than only wide-arithmetic markers. Structural index arithmetic is separated from oracle/policy rounding, exact value partitions, aggregate ceilings, the atom-aligned backing counter floor, and the engine-owned funding mirror. Executable evidence owns every nonstructural row. Public activation proves `41 = 20 + 21`; public batch redirect proves two one-atom per-side fees become `2 = 0 + 2`, with exact insurance and SPL reconciliation. |
...
partitions and the engine-proven atom-aligned backing-counter floor. The broader ratio-change and
split/merge product remains open; these tests do not imply that unbounded cross-product.
