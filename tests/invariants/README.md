...
tests, 109 public regressions, 213 stateful tests, 916 LiteSVM/CU tests, and 193 wrapper Kani
...
terminal-generation classification, and source-locks all 31 finding-blind `is_violation` oracles to
an explicit evidence class. Twenty-one oracles now carry classifier-bound public LoF evidence. In
...
remaining finding-blind public counterexample runner. A source-complete roster now covers all 31
...
activation-fee debit, the revoked-matcher terminal world, and all five oracle supersession families
raise the classifier-bound set to 21.
Source-complete retry and supersession disposition tables prevent weaker discoveries from
inheriting that label; the supersession table closes six terminal-LoF controls and both
...
deltas. The next retained-control targets are downstream terminal domain-loss quantification for
`FeeRedirectPolicy` and a complete funded route-mask liveness observation for `ResolvePolicy`.
...
| `cargo test --test v16_program_stateful_fuzz` | 213/213 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 257.14 seconds; every generated public action and successful crank composes the shared invariant oracles for both markets, including generated route selection for all four terminal retained-trade retry worlds, both exact external-debit retry families, the revoked-matcher paired terminal world with a fresh-grant nonvacuity witness, all five oracle-supersession terminal worlds, and fee-bearing maintenance/liquidation share worlds. The oracle worlds prove exact stale rollback and fresh terminal victim loss offset by counterparty gain or protocol-defined mint burn. The suite also owns the complete pending-obligation and close-ledger encumbrance census, direct per-domain `usable_positive_credit_num <= available_backing_num`, exact/bound claim transitions, credit-rate edge causality, and the independent resolved-payout quotient/remainder oracle. A persisted INV-086 seed additionally locks zero-amount withdrawal semantics: an admitted economic no-op may charge only the exact maintenance amount, move no SPL payout, and cannot satisfy value-withdrawal coverage. All authority, policy, lifecycle, liveness, accounting, and model campaigns pass. |
| Registry/manifest checks in the INV-079 module | 14/14 | includes the source-complete 31-oracle evidence roster, 21 exact terminal-LoF dispositions, all 11 retained-retry dispositions, all 14 retained-control supersession dispositions with six terminal-LoF rows, two attribution-only fee-share rows, and zero terminal-value candidates, complete-route terminal classification, and the recursive 55-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 109/109 | full exact engine-`a6e3c79`/SBF-`692152ce` rerun completed in 28.71 seconds; includes the generic 11-operation whole-market no-reuse matrix, deterministic terminal retained-trade, exact external-debit retry, revoked-matcher, all five oracle-supersession terminal worlds, and both fee-share attribution worlds, the source-complete 11-kind retry and 14-kind supersession disposition censuses, the source-complete account-incarnation and detached-signature censuses, all public regressions, 55 trace consumers, executable registries, and the 31-oracle INV-079 evidence roster |
...
| AUDIT-079 | OPEN-T | The LiteSVM trace schema records actual transaction signers, compiled account metas, exact authority-attributed tracked token/lamport deltas, rejected writable-account rollback with the fee-payer network charge separated from program effects, between-transaction economic mutation, and exact mint-supply deltas for terminal burns. The shared validator requires an allowlisted public construction sequence containing a real wrapper call and rejects malformed success, rollback, signer, account, payload, program, CU, token-owner, vault-participation, and quote-balance evidence. A recursive scan source-locks all 55 current trace consumers to validate or classify immediately. The normalized terminal classifier distinguishes exact loss/unauthorized gain, persistent funded lock, bounded exit, atomic rejection, and progress; real public exit/rejection witnesses plus negative anti-overclaim controls pass. Required, attempted, and progressing route masks prevent incomplete searches or successful no-op continuations from being labeled persistent DoS. Twenty-one of 31 finding-blind violation oracles carry classifier-bound exact LoF evidence, including all eight signed fee-consent families, four retained trade transports, the exact nonrecoverable activation-fee retry, fractional settlement with a complete progressing-route observation, stale pending-mark admission and inheritance matrices, revoked matcher consent, and all five oracle supersession families; the paid mark-reserve and paired pending-target oracles separately carry exact terminal evidence while retaining narrower labels for local rollback branches. All 11 retained-retry kinds and all 14 same-incarnation supersession kinds have source-complete dispositions. Recoverable insurance principal remains non-LoF; real fee-bearing terminal worlds classify liquidation and maintenance shares as attribution-only; no terminal-value candidate remains. The enforced roster limits the remaining replay, semantic, privileged, or local discoveries to their actual claims. Next quantify `FeeRedirectPolicy` terminal domain harm and complete the funded route-mask observation for `ResolvePolicy`. |
