...
The INV-080 dispatcher tranche now source-locks every explicit engine-error disposition, all 141
ordinary engine-result mappings, all 50 direct handler returns, and both deployed entrypoint
adapters. The only successful engine-error dispositions are optional live-account deregistration
(`LockActive` keeps the account) and selector `NonProgress` after independently observed market
accrual; public witnesses prove both outcomes mutate no unauthorized value and preserve liveness.
A multi-instruction LiteSVM transaction additionally proves an over-withdraw engine error prevents
a later configured matcher CPI trade from executing or consuming success-only return data, rolls
back every writable engine/SPL/matcher account, and leaves the same trade independently live.
