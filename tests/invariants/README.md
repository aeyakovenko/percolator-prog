...
`fdf11670d1133ade1647aac8280356cea1536b78`. The exact SBF artifact exercised by the public
LiteSVM/CU suites has SHA-256
`3b14d7b8a7d137b655b33d50f1364856ecbc6ad2ffc4eaf7585d23fb71f4672d`. Resolved-mode
...
close, B, and liquidation work, releases at most the bounded account/source lien set, and
recertifies the account. On the fixed pin one mutating automatic crank releases the lien, exact PnL
conversion succeeds, all owner capital is withdrawn, and the portfolio closes with zero remaining
funded value or lien. Engine runtime suites pass 185/185 and 234/234 with fuzz features; the focused
selector-priority and observation proofs plus all three affected function contracts pass. No
wrapper production branch or persisted field was added.

The adjacent INV-077 maximum-shape composition publicly creates two real counterparty-backed liens,
one from each source side, keeps them live while growing the same portfolio to fourteen active legs
and all 28 occupied source-domain slots, then flattens every leg without releasing the liens. One
observation-free automatic crank scans the complete source array, releases both domain reservations
at 1,019,584 CU, preserves the 28,000-atom PnL claim, and leaves exact conversion, withdrawal, and
portfolio close live at 947,446, 43,809, and 26,641 CU. The complete 845-test CU gate passes on the
same SBF artifact. This closes the full-scan/two-sided-lien cell, not the unconstructed all-28-lien
...
195/195 stateful tests pass. This was a loss of detection power in test code, not a deployed public
...
| Focused INV-028/071/073/079/082 flat source-lien exit | 4/4 independent public trade-route worlds, 185/185 engine base, 234/234 engine fuzz/reference, 2/2 focused Kani, and 3/3 affected function contracts | engine `592d538c` leaves a publicly flattened owner with positive PnL and a source lien after provider withdrawal: conversion and close reject, all four trade families fail to release the claim, and automatic crank reports `NoAction`. Engine PR 185 (`fdf11670`) selects one bounded `ReleaseSourceLiens` continuation after higher-priority work. Each named single/batch CPI/no-CPI world now proves exact pre-release rollback, one mutating crank, exact conversion, full capital withdrawal, portfolio close, zero residual funded value/lien, and custody/supply reconciliation; a required-route bitmask prevents partial escape probing from being classified as persistent DoS. |
| Focused INV-028/071/073/077/082 maximum-shape source-lien release | 1/1 public 14-leg/28-source/two-lien world plus 845/845 complete CU | both source-side counterparty liens are created through margin-constrained public trades, retained while all fourteen legs and 28 historical source slots are populated, and released only after every leg is publicly flattened. `ReleaseSourceLiens` lands without an observation tail at 1,019,584 CU; the unchanged 28,000-atom claim converts at 947,446 CU and the owner withdraws and closes at 43,809/26,641 CU. This covers the full scan with two real lien mutations; simultaneous liens in all 28 slots remain an explicit unconstructed maximum. |
...
| Focused INV-086 terminal reference composition | 12/12 public terminal worlds, one active-close-to-partial-receipt-to-terminal bridge, plus all 183 depth-two base words | every seed is rebuilt through public top-up, trade, mark, crank, resolve, and close instructions; all claim-priority worlds move real SPL value, exact/late expiry normalize on a recorded edge, route/claimant orders converge under independent position/OI/source-credit/encumbrance/stock/custody oracles, and the separate three-asset bridge preserves an active value-bearing close through resolution, finalizes it into a genuine partial receipt, moves additional SPL value, and terminates all five funded portfolios |
...
| `cargo check --tests` | pass | covered by the exact engine-`fdf11670` test builds on the 2026-08-21 PR135 production head |
| `cargo test --lib` | 7/7 | exact engine-`fdf11670` rerun on the 2026-08-21 PR135 production head |
| `cargo test --test v16_program_stateful_fuzz` | 195/195 | exact engine-`fdf11670`/SBF-`3b14d7b8` rerun in 174.47 seconds, including four independent flat-source-lien route worlds with complete terminal observations, source-complete validation of every current public trace consumer, and every prior rank/reference-model, lifecycle, source-lien, terminal, and route-composition check |
| Registry/manifest checks in the INV-079 module | 11/11 | rerun on the 2026-08-21 PR135 exact-pin test head; includes complete-route terminal classification and the recursive 25-consumer source guard |
| `cargo test --test v16_program_fuzz_regressions` | 95/95 | full engine-`fdf11670`/SBF-`3b14d7b8` exact-source rerun in 27.16 seconds; every current public trace consumer validates normalized reachability evidence, and the sealed benchmark census, audit ledger, special-method registry, and invariant registries remain green |
| `cargo test --test v16_cu` | 845/845 | full engine-`fdf11670`/SBF-`3b14d7b8` exact-source rerun passed in 197.50 seconds on the 2026-08-21 checkpoint; zero failures, including exact/late impaired-lien terminal disposition, maximum-shape two-sided source-lien release, the source-complete INV-085 arithmetic census, bigint boundary/provider references, exact host-to-SBF comparisons, and INV-080 error-disposition/dispatcher/entrypoint/later-consumer composition |
| `cargo kani --bin v16-kani --features kani --default-unwind 18 --output-format terse` | 171/171 | full wrapper-only exact-source rerun on the 2026-08-21 checkpoint with engine `fdf11670`; zero failures, including all 26 branch-witnessed proof classes with constructive covers, all 13 source-inventoried assumptions, the twelve INV-085 arithmetic relations, 16 provider-parser decomposition harnesses, and every prior decoder and rollback proof |
| Engine runtime/property suites | 185/185 base and 234/234 with `--features fuzz` | exact rerun on engine `fdf11670`. All unit, reference-model, arithmetic-discharge, source-credit, insolvency, and v16 specification tests pass, including the bounded flat-source-lien release continuation; the selector-priority and observation Kani proofs plus all three affected function contracts pass |
...
7. Extend the finite graph beyond depth two with targeted partial-order reduction and public-prefix
...
On engine pin `fdf11670d1133ade1647aac8280356cea1536b78`, the full 845-test `v16_cu` inventory is
