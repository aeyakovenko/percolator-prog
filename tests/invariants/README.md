...
| `cu/` | 714 | Full `v16_cu` public-route, metamorphic, rollback, liveness, arithmetic-differential, and max-shape CU inventory, with no standalone top-level tests |
...
| INV-017 | SVM/CU + Partial M | `cu/inv_017_signer_writable_role_and_account_alias_safety.rs` exhausts all ten semantic account-pair aliases plus both signer and all three writable downgrades for single and batch no-CPI trade from nonvacuous public fixtures, with exact market/portfolio/vault rollback; targeted custody, ledger, CPI, helper, reward, and close aliases remain route-specific rather than pairwise-complete |
...
| AUDIT-017 | OPEN-T | Single and batch no-CPI trade now exhaust all ten semantic account-pair aliases and every required signer/writable downgrade from successful public controls; all reject with exact economic rollback. CPI/matcher, custody, ledger, helper, reward, close, and remaining instruction schemas still need the same generated all-pairs matrix, with explicit successful controls for intentionally safe aliases. |
