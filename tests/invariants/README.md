...
| Focused INV-089 activation/reactivation equivalence | 14/14 public CU tests | both permissionless and privileged reuse reset an old generation's `u64::MAX` replay watermark; byte-complete persisted-slot differentials after public trade/oracle/backing history and after exact insurance spend, owner forfeit, provider settlement, and retirement match fresh activation after normalizing only generation IDs. Fresh same-generation trading plus full withdrawal remains live and engine/SPL custody stays exact. |
...
8. Extend INV-089's byte-complete fresh/reuse differential beyond matched trade, mark, backing, and
   spent-insurance history to publicly retired generations with prior claims, current certificates,
   maximum supported account shape, and unsupported-shape rejection. Replay-watermark poisoning and
   spent-only insurance normalization are closed; do not duplicate those cells with field-by-field
   assertions.
...
| AUDIT-089 | OPEN-T | Fresh/reuse authority and price checks are broad. The shared model proves both a public Active-to-DrainOnly-to-empty-Retired lifecycle and an empty-Recovery restart that advances generation, preserves authority/fee/insurance scope, frames unrelated assets/accounts/custody, and admits a fresh-generation trade under independent leg/OI checks. A byte-complete dynamic-slot differential independently found that wrapper replay watermarks survived engine/profile reset: an old generation could set `oracle_observation = u64::MAX` and permanently poison the replacement authority. Both privileged and permissionless reuse now reset all eleven per-generation lanes. The first persisted-state route composes matched trade, mark movement, backing top-up/withdrawal, retirement, and reuse. A second public-only route consumes an exact insurance budget through bankruptcy, explicitly forfeits only the winner's junior claim, returns all senior capital, preserves and withdraws the provider-attributed backing, retires the empty Recovery asset, and then proves fresh-slot byte equality, fresh-generation trading/withdrawal, and exact custody. Prior-claim history, current-certificate invalidation, maximum-shape, and unsupported-shape permutations remain open. |
