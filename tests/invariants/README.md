...
`a6e3c79f2d6c3afdfb82260951d8a5be85f8fa5d` (engine PR
[#192](https://github.com/aeyakovenko/percolator/pull/192), stacked on source-lien certification PR
[#191](https://github.com/aeyakovenko/percolator/pull/191)). This pin includes two production
...
the final vault, `c_tot`, and SPL vault are zero. The exact SBF is 1,247,048 bytes with SHA-256
...
harnesses, all green on the exact pin.
...
INV-086 now owns 64 public unilateral-close worlds. The owner-rebalance half crosses all four trade
transports, `effective - 1`, exact-effective, `effective + 1`, and raw-basis work requests, plus both
terminal claimant orders. The Recovery half crosses the same opening/boundary matrix through
...
OI deltas, zero SPL movement during reduction, and exact terminal payouts. The force-close fixture
has one ADL-scaled side and one unit-A side, so it does not close dual-nonunit-A histories. The next
tractable work is permissionless liquidation, Recovery forfeit, and a public history with both side
A indices non-unit before force-close; each must use the pre-state oracle rather than trust deployed
post-state counters.
