...
fn inv065_source_defines_test(source: &str, function: &str) -> bool {
    ...
    for row in CLASSES {
        ...
        for (path, witness) in row.public_witnesses {
            ...
            assert!(
                ...
                "lifecycle class '{}' points outside invariant test sources: {path}",
                row.class,
            );
            assert!(
                witness.starts_with("v16_"),
                "lifecycle class '{}' uses an unreviewed witness name: {witness}",
                row.class,
            );
            ...
            assert!(
                inv065_source_defines_test(source, witness),
...
/// Issue #170: matched, value-free spent-backing history on an emptied asset blocks reuse until
/// the asset's backing authority canonicalizes it (tag 70). Only that authority, at the current
/// asset generation and authority epoch, may clear it; the rejection is byte-atomic and the
/// transition is idempotent-rejecting once the history is gone.
#[test]
fn v16_program_backing_authority_canonicalizes_spent_history_before_reuse() {
    ...
    // Install the matched history a completed round trip leaves behind.
    ...
    let reuse = env.build_retained_activate_asset(ASSET, slot, INITIAL_PRICE);
    ...
    assert_eq!(env.market_data(false), before, "rejection is byte-atomic");
    ...
    assert_eq!(group.source_credit[ASSET as usize * 2].provider_receivable_num, 0);
    assert_eq!(group.source_credit[ASSET as usize * 2].spent_backing_num, 0);
    let cleared = env.market_data(false);
    env.expire_blockhash();
    assert!(
        env.canonicalize_spent_backing_history_for_actor(PROVIDER, ASSET).is_err(),
        "nothing left to canonicalize"
    );
    assert_eq!(env.market_data(false), cleared);
    env.expire_blockhash();
    let reuse = env.build_retained_activate_asset(ASSET, slot, INITIAL_PRICE);
    env.land_retained(reuse).expect("reuse succeeds once only history remained");
}
