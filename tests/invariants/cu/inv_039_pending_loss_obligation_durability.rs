...
fn v16_program_resolve_with_pending_obligation_defers_claim_until_debtor_settles() {
    ...
    for reverse_sides in [false, true] {
        ...
        // A bounded creditor continuation is allowed, but the zero-basis loss weight remains
        // attached until the opposing debtor is settled by a public continuation.
        ...
        close(&mut world, 0).expect("waiting claimant retry remains idempotent before debtor");
        world.check([0, -q, 0, 0], [true, false, false, false]);
        assert_eq!(
            world.frame(),
            before_retry,
            "waiting retry must not pay, forgive, or mutate before debtor settlement"
        );

        close(&mut world, 1).expect("permissionless debtor settlement remains available");
        world.check([0, 0, 0, 0], [true, false, false, false]);
