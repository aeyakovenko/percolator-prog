...
fn v16_attack_funded_asset_admin_cannot_seize_oracle_and_redistribute_user_value() {
    ...
    assert_eq!(
        ...
        ATTACK_MARK,
        "the seized oracle must control the settlement mark on the vulnerable program"
    );

    env.resolve_stale_permissionless(3)
        .expect("resolve the market with the seized mark");
