...
impl MixedModel {
    ...
    fn check(&self, world: &AttributionWorld) {
        ...
        for (actor, a) in world.actors.iter().enumerate() {
            ...
            assert_eq!(
                legs.len(),
                expected_legs,
                "owner {actor}: exact domain ownership; expected pending={:?} booked={:?} detached={:?}, actual legs={legs:?}",
                self.pending,
                self.booked,
                self.detached
            );
    ...
    fn close(&mut self, world: &mut AttributionWorld, actor: usize) {
        ...
        if actor == 0 {
            // Current resolved close keeps each continuation bounded: the mixed account first
            // books its debtor side, then releases the pending zero-basis credit leg, then
            // detaches its remaining payer leg.
            if !self.booked[1] {
                self.booked[1] = true;
                if self.detached[0] {
...
fn v16_program_mixed_creditor_debtor_preserves_pending_attribution_through_resolved_close_order() {
    ...
    for original_payer_first in [true, false] {
        ...
        assert_eq!(model.pending, [!original_payer_first, true]);
        if original_payer_first {
            model.close(&mut world, 0);
        } else {
            assert_eq!(
            ...
            assert!(
                !resolved_portfolio_is_terminal(&world.env, world.actors[0].portfolio),
                "the mixed account cannot delete before the absent original payer settles"
            );
            model.close(&mut world, 1);
            model.delete(&mut world, 1);
        }
