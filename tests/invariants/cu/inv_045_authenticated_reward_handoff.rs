...
fn v16_program_paid_discovery_fresh_handoff_authenticates_liquidation_and_keeper_exit() {
    ...
    let mut worlds = 0;
    for fresh_price in [MARK, MARK - 1_000] {
        for share in [3_333u16, 10_000] {
            ...
            for raw_print in [980_000, 900_000] {
                for publish_first in [false, true] {
                    ...
                    let loss = i128::from(ENTRY - ACCEPTED);
    ...
    println!("authenticated handoff: worlds={worlds}, late_rollbacks={late_rejections}, reward_rollbacks={rewarded_rollbacks}, peak_transaction_cu={peak_cu}");
}
