...
//! Secondary coverage: INV-045. The row-425 regression below verifies that a successful
//! position-changing trade cannot erase the canonical fractional price-movement carry
//! that a crank-before-trade ordering would preserve.
//!
    ...
    for row in CLASSES {
        ...
        for (path, witness) in row.public_witnesses {
            ...
            assert!(
                ...
                "liveness class '{}' uses an unreviewed witness name: {witness}",
                row.class,
            );
