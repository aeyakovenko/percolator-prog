...
//! `v16_program_underfunded_policy_handoff_and_resolve_exhaust_all_landing_orders` crosses all eight
//! retained market/asset-0 policy lanes, three boundary values, and all 3!
//! policy/handoff/resolve orders in the three-asset, five-portfolio underfunded terminal world.
//! Every stale old-authority operation rolls back exactly. Live-only backing/resolve policy
//! requests that land after terminal resolution, or resolve-policy updates after their prior low
//! threshold has matured, also reject exactly while the fresh incoming-authority resolve path
//! remains live. All 144 terminal outcomes are economically identical.
//!
...
//! authority + policy + terminal permutations cover every retained policy lane and its low,
//! midpoint, and maximum valid value in the shared three-asset, five-portfolio topology. Adding a
...
fn v16_program_underfunded_policy_handoff_and_resolve_exhaust_all_landing_orders() {
    ...
    assert_eq!(evidence.policy_kind_count, 8);
    assert_eq!(evidence.boundary_count, 3);
    assert_eq!(evidence.world_count, 144);
    assert_eq!(evidence.stale_resolve_rejection_count, 72);
    assert_eq!(evidence.stale_policy_rejection_count, 72);
    assert_eq!(evidence.terminal_policy_rejection_count, 9);
    assert_eq!(evidence.fresh_policy_rejection_count, 28);
    assert_eq!(evidence.partial_receipt_count, evidence.world_count);
    assert_eq!(evidence.value_moving_claim_count, evidence.world_count);
    assert!(evidence.claim_payout_atoms > 0);
}
