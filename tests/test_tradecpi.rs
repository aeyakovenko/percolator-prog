...
fn test_vulnerability_stale_pnl_pos_tot_after_force_close() {
    ...
    env.crank();
    env.flatten_positions_via_trade(
        &user_long, &lp.pubkey(), lp_idx, user_long_idx, &matcher_prog, &matcher_ctx,
    );
    env.set_slot(110);
    env.crank();
...
fn test_admin_force_close_account_happy_path() {
    ...
    env.crank();
    env.flatten_positions_via_trade(
        &user, &lp.pubkey(), lp_idx, user_idx, &mp, &matcher_ctx,
    );
    env.set_slot(160);
    env.crank();
...
fn test_honest_user_close_after_force_close_positive_pnl() {
    ...
    println!(
        "After flatten+resolve: user capital={}",
        cap_after
    );
    assert!(
        cap_after > 0,
